...
function refreshSocialFollowerDisplays(uid = '') {
  ...
  if (!targetUid || targetUid === ownUid) {
    /* v11.928: the shelf header host is `mylist-profile-controls` (index.html) —
       NOT `mylist-profile-host`, which exists nowhere. The wrong id meant `host`
       was always null, so this re-render NEVER fired: the v11.910 friend-commit
       refresh AND the derived-follower query's .finally() refresh both no-op'd,
       leaving the cold-start "0 followers" on the shelf until a category switch
       re-rendered it independently. renderMyListEditControls() self-guards on its
       own hosts, so it's a cheap no-op when the shelf isn't mounted. */
    const host = document.getElementById('mylist-profile-controls');
    if (host) {
