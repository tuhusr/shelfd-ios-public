...
function renderAllUsers(users, query = '') {
  ...
  users.forEach(u => {
    ...
    let action = '';
    if (isFriend) {
      action = `<button class="friend-action-btn friend-list-follow-btn is-following" onclick="event.stopPropagation(); openUserProfile('${u.uid}')">Following</button>`;
    } else if (sentRequest) {
      action = `<button class="friend-action-btn friend-pending-btn" onclick="event.stopPropagation(); cancelFriendRequest('${u.uid}')" title="Tap to cancel">Requested</button>`;
    } else {
      action = `<button class="friend-action-btn friend-add-btn" onclick="event.stopPropagation(); sendFriendRequest('${u.uid}')">Follow</button>`;
    }
