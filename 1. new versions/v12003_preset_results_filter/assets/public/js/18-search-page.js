...
(function() {
  ...
  /* ============================================================
     ...
     TMDB /discover query).
     ============================================================ */
  const PRESET_NONFILTERABLE_PRESETS = new Set(['popular', 'anticipated', 'upcoming']);
  ...
  async function fetchPresetRefinedDirect(config = {}) {
    const requestedMediaType = String(config.mediaType || 'mixed').trim().toLowerCase() || 'mixed';
    ...
    const requests = [];
    if (includeMovie) requests.push(fetchPresetTypedTmdb('discover/movie', { ...common, sort_by: sortByFor('movie'), ...dateParams('movie') }, 'movie'));
    if (includeTv) requests.push(fetchPresetTypedTmdb('discover/tv', { ...common, sort_by: sortByFor('tv'), ...dateParams('tv') }, 'tv'));
    let items = filterPresetItemsByMediaType(normalizePresetMediaItems((await Promise.all(requests)).flat(), requestedMediaType), requestedMediaType);
    const now = Date.now();
    ...
    return items.slice(0, PRESET_RESULT_LIMIT);
  }
  ...
  window.shelfdOpenPresetResultsFilter = function(panelId) {
    ...
    sheet.innerHTML = `
      ...
      <div class="preset-filter-body">
        ...
        ${renderPresetFilterChipGroup('Release', 'release', PRESET_REFINE_RELEASE_OPTIONS.map(o => ({ value: o.key, label: o.label })), [current.releaseKey || 'any'], true)}
  ...
  function readPresetFilterSheetRefine(sheet) {
    ...
    return {
      ...
      releaseKey: getSingle('release') || 'any',
