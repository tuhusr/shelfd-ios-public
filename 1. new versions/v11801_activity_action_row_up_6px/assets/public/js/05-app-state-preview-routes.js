...
function isShelfdProMember() {
  try {
    ...
    const uid = (window.currentUser && window.currentUser.uid)
      || (window.__shelfdUser && window.__shelfdUser.uid) || '';
...
async function openSharedMediaProfileRoute(route = parseScreenListMediaRoute(), opts = {}) {
  ...
  try {
    if (route.kind === 'game') {
      ...
      const gameSeed = { rawgId: numericRawg, title: String(opts.title || '').trim() };
  ...
  function handleAppUrlOpen(event) {
    ...
    try {
      ...
      if (mediaMatch) {
        ...
        __shelfdDeepLinkDiag('matched /media route: ' + route.kind + ' / ' + route.id);
        const fireRoute = () => {
          ...
          Promise.resolve(openSharedMediaProfileRoute(route))
...
(function bootResolveSharedMediaRouteFromUrl() {
  ...
  function tryResolveSharedMediaRoute() {
    ...
    try {
      ...
      Promise.resolve(fn(route)).then(ok => {
