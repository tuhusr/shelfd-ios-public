...
async function saveProfileSettingsPatch(patch = {}) {
  ...
  const providerPatchKeys = new Set(['steamConnection', 'trackerConnection', 'appleMusicConnection', 'appleMusicMetadataSummary']);
...
window.openUsernameHandleEdit = function() {
  ...
  row.hidden = true;
  edit.hidden = false;
  input.value = String(userProfile?.usernameHandle || '').trim();
...
window.saveUsernameHandleChange = async function() {
  ...
  const newHandle = String(input.value || '').trim();
...
window.saveDisplayNameChange = async function() {
  ...
  if (newName.length > 50) {
    setProfileEditError(errorEl, 'Display name must be 50 characters or fewer.');
    return;
  }
  if (saveBtn) { saveBtn.disabled = true; saveBtn.textContent = 'Saving…'; }
  setProfileEditError(errorEl, '');
