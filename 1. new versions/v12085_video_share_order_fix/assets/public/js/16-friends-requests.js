...
function buildFriendWatchlistDiffEvents(uid, previousState = null, nextState = {}) {
  ...
  keys.forEach(key => {
    ...
    // Activity Feed post â€” that change is private unless the user explicitly
...
function syncFriendActivityLiveListeners() {
  ...
  friends.forEach(uid => {
    /* v10.387: the parent watchlist/{uid} doc no longer carries the JSON
       sections â€” under schema v2 it's just a heartbeat ({ updatedAt,
...
let ownAcceptedFollowerRequestIds = [];
...
/* v11.847: session-scoped "Follow back" list. When you Accept an incoming follow
   request the requester is removed from incomingRequests (and is NOT added to
   friends unless you already follow them), so the row would otherwise vanish.
   We park just-accepted requesters here so a single Follow/Following/Requested
   button renders in their place until they become a mutual friend (then they
   migrate to the main friends list) or the page reloads (transient by design —
   never persisted, so old followers don't permanently clutter the list). */
let pendingFollowBackUids = [];
...
function captureOwnFriendRawState() {
  return {
    ...
    ownAcceptedFollowerRequestIds: ownAcceptedFollowerRequestIds.slice(),
...
function restoreOwnFriendRawState(snapshot = null) {
  ...
  ownAcceptedFollowerRequestIds = normalizeFriendUidList(snapshot.ownAcceptedFollowerRequestIds);
...
function computeEffectiveFriendState() {
  ...
  const ownAcceptedFollowerSet = new Set(ownAcceptedFollowerRequestIds);
  ...
  ownFriendIds.forEach(uid => {
    ...
    if (ownRemovedSet.has(uid)) return;
    nextFriends.add(uid);
  });
  ...
  const nextIncoming = normalizeFriendUidList(incomingFriendRequestSourceUids.filter(uid => {
    ...
    if (ownAcceptedFollowerSet.has(uid)) return false;
...
function buildFriendSelfRepairPlan() {
  ...
  const clearOutgoing = normalizeFriendUidList([...acceptedToPromote]);
...
/* v11.848: belt-and-suspenders for the sender side of an accept. A friend_accept
   NOTIFICATION (separate collection + listener) is an independent signal that
   someone accepted my follow request. The primary path â€” the realtime
   `acceptedFollowerRequests` array-contains listener â€” normally promotes them to
   friends and clears the "Requested" chip, but on iOS WKWebView that single
   listener can lag or miss while the webview is suspended/resumed, leaving the
   sender stuck on "Requested". If the actor is still in my outgoingRequests,
   promote them here too. Idempotent: a no-op once they're already promoted, and
   the self-repair write inside commit persists it to my own friends array so it
   sticks even after acceptedFriendSourceUids is later overwritten by the query. */
function reconcileFriendAcceptsFromNotifications(actorUids = []) {
  if (!currentUser) return;
  const me = String(currentUser.uid || '').trim();
  const toPromote = normalizeFriendUidList(actorUids).filter(uid =>
    uid && uid !== me
    && ownOutgoingFriendRequestIds.includes(uid)
    && !ownRemovedFriendIds.includes(uid)
    && !acceptedFriendSourceUids.includes(uid));
  if (!toPromote.length) return;
  const prev = captureFriendsCommitSnapshot();
  acceptedFriendSourceUids = normalizeFriendUidList([...acceptedFriendSourceUids, ...toPromote]);
  commitFriendsDataState(prev, { silent: true });
}
window.reconcileFriendAcceptsFromNotifications = reconcileFriendAcceptsFromNotifications;
...
function commitFriendsDataState(prev = captureFriendsCommitSnapshot(), opts = {}) {
  ...
  /* v11.910: My List opens before the friends/follower snapshot has hydrated on
     cold start, so the shelf header can initially render "0 followers" and only
     correct itself after a category switch forces renderMyListEditControls().
     Refresh the own-profile social surfaces directly on every friend commit;
     renderMyListEditControls() is keyed and cheap, so unchanged counts no-op. */
  try {
    if (typeof refreshSocialFollowerDisplays === 'function') {
      refreshSocialFollowerDisplays(currentUser?.uid || '');
    } else if (typeof renderMyListEditControls === 'function') {
      renderMyListEditControls();
    }
  } catch (e) {
    console.warn('[v11.910] mylist social header refresh skipped:', e);
  }
  if (prev.prevWatchTogetherIncomingKey !== watchTogetherIncomingRequestIds.join('|') || prev.prevWatchTogetherOutgoingKey !== watchTogetherOutgoingRequestIds.join('|') || prev.prevWatchTogetherApprovedKey !== watchTogetherApprovedRequestIds.join('|')) {
    hydrateWatchTogetherMirroredRequests().then(() => {
      const communityActive = (typeof getActiveMainTab === 'function' ? getActiveMainTab() : '') === 'community';
  ...
  /* v10.762: BELT-AND-SUSPENDERS RE-RENDER on friends data arrival.
     ...
     â€“ Activity FEED sub-tab (only the watch-activity sub-tab was wired)
     â€“ Discover "What Your Friends Are Watching" row (refreshDiscoverFriendStacks
  ...
  /* v10.764: BULLETPROOF FRIENDS-GRID REFRESH.
     ...
     swap tabs" indicate the existing render trigger at line 531 â€” gated
     by `nav-community.active` â€” can miss on cold start (timing race with
  ...
  const reqUidsToWarm = [...new Set([...incomingRequests, ...outgoingRequests])]
    .filter(uid => uid && (!usersMap[uid] || !usersMap[uid].name));
  if (reqUidsToWarm.length) {
    Promise.all(reqUidsToWarm.map(uid =>
      db.collection("users").doc(uid).get()
        .then(doc => { if (doc.exists) usersMap[doc.id] = { uid: doc.id, ...(doc.data() || {}) }; })
        .catch(() => {})
    ));
  }
}
...
function applyFriendsDataSnapshot(d = {}, opts = {}) {
  ...
  ownAcceptedFollowerRequestIds = normalizeFriendUidList(d.acceptedFollowerRequests);
...
function startFriendsDataListener() {
  ...
  attachDerivedFriendQuery('acceptedFollowerRequests', next => { acceptedFriendSourceUids = next; });
...
function updateRequestsBadges() {
  ...
  /* v11.965: incoming friend requests are NOT a "Notifications" item — they live
     inline in the Friends list and ping the Friends tab. The Notifications-tab
     badge therefore uses the unread count EXCLUDING friend_request. Falls back to
     the full count when the notifications listener hasn't reported yet. */
  const notifTabUnreadTotal = (typeof window.activityNotificationsNonRequestUnreadCount === 'number')
    ? window.activityNotificationsNonRequestUnreadCount
    : activityNotificationTotal;

  /* v11.590: split the two community bottom-nav entries so each pings for what
     actually lives under it.
       â€¢ ACTIVITY button (#mobile-nav-community) â†’ new feed activity + incoming
         Shared Watch requests (both live in the Activity tab).
       â€¢ FRIENDS button (#mobile-nav-friendslist) â†’ incoming friend requests
         (inline in the Friends list) + unread Notifications (the new
         Friends â†’ Notifications tab).
       â€¢ Desktop #nav-community is the single "Friends" main-nav entry â†’ union.
  ...
  /* v11.852: red dot on the Friends Feed sub-tab pill — mirrors the bottom-nav
     Activity dot so that once a user opens Activity (because a friend posted a
     review or finished a show), they can see the new posts live under Friends
     Feed. Hidden while that tab is already active; cleared the moment the feed is
     viewed (renderCurrentActivitySubTab → markFriendActivitySeen). Toggled by
     CLASS (not id) because the pill bar renders once per activity sub-tab pane. */
  const showFeedTabDot = !!friendActivityUnread
    && (typeof activeActivitySubTab === 'undefined' || activeActivitySubTab !== 'feed');
  document.querySelectorAll('.activity-feed-tab-dot').forEach(dot => {
    dot.style.display = showFeedTabDot ? 'inline-block' : 'none';
  });

  /* v11.965: red dot on the FRIENDS tab pill when there are pending incoming
     friend requests (they render inline at the top of the Friends list, so this
     is exactly where the user acts on them). Cleared automatically the moment a
     request is accepted/rejected because incomingRequests shrinks → this re-runs. */
  const friendsTabRequestDot = document.getElementById('friends-tab-request-dot');
  if (friendsTabRequestDot) {
    friendsTabRequestDot.style.display = friendIncomingTotal > 0 ? 'inline-block' : 'none';
  }
  ...
  /* v11.590: unread-count badge on the Notifications tab in the Friends tab bar
     ...
     v11.965: friend_request notifications are excluded here — they ping the
     Friends tab dot above instead. */
  const notifTabBadge = document.getElementById('notifications-tab-unread-badge');
  if (notifTabBadge) {
    if (notifTabUnreadTotal > 0) {
      notifTabBadge.textContent = notifTabUnreadTotal > 9 ? '9+' : String(notifTabUnreadTotal);
  ...
  /* #activity-unread-badge rides on the (always-hidden) Activity inner tab â€”
  ...
  /* v11.972: repaint friend-request notification cards' Accept/Deny ↔ Follow
     slots whenever friend data changes (this fires on every friend-data commit),
     so the buttons appear the instant incomingRequests loads and flip correctly
     after an accept/follow even when the notifications list isn't re-rendered. */
  if (typeof refreshAllNotifFriendRequestActions === 'function') refreshAllNotifFriendRequestActions();
  /* v11.973: show/hide the shelf "wants to follow you" Confirm/Delete banner as
     incoming requests load or get accepted/declined while viewing that user. */
  syncShelfFollowRequestBanner();
}
...
function switchFriendsTab(tab) {
  ...
  /* v11.589: the Requests tab + view were removed â€” incoming/sent friend
...
/* v810/v811: Add a Friend tab default state â€” surface the creator account
   ...
   visible the instant the tab opens (no "Loadingâ€¦" flash), then
...
async function openAddFriendDefault() {
  ...
  /* 1) Immediate render â€” use cached creator if available, otherwise a
  ...
  /* 2) Async refresh â€” pull the latest creator doc from Firestore, and if
...
function isValidActivitySubTab(tab = activeActivitySubTab) {
  /* v11.590: 'notifications' is no longer an Activity sub-tab â€” it moved to the
...
/* v11.624: Activity sub-tab slide-transition state. The horizontal cross-slide
   between Activity Feed Â· Shared Watch Â· News is owned by the transition runner
...
/* Page chrome that is NOT part of the sliding panes â€” the feed composer (shown
...
/* v11.648: the top-left header button (#activity-compose-btn) is shared across
   ...
   We swap the icon + label + action in place â€” one button, no layout change. */
...
function updateActivityTopLeftButton() {
  ...
  if (!btn) return;
  const shouldShow = activeFriendsTab === 'activity';
  if (!shouldShow) {
...
window.shelfdActivityTopLeftAction = function (event) {
  ...
  if (activeFriendsTab === 'activity' && typeof window.openActivityCreatePost === 'function') {
    window.openActivityCreatePost(event);
  }
};
...
function updateActivitySubtabUi() {
  ...
  /* Pane display is OWNED by the slide transition while it runs â€” skip it here so
...
const ACTIVITY_SUBTAB_PANE_IDS = ['friend-activity-feed', 'activity-news-feed', 'shared-watch-feed'];  // v11.671: feed Â· news Â· sharedWatch
...
/* The Activity sub-tab cross-slide (Feed Â· Shared Watch Â· News), rebuilt v11.624.
...
function runActivitySubTabSpatialTransition(previousSubTab = 'feed', nextSubTab = 'feed', previousPane = null, nextPane = null) {
  ...
  /* v12.082: BLACK-FLASH FIX. The News pane lazily mounts ONE live cross-origin
     YouTube <iframe> for the most-visible video card (scroll autoplay). Animating
     the pane's transform during the slide forces WKWebView to re-composite that
     iframe's texture cross-process, which stalls and flashes BLACK — the "app is
     refreshing" the user reported when swapping News↔Friends. Tear the inline
     video down BEFORE the slide (the user is leaving the feed anyway) so there is
     no live iframe to composite; autoplay re-arms after settle on re-entry.
     Runs even on the instant-swap bail below, since that path still shows the pane. */
  if ((previousSubTab === 'news' || nextSubTab === 'news') && typeof window.shelfdStopNewsInlineVideo === 'function') {
    try { window.shelfdStopNewsInlineVideo(); } catch (e) {}
  }
  ...
  /* Show both panes BEFORE measuring so the height hold is correct, then pin the
     container height (min-height is a floor â€” the entering pane still grows it if
...
async function switchActivitySubTab(tab = 'feed') {
  ...
  /* 1) Page chrome up front (composer show/hide via the community-view subtab
        class) so the slide runs on a STABLE layout â€” no mid-slide vertical jump. */
  applyActivitySubTabChrome();

  /* 2) START THE SLIDE IMMEDIATELY â€” do NOT await content. This is the core fix:
  ...
  /* 3) Render the now-active pane's content (async fine â€” guarded so its own
  ...
  if (!hasIncoming && !hasOutgoing) {
    feed.innerHTML = `${headerHtml}<div class="friend-watch-clean-shell empty">
      <div class="friend-watch-clean-hero">
        <div class="friend-watch-clean-icon">ðŸŽ¬</div>
...
async function renderSharedWatchActivity(skipHydrate = false) {
  ...
  feed.innerHTML = `${headerHtml}${sharedWatchDescriptionHtml}<div class="shared-watch-dashboard">
    ...
    <div class="shared-watch-dashboard-lane planned"><div class="shared-watch-lane-title"><span>ðŸ¿ Planning Together</span><em>${planned.length}</em></div>${buildSharedWatchDashboardCards(planned)}</div>
    <div class="shared-watch-dashboard-lane watched"><div class="shared-watch-lane-title"><span>âœ¨ Watched Together</span><em>${watched.length}</em></div>${buildSharedWatchDashboardCards(watched)}</div>
  </div>`;
}
...
async function loadActivityTabFeed() {
  ...
  try {
    ...
    /* v11.625: keep an already-rendered feed visible while it refreshes in the
       ...
       v12.078: also skip the full re-render entirely when cache is fresh and the
       feed already has content — eliminates the visible “refresh” flash when the
       user taps back to Friends Feed from another sub-tab. */
    if (isStaleLoad()) return;
    const dayLimit = (typeof getFriendActivityDayLimit === 'function') ? getFriendActivityDayLimit() : 7;
    const cacheWasFresh = typeof getFreshFriendActivityCache === 'function' && !!getFreshFriendActivityCache(dayLimit);
    const feedAlreadyRendered = !!feed.querySelector('.activity-feed-list');
    if (!feedAlreadyRendered) feed.innerHTML = buildSkeletonHTML();
    const activities = await fetchAllFriendActivities(dayLimit);
    if (isStaleLoad()) return;
    if (cacheWasFresh && feedAlreadyRendered) return;
...
async function buildFriendRequestCards(uids = [], type = 'incoming') {
  ...
  const unknown = uids.filter(uid => !usersMap[uid] || !usersMap[uid].name);
  if (unknown.length) {
    try {
      const docs = await Promise.all(unknown.map(uid => db.collection("users").doc(uid).get()));
      docs.forEach(doc => { if (doc.exists) usersMap[doc.id] = { uid: doc.id, ...(doc.data() || {}) }; });
    } catch(e) {
  ...
  /* v10.224: requests page rebuilt in the same IG row style as the Friends
     tab â€” circular avatar, handle line + display name underneath, trailing
  ...
  uids.forEach(uid => {
    ...
    const handle = getShelfdFriendHandle(u, 'User');
    const uidAttr = escAttr(u.uid);
    const actionsHtml = type === 'incoming'
      ? `<div class="shelfd-friend-req-actions">
           <button type="button" class="shelfd-friend-req-btn primary" onclick="event.stopPropagation(); acceptFriendRequest('${uidAttr}')">Accept</button>
           <button type="button" class="shelfd-friend-req-btn secondary" onclick="event.stopPropagation(); rejectFriendRequest('${uidAttr}')">Deny</button>
         </div>`
      : `<div class="shelfd-friend-req-actions">
           <button type="button" class="shelfd-friend-req-btn secondary shelfd-friend-req-btn-requested" aria-label="Requested â€” tap to cancel friend request" onclick="event.stopPropagation(); cancelFriendRequest('${uidAttr}')">Requested</button>
         </div>`;
    html += `<div class="shelfd-friend-row shelfd-friend-req-row" data-friend-uid="${uidAttr}" onclick="viewUserFromMap('${uidAttr}')">
      ...
      <div class="shelfd-friend-text">
        <strong class="shelfd-friend-handle">@${escHtml(handle)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(u) : ''}</strong>
      </div>
      ${actionsHtml}
    </div>`;
  });
...
async function renderRequestsList(skipWatchTogetherHydrate = false) {
  ...
  // Do NOT force activeRequestsSubTab here â€” preserve whatever tab the user chose.
  updateRequestSubtabUi();

  // â”€â”€ Watch Together subtab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (activeRequestsSubTab === 'watchTogether') {
    ...
    if (!incomingGroups.length && !outgoingGroups.length) {
      ...
      outGrid.innerHTML = `<div class="friends-empty" style="grid-column:1/-1;">
        <div class="friends-empty-icon">ðŸŽ¬</div>
  ...
  if (incomingHeading) {
    incomingHeading.textContent = 'Follow Requests';
...
/* v11.847: the single Follow/Requested/Following button shown on a "Follow back"
   row after you Accept an incoming request. Mirrors getFriendListFollowButtonHTML
   (my outgoing relationship toward them) but styled with the request-row button
   classes so it sits cleanly in the inline list. Reuses the shared, tested
   toggleFriendListFollow handler (Follow â†’ sendFriendRequest; Requested â†’ cancel;
   Following â†’ unfollow sheet) so behaviour matches the rest of the app. */
function getInlineFollowBackButtonHTML(uid) {
  const u = String(uid || '').trim();
  if (!currentUser || !u || u === currentUser.uid) return '';
  if (typeof isPreviewMode === 'function' && isPreviewMode()) return '';
  if (friends.includes(u)) {
    return `<button type="button" class="shelfd-friend-req-btn secondary shelfd-followback-btn is-following" onclick="event.stopPropagation(); toggleFriendListFollow('${escAttr(u)}')">Following</button>`;
  }
  if (outgoingRequests.includes(u)) {
    return `<button type="button" class="shelfd-friend-req-btn secondary shelfd-followback-btn is-requested" aria-label="Requested â€” tap to cancel" onclick="event.stopPropagation(); toggleFriendListFollow('${escAttr(u)}')">Requested</button>`;
  }
  return `<button type="button" class="shelfd-friend-req-btn primary shelfd-followback-btn is-follow" onclick="event.stopPropagation(); toggleFriendListFollow('${escAttr(u)}')">Follow</button>`;
}

/* v11.847: build the "Follow back" rows from pendingFollowBackUids. Prefers the
   already-cached profile (the incoming card loaded it moments ago) so the row
   paints instantly on Accept; only fetches genuinely-unknown uids. */
async function buildFollowBackCards(uids = []) {
  if (!uids.length) return '';
  const unknown = uids.filter(uid => !usersMap[uid] || !usersMap[uid].name);
  if (unknown.length) {
    try {
      const docs = await Promise.all(unknown.map(uid => db.collection("users").doc(uid).get()));
      docs.forEach(doc => { if (doc.exists) usersMap[doc.id] = { uid: doc.id, ...(doc.data() || {}) }; });
    } catch (e) { console.warn('Follow-back profile load failed:', e); }
  }
  let html = '';
  uids.forEach(uid => {
    const u = usersMap[uid] || { uid };
    if (!u.uid) return;
    const fallbackAvatar = '/default-avatar.svg#' + encodeURIComponent(u.name || '?') + '&background=1e2028&color=60a5fa';
    const avatar = u.photo || fallbackAvatar;
    const handle = getShelfdFriendHandle(u, 'User');
    const uidAttr = escAttr(u.uid);
    html += `<div class="shelfd-friend-row shelfd-friend-req-row shelfd-friends-followback-row" data-friend-uid="${uidAttr}" onclick="viewUserFromMap('${uidAttr}')">
      <img class="shelfd-friend-avatar" src="${escAttr(avatar)}" alt="" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${escAttr(fallbackAvatar)}'">
      <div class="shelfd-friend-text">
        <strong class="shelfd-friend-handle">@${escHtml(handle)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(u) : ''}</strong>
      </div>
      <div class="shelfd-friend-req-actions followback-action-slot">${getInlineFollowBackButtonHTML(u.uid)}</div>
    </div>`;
  });
  return html;
}

/* v11.847: repaint a Follow-back row's button in place (no list rebuild) after
   the relationship changes. Hooked into the central syncFollowButtonsForUid hub
   so a Follow/cancel from ANY surface keeps this row in sync. */
function refreshInlineFollowBackButton(uid) {
  const u = String(uid || '').trim();
  if (!u) return;
  document.querySelectorAll(`.shelfd-friends-followback-row[data-friend-uid="${escAttr(u)}"] .followback-action-slot`).forEach(slot => {
    slot.innerHTML = getInlineFollowBackButtonHTML(u);
  });
}
...
/* v11.589: render incoming + sent friend requests inline at the top of the
   ...
   Incoming requests show Confirm/Delete, sent requests show Cancel â€” the exact
   ...
   (commitFriendsDataState â†’ renderFriendsList â†’ renderShelfdFriendsLayout). */
async function hydrateInlineFriendRequests() {
  ...
  /* v11.847: prune the session Follow-back list â€” drop anyone who has since
     become a mutual friend (they now live in the main friends list) or who is
     somehow back in the incoming queue â€” then render the rest as single-button
     "Follow back" rows so an Accept never leaves an empty/vanished spot. */
  const myUid = String(currentUser?.uid || '').trim();
  pendingFollowBackUids = pendingFollowBackUids.filter(uid =>
    uid && uid !== myUid && !friends.includes(uid) && !incomingItems.includes(uid));
  const followBackItems = pendingFollowBackUids.slice();
  /* A Follow-back row you then tap "Follow" on enters outgoingRequests â€” keep it
     ONLY in the Follow-back group (showing Requested), never also duplicated under
     "Sent friend requests". */
  const outgoingItems = (Array.isArray(outgoingRequests) ? outgoingRequests.slice() : [])
    .filter(uid => !pendingFollowBackUids.includes(uid));
  if (!incomingItems.length && !outgoingItems.length && !followBackItems.length) {
    host.hidden = true;
    host.innerHTML = '';
    return;
  }
  const allReqUids = [...new Set([...incomingItems, ...outgoingItems, ...followBackItems])];
  const unknownReqUids = allReqUids.filter(uid => !usersMap[uid] || !usersMap[uid].name);
  if (unknownReqUids.length) {
    try {
      const prewarmDocs = await Promise.all(unknownReqUids.map(uid => db.collection("users").doc(uid).get()));
      prewarmDocs.forEach(doc => { if (doc.exists) usersMap[doc.id] = { uid: doc.id, ...(doc.data() || {}) }; });
    } catch (_) {}
  }
  let incomingCards = '';
  let followBackCards = '';
  ...
  try { if (followBackItems.length) followBackCards = await buildFollowBackCards(followBackItems); } catch (_) { followBackCards = ''; }
  ...
  /* A newer renderFriendsList() may have replaced the container while we awaited
     the Firestore reads â€” bail so we don't write stale cards into a detached node
     (the newer render runs its own hydrate against the fresh container). */
  if (!host.isConnected) return;
  let html = '';
  if (incomingCards) {
    html += `<div class="shelfd-friends-req-group">
      <div class="shelfd-friends-req-heading">Follow requests<span class="shelfd-friends-req-count">${incomingItems.length}</span></div>
      <div class="shelfd-friends-req-list">${incomingCards}</div>
    </div>`;
  }
  if (followBackCards) {
    html += `<div class="shelfd-friends-req-group">
      <div class="shelfd-friends-req-heading">Follow back</div>
      <div class="shelfd-friends-req-list">${followBackCards}</div>
    </div>`;
  }
  if (outgoingCards) {
    html += `<div class="shelfd-friends-req-group">
      <div class="shelfd-friends-req-heading">Sent friend requests</div>
...
let _friendsListRenderSig = '';
function _computeFriendsListSig() {
  return [
    Array.isArray(friends) ? friends.slice().sort().join(',') : '',
    Array.isArray(incomingRequests) ? incomingRequests.slice().sort().join(',') : '',
    Array.isArray(outgoingRequests) ? outgoingRequests.slice().sort().join(',') : '',
    Array.isArray(pendingFollowBackUids) ? pendingFollowBackUids.slice().sort().join(',') : '',
    typeof shelfdFriendsSortMode !== 'undefined' ? shelfdFriendsSortMode : ''
  ].join('|');
}
...
async function renderFriendsList() {
  ...
  /* v12.057: skip redundant renders when friends/requests data hasn't changed
     and the grid already has content. This eliminates the 2-3 sequential
     innerHTML rebuilds that cause visible flashes each time the Friends tab is
     opened. The sig covers every array that affects the rendered output; when
     any of those change (new friend, accepted request, etc.) the sig differs
     and the render proceeds normally. */
  const _sig = _computeFriendsListSig();
  if (_sig && _sig === _friendsListRenderSig && grid.children.length > 0) return;
  if (isPreviewMode()) {
  ...
  if (friends.length === 0) {
    ...
    const hasPendingRequests = (Array.isArray(incomingRequests) && incomingRequests.length > 0)
      ...
      || (Array.isArray(pendingFollowBackUids) && pendingFollowBackUids.length > 0);
  ...
  /* v10.222: IG-style remodel of the friends list â€” search bar, follow
  ...
  function buildFriendsListHtml(profiles) {
    ...
    safeProfiles.forEach(u => {
      ...
      listHtml += `<div class="shelfd-friend-row shelfd-friends-followback-row" data-friend-uid="${escAttr(u.uid)}" data-friend-followed-at-ms="${escAttr(followedAtMs)}" data-friend-filter="${escAttr(filterTokens)}" onclick="viewUserFromMap('${escAttr(u.uid)}')">
        <img class="shelfd-friend-avatar" src="${escAttr(avatar)}" alt="" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${escAttr(fallbackAvatar)}'">
        <div class="shelfd-friend-text">
          <strong class="shelfd-friend-handle">${escHtml(handle)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(u) : ''}</strong>
          ${displayNameHtml}
    ...
    const emptyHtml = `<div class="shelfd-friends-empty"><div class="friends-empty-icon">ðŸ‘¥</div><p>No friends found</p></div>`;
  ...
  function buildCategoriesHtml(profiles) {
    ...
    // signals over current cached profile data â€” Shelfd is a symmetric-friend
  ...
  function renderShelfdFriendsLayout(profiles) {
    _friendsListRenderSig = _sig;
    const listHtml = buildFriendsListHtml(profiles);
    /* v11.589: search bar â†’ inline friend requests â†’ sort header â†’ friend list.
...
/* v10.222: live search filter for the IG-style friends list. Toggles
   display:none on rows whose handle/name don't match â€” no full re-render.
   v10.763: query strips a leading '@' so typing "@johndoe" matches the
   same as "johndoe" â€” what every other social app does. The data-friend-filter
...
/* v10.222: stub category opener â€” UI rows are rendered to match the
...
async function searchUsersByUsername(query) {
  const normalized = typeof normalizeShelfdUserSearchQuery === 'function'
  ...
  try {
    ...
    const addUserRecord = (raw = {}, fallbackUid = '') => {
      const uid = String(raw.uid || fallbackUid || '').trim();
      if (!uid || uid === currentUser?.uid) return;
      const user = { ...raw, uid };
      /* v924: removed `if (user.isPublic === false) return` â€” saveUserProfile
    ...
    const addUserDoc = (doc) => addUserRecord(doc.data() || {}, doc.id || '');

    try {
      const handleMatches = typeof fetchShelfdUsersByHandlePrefix === 'function'
        ? await fetchShelfdUsersByHandlePrefix(normalized, { limit: 20, excludeUid: currentUser?.uid })
        : [];
      handleMatches.forEach(user => addUserRecord(user, user?.uid || ''));
    } catch (handleError) {
      console.warn('Handle-first people search lookup failed:', handleError);
    }

    /* v923: added usernameHandleLower (the actual field written by the
       signup flow â€” the old 'usernameLower' key was never stored),
...
function renderAllUsers(users, query = '') {
  ...
  users.forEach(u => {
    ...
    const handle = getShelfdFriendHandle(u, 'user');
    const usernameText = '@' + handle;
    const safeUid = escAttr(u.uid);

      html += `
        <div class="user-card" data-user-search-uid="${safeUid}" style="justify-content:space-between;">
          <div style="display:flex;align-items:center;gap:14px;flex:1;min-width:0;cursor:pointer;" onclick="viewUserFromMap('${u.uid}')">
            ...
            <div style="min-width:0;">
              <div class="user-card-name">${escHtml(usernameText)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(u) : ''}</div>
            </div>
        </div>
        <span class="people-search-action-slot">${getPeopleSearchActionButtonHTML(u.uid)}</span>
      </div>`;
  });
...
function refilterPeople() {
  ...
  if (inlineInput && inlineGrid && inlineGrid.style.display !== 'none') {
    ...
    if (Array.isArray(allUsersCache) && allUsersCache.length) {
      renderAllUsers(allUsersCache, inlineInput.value || '');
      return;
    }
...
async function sendFriendRequest(uid) {
  ...
  /* Every account uses the same private one-way follow request pipeline.
     The button flips to Requested until the target user accepts. */
  ...
  /* v10.145: live notification trigger â€” recipient gets a notification
  ...
  refilterPeople();
  refreshProfileSocialModal();
  /* v11437: no success toast â€” the Follow â†’ Pending button state change is the
...
async function cancelFriendRequest(uid) {
  ...
  refilterPeople();
  refreshProfileSocialModal();
  /* v11437: no success toast â€” the Pending â†’ Follow button state change is the
     visual confirmation. */
}

// Accept a request someone sent me. This lets them follow/view me; it does not make me follow them back.
async function acceptFriendRequest(uid) {
  if (typeof requireShelfdSignedInAction === 'function' && !requireShelfdSignedInAction()) return;
  const rawBefore = captureOwnFriendRawState();
  const prev = captureFriendsCommitSnapshot();
  ownAcceptedFollowerRequestIds = normalizeFriendUidList([...ownAcceptedFollowerRequestIds, uid]);
  ownRejectedFriendRequestIds = ownRejectedFriendRequestIds.filter(id => id !== uid);
  ownRemovedFriendIds = ownRemovedFriendIds.filter(id => id !== uid);
  /* v11.847: park the requester in the session Follow-back list BEFORE the commit
     so the commit-driven renderFriendsList()/hydrate already paints a single
     Follow/Following button where the Accept/Deny pair was (case 1: I don't follow
     them â†’ Follow; case 2: I already follow them â†’ Following). Skip when I already
     follow them â€” they belong in the main friends list and the hydrate prune keeps
     them out of the Follow-back group. */
  if (uid && !friends.includes(uid) && !pendingFollowBackUids.includes(uid)) {
    pendingFollowBackUids.push(uid);
  }
  commitFriendsDataState(prev, { silent: true, skipSelfRepair: true });
  /* Instant in-place swap as a belt-and-suspenders paint (covers a suppressed
     renderFriendsList or a host that was replaced mid-render) so Accept is never
     a visual no-op like the old behaviour. */
  try {
    const acceptedRow = document.querySelector(`.shelfd-friend-req-row[data-friend-uid="${escAttr(uid)}"]`);
    if (acceptedRow) {
      acceptedRow.classList.add('shelfd-friends-followback-row');
      const actionSlot = acceptedRow.querySelector('.shelfd-friend-req-actions');
      if (actionSlot) {
        actionSlot.classList.add('followback-action-slot');
        actionSlot.innerHTML = getInlineFollowBackButtonHTML(uid);
      }
    }
  } catch (_) {}
  try {
    ...
    const arrayRemove = firebase.firestore.FieldValue.arrayRemove;
    await db.collection("users").doc(currentUser.uid).set({
      acceptedFollowerRequests: arrayUnion(uid),
      rejectedFriendRequests: arrayRemove(uid),
      removedFriends: arrayRemove(uid)
    }, { merge: true });
  } catch(e) {
    console.error("acceptFriendRequest failed:", e);
    pendingFollowBackUids = pendingFollowBackUids.filter(id => id !== uid);
    const rollback = captureFriendsCommitSnapshot();
    restoreOwnFriendRawState(rawBefore);
    commitFriendsDataState(rollback, { silent: true, skipSelfRepair: true });
  ...
  /* v10.145: live notification trigger â€” original requester gets a
  ...
  else if (typeof hydrateInlineFriendRequests === 'function') hydrateInlineFriendRequests();
  refilterPeople();
  refreshProfileSocialModal();
  /* No success toast; the Accept/Deny pair flips to a single Follow-back button
     (Follow / Following), and the requester sees Following once their listener
...
async function rejectFriendRequest(uid) {
  ...
  ownAcceptedFollowerRequestIds = ownAcceptedFollowerRequestIds.filter(id => id !== uid);
  commitFriendsDataState(prev, { silent: true, skipSelfRepair: true });
  try {
    const arrayUnion = firebase.firestore.FieldValue.arrayUnion;
    const arrayRemove = firebase.firestore.FieldValue.arrayRemove;
    await db.collection("users").doc(currentUser.uid).set({
      ...
      acceptedFollowerRequests: arrayRemove(uid)
    }, { merge: true });
  } catch(e) {
  ...
  modal.innerHTML = `
    <div class="plm-sheet">
      <div class="plm-header">
        ...
        <button class="plm-close" onclick="closeRemoveFriendModal()">âœ•</button>
      </div>
...
/* v11.977: REMOVE A FOLLOWER (undo an Accept). Followers are exactly the people
   in MY acceptedFollowerRequests (requests I accepted). Removing one revokes that
   accept: they stop following me, and if they still have a request out to me it
   re-surfaces as a fresh Accept/Deny (the notif card + shelf banner no longer see
   them as "accepted"). This is also how the dev/creator account clears the stale
   followers it accumulated back when it behaved "always public" — there is NO
   auto-accept anywhere now, so once cleared they stay cleared unless re-accepted. */
async function removeFollower(uid) {
  ...
  const rawBefore = captureOwnFriendRawState();
  const prev = captureFriendsCommitSnapshot();
  ownAcceptedFollowerRequestIds = ownAcceptedFollowerRequestIds.filter(id => id !== u);
  pendingFollowBackUids = pendingFollowBackUids.filter(id => id !== u);
  commitFriendsDataState(prev, { silent: true, skipSelfRepair: true });
  /* instant feedback: drop their row from the open Followers panel */
  try {
    const sel = (window.CSS && CSS.escape) ? CSS.escape(u) : u;
    document.querySelectorAll(`#profile-social-modal .profile-social-panel[data-kind="followers"] .profile-social-row[data-uid="${sel}"]`).forEach(r => r.remove());
  } catch (_) {}
  try {
    const arrayRemove = firebase.firestore.FieldValue.arrayRemove;
    await db.collection("users").doc(currentUser.uid).set({
      acceptedFollowerRequests: arrayRemove(u)
    }, { merge: true });
  } catch (e) {
    console.error("removeFollower failed:", e);
    const rollback = captureFriendsCommitSnapshot();
    restoreOwnFriendRawState(rawBefore);
    commitFriendsDataState(rollback, { silent: true, skipSelfRepair: true });
  ...
  updateRequestsBadges();
  updateFriendsCountBadge();
  if (typeof refreshProfileSocialModal === 'function') refreshProfileSocialModal();
...
/* v11.982: Instagram-style "Remove follower?" bottom sheet (avatar + title +
   subtitle + red Remove, with a separate Cancel pill). Remove → removeFollower()
   takes them out of MY followers only (does NOT unfollow them). */
function confirmRemoveFollower(uid) {
  if (typeof requireShelfdSignedInAction === 'function' && !requireShelfdSignedInAction()) return;
  const u = String(uid || '').trim();
  if (!u) return;
  const src = (usersMap && usersMap[u]) || { uid: u };
  const handle = (typeof getShelfdFriendHandle === 'function')
    ? getShelfdFriendHandle(src, 'User')
    : String(src.usernameHandle || src.name || 'this user').trim().replace(/^@+/, '');
  const fallbackAvatar = '/default-avatar.svg#' + encodeURIComponent(src.name || handle || '?') + '&background=1e2028&color=60a5fa';
  const avatar = src.photo || fallbackAvatar;
  const existing = document.getElementById('remove-follower-sheet');
  if (existing) existing.remove();
  const sheet = document.createElement('div');
  sheet.id = 'remove-follower-sheet';
  sheet.className = 'remove-follower-sheet-overlay';
  sheet.setAttribute('role', 'dialog');
  sheet.innerHTML = `
    <div class="remove-follower-sheet">
      <div class="remove-follower-sheet-card">
        <img class="remove-follower-sheet-avatar" src="${escAttr(avatar)}" alt="" decoding="async" onerror="this.onerror=null;this.src='${escAttr(fallbackAvatar)}'">
        <div class="remove-follower-sheet-title">Remove follower?</div>
        <div class="remove-follower-sheet-sub">We won't tell <strong>${escHtml(handle)}</strong> they were removed from your followers.</div>
        <button type="button" class="remove-follower-sheet-remove" onclick="removeFollower('${escAttr(u)}'); closeRemoveFollowerSheet();">Remove</button>
      </div>
      <button type="button" class="remove-follower-sheet-cancel" onclick="closeRemoveFollowerSheet()">Cancel</button>
    </div>`;
  sheet.addEventListener('click', e => { if (e.target === sheet) closeRemoveFollowerSheet(); });
  document.body.appendChild(sheet);
  requestAnimationFrame(() => requestAnimationFrame(() => sheet.classList.add('is-open')));
}
window.confirmRemoveFollower = confirmRemoveFollower;

function closeRemoveFollowerSheet() {
  const sheet = document.getElementById('remove-follower-sheet');
  if (!sheet) return;
  sheet.classList.remove('is-open');
  window.setTimeout(() => { if (sheet.isConnected) sheet.remove(); }, 260);
}
window.closeRemoveFollowerSheet = closeRemoveFollowerSheet;
...
async function removeFriend(uid) {
  ...
  /* v11.979: COMPLETE the unfollow on the OTHER side too. "I follow them" is
     recorded as MY uid living in THEIR `acceptedFollowerRequests` (they accepted
     my request). Editing only my own doc left that stale, so re-following re-derived
     as mutual instantly ("Following" instead of "Requested"). Remove my own uid
     from their accepted-followers so the follow is fully torn down and a future
     follow has to be requested + accepted again. Best-effort: a non-fatal
     cross-write (needs the Firestore rule that lets a user arrayRemove THEIR OWN
     uid from another user's acceptedFollowerRequests). */
  try {
    const arrayRemove = firebase.firestore.FieldValue.arrayRemove;
    await db.collection("users").doc(uid).set({
      acceptedFollowerRequests: arrayRemove(currentUser.uid)
    }, { merge: true });
  } catch (crossErr) {
    console.warn('[v11.979] unfollow cross-clear blocked (add the self-arrayRemove rule):', crossErr && crossErr.message ? crossErr.message : crossErr);
  }
  updateFriendsCountBadge();
  if (activeFriendsTab === 'friends') renderFriendsList();
  refilterPeople();
  refreshProfileSocialModal();
  if (typeof syncFollowButtonsForUid === 'function') syncFollowButtonsForUid(uid);
  /* v11437: no success toast â€” the Following â†’ Follow button state change is the
     visual confirmation. */
}

/* Viewing-banner follow button states represent only my outgoing relationship:
   Follow, Requested, or Following. Incoming requests are accepted/denied from
   the Follow Requests and Notifications surfaces. */
...
function getFriendListFollowButtonHTML(uid) {
  ...
  if (outgoingRequests.includes(u)) {
    return `<button type="button" class="friend-list-follow-btn is-requested" onclick="toggleFriendListFollow('${escAttr(u)}')">Requested</button>`;
  }
  return `<button type="button" class="friend-list-follow-btn is-follow" onclick="toggleFriendListFollow('${escAttr(u)}')">Follow</button>`;
}

function getPeopleSearchActionButtonHTML(uid) {
  const u = String(uid || '').trim();
  if (!currentUser || !u || u === currentUser.uid) return '';
  if (typeof isPreviewMode === 'function' && isPreviewMode()) return '';
  if (friends.includes(u)) {
    return `<button type="button" class="friend-action-btn friend-list-follow-btn is-following" onclick="event.stopPropagation(); openUserProfile('${escAttr(u)}')">Following</button>`;
  }
  if (outgoingRequests.includes(u)) {
    return `<button type="button" class="friend-action-btn friend-pending-btn" onclick="event.stopPropagation(); toggleFriendListFollow('${escAttr(u)}')" title="Tap to cancel">Requested</button>`;
  }
  return `<button type="button" class="friend-action-btn friend-add-btn" onclick="event.stopPropagation(); toggleFriendListFollow('${escAttr(u)}')">Follow</button>`;
}

function refreshPeopleSearchFollowButton(uid) {
  const u = String(uid || '').trim();
  if (!u) return;
  document.querySelectorAll(`.user-card[data-user-search-uid="${escAttr(u)}"] .people-search-action-slot`).forEach(slot => {
    slot.innerHTML = getPeopleSearchActionButtonHTML(u);
  });
}
...
async function toggleFriendListFollow(uid) {
  ...
  /* v11376: on the shelf/profile banner, tapping "Following" must NOT instantly
     unfollow â€” it opens an Instagram-style confirm sheet whose only action is
  ...
  let action = null;
  if (outgoingRequests.includes(u)) action = (typeof cancelFriendRequest === 'function') ? cancelFriendRequest : null;
...
// View another user's list â€” open to everyone (v11344; was mutual-friends only)
function buildFriendListSocialCountsHtml(uid, sourceUser = {}, shelfPrivate = false) {
  const u = String(uid || sourceUser?.uid || '').trim();
  if (!u) return '';
  const followersCount = (typeof getSocialIdsForProfile === 'function') ? getSocialIdsForProfile({ ...sourceUser, uid: u }, 'followers').length : 0;
  const followingCount = (typeof getSocialIdsForProfile === 'function') ? getSocialIdsForProfile({ ...sourceUser, uid: u }, 'following').length : 0;
  const separator = '<span class="friend-list-social-sep" aria-hidden="true">&middot;</span>';
  if (shelfPrivate) {
    return `<span class="friend-list-social-count is-locked"><strong>${followersCount.toLocaleString('en-US')}</strong> followers</span>
        ${separator}
        <span class="friend-list-social-count is-locked"><strong>${followingCount.toLocaleString('en-US')}</strong> following</span>`;
  }
  const safeUid = escAttr(u);
  return `<button type="button" class="friend-list-social-count" onclick="openShelfUserSocialPage('${safeUid}','followers')"><strong>${followersCount.toLocaleString('en-US')}</strong> followers</button>
        ${separator}
        <button type="button" class="friend-list-social-count" onclick="openShelfUserSocialPage('${safeUid}','following')"><strong>${followingCount.toLocaleString('en-US')}</strong> following</button>`;
}

function refreshViewingUserSocialCounts() {
  if (!viewingUser?.uid) return;
  const row = document.querySelector('.friend-list-viewing-banner .friend-list-social-counts');
  if (!row) return;
  const uid = String(viewingUser.uid || '').trim();
  ...
  row.innerHTML = buildFriendListSocialCountsHtml(uid, viewingUser, shelfPrivate);
}

function resetFriendShelfScrollToTop() {
  const myListView = document.getElementById('mylist-view');
  const mainContent = myListView?.querySelector('.main-content');
  const scrollRoot = document.scrollingElement || document.documentElement;
  [myListView, mainContent, scrollRoot, document.documentElement, document.body].forEach(el => {
    if (!el) return;
...
function scheduleFriendShelfScrollReset() {
  resetFriendShelfScrollToTop();
  requestAnimationFrame(() => {
    resetFriendShelfScrollToTop();
    requestAnimationFrame(resetFriendShelfScrollToTop);
  });
}
...
async function viewUserList(uid, name, photo, options = {}) {
  ...
  /* Load the target shell first; the render layer below decides whether the
     shelf content is visible or replaced by the private-account lock. */
  ...
  // Load their profile + list in parallel; non-following viewers get the locked
  // private shelf state after the profile shell is painted.
  let userDoc = null;
  ...
  /* v11380: bail if a newer open / a back-out happened during the load above â€”
  ...
  /* v11380: a back-out / different open during the sort above invalidates this
     load â€” abort before painting the friend shelf so it can't override the own
  ...
  /* Private-account gate. When the viewer is not following this user through an
     accepted request, counts become non-interactive and the list area locks. */
  const shelfPrivate = (typeof isShelfUserShelfPrivate === 'function')
    ? isShelfUserShelfPrivate(uid)
    : !(Array.isArray(friends) && friends.includes(uid));
  let socialCountsHtml = shelfPrivate
    ? `<span class="friend-list-social-count is-locked"><strong>${followersCount.toLocaleString('en-US')}</strong> followers</span>
        <span class="friend-list-social-sep" aria-hidden="true">Â·</span>
        <span class="friend-list-social-count is-locked"><strong>${followingCount.toLocaleString('en-US')}</strong> following</span>`
  ...
  if (typeof buildFriendListSocialCountsHtml === 'function') {
    socialCountsHtml = buildFriendListSocialCountsHtml(uid, sourceUser, shelfPrivate);
  }
  if (bannerArea) bannerArea.innerHTML = `<div class="viewing-banner friend-list-viewing-banner">
    ${getShelfFollowRequestBannerHTML(uid)}
    <div class="viewing-user-profile-center">
      ...
      ${friendDisplayName ? `<div class="viewing-user-display-name">${escHtml(friendDisplayName)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(sourceUser) : ''}</div>` : ''}
    ...
    <button class="friend-list-floating-back-btn" type="button" onclick="backToMyList()" aria-label="Back">â€¹</button>
  </div>`;
  if (typeof hydrateDerivedFollowerIdsForProfile === 'function') {
    hydrateDerivedFollowerIdsForProfile(sourceUser, { refresh: false })
      .then(() => {
        if (viewingUser?.uid !== uid) return;
        refreshViewingUserSocialCounts();
      })
      .catch(() => {});
  }
  ...
  render();
  scheduleFriendShelfScrollReset();
  bindFriendShelfSwipeBack();
  await startFriendHomeEnterTransition(options);
  scheduleFriendShelfScrollReset();
  persistUiState();
  /* v11.979: re-evaluate the "wants to follow you" Confirm/Delete banner a few
     times after the shelf settles. On iOS cold-launch from a push, the friends /
     notifications listeners can report AFTER viewUserList paints, so the first
     inline render misses the pending request; these catch it once data lands. */
  if (typeof syncShelfFollowRequestBanner === 'function') {
    [250, 800, 1800].forEach(ms => window.setTimeout(() => {
      try { if (viewingUser && String(viewingUser.uid) === uid) syncShelfFollowRequestBanner(); } catch (_) {}
    }, ms));
  }
...
/* v11.973: when you open a user's shelf (e.g. by tapping their "wants to follow
   you" push), a mini Confirm/Delete banner sits at the top of the viewing banner
   — above the profile picture, below the header — IF that user has a pending
   incoming follow request to you. Confirm writes the accept (they now follow
   you); Delete declines. Either way the mini banner disappears. It's the only
   place this banner lives; the action reuses acceptFriendRequest/rejectFriendRequest
   so the data write matches every other accept surface. */
function shelfHasUnresolvedFollowRequest(uid) {
  const u = String(uid || '').trim();
  if (!u) return false;
  /* v11.974: prefer the robust determiner (a friend_request notification is a
     reliable signal even when the incoming-requests derived query lags on iOS);
     fall back to the derived list if that helper isn't loaded yet. */
  if (typeof shelfdActorRequestedMeUnresolved === 'function') return shelfdActorRequestedMeUnresolved(u);
  return Array.isArray(incomingRequests) && incomingRequests.includes(u);
}

function getShelfFollowRequestBannerHTML(uid) {
  const u = String(uid || '').trim();
  if (!u || !currentUser || u === currentUser.uid) return '';
  if (typeof isPreviewMode === 'function' && isPreviewMode()) return '';
  if (!shelfHasUnresolvedFollowRequest(u)) return '';
  const src = (viewingUser && String(viewingUser.uid) === u) ? viewingUser : (usersMap[u] || { uid: u });
  const handle = (typeof getShelfdFriendHandle === 'function')
    ? getShelfdFriendHandle(src, 'User')
    : String(src.usernameHandle || src.username || src.name || 'User').trim().replace(/^@+/, '');
  return `<div class="shelf-follow-request-banner" data-shelf-follow-request-uid="${escAttr(u)}">
    <span class="shelf-follow-request-text">${escHtml(handle)} wants to follow you</span>
    <div class="shelf-follow-request-actions">
      <button type="button" class="shelf-follow-request-btn confirm" onclick="event.stopPropagation(); confirmShelfFollowRequest('${escAttr(u)}')">Confirm</button>
      <button type="button" class="shelf-follow-request-btn delete" onclick="event.stopPropagation(); deleteShelfFollowRequest('${escAttr(u)}')">Delete</button>
    </div>
  </div>`;
}

function removeShelfFollowRequestBanner() {
  document.querySelectorAll('.shelf-follow-request-banner').forEach(el => el.remove());
}

/* Keep the shelf follow-request banner in sync with live state: show it while
   viewing a user who has a pending request to me, hide it otherwise. Called from
   updateRequestsBadges so it appears the moment incomingRequests loads and
   vanishes the instant the request is accepted/declined from anywhere. */
function syncShelfFollowRequestBanner() {
  const banner = document.querySelector('.friend-list-viewing-banner');
  const viewingUid = (document.body.classList.contains('viewing-other-user') && viewingUser)
    ? String(viewingUser.uid || '').trim() : '';
  const shouldShow = !!viewingUid
    && !!currentUser
    && viewingUid !== currentUser.uid
    && shelfHasUnresolvedFollowRequest(viewingUid)
    && !(typeof isPreviewMode === 'function' && isPreviewMode());
  const existing = banner ? banner.querySelector('.shelf-follow-request-banner') : null;
  if (!shouldShow) {
    if (existing) existing.remove();
    return;
  }
  if (existing || !banner) return;
  const html = getShelfFollowRequestBannerHTML(viewingUid);
  if (html) banner.insertAdjacentHTML('afterbegin', html);
}

async function confirmShelfFollowRequest(uid) {
  const u = String(uid || '').trim();
  if (!u) return;
  removeShelfFollowRequestBanner();
  try { if (typeof acceptFriendRequest === 'function') await acceptFriendRequest(u); }
  catch (e) { console.warn('confirmShelfFollowRequest failed:', e && e.message ? e.message : e); }
}

async function deleteShelfFollowRequest(uid) {
  const u = String(uid || '').trim();
  if (!u) return;
  removeShelfFollowRequestBanner();
  /* v11.978: also clear the matching friend_request notification card so declining
     from the shelf removes it everywhere, not just the banner. */
  if (typeof deleteFriendRequestNotificationForActor === 'function') deleteFriendRequestNotificationForActor(u);
  try { if (typeof rejectFriendRequest === 'function') await rejectFriendRequest(u); }
  catch (e) { console.warn('deleteShelfFollowRequest failed:', e && e.message ? e.message : e); }
  /* belt-and-suspenders: the reject marks me as having declined, so the determiner
     now returns false — make sure the banner can't be re-added by a later sync. */
  if (typeof syncShelfFollowRequestBanner === 'function') { try { syncShelfFollowRequestBanner(); } catch (_) {} }
}
...
/* v11360: dedicated, finger-tracking left-edge swipe-back for the friend's
   ...
   Modeled on the canonical bindDiscoverMediaProfileSwipeBack:
     â€¢ left-edge (â‰¤28px) engage, 14px horizontal threshold (beats vertical 1.3Ã—)
     â€¢ rAF-batched composited translate3d (ProMotion/120Hz-safe)
     â€¢ pins #mylist-view as a fixed overlay + reveals the friends page behind it
     â€¢ commit at 34% width OR a fast flick â†’ slide off + backToMyList(skipSlide)
     â€¢ otherwise spring back to rest
...
function bindFriendShelfSwipeBack() {
  ...
  /* SMOOTHNESS NOTE (v11361 rebuild): the previous version pinned #mylist-view
     position:fixed on engage â€” that forced a synchronous reflow AND a vertical
     ...
     a pure composited transform â€” no reflow, no scroll jump, no per-frame
  ...
  /* Reveal WHATEVER page you came from behind the sliding shelf (universal â€”
     friends list, activity, discover, games/anime/music discover, â€¦), resolved
  ...
  const beginDrag = () => {
    ...
    /* shelf stays IN FLOW (relative, no offset â†’ no scroll-jump) but gets z:2 so
  ...
  /* Anywhere-swipe back: NOT finger-tracked. Once a clear leftâ†’right swipe is
  ...
  const handleDown = (event) => {
    ...
    /* never hijack text inputs. Buttons/links/cards are allowed â€” a tap still
  ...
  const handleMove = (event) => {
    ...
    if (!engaged) {
      if (edgeZone) {
        ...
        /* ANYWHERE: not finger-tracked â€” a clear leftâ†’right swipe (distance or a
        ...
        if (triggered) {
          ...
          armed = false; // vertical â†’ leave it to native scroll
        }
        return;
      }
    }
...
/* v10.995: smooth 400ms slide-off-right when leaving someone's My List.
   Previously there was NO exit animation â€” the friend view just snapped
   ...
   restores visibility. Transform + opacity only â†’ ProMotion/120Hz-safe
...
async function backToMyList(targetTab = null, options = {}) {
  ...
  if (skipSlide) {
    ...
    /* the swipe revealed the previous page as a fixed z-index:-1 backdrop â€” strip
    ...
    document.body.classList.remove('friend-shelf-swiping');
    /* v11365: the revealed page is already current â€” suppress the friends-list
  ...
  const exitTask = (async () => {
    ...
    /* v11380 â€” OWNERSHIP FIX: repaint #mylist-view with MY OWN data right now,
...
function syncViewingUserHeaderBackButton(enabled = false) {
  const dedicatedBackBtn = document.getElementById('friend-shelf-header-back-btn');
  if (dedicatedBackBtn) {
    dedicatedBackBtn.hidden = !enabled;
    dedicatedBackBtn.style.display = enabled ? 'inline-flex' : '';
    dedicatedBackBtn.setAttribute('onclick', 'backToMyList()');
    dedicatedBackBtn.setAttribute('aria-label', 'Back');
    dedicatedBackBtn.setAttribute('title', 'Back');
  }
  ...
  if (enabled) {
    ...
    /* v10.266: instead of just text-swapping the cyan Import pill, also add
       ...
       cyan look. v10.995: dropped the "Back" text label â€” match the
       40Ã—40 circle arrow-only back button used on the viewing-other
...
function startFriendProfileListEnterTransition() {
  ...
  try {
    const headerEl = document.querySelector('.header');
    headerBottom = headerEl ? Math.max(0, Math.round(headerEl.getBoundingClientRect().bottom)) : 0;
  } catch (_) {
    headerBottom = 0;
  }
  ...
  myListView.style.position = 'fixed';
  myListView.style.inset = '0';
  if (headerBottom > 0) myListView.style.top = `${headerBottom}px`;
  myListView.style.zIndex = '2400';
...
function startFriendHomeEnterTransition(options = {}) {
  ...
  /* v11358: the slide-in pins #mylist-view position:fixed, which detaches it
     from the top header and the body's safe-area padding â€” so its content
