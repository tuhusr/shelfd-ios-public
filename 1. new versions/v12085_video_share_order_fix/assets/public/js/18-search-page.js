...
(function() {
  ...
  const FILTER_RESULT_BATCH_SIZE = 15;
  const FILTER_RESULT_FETCH_PAGE_SIZE = 25;
  const PRESET_RESULT_LIMIT = 30;
  const PRESET_GAME_RESULT_LIMIT = 21;
  const PRESET_PAGE_COUNT = 2;
  const PRESET_REFINE_INITIAL = 21;
  const PRESET_REFINE_BATCH = 12;
  const PRESET_REFINE_PAGE_SIZE = 3;
  ...
  /* v12.003: per-results-panel config store so the "Filter" refine sheet can
     read the panel's base preset config and re-run it with a `refine` spec. */
  const presetResultConfigByPanel = new Map();
  const presetResultBuffer = new Map();
  ...
  function normalizeTmdbItem(item) {
    ...
    const pushAliasDeep = (val) => {
      if (!val) return;
      if (Array.isArray(val)) {
        val.forEach(pushAliasDeep);
        return;
      }
      if (typeof val === 'object') {
        [
          ...
          val.data?.title,
          val.data?.name,
          val.data?.english_title
        ].forEach(pushAliasDeep);
        return;
      }
      pushAlias(val);
    };
    if (isJikan) {
      ...
      pushAliasDeep(item.titles);
    } else {
      if (isMovie) pushAlias(item.original_title);
      else pushAlias(item.original_name);
      pushAliasDeep(item.__shelfdAliases);
      pushAliasDeep(item.alternative_titles?.titles);
      pushAliasDeep(item.alternative_titles?.results);
      pushAliasDeep(item.translations?.translations);
    }
    return {
  ...
  function getSearchPresetMediaType() {
    const filter = String(activeFilter || 'all').trim().toLowerCase();
    if (filter === 'movie' || filter === 'tv' || filter === 'anime' || filter === 'game') return filter;
    return 'mixed';
  }

  function getPresetHubMediaType() {
    return activePresetSurface === 'discover' ? getDiscoverPresetMediaType() : getSearchPresetMediaType();
  }
  ...
  function renderPresetPanelShell({ title, eyebrow = 'Discovery', subtitle = '', body = '', className = '', filterPanelId = '' } = {}) {
    /* v12.003: results panels of filterable presets get a working refine button
       (shelfdOpenPresetResultsFilter); every other panel keeps the legacy
       discover-surface filter button, which CSS hides unless .is-discover-filterable. */
    const filterBtn = filterPanelId
      ? `<button type="button" class="shelfd-search-preset-filter-btn is-preset-results-filter" onclick="shelfdOpenPresetResultsFilter('${escAttr(filterPanelId)}')" aria-label="Filter results">
      ...
      : `<button type="button" class="shelfd-search-preset-filter-btn" onclick="openDiscoverUniversalSearchFilterSheet()" aria-label="Filter Discovery search">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M7 12h10M10 17h4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
            <span>Filter</span>
          </button>`;
    return `
  ...
  function openPresetPanel(panel, surface = activePresetSurface) {
    ...
    if (surface === 'discover') {
      const source = typeof window.getDiscoverUniversalSearchSource === 'function'
        ? window.getDiscoverUniversalSearchSource()
        : '';
      el.classList.toggle('is-discover-filterable', source === 'movie' || source === 'tv');
    }
  ...
  function applyPresetSurfaceScope(config = {}, surface = activePresetSurface) {
    const scopedType = surface === 'discover' ? getDiscoverPresetMediaType() : getSearchPresetMediaType();
    return {
      ...config,
  ...
  function openPresetRoot(key = '', surface = activePresetSurface) {
    ...
    if (key === 'country') {
      openPresetPanel({
        ...
        html: renderPresetPanelShell({
          title: 'Country',
          subtitle: 'Browse titles by origin country.',
          body: renderOptionList(PRESET_COUNTRY_OPTIONS, { action: 'country-option' })
        })
      }, surface);
      return;
    }
    if (key === 'language') {
      openPresetPanel({
        ...
        html: renderPresetPanelShell({
          title: 'Language',
          subtitle: 'Browse titles by original language.',
          body: renderOptionList(PRESET_LANGUAGE_OPTIONS, { action: 'language-option' })
        })
      }, surface);
      return;
    }
  ...
  function handlePresetPanelClick(event) {
    ...
    if (action === 'release-option') {
      ...
      if (key === 'upcoming') {
        openPresetResultsPanel({
          title: option.label,
          preset: 'upcoming',
          subtitle: isGamePreset ? 'Upcoming games ranked by RAWG engagement.' : 'Upcoming movies, TV, and anime-style titles.'
        }, surface);
      } else if (key.startsWith('decade:')) {
        const decadeStart = Number(key.split(':')[1]);
        const currentYear = new Date().getFullYear();
        const decadeEnd = Math.min(decadeStart + 9, currentYear);
        const years = [];
        for (let yr = decadeEnd; yr >= decadeStart; yr--) years.push(yr);
        openPresetPanel({
          id: `decade-years-${decadeStart}`,
          html: renderPresetPanelShell({
            title: option.label,
            eyebrow: 'Discovery',
            subtitle: `Browse titles released from ${decadeStart}–${decadeEnd}.`,
            body: renderOptionList(
              years.map(yr => ({ key: `year:${yr}`, label: String(yr) })),
              { action: 'release-year-option' }
            )
          })
        }, surface);
      }
    } else if (action === 'release-year-option') {
      const key = actionBtn.dataset.presetKey || '';
      ...
      openPresetResultsPanel({
        ...
        preset: 'year',
        yearValue: yr,
        subtitle: `Titles released in ${yr}.`
      }, surface);
    ...
    } else if (action === 'country-option') {
      const country = PRESET_COUNTRY_OPTIONS.find(item => String(item.key) === String(actionBtn.dataset.presetKey || ''));
      if (!country) return;
      openPresetResultsPanel(applyPresetSurfaceScope({
        title: country.label,
        preset: 'country',
        countryCode: country.key,
        subtitle: `Titles from ${country.label}.`
      }, surface), surface);
    } else if (action === 'language-option') {
      const language = PRESET_LANGUAGE_OPTIONS.find(item => String(item.key) === String(actionBtn.dataset.presetKey || ''));
      if (!language) return;
      openPresetResultsPanel(applyPresetSurfaceScope({
        title: language.label,
        preset: 'language',
        languageCode: language.key,
        subtitle: `${language.label}-language titles.`
      }, surface), surface);
  ...
  function openPresetResultsPanel(config = {}, surface = activePresetSurface) {
    ...
    const panelId = `results:${config.preset || 'custom'}:${config.title || Date.now()}`.replace(/[^a-zA-Z0-9:_-]/g, '-');
    presetResultConfigByPanel.set(panelId, scopedConfig);
    /* v12.003: only the search-surface results pages of filterable presets get
       the working refine Filter button (not Most Popular / Most Anticipated, and
       not game/music media types). */
    const filterPanelId = (surface === 'search' && isPresetResultsFilterable(scopedConfig)) ? panelId : '';
    openPresetPanel({
      id: panelId,
      html: renderPresetPanelShell({
  ...
  async function fetchPresetTmdbRange(path, params, type, startPage, count) {
    const pages = Array.from({ length: Math.max(1, count) }, (_, i) => startPage + i);
    const settled = await Promise.allSettled(pages.map(async page => {
      if (typeof window.fetchTmdbProxy === 'function') {
        const res = await window.fetchTmdbProxy(path, { ...params, page: String(page) });
        if (!res.ok) return [];
        const json = await res.json();
        return (json.results || []).filter(it => it?.id && it?.poster_path);
      }
      return [];
    }));
    return settled
      ...
      .flatMap(r => r.value || [])
      .map(it => markPresetMediaType(it, type));
  }
  ...
  async function fetchGamePresetDirect(config = {}) {
    ...
    if (preset === 'game-genre') {
      ...
      const yr = Number(config.yearValue || 0);
      if (yr) {
        rawgPromise = fetchPresetRawgPages({
          ...baseParams,
          dates: `${yr}-01-01,${yr}-12-31`
        }, PRESET_POPULAR_PAGE_COUNT).catch(error => {
          console.warn('RAWG year games preset failed; using IGDB fallback if available.', error);
          return [];
        });
        igdbConfig = { ...igdbConfig, fromDate: `${yr}-01-01`, toDate: `${yr}-12-31` };
      }
    } else {
  ...
  async function fetchPresetDirect(config = {}) {
    ...
    if (hasActivePresetRefine(config)) return fetchPresetRefinedDirect(config);
    if (preset === 'rated') {
      let ratedCandidates = [];
    ...
    if ((preset === 'anticipated' || preset === 'upcoming') && typeof window.fetchAndRankAnticipated === 'function') {
      const anticipatedScope = requestedMediaType === 'anime' ? 'anime' : (requestedMediaType === 'movie' || requestedMediaType === 'tv' ? requestedMediaType : 'mixed');
      return window.fetchAndRankAnticipated(PRESET_RESULT_LIMIT, PRESET_PAGE_COUNT, anticipatedScope);
    }
    ...
    if (preset === 'year') {
      ...
      if (includeTv) requests.push(fetchPresetTypedTmdb('discover/tv', {
        ...baseParams,
        'first_air_date.gte': `${yr}-01-01`,
        'first_air_date.lte': `${yr}-12-31`
      }, 'tv'));
      return enrichAndRankPresetItems((await Promise.all(requests)).flat(), 'popular', requestedMediaType);
    }
    ...
    if (preset === 'country' || preset === 'language') {
      const countryCode = String(config.countryCode || '').trim().toUpperCase();
      const languageCode = String(config.languageCode || '').trim().toLowerCase();
      const scopedParams = {
        ...baseParams,
        ...(preset === 'country' && countryCode ? { with_origin_country: countryCode } : {}),
        ...(preset === 'language' && languageCode ? { with_original_language: languageCode } : {})
      };
      if (includeMovie) requests.push(fetchPresetTypedTmdb('discover/movie', scopedParams, 'movie'));
      if (includeTv) requests.push(fetchPresetTypedTmdb('discover/tv', scopedParams, 'tv'));
      return enrichAndRankPresetItems((await Promise.all(requests)).flat(), 'popular', requestedMediaType);
    }

    if (preset === 'service') {
      if (!config.providerId) return fetchPresetDirect({ ...config, preset: 'popular' });
      if (includeMovie) requests.push(fetchPresetTypedTmdb('discover/movie', {
        ...baseParams,
  ...
  async function loadPresetResults(config = {}, panelId = '') {
    ...
    if (panelId) presetResultConfigByPanel.set(panelId, config);
    if (panelId) syncPresetFilterButtonState(panelId);
    const selector = `[data-preset-results="${CSS.escape(String(panelId))}"]`;
    const target = () => document.querySelector(selector);
    const isFiltered = hasActivePresetRefine(config);
    if (isFiltered && panelId) {
      presetResultBuffer.set(panelId, {
        items: [], visible: PRESET_REFINE_INITIAL, config,
        nextTmdbPage: 1 + PRESET_REFINE_PAGE_SIZE,
    ...
    try {
      ...
      if (!root) return;
      if (!items?.length) {
        root.innerHTML = `<div class="shelfd-search-preset-empty-state">No titles found for this filter right now.</div>`;
        return;
      }
      if (isFiltered && panelId) {
        const buf = presetResultBuffer.get(panelId);
        if (buf) {
          buf.items = items;
          if (items.length <= PRESET_REFINE_INITIAL) buf.exhausted = true;
        }
        root.innerHTML = renderPresetResultGridPaged(panelId);
      } else {
        root.innerHTML = renderPresetResultGrid(items);
      }
    } catch (error) {
      console.error('Search preset results failed:', config, error);
      if (panelId) presetResultBuffer.delete(panelId);
      const root = target();
  ...
  function renderPresetResultGridPaged(panelId) {
    const buf = presetResultBuffer.get(panelId);
    if (!buf) return '';
    const shown = buf.items.slice(0, buf.visible);
    const hasMore = (buf.visible < buf.items.length) || !buf.exhausted;
    const gridHtml = `<div class="shelfd-search-preset-grid">${shown.map(renderPresetResultCard).join('')}</div>`;
    if (!shown.length) return `<div class="shelfd-search-preset-empty-state">No titles found for this filter right now.</div>`;
    if (!hasMore) return gridHtml;
    const btnLabel = buf.loading ? 'Loading…' : 'Load more';
    return gridHtml + `<div class="preset-load-more-wrap"><button type="button" class="preset-load-more-btn" onclick="shelfdLoadMorePresetResults('${escAttr(panelId)}')"${buf.loading ? ' disabled' : ''}>${btnLabel}</button></div>`;
  }

  /* ============================================================
     v12.003 — Preset results "Filter" refine sheet
     Top-right Filter button on filterable preset results pages
     (every preset except Most Popular / Most Anticipated, and not
     for game/music media types). Opens a bottom sheet to refine the
     current results by sort + genre / service / country / language /
     release window; Apply re-runs loadPresetResults with a `refine`
     spec routed through fetchPresetRefinedDirect (one combined
     TMDB /discover query).
     ============================================================ */
  const PRESET_NONFILTERABLE_PRESETS = new Set(['upcoming', 'anticipated']);
  const PRESET_REFINE_SORT_OPTIONS = [
    { key: 'popular', label: 'Most Popular' },
    { key: 'rated', label: 'Highest Rated' },
    { key: 'newest', label: 'Newest' },
    { key: 'oldest', label: 'Oldest' }
  ];
  const PRESET_REFINE_RELEASE_OPTIONS = [
    { key: 'any', label: 'Any time' },
    { key: 'year', label: 'This year' },
    { key: 'recent5', label: 'Last 5 years' },
  ...
  function presetCssEscape(value) {
    return (window.CSS && CSS.escape) ? CSS.escape(String(value)) : String(value).replace(/["\\]/g, '\\$&');
  }

  function isPresetResultsFilterable(config = {}) {
    const preset = String(config.preset || '');
    const mt = String(config.mediaType || 'mixed').trim().toLowerCase();
    if (PRESET_NONFILTERABLE_PRESETS.has(preset)) return false;
    if (mt === 'game' || mt === 'music') return false;
    return true;
  }

  function defaultRefineForConfig(config = {}) {
    const preset = String(config.preset || '');
    const refine = { genres: [], providers: [], country: '', language: '', releaseKey: 'any', sortBy: 'popular' };
    if (preset === 'genre' && config.genreId) refine.genres = [String(config.genreId)];
    if (preset === 'service' && config.providerId) refine.providers = [String(config.providerId)];
    if (preset === 'country' && config.countryCode) refine.country = String(config.countryCode).toUpperCase();
    if (preset === 'language' && config.languageCode) refine.language = String(config.languageCode).toLowerCase();
    if (preset === 'decade' && config.decadeStart) refine.releaseKey = `decade:${config.decadeStart}`;
    if (preset === 'rated') refine.sortBy = 'rated';
    return refine;
  }

  function refineIsNonDefault(refine = null) {
    if (!refine) return false;
    return !!(
      (refine.genres && refine.genres.length) ||
      (refine.providers && refine.providers.length) ||
      refine.country ||
      refine.language ||
      (refine.releaseKey && refine.releaseKey !== 'any') ||
      (refine.sortBy && refine.sortBy !== 'popular')
    );
  }

  function hasActivePresetRefine(config = {}) {
    return !!(config && config.refine);
  }

  function getRefineReleaseRange(key = 'any') {
    if (!key || key === 'any') return {};
    if (key === 'year') return { gte: getPresetStartOfCurrentYear(), lte: getPresetDate(0) };
    if (key === 'recent5') {
      const d = new Date(); d.setHours(0, 0, 0, 0); d.setFullYear(d.getFullYear() - 5);
      return { gte: d.toISOString().slice(0, 10), lte: getPresetDate(0) };
    }
    const customYr = String(key).match(/^year:(\d{4})$/);
    if (customYr) { const yr = customYr[1]; return { gte: `${yr}-01-01`, lte: `${yr}-12-31` }; }
    const m = String(key).match(/^decade:(\d{4})$/);
    if (m) { const s = Number(m[1]); return { gte: `${s}-01-01`, lte: `${s + 9}-12-31` }; }
    return {};
  }

  function renderReleaseFilterSection(currentReleaseKey) {
    const isCustom = /^year:\d{4}$/.test(String(currentReleaseKey || ''));
    const selected = new Set(isCustom ? [] : [currentReleaseKey || 'any']);
    const chips = PRESET_REFINE_RELEASE_OPTIONS.map(opt => {
      const on = selected.has(String(opt.key));
      return `<button type="button" class="preset-filter-chip${on ? ' is-selected' : ''}" data-group="release" data-value="${escAttr(opt.key)}" aria-pressed="${on ? 'true' : 'false'}" onclick="shelfdTogglePresetFilterChip(this)">${escHtml(opt.label)}</button>`;
    }).join('');
    return `<div class="preset-filter-group" data-group="release" data-single="1">
      <div class="preset-filter-group-title">Release</div>
      <div class="preset-filter-chips">${chips}</div>
      <div class="preset-custom-year-row">
        <input type="number" class="preset-custom-year-input" placeholder="Any year, e.g. 1975" min="1880" max="2035" inputmode="numeric" pattern="[0-9]*" />
        <button type="button" class="preset-custom-year-btn" onclick="shelfdConfirmPresetYearFilter(this.previousElementSibling)">Set</button>
      </div>
    </div>`;
  }

  async function fetchPresetRefinedDirect(config = {}, startPage = 1, pageCount = PRESET_REFINE_PAGE_SIZE) {
    const requestedMediaType = String(config.mediaType || 'mixed').trim().toLowerCase() || 'mixed';
    const refine = config.refine || {};
    const includeMovie = requestedMediaType === 'mixed' || requestedMediaType === 'movie';
    const includeTv = requestedMediaType === 'mixed' || requestedMediaType === 'tv' || requestedMediaType === 'anime';
    const sortKey = refine.sortBy || 'popular';
    const common = { include_adult: 'false', watch_region: 'US' };
    const genres = [...new Set((refine.genres || []).map(g => String(g)).filter(Boolean))];
    if (genres.length) common.with_genres = genres.join('|');
    const providers = [...new Set((refine.providers || []).map(p => String(p)).filter(Boolean))];
    if (providers.length) {
      common.with_watch_providers = providers.join('|');
      common.with_watch_monetization_types = 'flatrate|free|ads|rent|buy';
    }
    if (refine.country) common.with_origin_country = String(refine.country).toUpperCase();
    if (refine.language) common.with_original_language = String(refine.language).toLowerCase();
    if (sortKey === 'rated') common['vote_count.gte'] = '50';
    const range = getRefineReleaseRange(refine.releaseKey);
    const sortByFor = (type) => {
      if (sortKey === 'rated') return 'vote_average.desc';
      if (sortKey === 'newest') return type === 'movie' ? 'primary_release_date.desc' : 'first_air_date.desc';
      if (sortKey === 'oldest') return type === 'movie' ? 'primary_release_date.asc' : 'first_air_date.asc';
      return 'popularity.desc';
    };
    const dateParams = (type) => {
      const p = {};
      const gteKey = type === 'movie' ? 'primary_release_date.gte' : 'first_air_date.gte';
      const lteKey = type === 'movie' ? 'primary_release_date.lte' : 'first_air_date.lte';
      if (range.gte) p[gteKey] = range.gte;
      if (range.lte) p[lteKey] = range.lte;
      return p;
    };
    const [movieItems, tvItems] = await Promise.all([
      includeMovie ? fetchPresetTmdbRange('discover/movie', { ...common, sort_by: sortByFor('movie'), ...dateParams('movie') }, 'movie', startPage, pageCount) : Promise.resolve([]),
      includeTv ? fetchPresetTmdbRange('discover/tv', { ...common, sort_by: sortByFor('tv'), ...dateParams('tv') }, 'tv', startPage, pageCount) : Promise.resolve([])
    ]);
    let items = filterPresetItemsByMediaType(normalizePresetMediaItems([...movieItems, ...tvItems], requestedMediaType), requestedMediaType);
    const now = Date.now();
    items = items.filter(item => {
      const releaseDate = getPresetReleaseDate(item);
      if (!releaseDate) return false;
      const ms = Date.parse(`${String(releaseDate).slice(0, 10)}T00:00:00`);
      return Number.isFinite(ms) && ms <= now;
    });
    if (sortKey === 'rated') {
      if (items.length && typeof window.enrichItemsWithImdbRatings === 'function') {
        try { await window.enrichItemsWithImdbRatings(items, requestedMediaType); } catch (_) {}
      }
      const mean = getPresetImdbMeanRating(items);
      items.sort((a, b) => getPresetWeightedRatingScore(b, mean) - getPresetWeightedRatingScore(a, mean));
    } else if (sortKey === 'newest' || sortKey === 'oldest') {
      items.sort((a, b) => {
        const da = Date.parse(`${getPresetReleaseDate(a)}T00:00:00`) || 0;
        const db = Date.parse(`${getPresetReleaseDate(b)}T00:00:00`) || 0;
        return sortKey === 'newest' ? db - da : da - db;
      });
    } else {
      items.sort((a, b) => Number(b.popularity || 0) - Number(a.popularity || 0));
    }
    return items; // no slice — caller handles pagination
  }

  function syncPresetFilterButtonState(panelId) {
    const panel = document.querySelector(`.shelfd-search-preset-panel[data-panel-id="${presetCssEscape(panelId)}"]`);
    if (!panel) return;
    const btn = panel.querySelector('.shelfd-search-preset-filter-btn.is-preset-results-filter');
    if (!btn) return;
    const config = presetResultConfigByPanel.get(panelId);
    btn.classList.toggle('has-active-filter', refineIsNonDefault(config && config.refine));
  }

  function renderPresetFilterChipGroup(title, group, options, selectedValues, single) {
    const selected = new Set((Array.isArray(selectedValues) ? selectedValues : [selectedValues]).map(v => String(v)).filter(Boolean));
    const chips = options.map(opt => {
      const value = String(opt.value);
      const on = selected.has(value);
      return `<button type="button" class="preset-filter-chip${on ? ' is-selected' : ''}" data-group="${escAttr(group)}" data-value="${escAttr(value)}" aria-pressed="${on ? 'true' : 'false'}" onclick="shelfdTogglePresetFilterChip(this)">${escHtml(opt.label)}</button>`;
    }).join('');
    return `<div class="preset-filter-group" data-group="${escAttr(group)}" data-single="${single ? '1' : '0'}"><div class="preset-filter-group-title">${escHtml(title)}</div><div class="preset-filter-chips">${chips}</div></div>`;
  }

  window.shelfdTogglePresetFilterChip = function(btn) {
    if (!btn) return;
    const group = btn.closest('.preset-filter-group');
    const single = group && group.dataset.single === '1';
    if (group && group.dataset.group === 'release') {
      const inp = group.querySelector('.preset-custom-year-input');
      if (inp) { inp.value = ''; inp.classList.remove('is-set', 'is-error'); }
      delete group.dataset.customYear;
    }
    if (single) {
      group.querySelectorAll('.preset-filter-chip.is-selected').forEach(el => {
        if (el !== btn) { el.classList.remove('is-selected'); el.setAttribute('aria-pressed', 'false'); }
      });
  ...
  window.shelfdClosePresetResultsFilter = function() {
    const sheet = document.getElementById('preset-filter-sheet');
    const bd = document.getElementById('preset-filter-backdrop');
    if (sheet) { sheet.classList.remove('is-open'); setTimeout(() => { try { sheet.remove(); } catch (_) {} }, 260); }
    if (bd) { bd.classList.remove('is-open'); setTimeout(() => { try { bd.remove(); } catch (_) {} }, 260); }
  };

  window.shelfdOpenPresetResultsFilter = function(panelId) {
    const id = String(panelId || '');
    const config = presetResultConfigByPanel.get(id);
    if (!config) return;
    const current = config.refine || defaultRefineForConfig(config);
    window.shelfdClosePresetResultsFilter();
    const bd = document.createElement('div');
    bd.id = 'preset-filter-backdrop';
    bd.className = 'preset-filter-backdrop';
    bd.onclick = window.shelfdClosePresetResultsFilter;
    document.body.appendChild(bd);
    const sheet = document.createElement('div');
    sheet.id = 'preset-filter-sheet';
    sheet.className = 'preset-filter-sheet';
    sheet.dataset.panelId = id;
    sheet.setAttribute('role', 'dialog');
    sheet.setAttribute('aria-modal', 'true');
    sheet.setAttribute('aria-label', 'Filter results');
    const genreOptions = PRESET_GENRE_OPTIONS.map(g => ({ value: g.id, label: g.label }));
    const serviceOptions = PRESET_SERVICE_OPTIONS.filter(s => s.providerId).map(s => ({ value: s.providerId, label: s.label }));
    const countryOptions = PRESET_COUNTRY_OPTIONS.map(c => ({ value: c.key, label: c.label }));
    const languageOptions = PRESET_LANGUAGE_OPTIONS.map(l => ({ value: l.key, label: l.label }));
    sheet.innerHTML = `
      <div class="preset-filter-handle" aria-hidden="true"></div>
      <div class="preset-filter-head">
        <strong>Filter results</strong>
        <button type="button" class="preset-filter-close" aria-label="Close" onclick="shelfdClosePresetResultsFilter()">&times;</button>
      </div>
      <div class="preset-filter-body">
        ${renderPresetFilterChipGroup('Sort by', 'sort', PRESET_REFINE_SORT_OPTIONS.map(o => ({ value: o.key, label: o.label })), [current.sortBy || 'popular'], true)}
        ${renderReleaseFilterSection(current.releaseKey)}
        ${renderPresetFilterChipGroup('Genre', 'genre', genreOptions, current.genres || [], false)}
        ${renderPresetFilterChipGroup('Service', 'service', serviceOptions, current.providers || [], false)}
        ${renderPresetFilterChipGroup('Country', 'country', countryOptions, current.country ? [current.country] : [], true)}
        ${renderPresetFilterChipGroup('Language', 'language', languageOptions, current.language ? [current.language] : [], true)}
      </div>
      <div class="preset-filter-actions">
        <button type="button" class="preset-filter-clear" onclick="shelfdClearPresetResultsFilter()">Clear</button>
        <button type="button" class="preset-filter-apply" onclick="shelfdApplyPresetResultsFilter()">Apply</button>
      </div>`;
    document.body.appendChild(sheet);
    const customYrMatch = String(current.releaseKey || '').match(/^year:(\d{4})$/);
    if (customYrMatch) {
      const rg = sheet.querySelector('.preset-filter-group[data-group="release"]');
      const inp = rg && rg.querySelector('.preset-custom-year-input');
      if (rg) rg.dataset.customYear = customYrMatch[1];
      if (inp) { inp.value = customYrMatch[1]; inp.classList.add('is-set'); }
    }
    requestAnimationFrame(() => { bd.classList.add('is-open'); requestAnimationFrame(() => sheet.classList.add('is-open')); });
  };

  window.shelfdConfirmPresetYearFilter = function(inputEl) {
    if (!inputEl) return;
    const group = inputEl.closest('.preset-filter-group');
    if (!group) return;
    const yr = parseInt(inputEl.value, 10);
    const maxYr = new Date().getFullYear() + 5;
    if (!yr || yr < 1880 || yr > maxYr) {
      inputEl.classList.add('is-error');
      inputEl.classList.remove('is-set');
      setTimeout(() => inputEl.classList.remove('is-error'), 820);
      return;
    }
    group.querySelectorAll('.preset-filter-chip.is-selected').forEach(el => {
      el.classList.remove('is-selected'); el.setAttribute('aria-pressed', 'false');
    });
    group.dataset.customYear = String(yr);
    inputEl.classList.add('is-set');
    inputEl.classList.remove('is-error');
  };

  function readPresetFilterSheetRefine(sheet) {
    const getSingle = (group) => {
      const el = sheet.querySelector(`.preset-filter-group[data-group="${group}"] .preset-filter-chip.is-selected`);
      return el ? String(el.dataset.value) : '';
    };
    const getMulti = (group) => Array.from(sheet.querySelectorAll(`.preset-filter-group[data-group="${group}"] .preset-filter-chip.is-selected`)).map(el => String(el.dataset.value));
    const releaseGroup = sheet.querySelector('.preset-filter-group[data-group="release"]');
    const chipRelease = getSingle('release');
    const customYr = releaseGroup && releaseGroup.dataset.customYear ? `year:${releaseGroup.dataset.customYear}` : '';
    return {
      sortBy: getSingle('sort') || 'popular',
      releaseKey: chipRelease || customYr || 'any',
      genres: getMulti('genre'),
      providers: getMulti('service'),
      country: getSingle('country'),
      language: getSingle('language')
    };
  }

  function rerunPresetPanelWithConfig(panelId, nextConfig) {
    presetResultConfigByPanel.set(panelId, nextConfig);
    const root = document.querySelector(`[data-preset-results="${presetCssEscape(panelId)}"]`);
    if (root) root.innerHTML = renderPresetResultSkeleton();
    syncPresetFilterButtonState(panelId);
    loadPresetResults(nextConfig, panelId);
  }

  window.shelfdApplyPresetResultsFilter = function() {
    const sheet = document.getElementById('preset-filter-sheet');
    if (!sheet) return;
    const panelId = String(sheet.dataset.panelId || '');
    const base = presetResultConfigByPanel.get(panelId);
    const refine = readPresetFilterSheetRefine(sheet);
    window.shelfdClosePresetResultsFilter();
    if (!base) return;
    presetResultBuffer.delete(panelId);
    rerunPresetPanelWithConfig(panelId, { ...base, refine });
  };

  window.shelfdClearPresetResultsFilter = function() {
    const sheet = document.getElementById('preset-filter-sheet');
    if (!sheet) return;
    const panelId = String(sheet.dataset.panelId || '');
    window.shelfdClosePresetResultsFilter();
    ...
    if (!base) return;
    const cleared = { ...base };
    delete cleared.refine;
    presetResultBuffer.delete(panelId);
    rerunPresetPanelWithConfig(panelId, cleared);
  };

  window.shelfdLoadMorePresetResults = async function(panelId) {
    const buf = presetResultBuffer.get(panelId);
    if (!buf || buf.loading) return;
    const newVisible = buf.visible + PRESET_REFINE_BATCH;
    const root = document.querySelector(`[data-preset-results="${presetCssEscape(panelId)}"]`);
    // Enough in buffer — just reveal more
    if (newVisible <= buf.items.length) {
      buf.visible = newVisible;
      if (root) root.innerHTML = renderPresetResultGridPaged(panelId);
      return;
    }
    // Nothing more to fetch
    if (buf.exhausted) {
      buf.visible = buf.items.length;
      if (root) root.innerHTML = renderPresetResultGridPaged(panelId);
      return;
    }
    // Fetch next batch of TMDB pages
    buf.loading = true;
    if (root) root.innerHTML = renderPresetResultGridPaged(panelId);
    try {
      const freshItems = await fetchPresetRefinedDirect(buf.config, buf.nextTmdbPage, PRESET_REFINE_PAGE_SIZE);
      buf.nextTmdbPage += PRESET_REFINE_PAGE_SIZE;
      const existingIds = new Set(buf.items.map(it => String(it.id || '')));
      const deduped = freshItems.filter(it => it?.id && !existingIds.has(String(it.id)));
      if (!deduped.length) {
        buf.exhausted = true;
      } else {
        buf.items.push(...deduped);
      }
      buf.visible = Math.min(newVisible, buf.items.length);
      buf.loading = false;
      if (root) root.innerHTML = renderPresetResultGridPaged(panelId);
    } catch (_) {
      buf.loading = false;
  ...
  function shelfdCompactSearchKey(s) {
    return shelfdSearchNormalize(s).replace(/\s+/g, '');
  }

  function shelfdIsRomanizedNearVariant(a = '', b = '') {
    const ak = shelfdCompactSearchKey(a);
    const bk = shelfdCompactSearchKey(b);
  ...
  const SHELFD_ACRONYM_STOP_WORDS = new Set([
    'the', 'a', 'an', 'and', 'of', 'on', 'in', 'to', 'for', 'with', 'from', 'at', 'by'
  ]);
  function shelfdGenerateAcronyms(rawText = '') {
    const text = shelfdSearchNormalize(rawText);
    if (!text) return [];
    const words = text.split(' ').filter(Boolean);
    const meaningful = words.filter(word => !SHELFD_ACRONYM_STOP_WORDS.has(word));
    const source = meaningful.length >= 2 ? meaningful : words;
    if (source.length < 2 || source.length > 7) return [];
    const letters = source.map(word => word.charAt(0)).join('');
    const out = [];
    if (letters.length >= 3 && letters.length <= 7) out.push(letters);
    if (letters.length === 2 && text.length >= 10) out.push(letters);
    const last = source[source.length - 1] || '';
    const roman = shelfdRomanToInt(last);
    if (roman && source.length > 1) {
      const base = source.slice(0, -1).map(word => word.charAt(0)).join('');
      if (base.length >= 2) {
        out.push(base + roman);
        out.push(base + last);
      }
  ...
  function shelfdRomanToInt(value = '') {
    const s = String(value || '').trim().toLowerCase();
    const map = { i: 1, ii: 2, iii: 3, iv: 4, v: 5, vi: 6, vii: 7, viii: 8, ix: 9, x: 10 };
    return map[s] || 0;
  }

  function shelfdNormalizeRomanNumeralTokens(value = '') {
    return shelfdSearchNormalize(value)
      .split(' ')
      .filter(Boolean)
      .map(token => String(shelfdRomanToInt(token) || token))
      .join(' ');
  }

  function shelfdAcronymScore(rawText = '', q = '') {
    const query = shelfdCompactSearchKey(q);
    if (!query || query.length < 2 || query.length > 7) return 0;
    const acronyms = shelfdGenerateAcronyms(rawText);
    if (!acronyms.length) return 0;
    if (acronyms.includes(query)) return query.length <= 2 ? 720 : 920;
    return acronyms.some(a => a.startsWith(query) && query.length >= 3) ? 620 : 0;
  }
  ...
  function shelfdScoreCandidate(rawText, q, qNoSpace, qWords) {
    ...
    const tRoman = shelfdNormalizeRomanNumeralTokens(rawText);
    const qRoman = shelfdNormalizeRomanNumeralTokens(q);
    const tRomanNoSpace = tRoman.replace(/\s+/g, '');
    const qRomanNoSpace = qRoman.replace(/\s+/g, '');
    if (tRoman && qRoman && (tRoman === qRoman || tRomanNoSpace === qRomanNoSpace)) {
      return { bucket: 100, sub: 940 };
    }
    /* 100 (near) — short romanization/localization suffixes such as
       "hope" -> "hopeu". Keep it below a literal exact match inside
       the same bucket, but strong enough to compete with exact low-
       confidence rows from other sources. */
    if (shelfdIsRomanizedNearVariant(t, q)) {
      const lenRatio = Math.min(tNoSpace.length, qNoSpace.length) / Math.max(1, Math.max(tNoSpace.length, qNoSpace.length));
      return { bucket: 100, sub: Math.round(850 + 130 * lenRatio) };
    }
    const acronym = shelfdAcronymScore(rawText, q);
    if (acronym >= 900) return { bucket: 100, sub: 900 + Math.min(80, acronym - 900) };
    if (acronym > 0) return { bucket: 80, sub: acronym };
  ...
  function shelfdSourceConfidence(row) {
    if (!row) return 0;
    let score = 0;
    const kind = row.kind || '';
    if (kind === 'movie' || kind === 'tv') score += 0.34;
    else if (kind === 'anime') score += row.isJikan || row.malId ? 0.36 : 0.24;
    else if (kind === 'game') score += row.raw?.source === 'igdb' || row.raw?.igdbId || row.raw?.igdb_id ? 0.36 : 0.30;
    else if (kind === 'music') score += row.deezerSource ? 0.26 : 0.14;
    else if (kind === 'artist') score += row.deezerSource ? 0.28 : 0.16;
    else if (kind === 'person') score += 0.22;

    score += Math.min(0.24, shelfdQualityScore(row) * 0.045);
    score += Math.min(0.22, shelfdAuthority(row) * 0.045);
    if (!row.poster && (kind === 'music' || kind === 'artist' || kind === 'person')) score -= 0.06;
    if (!row.id) score -= 0.08;
    return Number(Math.max(0, score).toFixed(4));
  }

  function shelfdMusicSubtypeScore(row, qN = '') {
    if (!row || (row.kind !== 'music' && row.kind !== 'artist')) return 0;
    const titleMatch = shelfdScoreCandidate(row.title, qN, qN.replace(/\s+/g, ''), qN.split(' ').filter(Boolean));
    const artistMatch = row.artist ? shelfdScoreCandidate(row.artist, qN, qN.replace(/\s+/g, ''), qN.split(' ').filter(Boolean)) : { bucket: 0, sub: 0 };
    if (row.kind === 'artist') return titleMatch.bucket >= 100 ? 0.42 : titleMatch.bucket >= 90 ? 0.30 : 0.08;
    if (row.subkind === 'album') return titleMatch.bucket >= 100 ? 0.28 : titleMatch.bucket >= 90 ? 0.20 : artistMatch.bucket >= 90 ? 0.08 : 0.04;
    if (row.subkind === 'track') return titleMatch.bucket >= 100 ? 0.18 : titleMatch.bucket >= 90 ? 0.12 : 0;
    return 0;
  }

  function shelfdRowAcronymScore(row, qN = '') {
    const aliases = Array.isArray(row?.aliases) ? row.aliases : [];
    const score = Math.max(
      shelfdAcronymScore(row?.title || '', qN),
      ...aliases.map(alias => shelfdAcronymScore(alias, qN))
    );
    return Number((score / 1000).toFixed(4));
  }

  function shelfdAllTabConfidence(row) {
    if (!row || !row._bucket) return 0;
    const kind = row.kind || '';
    const isShelfTitle = kind === 'movie' || kind === 'tv' || kind === 'anime' || kind === 'game';
    const isMusicOrPerson = kind === 'music' || kind === 'artist' || kind === 'person';
    let score = 0;

    if (row._bucket >= 100) score += 0.35;
    else if (row._bucket >= 95) score += 0.30;
    else if (row._bucket >= 90) score += 0.22;
    else if (row._bucket >= 80) score += 0.12;

    if (isShelfTitle) score += kind === 'movie' ? 0.34 : 0.32;
    else if (kind === 'music') score += 0.05;

    const quality = shelfdQualityScore(row);
    score += Math.min(0.22, quality * 0.045);
    const auth = shelfdAuthority(row);
    score += Math.min(0.24, auth * 0.045);
    if (isMusicOrPerson && row._bucket >= 100) {
      score += Math.min(0.45, auth * 0.08);
    }

    const year = parseInt(row.year || '0', 10) || 0;
    if (isShelfTitle && year) {
      const currentYear = new Date().getFullYear();
      const delta = Math.abs(currentYear - year);
      if (delta <= 2) score += 0.28;
      else if (delta <= 6) score += 0.12;
    }

    if (!row.poster) score -= 0.12;
    if (!row.id) score -= 0.08;
    if (!row.year && isShelfTitle) score -= 0.06;
    score += Math.min(0.18, row._sourceConfidence || 0);
    score += Math.min(0.16, row._acronymScore || 0);
  ...
  function shelfdNearDuplicateTitleKey(title) {
    const key = shelfdCompactSearchKey(title);
    if (key.length < 4) return key;
    return key.replace(/[aeiouy]{1,2}$/i, '');
  }

  function shelfdDedupeRows(rows) {
    const seenIds = new Set();
    const seenSig = new Map();
    const seenNearSig = new Map();
    const out = [];
    for (const r of rows) {
      if (!r) continue;
      if (r.id) {
      ...
      if (existing) {
        if (shelfdQualityScore(r) > shelfdQualityScore(existing)) {
          ...
          seenSig.set(sig, r);
          seenNearSig.set(`${r.kind || ''}|${shelfdNearDuplicateTitleKey(r.title)}|${r.year || ''}|${r.subkind || ''}`, r);
          out.push(r);
        }
        continue;
      }
      const nearSig = `${r.kind || ''}|${shelfdNearDuplicateTitleKey(r.title)}|${r.year || ''}|${r.subkind || ''}`;
      const nearExisting = seenNearSig.get(nearSig);
      if (nearExisting && shelfdIsRomanizedNearVariant(nearExisting.title, r.title)) {
        const rScore = shelfdQualityScore(r) + (r._authority || 0);
        const existingScore = shelfdQualityScore(nearExisting) + (nearExisting._authority || 0);
        if (rScore > existingScore) {
          const idx = out.indexOf(nearExisting);
          if (idx >= 0) out.splice(idx, 1);
          seenSig.delete(`${nearExisting.kind || ''}|${shelfdSearchNormalize(nearExisting.title)}|${nearExisting.year || ''}|${nearExisting.subkind || ''}`);
          seenSig.set(sig, r);
          seenNearSig.set(nearSig, r);
          out.push(r);
        }
        continue;
      }
  ...
  function shelfdIntentGroup(row) {
    if (!row) return '';
    if (row.kind === 'music' || row.kind === 'artist') {
      if (row.kind === 'artist') return 'music_artist';
      if (row.subkind === 'track') return 'music_track';
      return 'music_album';
    }
    if (row.kind === 'person') return 'person';
    return row.kind || '';
  }

  function shelfdBaseIntentForKind(kind = '') {
    if (kind === 'music_artist' || kind === 'music_album' || kind === 'music_track') return 'music';
    return kind;
  }

  function shelfdInferQueryIntent(query = '', rows = []) {
    const qN = shelfdSearchNormalize(query);
    const qKey = shelfdCompactSearchKey(query);
    const explicit = shelfdDetectIntent(query);
    const scores = {
      movie: 0,
      tv: 0,
      anime: 0,
    ...
    explicit.forEach(kind => {
      if (kind === 'music') {
        scores.music_artist += 1.1;
        scores.music_album += 0.9;
        scores.music_track += 0.85;
      } else if (Object.prototype.hasOwnProperty.call(scores, kind)) {
        scores[kind] += 1.1;
      }
    });

    for (const row of rows.slice(0, 80)) {
      if (!row || !row._bucket) continue;
      const group = shelfdIntentGroup(row);
      if (!Object.prototype.hasOwnProperty.call(scores, group)) continue;
      const relevance = row._bucket >= 100 ? 1.3
        : row._bucket >= 95 ? 1.12
        : row._bucket >= 90 ? 0.95
        : row._bucket >= 80 ? 0.62
        : row._bucket >= 70 ? 0.34
        : 0.12;
      const authority = Math.min(0.45, (row._authority || 0) * 0.08);
      const source = Math.min(0.32, row._sourceConfidence || 0);
      const acronym = Math.min(0.7, row._acronymScore || 0);
      const subtype = Math.min(0.35, row._musicSubtypeScore || 0);
      scores[group] += relevance + authority + source + acronym + subtype;
      if (row.kind === 'music' || row.kind === 'artist') {
        const artistExact = row.kind === 'artist' && shelfdCompactSearchKey(row.title) === qKey;
        if (artistExact) scores.music_artist += 0.9;
      }
      if (row.kind === 'anime' && row._bucket >= 90) scores.anime += 0.18;
      if (row.kind === 'game' && row._acronymScore >= 0.9) scores.game += 0.65;
      if (row.kind === 'tv' && row._bucket >= 95 && qN.split(' ').length > 1) scores.tv += 0.14;
    }

    const sorted = Object.entries(scores).sort((a, b) => b[1] - a[1]);
    const [primary, primaryScore] = sorted[0] || ['broad_title', 0];
    const [, secondScore] = sorted[1] || ['', 0];
    if (!primary || primaryScore < 1.25 || primaryScore - secondScore < 0.35) {
      return { primary: 'broad_title', scores, confidence: 0 };
    }
    return {
      primary,
      scores,
      confidence: Math.min(1, (primaryScore - secondScore) / 2.5)
    };
  }

  function shelfdQueryIntentBoost(row, intent) {
    if (!row || !intent || intent.primary === 'broad_title') return 0;
    const group = shelfdIntentGroup(row);
    const baseGroup = shelfdBaseIntentForKind(group);
    const primaryBase = shelfdBaseIntentForKind(intent.primary);
    let score = 0;
    if (group === intent.primary) score += 0.75;
    else if (baseGroup && baseGroup === primaryBase) score += 0.46;
    else if (row._bucket >= 100) score += 0.08;
    else score -= 0.08;
    score *= Math.max(0.35, intent.confidence || 0);
    if (row._bucket < 70) score *= 0.4;
    return Number(score.toFixed(4));
  }

  function shelfdOwnLibraryDataForSearch() {
    try {
      if (typeof data !== 'undefined' && data) return data;
      if (typeof ownDataCache !== 'undefined' && ownDataCache) return ownDataCache;
    } catch (_) {}
    return null;
  }

  function shelfdLibrarySectionsForRow(row) {
    if (!row) return [];
    if (row.kind === 'movie') return ['movies'];
    if (row.kind === 'tv') return ['shows'];
    if (row.kind === 'anime') return ['anime'];
    if (row.kind === 'game') return ['games'];
    if (row.kind === 'music') return ['music'];
    return [];
  }

  function shelfdProviderKeysForLibraryItem(item = {}, section = '') {
    const keys = new Set();
    const add = (prefix, value) => {
      const v = String(value || '').trim();
      if (v) keys.add(`${prefix}:${v.toLowerCase()}`);
    };
    add('id', item.id);
    add('tmdb', item.tmdbId || item.tmdb_id);
    add('mal', item.malId || item.mal_id || item.__mal_id);
    add('rawg', item.rawgId || item.rawg_id);
    add('igdb', item.igdbId || item.igdb_id || item.sourceId);
    add('deezer', item.deezerId || item.deezerAlbumId);
    add('mbid', item.mbid || item.musicBrainzId || item.releaseGroupId);
    if (section === 'games') add('game', item.gameIdentityKey || item.rawgSlug || item.slug);
    return keys;
  }

  function shelfdProviderKeysForRow(row = {}) {
    const keys = new Set();
    const add = (prefix, value) => {
      const v = String(value || '').trim();
      if (v) keys.add(`${prefix}:${v.toLowerCase()}`);
    ...
    add('id', row.id);
    if (row.kind === 'movie' || row.kind === 'tv') add('tmdb', row.id);
    if (row.kind === 'anime') add('mal', row.malId || row.id);
    if (row.kind === 'game') {
      add('rawg', row.rawgId);
      add('igdb', row.raw?.igdbId || row.raw?.igdb_id || row.raw?.sourceId);
      add('game', row.raw?.gameIdentityKey || row.raw?.slug);
      if (!row.rawgId && row.id) add(String(row.id).startsWith('igdb:') ? 'igdb' : 'rawg', String(row.id).replace(/^igdb:/i, ''));
    }
    if (row.kind === 'music') {
      add('deezer', row.deezerId);
      add('mbid', row.id);
    }
    return keys;
  }

  function shelfdIsRowAlreadyInOwnLibrary(row = {}) {
    const own = shelfdOwnLibraryDataForSearch();
    if (!own || !row || row.kind === 'person' || row.kind === 'artist') return false;
    const sections = shelfdLibrarySectionsForRow(row);
    if (!sections.length) return false;
    const rowKeys = shelfdProviderKeysForRow(row);
    const rowTitle = shelfdSearchNormalize(row.title);
    const rowYear = String(row.year || '').trim();
    for (const section of sections) {
      const items = Array.isArray(own?.[section]) ? own[section] : [];
      for (const item of items) {
        const itemKeys = shelfdProviderKeysForLibraryItem(item, section);
        for (const key of rowKeys) {
          if (itemKeys.has(key)) return true;
        }
        const itemTitle = shelfdSearchNormalize(
          (typeof getDisplayTitleForItem === 'function' ? getDisplayTitleForItem(item, section) : '') ||
          item.title ||
          item.name ||
          ''
        );
        const itemYear = String(item.year || item.releaseYear || item.firstAirYear || '').trim();
        if (rowTitle && itemTitle && rowTitle === itemTitle && (!rowYear || !itemYear || rowYear === itemYear)) return true;
      }
    }
    return false;
  }
  ...
  function shelfdCompareRows(a, b) {
    ...
    /* Pure query/provider intent inference for All-tab category balance. */
    if ((a._intentBoost || 0) !== (b._intentBoost || 0)) return (b._intentBoost || 0) - (a._intentBoost || 0);
    /* All-tab media confidence: metadata + recency + category fit. */
    if ((a._categoryConfidence || 0) !== (b._categoryConfidence || 0)) {
      return (b._categoryConfidence || 0) - (a._categoryConfidence || 0);
    }
    if ((a._sourceConfidence || 0) !== (b._sourceConfidence || 0)) return (b._sourceConfidence || 0) - (a._sourceConfidence || 0);
    if ((a._musicSubtypeScore || 0) !== (b._musicSubtypeScore || 0)) return (b._musicSubtypeScore || 0) - (a._musicSubtypeScore || 0);
    if ((a._acronymScore || 0) !== (b._acronymScore || 0)) return (b._acronymScore || 0) - (a._acronymScore || 0);
  ...
  /* ---------- Search ---------- */
  function normalizeShelfdUniversalSearchFilter(filter = 'all') {
    const key = String(filter || 'all').trim().toLowerCase();
    if (!key || key === 'all' || key === 'everything' || key === 'mixed') return 'all';
    if (key === 'movies') return 'movie';
    if (key === 'shows' || key === 'tvshows' || key === 'tv_shows') return 'tv';
    if (key === 'games' || key === 'gaming' || key === 'rawg') return 'game';
    if (key === 'actors' || key === 'people' || key === 'person') return 'person';
    if (key === 'artist' || key === 'album' || key === 'song' || key === 'track') return 'music';
    if (['movie', 'tv', 'anime', 'game', 'person', 'music'].includes(key)) return key;
    return 'all';
  }

  async function collectShelfdUniversalSearchRows(rawQuery = '', filter = 'all', options = {}) {
    const query = String(rawQuery || '').trim();
    const searchFilter = normalizeShelfdUniversalSearchFilter(filter);
    const limit = Math.max(1, Number(options.limit || SEARCH_LIMIT) || SEARCH_LIMIT);
    if (!query) return [];
    ...
    try {
      const tasks = [];
      const wantTmdb = searchFilter === 'all' || searchFilter === 'movie' || searchFilter === 'tv';
      const wantJikanAnime = searchFilter === 'all' || searchFilter === 'anime';
      const wantGames = searchFilter === 'all' || searchFilter === 'game';
      const wantPeople = searchFilter === 'all' || searchFilter === 'person';
      const wantMusic = searchFilter === 'all' || searchFilter === 'music';
      ...
      if (wantJikanAnime && window.JikanAnime?.searchAnime) {
        tasks.push(window.JikanAnime.searchAnime(query, 18).then(arr => {
    ...
    if (typeof window.enrichItemsWithImdbRatings === 'function' && tmdbItems.length) {
      try { await window.enrichItemsWithImdbRatings(tmdbItems); } catch (_) {}
    }
    ...
    if (searchFilter === 'all') {
      rows = [
        ...
        ...jikanItems.map(normalizeTmdbItem),
    ...
    const intents = searchFilter === 'all' ? shelfdDetectIntent(query) : new Set();

    if (searchFilter === 'music') {
      return shelfdDedupeRows(rows)
        .map(row => {
          const m = shelfdBestBucketAcross(row, qN, qNoSpace, qWords);
          row._bucket = m.bucket;
          row._sub = m.sub;
          row._authority = shelfdAuthority(row);
          row._sourceConfidence = shelfdSourceConfidence(row);
          row._musicSubtypeScore = shelfdMusicSubtypeScore(row, qN);
          row._acronymScore = shelfdRowAcronymScore(row, qN);
          row._alreadyInLibrary = shelfdIsRowAlreadyInOwnLibrary(row);
          return row;
        })
    ...
    rows = shelfdDedupeRows(rows)
      .map(row => {
        ...
        row._musicSubtypeScore = shelfdMusicSubtypeScore(row, qN);
        row._acronymScore = shelfdRowAcronymScore(row, qN);
        row._categoryConfidence = searchFilter === 'all' ? shelfdAllTabConfidence(row) : 0;
        row._alreadyInLibrary = shelfdIsRowAlreadyInOwnLibrary(row);
        return row;
      })
      .filter(row => row._bucket > 0)
    ...
    if (searchFilter === 'all') {
      const inferredIntent = shelfdInferQueryIntent(query, rows);
      rows.forEach(row => { row._intentBoost = shelfdQueryIntentBoost(row, inferredIntent); });
    }
    rows.sort(shelfdCompareRows);

    if (searchFilter === 'all' && rows.length > 6 && intents.size === 0) {
      const TOP_WINDOW = 10;
      const DEFAULT_CAP = 4;
    ...
    if (activeFilter === 'music') {
      ...
      rows = shelfdDedupeRows(rows)
        .map(row => {
          ...
          row._musicSubtypeScore = shelfdMusicSubtypeScore(row, qN);
          row._acronymScore = shelfdRowAcronymScore(row, qN);
          row._categoryConfidence = activeFilter === 'all' ? shelfdAllTabConfidence(row) : 0;
          row._alreadyInLibrary = shelfdIsRowAlreadyInOwnLibrary(row);
          return row;
        })
  ...
  function renderResultRow(r, index) {
    ...
    */
    ...
    const shelfPillHtml = r._alreadyInLibrary
      ? `<span class="shelfd-search-row-shelf-pill">On your Shelf</span>`
      : '';
    const yearTypeHtml = metaText
      ? `<span class="shelfd-search-row-meta">${escHtml(metaText)}${shelfPillHtml}</span>`
      : shelfPillHtml
        ? `<span class="shelfd-search-row-meta">${shelfPillHtml}</span>`
      : '';
  ...
  window.ShelfdUniversalSearchEngine = {
    search: collectShelfdUniversalSearchRows,
    normalizeFilter: normalizeShelfdUniversalSearchFilter,
    renderRows(rows = [], query = '', options = {}) {
      const list = Array.isArray(rows) ? rows : [];
      if (query) activeQuery = String(query || '').trim();
      lastResultRows = list;
      const html = list.map((r, i) => renderResultRow(r, i)).join('');
      if (!options || options.backfill !== false) {
        backfillResultCredits(list, queryToken);
      }
      return html;
    },
    rememberRows(rows = [], query = '') {
      if (query) activeQuery = String(query || '').trim();
      lastResultRows = Array.isArray(rows) ? rows : [];
    }
  };
  ...
  function setActiveFilter(filter) {
    ...
    updateShelfdSearchFilterButtonState();
  ...
  function getShelfdSearchFilterSource() {
    const filter = String(activeFilter || 'all').trim().toLowerCase();
    if (filter === 'movie' || filter === 'tv' || filter === 'anime') return filter;
    if (filter === 'all') return 'all';
    return '';
  }

  function isShelfdSearchTopFilterable() {
    const source = getShelfdSearchFilterSource();
    if (!source) return false;
    if (typeof window.isShelfdSearchFilterableSource === 'function') {
      return !!window.isShelfdSearchFilterableSource(source);
    }
    return true;
  }

  function updateShelfdSearchFilterButtonState() {
    const btn = document.getElementById('shelfd-search-filter-btn');
    if (!btn) return;
    const source = getShelfdSearchFilterSource();
    const filterable = !!source && isShelfdSearchTopFilterable();
    const count = filterable && typeof window.getShelfdSearchFilterCount === 'function'
      ? Number(window.getShelfdSearchFilterCount(source) || 0)
      : 0;
    btn.style.display = filterable ? 'inline-flex' : 'none';
    btn.classList.toggle('has-active-filter', !!count);
    btn.setAttribute('aria-label', count ? `Filter search. ${count} active` : 'Filter search');
    const label = btn.querySelector('.discover-universal-search-filter-label');
    if (label) label.textContent = count ? `Filter ${count}` : 'Filter';
  }

  function getFilteredSearchRows(items = [], query = activeQuery, options = {}) {
    const limit = Number.isFinite(Number(options.limit)) ? Math.max(1, Number(options.limit)) : Infinity;
    let rows = (Array.isArray(items) ? items : [])
      .map(normalizeTmdbItem)
      .filter(row => {
        if (!row || !row.id) return false;
        if (activeFilter === 'movie') return row.kind === 'movie';
        if (activeFilter === 'tv') return row.kind === 'tv';
        if (activeFilter === 'anime') return row.kind === 'anime';
        return row.kind === 'movie' || row.kind === 'tv' || row.kind === 'anime';
      })
      .map(row => {
        row._alreadyInLibrary = shelfdIsRowAlreadyInOwnLibrary(row);
        return row;
    ...
    const q = shelfdSearchNormalize(query || '');
    if (!q) return Number.isFinite(limit) ? rows.slice(0, limit) : rows;

    const qNoSpace = q.replace(/\s+/g, '');
    const qWords = q.split(' ').filter(Boolean);
    rows = rows
      .map(row => {
        const match = shelfdBestBucketAcross(row, q, qNoSpace, qWords);
        row._bucket = match.bucket;
        row._sub = match.sub;
        row._authority = shelfdAuthority(row);
        row._sourceConfidence = shelfdSourceConfidence(row);
        return row;
      ...
      .filter(row => row._bucket > 0)
      .sort(shelfdCompareRows);
    return Number.isFinite(limit) ? rows.slice(0, limit) : rows;
  }

  function getFilteredSearchFetchQuery() {
    return activeFilter === 'anime' ? activeQuery : '';
  }

  async function fetchShelfdSearchFilteredRows(pageCount = 1) {
    const safePageCount = Math.max(1, Number(pageCount || 1));
    const maxResults = Math.max(FILTER_RESULT_BATCH_SIZE, safePageCount * FILTER_RESULT_FETCH_PAGE_SIZE);
    const items = typeof window.fetchDiscoverFilteredMediaItems === 'function'
      ? await window.fetchDiscoverFilteredMediaItems(getFilteredSearchFetchQuery(), {
          pageCount: safePageCount,
          maxResults
        })
      : [];
    return getFilteredSearchRows(items, activeQuery, { limit: maxResults });
  }

  function renderFilteredSearchLoadMoreButton() {
    if (!filteredSearchState.active) return '';
    const hasBufferedRows = filteredSearchState.visibleCount < filteredSearchState.rows.length;
    if (!hasBufferedRows && filteredSearchState.exhausted) return '';
    const label = filteredSearchState.loading ? 'Loading...' : 'Load More';
    return `<div class="shelfd-search-load-more-wrap">
      <button class="shelfd-search-load-more-btn" type="button" onclick="loadMoreShelfdSearchFilteredResults()"${filteredSearchState.loading ? ' disabled aria-busy="true"' : ''}>${label}</button>
    </div>`;
  }
  ...
  function renderFilteredSearchResults() {
    const results = $('shelfd-search-results');
    if (!results) return;
    const visibleRows = filteredSearchState.rows.slice(0, filteredSearchState.visibleCount);
    results.innerHTML = visibleRows.map((row, i) => renderResultRow(row, i)).join('') + renderFilteredSearchLoadMoreButton();
    lastResultRows = visibleRows;
    backfillResultCredits(visibleRows, queryToken);
  }

  window.updateShelfdSearchFilterButtonState = updateShelfdSearchFilterButtonState;

  window.openShelfdSearchFilterSheetForSearch = function() {
    const source = getShelfdSearchFilterSource();
    if (!source || typeof window.openShelfdSearchFilterSheet !== 'function') return;
    updateShelfdSearchFilterButtonState();
    window.openShelfdSearchFilterSheet(source);
  };

  window.applyShelfdSearchDiscoveryFilters = async function() {
    const empty = $('shelfd-search-empty');
    const results = $('shelfd-search-results');
    const status = $('shelfd-search-status');
    ...
    status.hidden = false;
    status.innerHTML = renderSkeletonRows(5);
    const body = $('shelfd-search-body');
    if (body) {
      try { body.scrollTo({ top: 0, behavior: 'smooth' }); } catch (_) { try { body.scrollTop = 0; } catch (__) {} }
    }

    try {
      const rows = await fetchShelfdSearchFilteredRows(filteredSearchState.fetchPageCount);
      ...
      if (!rows.length) {
        ...
        status.innerHTML = `
          <div class="shelfd-search-noresults">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <strong>No titles found for these filters.</strong>
            <p>Try clearing one filter or switching categories.</p>
          </div>`;
        results.hidden = true;
        results.innerHTML = '';
        return;
      ...
      status.innerHTML = '';
      results.hidden = false;
      filteredSearchState.rows = rows;
      filteredSearchState.visibleCount = Math.min(FILTER_RESULT_BATCH_SIZE, rows.length);
      renderFilteredSearchResults();
    } catch (error) {
      console.error('Shelfd search filter failed:', error);
      filteredSearchState.active = false;
      status.hidden = false;
      status.innerHTML = '<div class="shelfd-search-noresults"><strong>Filters could not load.</strong><p>Try again.</p></div>';
      results.hidden = true;
      results.innerHTML = '';
    }
  };

  window.loadMoreShelfdSearchFilteredResults = async function() {
    if (!filteredSearchState.active || filteredSearchState.loading) return;
    const nextVisibleCount = filteredSearchState.visibleCount + FILTER_RESULT_BATCH_SIZE;
    if (nextVisibleCount <= filteredSearchState.rows.length) {
      filteredSearchState.visibleCount = nextVisibleCount;
      renderFilteredSearchResults();
      return;
    }
    if (filteredSearchState.exhausted) return;

    const currentToken = queryToken;
    const previousRowCount = filteredSearchState.rows.length;
    filteredSearchState.loading = true;
    renderFilteredSearchResults();
    try {
      const nextPageCount = filteredSearchState.fetchPageCount + 1;
      const rows = await fetchShelfdSearchFilteredRows(nextPageCount);
      if (currentToken !== queryToken) return;
      filteredSearchState.fetchPageCount = nextPageCount;
      filteredSearchState.rows = rows;
      if (rows.length <= previousRowCount) filteredSearchState.exhausted = true;
      filteredSearchState.visibleCount = Math.min(nextVisibleCount, rows.length);
    } catch (error) {
      console.error('Shelfd search load more failed:', error);
      filteredSearchState.exhausted = true;
    } finally {
      if (currentToken === queryToken) {
        filteredSearchState.loading = false;
        renderFilteredSearchResults();
      }
    }
  };
  ...
  function promoteReaderSearchProfileOpen() {
    const page = $('shelfd-search-page');
    if (!page || page.dataset.searchReturnTarget !== 'news-reader') return false;
    try {
      document.body.classList.add('shelfd-search-profile-top-open');
      page.classList.add('shelfd-search-page--profile-open');
    } catch (_) {}
    return true;
  }
  ...
  window.handleSearchPageMediaClick = function(event, type, id) {
    ...
    promoteReaderSearchProfileOpen();
    if (typeof window.openDiscoverMediaProfile === 'function') {
      try {
  ...
  /* v11.755: hard "close everything" used by the top-right X on the search
     page. Unlike handleSearchBack (which pops ONE layer per tap — media
     profile → query → page), this tears down every open layer in a single tap
     so the user never has to back out 3×. */
  window.closeSearchPageFully = function() {
    /* 1. Tear down any media profile(s) opened from the results. Clear its
          back-stack first so closeDiscoverMediaProfile doesn't pop to a
          previous "More Like This" title — we want it gone entirely. */
    try {
      if (Array.isArray(window.discoverMediaProfileBackStack)) window.discoverMediaProfileBackStack.length = 0;
      if (document.getElementById('discover-media-profile') && typeof window.closeDiscoverMediaProfile === 'function') {
        window.closeDiscoverMediaProfile('search-full-close');
      }
    } catch (_) {}
    /* 2. Collapse every open preset panel on the search surface. */
    try { closeAllPresetPanels({ surface: 'search', immediate: true }); } catch (_) {}
    /* 3. Reset the query box so the page reopens fresh next time. */
    try {
      const input = document.getElementById('shelfd-search-input');
      const clearBtn = document.getElementById('shelfd-search-clear-btn');
      if (input) input.value = '';
