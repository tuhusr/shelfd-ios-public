...
const DISCOVERY_HUB_TAP_MS = 300;
const DISCOVERY_HUB_TAP_EASE = 'cubic-bezier(0.215, 0.61, 0.355, 1)'; // easeOutCubic — sharp, fast-start
...
function transitionDiscoveryHub(previousHub, nextHub, preClone) {
  const token = ++discoveryHubTransitionToken;
  const prevPanel = getDiscoveryHubPanel(previousHub);
  const nextPanel = getDiscoveryHubPanel(nextHub);
  if (!nextPanel) return;

  const prevIdx = DISCOVERY_HUB_ORDER.indexOf(normalizeDiscoveryHub(previousHub));
  const nextIdx = DISCOVERY_HUB_ORDER.indexOf(normalizeDiscoveryHub(nextHub));
  const dir = nextIdx > prevIdx ? 1 : -1;
  const W = window.innerWidth || 390;
  const samePanel = prevPanel === nextPanel;

  function styleFixed(el, z, startX, topPx) {
  ...
  function clearFixed(el) {
    if (!el) return;
    ['position', 'top', 'left', 'right', 'bottom', 'overflow', 'z-index',
     'pointer-events', 'will-change', 'transform', 'transition', 'margin'].forEach(function(p) {
      el.style.removeProperty(p);
    });
  }

  // Read ALL geometry before any DOM writes
  const anchor = prevPanel || nextPanel;
  const rect = anchor.getBoundingClientRect();
  const topPx = Math.max(0, Math.round(rect.top));
  const srcToggle = anchor.querySelector('.discover-hub-toggle');
  const toggleRect = srcToggle ? srcToggle.getBoundingClientRect() : null;

  let outgoing, incoming, outIsClone;

  if (samePanel && preClone) {
    outgoing = preClone;
    outgoing.setAttribute('data-discover-hub-tap-out', '1');
    outIsClone = true;
    incoming = nextPanel;
    styleFixed(outgoing, 4, 0, topPx);
    document.body.appendChild(outgoing);
    styleFixed(incoming, 5, dir * W, topPx);
  } else if (!samePanel && prevPanel) {
    outgoing = prevPanel;
    outIsClone = false;
    incoming = nextPanel;
    getDiscoveryHubPanels().forEach(function(p) {
      if (p !== prevPanel && p !== nextPanel) p.style.display = 'none';
    });
    styleFixed(outgoing, 4, 0, topPx);
    styleFixed(incoming, 5, dir * W, topPx);
  } else {
    getDiscoveryHubPanels().forEach(function(p) {
      clearDiscoveryHubTransitionClasses(p);
      p.style.display = p === nextPanel ? 'block' : 'none';
    });
    return;
  }

  // Pin the hub toggle bar as a static overlay so it doesn't slide with the panels
  var staticToggle = null;
  if (srcToggle && toggleRect) {
    var tClone = srcToggle.cloneNode(true);
    tClone.setAttribute('data-discover-hub-tap-toggle', '1');
    // Mirror all computed longhands — avoids shorthand gaps (e.g. padding) on WebKit
    var mirrorStyles = function(from, to) {
      var cs = window.getComputedStyle(from);
      for (var i = 0; i < cs.length; i++) {
        var p = cs[i];
        to.style.setProperty(p, cs.getPropertyValue(p));
      }
    };
    mirrorStyles(srcToggle, tClone);
    var srcBtns = srcToggle.querySelectorAll('.discover-hub-btn');
    var cloneBtns = tClone.querySelectorAll('.discover-hub-btn');
    for (var bi = 0; bi < cloneBtns.length && bi < srcBtns.length; bi++) {
      mirrorStyles(srcBtns[bi], cloneBtns[bi]);
    }
    // Strip active state so no purple bubble shows while sliding
    cloneBtns.forEach(function(b) { b.classList.remove('active'); });
    // Fixed overlay positioning (overrides mirrored in-flow values)
    tClone.style.position = 'fixed';
    tClone.style.top = Math.round(toggleRect.top) + 'px';
    tClone.style.left = Math.round(toggleRect.left) + 'px';
    tClone.style.width = Math.round(toggleRect.width) + 'px';
    tClone.style.margin = '0';
    tClone.style.zIndex = '6';
    tClone.style.pointerEvents = 'none';
    tClone.style.transition = 'none';
    tClone.style.transform = 'none';
    // Opaque backing so text rasterises identically on iOS GPU layer (prevents font-weight shimmer)
    var bgNode = srcToggle.parentElement;
    while (bgNode) {
      var bgVal = window.getComputedStyle(bgNode).backgroundColor || '';
      var m = bgVal.match(/^rgba?\(([^)]+)\)/);
      if (m) {
        var parts = m[1].split(',').map(Number);
        if ((parts.length >= 4 ? parts[3] : 1) >= 0.99) {
          tClone.style.setProperty('background-color', bgVal, 'important');
          break;
        }
      }
    ...
    void tClone.offsetHeight; // rasterise at engage, not mid-slide
    staticToggle = tClone;
    // Hide the real toggle inside both sliding panels
    [outgoing, incoming].forEach(function(el) {
      if (!el) return;
      var tg = el.querySelector('.discover-hub-toggle');
  ...
  var ease = 'transform ' + DISCOVERY_HUB_TAP_MS + 'ms ' + DISCOVERY_HUB_TAP_EASE;
  outgoing.style.transition = ease;
  outgoing.style.transform = 'translate3d(' + (-dir * W) + 'px,0,0)';
  ...
  function finish() {
    ...
    if (token !== discoveryHubTransitionToken) {
      if (outIsClone && outgoing) outgoing.remove();
      return;
    }
    if (outIsClone && outgoing) {
      outgoing.remove();
    } else if (outgoing) {
...
function switchDiscoveryHub(hub) {
  ...
  // Scroll to top first so all geometry measurements happen at position 0.
  // This prevents the movies↔tv section-reflow from shifting the toggle position.
  if (getActiveMainTab() === 'discover') {
    window.scrollTo({ top: 0, behavior: 'auto' });
  }

  // Capture pre-update clone for movies↔tv (same DOM panel) before sync changes sections
  let preClone = null;
  if (samePanel && prevPanel && getActiveMainTab() === 'discover') {
    preClone = prevPanel.cloneNode(true);
  }
  ...
  if (getActiveMainTab() === 'discover') {
    transitionDiscoveryHub(previousHub, nextHub, preClone);
    loadActiveDiscoveryHub();
  }
  persistUiState();
}
...
const MAIN_NAV_SLIDE_MS = 360;
const MAIN_NAV_SLIDE_EASE = 'cubic-bezier(0.25, 0.46, 0.45, 0.94)';
...
function scheduleMainNavPostTransitionHydration(callback, delayOrMobile = false) {
  if (typeof callback !== 'function') return;
  const delay = typeof delayOrMobile === 'number'
    ? delayOrMobile
    : delayOrMobile
...
// v12.048: Full-width 360ms horizontal slide matching the discovery hub tap animation.
// Clones outgoing panel(s) as body-level overlays so setMainNavVisibility can run freely,
// then slides them out while the real incoming panel slides in simultaneously.
async function animateMainNavSlide(fromTab, toTab, revealNextPanel) {
  const W = window.innerWidth || 390;
  const dir = getMainNavTransitionDirection(fromTab, toTab);

  // Scroll to top so geometry is measured at position 0
  window.scrollTo({ top: 0, behavior: 'auto' });

  // Snapshot outgoing panels before reveal changes visibility
  const outPanels = getVisibleMainNavPanels(getMainNavPanels(fromTab));

  // Clone each outgoing panel as a fixed body-level overlay
  const clones = outPanels.map(function(el) {
    var rect = el.getBoundingClientRect();
    var clone = el.cloneNode(true);
    clone.setAttribute('data-main-nav-slide-out', '1');
  ...
  // Reveal the incoming panel (runs setMainNavVisibility + schedules hydration)
  await revealNextPanel();

  // Get incoming panels (now visible after reveal)
  var inPanels = getVisibleMainNavPanels(getMainNavPanels(toTab));

  if (!inPanels.length && !clones.length) return;

  // Fix incoming panels off-screen in the slide direction
  inPanels.forEach(function(el) {
    var rect = el.getBoundingClientRect();
    el.style.position = 'fixed';
  ...
  // Force paint before applying transitions
  if (inPanels[0]) void inPanels[0].offsetHeight;
  if (clones[0]) void clones[0].offsetHeight;

  var ease = 'transform ' + MAIN_NAV_SLIDE_MS + 'ms ' + MAIN_NAV_SLIDE_EASE;
  ...
  return new Promise(function(resolve) {
    function finish() {
      clones.forEach(function(c) { try { c.remove(); } catch(_) {} });
      inPanels.forEach(function(el) {
        ['position', 'top', 'left', 'right', 'bottom', 'overflow', 'z-index',
         'pointer-events', 'will-change', 'transform', 'transition', 'margin'].forEach(function(p) {
          el.style.removeProperty(p);
    ...
    var watchEl = inPanels[0] || clones[0];
    if (watchEl) watchEl.addEventListener('transitionend', finish, { once: true });
    setTimeout(finish, MAIN_NAV_SLIDE_MS + 80);
  });
}
  ...
  try {
    const revealNextMainNavPanel = async () => {
      if (normalizedTab === 'community') {
        ...
        /* v12.056: pre-swap sub-tab DOM before the community-view is revealed so
           the correct sub-tab is visible from the first painted frame. Without this,
           the view shows whichever sub-tab was last active (often friends-list) for
           the entire 390ms hydration delay — causing a visible flash of the friends
           list when navigating Friends → MyList → Activity. DOM-only: no content
           loading, no runFriendsTabWorkWhenSmooth; loadCommunity at 390ms handles that. */
        const activityTabView = document.getElementById('activity-tab-view');
        ...
        const communityViewEl = document.getElementById('community-view');
        if (activityTabView) activityTabView.style.display = friendsListMode ? 'none' : 'block';
        if (friendsListView) friendsListView.style.display = friendsListMode ? 'block' : 'none';
        if (addFriendView && !friendsListMode) addFriendView.style.display = 'none';
        if (notificationsView && !friendsListMode) notificationsView.style.display = 'none';
        if (communityViewEl) communityViewEl.classList.toggle('friends-activity-active', !friendsListMode);
        /* v12.057: pre-render the friends list while the view is still hidden so
           content is already in the DOM when setMainNavVisibility reveals it.
           renderFriendsList has a sig-check guard — if data hasn't changed since
           last render it returns immediately (no-op), so this is free on repeat visits.
           Profiles are cached after first load so the sync path runs for most visits. */
        if (friendsListMode && typeof renderFriendsList === 'function') {
        ...
        scheduleMainNavPostTransitionHydration(hydrateCommunity, 390);
      }
      setMainNavVisibility(normalizedTab);
      if (normalizedTab === 'discover') {
        scheduleMainNavPostTransitionHydration(() => loadActiveDiscoveryHub(), 390);
      }
      if (normalizedTab === 'mylist' && viewingUser) {
        scheduleMainNavPostTransitionHydration(() => backToMyList(), 390);
      } else if (normalizedTab === 'mylist') {
        /* v12.055: only re-render on entry if the grid is empty (first paint
           or after data cleared). When it already has cards — e.g. returning
           from Discover — the grid is correct; a full innerHTML rebuild here
           forces every poster image to re-decode and causes a 1-frame black
           flash. The realtime Firestore listener calls render() on any data
           change, so skipping it here loses no freshness. */
        scheduleMainNavPostTransitionHydration(() => {
          const grid = document.getElementById('cards-grid');
          if ((!grid || !grid.children.length) && typeof render === 'function') render();
        }, 390);
      }
    };
