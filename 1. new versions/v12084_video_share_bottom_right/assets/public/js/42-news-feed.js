...
(function () {
    ...
    return '<article class="news-card" role="button" tabindex="0" data-news-url="' + escA(it.url) + '"' +
      ...
      '<div class="news-card-main">' +
        '<div class="news-card-meta">' +
          ...
          '<div class="news-card-actions">' + followBtn + ((isVideo && vid) ? '' : heartBtn) + ((isVideo && vid) ? '' : shareBtn) + '</div>' +
        '</div>' +
  ...
  function openNewsYouTube(id, meta) {
    ...
    var src = 'https://www.youtube.com/embed/' + encodeURIComponent(id) + '?autoplay=1&playsinline=1&rel=0&modestbranding=1';   // v11.968: youtube.com (not nocookie) so the embed carries trust cookies → no bot wall
  ...
  /* v11.967: GESTURE-GATED sound (Instagram/TikTok model). Default is now OFF
     (muted) — the FIRST video a user meets autoplays SILENTLY with the unmute
     speaker badge. The moment they tap unmute on ANY video (a real user gesture →
     setNewsSoundPref(true) inside toggleInlineNewsVideoMute), the pref flips ON and
     EVERY later video auto-plays WITH sound, via the bot-safe muted-load → API-unMute
     path (NEVER mute=0, which tripped YouTube's bot wall — see mountAutoplayVideo).
     The choice persists per device; muting any video turns it back off. Returning
     users who already opted into sound keep it. (Was default-ON before v11.967, which
     auto-unmuted even the very first video with no opt-in.) */
  function newsSoundPrefOn() {
    try { return localStorage.getItem(NEWS_SOUND_KEY) === '1'; } catch (e) { return false; }
  }
  ...
  function newsVideoEmbedSrc(ytId, muted) {
    ...
    /* v11.727: CONTINUOUS LOOP — loop=1 + playlist=<same id> auto-replays the video
       ...
       v11.968: domain is youtube.com, NOT youtube-nocookie.com. The nocookie domain
       deliberately sets NO cookies, so in a WKWebView (which starts with no YouTube
       session) every embed is a cold, sessionless, consentless request → YouTube's
       anti-bot system challenges it ("Sign in to confirm you're not a bot"). The
       regular domain lets YouTube set/carry its trust+consent cookies so the client
       looks established instead of bot-like. (Full reliability still needs the native
       WKWebView cookie persistence — see the iOS handoff — but nocookie can NEVER be
       trusted, so this is the required web-side foundation.) */
  ...
  function postInlineNewsVideoCommand(func) {
    try {
      ...
      /* v11.968: target origin MUST match the iframe's domain (now youtube.com) or
         the command is dropped (postMessage origin check). */
      frame.contentWindow.postMessage(JSON.stringify({ event: 'command', func: func, args: [] }), 'https://www.youtube.com');
      return true;
    } catch (e) { return false; }
  }
  ...
  /* v11.966: nudgeActiveVideoSound (added v11.947 for the unmuted-mount path) was
     removed — we mount MUTED again, so applyAutoplayUnmute (state-tracked) is the
     correct unMute path. See mountAutoplayVideo for why unmuted-mount was reverted. */
  ...
  function _mountNewsVideo(card, meta, ytId, muted, isAutoplay) {
    ...
    try {
      ...
      if (vframe) vframe.addEventListener('load', function () {
        /* v11.966: REPEAT the API handshake for ~2.4s. A single post on `load` can
           race the player's internal API init — if it lands too early it's dropped,
           the PLAYING (state 1) event never reaches us, and the auto-unMute never
           fires → that source autoplays silently (the v11.947 "some sources have no
           sound" complaint). Re-posting registers the event channel reliably, so the
           PLAYING event arrives and sound lands on every source — all WITHOUT the
           unmuted-mount that tripped YouTube's bot wall. */
        [0, 150, 400, 800, 1500, 2400].forEach(function (d) {
          setTimeout(function () {
            try { if (state.inlineVideoMedia === media && vframe.contentWindow) vframe.contentWindow.postMessage(JSON.stringify({ event: 'listening', id: ytId, channel: 'widget' }), '*'); } catch (e) {}
          }, d);
        });
      });
      applyYouTubeOriginalAspectToMedia(media, ytId);
  ...
  function mountAutoplayVideo(card) {
    ...
    /* v11.966: ALWAYS mount MUTED, then unMute via the YT JS API AFTER the player
       reports it is actually PLAYING. This REVERTS the v11.947 "mount unmuted
       (mute=0) from frame 1" change, which tripped YouTube's "Sign in to confirm
       you're not a bot" wall inside the embedded player: unmuted programmatic
       autoplay is the #1 signal their anti-bot system flags, and the native
       WKWebView autoplay flag (mediaTypesRequiringUserActionForPlayback=[]) only
       relaxes the WebVIEW's LOCAL gate — it does NOT exempt us from YouTube's
       SERVER-SIDE bot check. Muted autoplay is the only embed pattern YouTube
       allows; unMuting AFTER playback has begun is just a volume change that does
       NOT re-trip the autoplay/bot gate. Sound reliability ("some sources silent",
       the v11.947 complaint) is handled WITHOUT unmuted-mount by the repeated
       `listening` handshake in _mountNewsVideo (so the PLAYING event reliably
       arrives → unMute fires) plus these timed fallbacks. */
    var wantSound = newsSoundPrefOn() && newsSoundCapable();
    _mountNewsVideo(card, meta, youtubeVideoId(meta.url), true, true);   // muted at load (YouTube-compliant; no bot wall)
    state.inlineVideoWantSound = wantSound;
    if (wantSound) {
      var media = state.inlineVideoMedia;
      [400, 900, 1600, 2600, 3800].forEach(function (d) {   // unMute once playing; primary trigger is the PLAYING-state event
        setTimeout(function () { applyAutoplayUnmute(media); }, d);
      });
    }
  }
  ...
  window.shelfdMountNewsInlineVideo = function (media, meta, opts) {
    ...
    /* v11.966: mount MUTED, then unMute after playback starts (mirrors the feed).
       Reverts the v11.947 direct-unmuted mount that tripped YouTube's bot-check wall
       on the embedded player. */
    var readerWantSound = !!opts.wantSound && opts.muted !== false && newsSoundPrefOn() && newsSoundCapable();
    _mountNewsVideo(shimCard, meta, ytId, true, true);
    if (readerWantSound) {
      state.inlineVideoWantSound = true;
      [400, 900, 1600, 2600, 3800].forEach(function (d) {
        setTimeout(function () { applyAutoplayUnmute(media); }, d);
      });
    }
    return true;
  ...
  /* v12.025: engagement API for the News Reader video page. A shared video link
     opens the reader (not the feed), so the reader needs to reuse the feed's
     like / comment / view system. These all operate on rows carrying
     [data-news-engagement][data-news-video-id], so any row the reader renders
     with those attributes is picked up automatically by the same hydrate/render
     pipeline the feed uses. */
  window.shelfdNewsVideoEngagement = {
    hydrate: function () { try { hydrateVideoEngagementStats(); } catch (e) {} },
    render: function (vidId) { try { renderVideoEngagement(vidId); } catch (e) {} },
    like: function (vidId, on) { try { toggleShelfdVideoLike(vidId, !!on); } catch (e) {} },
    isLiked: function (vidId) { try { return !!shelfdVideoLikeInfo(vidId).liked; } catch (e) { return false; } },
    openComments: function (vidId, title) { try { openYouTubeCommentsSheet(vidId, title || ''); } catch (e) {} }
  };
