...
const MODAL_STATUS_OPTIONS = {
  ...
  games: [
    ...
    { status: 'party',    label: 'Party Games' },
    { status: 'planned',  label: 'Planning' },
...
function getDiscoverLibraryStatusLabel(status, section) {
  ...
  if (normalized === 'watched') return section === 'games' ? 'Played' : 'Watched';
  if (normalized === 'watching') return section === 'games' ? 'Playing' : 'Watching';
...
async function submitModal(status, rating = 0, itemOverride = null) {
  ...
  if (targetSection === 'games' && !validStatuses.includes('competitive')) validStatuses.push('competitive');
  ...
  if (targetSection === 'games' && ['watching', 'live', 'competitive', 'party'].includes(status)) {
    activeTab = 'watching';
    if (typeof activeGamePlayingFilter !== 'undefined') {
      activeGamePlayingFilter = status === 'competitive'
        ? 'competitive'
        : status === 'watching'
          ? 'watching'
          : status === 'party'
