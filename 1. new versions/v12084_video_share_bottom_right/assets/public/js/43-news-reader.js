...
(function () {
  ...
  function readerVideoShellHtml(meta) {
    ...
    var imgHtml = img
      ...
      : '';
    /* v12.025: engagement row (likes · comment button · views) under the video,
       mirroring the feed card. The [data-news-engagement][data-news-video-id]
       attributes let the feed's shared hydrate/render pipeline populate it. */
    var engageHtml = ytId
      ? '<div class="news-ytc-engage news-reader-ytc-engage" data-news-engagement data-news-video-id="' + escA(ytId) + '">' +
  ...
  function renderReaderYouTube(token, ytId, meta) {
    ...
    var src = 'https://www.youtube.com/embed/' + encodeURIComponent(ytId) + '?autoplay=1&playsinline=1&rel=0&modestbranding=1&controls=1&fs=1';   // v11.968: youtube.com (not nocookie) so the embed carries trust cookies → no bot wall
  ...
  function openNewsReaderOverlay(meta) {
    ...
    if (heartBtn) {
      /* v12.025: on a VIDEO reader the heart is Like + Save (mirrors the feed) —
         it keeps the article-bookmark visual state and ALSO toggles the communal
         video like, so the like count everyone sees moves when you tap it. */
      heartBtn.addEventListener('click', function (e) {
        e.preventDefault();
        var on;
        if (typeof window.shelfdNewsToggleArticleHeart === 'function') {
          on = window.shelfdNewsToggleArticleHeart(normMeta.url, normMeta);
        } else {
          on = !heartBtn.classList.contains('is-bookmarked');
        }
        heartBtn.classList.toggle('is-bookmarked', !!on);
        heartBtn.setAttribute('aria-pressed', on ? 'true' : 'false');
        if (ytId && window.shelfdNewsVideoEngagement) window.shelfdNewsVideoEngagement.like(ytId, on);
        if (on) {
    ...
    if (ytId) {
      renderReaderYouTube(token, ytId, normMeta);   // trailers play, no extraction
      /* v12.025: wire the comment button, load like/view/comment counts, and let a
         tap on the video (re)mount it — cold-open autoplay can be blocked, so the
         user must always be able to start playback by tapping. */
      overlay.addEventListener('click', function (e) {
        var btn = e.target && e.target.closest ? e.target.closest('[data-news-comments]') : null;
        if (!btn) return;
        e.preventDefault();
        if (e.stopPropagation) e.stopPropagation();
        if (window.shelfdNewsVideoEngagement) window.shelfdNewsVideoEngagement.openComments(ytId, normMeta.title || '');
      });
      var videoEl = overlay.querySelector('[data-news-reader-video]');
      if (videoEl) {
        videoEl.addEventListener('click', function (e) {
          var volBtn = e.target && e.target.closest ? e.target.closest('[data-news-video-volume]') : null;
          if (volBtn) return;                          // mute toggle owns its taps
          if (videoEl.querySelector('iframe')) return; // already mounted/playing
          renderReaderYouTube(token, ytId, normMeta);
        });
      }
      if (window.shelfdNewsVideoEngagement) {
        window.shelfdNewsVideoEngagement.hydrate();
        // retries: the feed module's stats/likes caches or Firestore may init slightly later
        setTimeout(function () { if (token === openToken && window.shelfdNewsVideoEngagement) window.shelfdNewsVideoEngagement.hydrate(); }, 600);
        setTimeout(function () { if (token === openToken && window.shelfdNewsVideoEngagement) window.shelfdNewsVideoEngagement.hydrate(); }, 1800);
      }
      return;
    }
