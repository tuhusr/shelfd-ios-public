...
function syncProfileSocialRowButtons(modal) {
  ...
  if (!modal) return;
  modal.querySelectorAll('.profile-social-row[data-uid]').forEach(repaintProfileSocialRowActions);
}
function syncFollowButtonsForUid(uid) {
  ...
  if (modal) {
    modal.querySelectorAll('.profile-social-row[data-uid]').forEach(row => {
      ...
      repaintProfileSocialRowActions(row);
    });
  }
}
...
/* v11.982: right-side action(s) for a social-list row.
   On MY OWN Followers tab each follower shows [Message | Follow back | Requested]
   + an X that opens the "Remove follower?" sheet. Message opens/creates the DM;
   Follow back/Requested is my follow status TOWARD them (mutual → Message). The X
   removes them as a follower ONLY (does NOT unfollow them). Every other tab / any
   other user's lists keep the single follow button. */
function renderProfileSocialRowActionsHTML(user, kind = '') {
  if (!user || !user.uid) return '';
  const own = (typeof isViewingOtherProfile !== 'function') || !isViewingOtherProfile();
  const isOwnFollowersTab = kind === 'followers'
    && own
    && (typeof isPreviewMode !== 'function' || !isPreviewMode())
    && currentUser && user.uid !== currentUser.uid;
  if (!isOwnFollowersTab) return getProfileSocialFollowButtonHTML(user);
  const u = escAttr(user.uid);
  const uid = user.uid;
  let leftBtn;
  if (Array.isArray(friends) && friends.includes(uid)) {
    leftBtn = `<button type="button" class="profile-social-msg-btn" onclick="event.stopPropagation(); openDirectMessageFromUser('${u}')">Message</button>`;
  } else if (Array.isArray(outgoingRequests) && outgoingRequests.includes(uid)) {
    leftBtn = `<button type="button" class="profile-social-follow-btn is-requested" onclick="event.stopPropagation(); toggleProfileSocialFollow('${u}')">Requested</button>`;
  } else {
    leftBtn = `<button type="button" class="profile-social-follow-btn is-followback" onclick="event.stopPropagation(); toggleProfileSocialFollow('${u}')">Follow back</button>`;
  }
  const xBtn = `<button type="button" class="profile-social-x-btn" aria-label="Remove follower" onclick="event.stopPropagation(); confirmRemoveFollower('${u}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="6" y1="6" x2="18" y2="18"/><line x1="18" y1="6" x2="6" y2="18"/></svg></button>`;
  return `<span class="profile-social-row-actions">${leftBtn}${xBtn}</span>`;
}

/* Repaint a row's right-side action(s) in place from live state. Handles both the
   followers-tab action group and the single follow button on the other tabs. */
function repaintProfileSocialRowActions(row) {
  if (!row) return;
  const uid = row.getAttribute('data-uid');
  if (!uid) return;
  const kind = row.getAttribute('data-social-kind') || '';
  const user = (typeof usersMap === 'object' && usersMap && usersMap[uid]) ? usersMap[uid] : { uid };
  const actions = row.querySelector('.profile-social-row-actions');
  if (actions) {
    const tpl = document.createElement('template');
    tpl.innerHTML = String(renderProfileSocialRowActionsHTML(user, kind) || '').trim();
    const fresh = tpl.content.firstElementChild;
    if (fresh) actions.replaceWith(fresh); else actions.remove();
    return;
  }
...
function renderProfileSocialUserRow(user, kind = '') {
  ...
  return `<div class="profile-social-row" data-uid="${uidAttr}" data-social-kind="${escAttr(kind)}">
    <button type="button" class="profile-social-row-main" onclick="openProfileSocialUser('${uidAttr}')">
      ...
      <span class="profile-social-row-copy">
        <span class="profile-social-row-name">${escHtml(primary)}${typeof renderShelfdOwnerBadgeHTML === 'function' ? renderShelfdOwnerBadgeHTML(user) : ''}</span>
        ${secondary ? `<span class="profile-social-row-sub">${escHtml(secondary)}</span>` : ''}
      </span>
    </button>
    ${renderProfileSocialRowActionsHTML(user, kind)}
  </div>`;
}
