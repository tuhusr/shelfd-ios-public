...
(function initShelfdMovieRatingDuel() {
  ...
  /* v11.030: position-based auto-scroll during a tier drag. Direction is
     decided by WHERE in the tier the held poster sits (its X relative to
     the row's horizontal center), not by proximity to a fixed edge zone:
       • held in the LEFT portion  → scroll RIGHT  (scrollLeft increases)
       • held in the RIGHT portion → scroll LEFT   (scrollLeft decreases)
     A centered dead-zone (middle ~44%) means holding mid-row does NOT
     scroll — this fixes the accidental horizontal scroll the edit-mode
     long-press used to trigger. Speed ramps with distance from center so
     the nearer the row's end you hold, the faster it travels. Returns the
     actual pixels scrolled (0 when clamped / in the dead-zone) so the
     caller can skip needless placeholder churn. */
  function autoScrollMovieDuelTierRow(scrollEl, clientX = 0) {
    ...
    const norm = (clientX - center) / half; /* -1 far-left .. +1 far-right */
    const DEAD_ZONE = 0.22;
    if (Math.abs(norm) <= DEAD_ZONE) return 0;
    const MAX_SPEED = 18;
    const ramp = (Math.abs(norm) - DEAD_ZONE) / (1 - DEAD_ZONE); /* 0..1 */
    /* held left (norm<0) → +delta (scroll right); held right → -delta. */
    const delta = -Math.sign(norm) * Math.max(1, Math.round(ramp * MAX_SPEED));
    const before = scrollEl.scrollLeft;
    scrollEl.scrollLeft = before + delta;
    return scrollEl.scrollLeft - before;
  }

  /* v11.030: continuous auto-scroll pump. Runs on rAF for the life of a
     drag so the row keeps scrolling even when the finger is held
     perfectly still in the left/right portion (the old edge logic only
     scrolled on pointermove, so a stationary hold did nothing). Uses the
     drag's last known pointer position; re-places the drop placeholder
     only on frames that actually scrolled. */
  function stepMovieDuelTierAutoScroll() {
    const state = movieDuelTierDragState;
    if (!state || !state.dragging) {
      if (state) state.autoScrollRaf = 0;
      return;
    }
    const scrollEl = getMovieDuelTierScrollFromPoint(state.lastX, state.lastY);
    if (scrollEl) {
      const scrolled = autoScrollMovieDuelTierRow(scrollEl, state.lastX);
      if (scrolled) placeMovieDuelTierDragPlaceholder(scrollEl, state.lastX);
    }
    state.autoScrollRaf = requestAnimationFrame(stepMovieDuelTierAutoScroll);
  }
  ...
  function startMovieDuelTierLongPressDrag(event) {
    ...
    /* v11.030: kick off the continuous auto-scroll pump for this drag. */
    if (!state.autoScrollRaf) state.autoScrollRaf = requestAnimationFrame(stepMovieDuelTierAutoScroll);
  ...
  function onMovieDuelTierPointerMove(event) {
    ...
    if (scrollEl) {
      ...
      /* v11.030: auto-scroll is now driven by the rAF pump
         (stepMovieDuelTierAutoScroll) using the drag's last pointer
         position, so it keeps scrolling on a stationary hold. No need to
         call it per-move here. */
    }
