...
function confirmRemoveFollower(uid) {
  ...
  const name = (typeof getShelfdFriendHandle === 'function')
    ? '@' + getShelfdFriendHandle(src, 'User')
    : String(src.usernameHandle || src.name || 'this user').trim();
  const existing = document.getElementById('remove-follower-modal');
  if (existing) existing.remove();
  const modal = document.createElement('div');
  modal.id = 'remove-follower-modal';
  modal.className = 'plm-overlay';
  modal.innerHTML = `
    <div class="plm-sheet">
      <div class="plm-header">
        <span class="plm-title">Remove Follower</span>
        <button class="plm-close" onclick="closeRemoveFollowerModal()">&times;</button>
      </div>
      <p style="color:#a9a0c6;font-size:13px;line-height:1.5;">Remove <strong style="color:#f7f3ff;">${escHtml(name)}</strong> as a follower? They won't be notified, and they'd have to request to follow you again.</p>
      <div class="plm-actions">
        <button class="plm-remove-btn" style="flex:1;" onclick="removeFollower('${escAttr(u)}'); closeRemoveFollowerModal();">Remove</button>
        <button class="plm-save-btn" onclick="closeRemoveFollowerModal()">Cancel</button>
      </div>
    </div>`;
  modal.addEventListener('click', e => { if (e.target === modal) closeRemoveFollowerModal(); });
  document.body.appendChild(modal);
  requestAnimationFrame(() => modal.classList.add('plm-open'));
}
...
function closeRemoveFollowerModal() {
  const modal = document.getElementById('remove-follower-modal');
  if (!modal) return;
  modal.classList.remove('plm-open');
  setTimeout(() => modal.remove(), 230);
}
