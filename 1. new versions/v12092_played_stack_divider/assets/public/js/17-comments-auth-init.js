...
function renderCommentsInput() {
  ...
  const photo = (userProfile && userProfile.photo) || currentUser.photoURL || `/default-avatar.svg#${encodeURIComponent((userProfile && userProfile.name) || currentUser.displayName || '?')}&background=1e2028&color=60a5fa`;
...
function renderCommentsUI(comments) {
  ...
  list.innerHTML = filtered.map(c => {
      const photo = c.photo || '/default-avatar.svg#' + encodeURIComponent(c.name || 'U') + '&background=1e2028&color=60a5fa';
...
function syncShelfdDevAccountClasses(userLike = null) {
  let isCreatorDevAccount = false;
  try {
    if (typeof isCreatorAdmin === 'function') {
      isCreatorDevAccount = !!isCreatorAdmin(userLike);
    } else {
      const uid = String(userLike?.uid || userLike?.id || '').trim();
      isCreatorDevAccount = uid === 'KihPpiqSsFMpn5Tee4xZWFWapg62';
    }
  } catch (_) {}
  try {
    document.body?.classList.toggle('shelfd-dev-account', isCreatorDevAccount);
    document.body?.classList.toggle('shelfd-dev-mylist-status-preview', isCreatorDevAccount);
  } catch (_) {}
  try { window.__shelfdDevAccountActive = isCreatorDevAccount; } catch (_) {}
}
...
auth.onAuthStateChanged(async (user) => {
  ...
  const gameProfileRoute = typeof parseScreenListGameProfileRoute === 'function' ? parseScreenListGameProfileRoute() : null;
  const newsArticleRoute = typeof parseScreenListNewsArticleRoute === 'function' ? parseScreenListNewsArticleRoute() : null;
  ...
  if (user) {
    ...
    if (window.__shelfdSignupInProgress) {
      ...
      syncShelfdDevAccountClasses(user);
      exitPreviewMode();
    ...
    if (gameProfileRoute?.itemId && typeof openSharedGameProfileRoute === 'function') {
      await openSharedGameProfileRoute(gameProfileRoute);
      markScreenListAppReadyForSplash();
      return;
    }
    ...
    landingPublicProfileActive = false;
    currentUser = null;
    syncShelfdDevAccountClasses(null);
    DOC_REF = null;
    userProfile = null;
    ownDataCache = null;
    myData = null;
