...
function getActivityWrittenReviewText(activity = {}) {
...
function getActivityRatingValue(activity = {}) {
  const item = activity?.item || {};
  return Number(activity?.rating || activity?.activityRating || item?.rating || 0);
}

function hasActivityWrittenReview(activity = {}) {
  return getActivityWrittenReviewText(activity).length > 0;
}

function isActivityMediaReviewPost(activity = {}) {
  return (activity?.type === 'media-review' || activity?.eventType === 'review') && hasActivityWrittenReview(activity);
}
...
function getActivityEventType(activity) {
  ...
  if (activity.type === 'media-review') return hasActivityWrittenReview(activity) ? 'review' : (getActivityRatingValue(activity) > 0 ? 'rated' : 'updated');
  if (activity.type === 'activity-stack') return activity.stackEventType || activity.eventType || 'added';
  const item = activity.item || {};
  const evType = activity.eventType;
  if (evType) {
    if (evType === 'review' && !hasActivityWrittenReview(activity)) {
      return getActivityRatingValue(activity) > 0 ? 'rated' : 'updated';
    }
    return evType;
  }
...
function getScreenListActivityDeleteCandidates(activity = {}, fallbackId = '') {
  ...
  const eventType = String(getActivityEventType(activity) || '').trim().toLowerCase();
  const isReviewPost = isActivityMediaReviewPost(activity) || eventType === 'review';
  if (!isReviewPost) {
...
function canCurrentUserNoteActivity(activity = {}) {
  ...
  if (isActivityMediaReviewPost(activity)) return false;
  const eventType = getActivityEventType(activity);
  const item = activity.item || {};
  const section = item.librarySection || item.mediaCategory || '';
...
function getScreenListShareIconSvg() {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12"/><path d="M8 7l4-4 4 4"/><path d="M5 12v7a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-7"/></svg>`;
}

/* v12.032: like + comment glyphs that match the NEWS FEED video-card engagement
   row exactly (thin outline heart + outline comment bubble, 18px, stroke 1.9),
   the only difference being the activity feed renders them in 100%-white. The
   liked heart fills (white). Kept separate from getScreenListHeartIconSvg /
   getScreenListReplyIconSvg so the media-review like button (js/06) is untouched. */
function getActivityFeedLikeIconSvg(isLiked = false) {
  return `<svg viewBox="0 0 24 24" fill="${isLiked ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>`;
}
function getActivityFeedCommentIconSvg() {
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 11.5a8.38 8.38 0 0 1-8.5 8.5 8.5 8.5 0 0 1-3.8-.9L3 21l1.9-5.7A8.38 8.38 0 0 1 4 11.5 8.5 8.5 0 0 1 12.5 3 8.38 8.38 0 0 1 21 11.5z"/></svg>`;
}
...
/* Collapse duplicate completion cards when a real "Wrote a Review" post exists
   for the same (uid, mediaKey) in the same save window.
   ─────────────────────────────────────────────────────────────────────
   ...
   writing a review created TWO activity cards:
     ...
     2. The `media-review` feed post the composer creates ("Wrote a
        Review {title}").
   The review card carries the actual full review text, so keep the review post
   and drop only the nearby completion duplicate. Older completion cards remain
   visible, which lets a deleted-and-rewritten review create a fresh activity.

   This intentionally runs BEFORE `mergeRelatedLibraryActivities` so
   the dropped completion activities never enter the merge keying logic. */
function filterRedundantMediaReviewWhenCompletionExists(activities = []) {
  ...
  const REVIEW_COMPLETION_DEDUPE_WINDOW_MS = 2 * 60 * 1000;
  const keyOf = (a) => {
  ...
  const reviewTimesByKey = new Map();
  for (const a of activities) {
    if (!a) continue;
    if (!isActivityMediaReviewPost(a)) continue;
    const key = keyOf(a);
    if (!key) continue;
    const time = parseFriendActivityTime(a.timestamp || a.updatedAt || a.createdAt || a.item?.dateModified || a.item?.dateAdded);
    const times = reviewTimesByKey.get(key) || [];
    times.push(time || 0);
    reviewTimesByKey.set(key, times);
  }
  if (!reviewTimesByKey.size) return activities;
  return activities.filter(a => {
    if (!a) return false;
    const isReview = isActivityMediaReviewPost(a);
    ...
    if (!isCompletionLike) return true;
    const key = keyOf(a);
    const reviewTimes = key ? reviewTimesByKey.get(key) : null;
    if (!reviewTimes || !reviewTimes.length) return true;
    const completionTime = parseFriendActivityTime(a.timestamp || a.item?.dateModified || a.item?.dateAdded || a.updatedAt || a.createdAt);
    if (!completionTime) return true;
    return !reviewTimes.some(time => time > 0 && Math.abs(completionTime - time) <= REVIEW_COMPLETION_DEDUPE_WINDOW_MS);
  });
}
...
function getScreenListActivitySectionPlural(section = '', count = 0) {
  ...
  if (section === 'music') return total === 1 ? 'album' : 'albums';
...
function getScreenListActivityDestinationLabel(status = '', section = '') {
  ...
  if (section === 'games') {
    if (clean === 'planned') return 'Planning';
  ...
  if (section === 'music') {
    if (clean === 'planned') return 'Planning';
    if (clean === 'watched') return 'Listened';
  }
  if (clean === 'planned') return 'Planning';
...
function getScreenListStackedActivityClassification(activity = {}) {
  ...
  if (eventType === 'review' || isActivityMediaReviewPost(activity)) {
...
function getScreenListStackedActivityCountWord(count = 0) {
  const total = Math.max(0, Number(count) || 0);
  const words = {
    0: 'zero',
    1: 'one',
  ...
  return words[total] || String(total);
}
...
function getScreenListStackedActivityDisplayParts(group = [], type = '') {
  ...
  if (section === 'music' && (type === 'rated' || type === 'status:watched')) {
    return { action: 'Listened to', title: `${total} ${plural}`, detail: 'and rated them' };
  }
  ...
  if (type === 'review') {
    const countWord = getScreenListStackedActivityCountWord(total);
    return { action: 'Wrote a review for', title: `${countWord} title${total === 1 ? '' : 's'}` };
  }
  if (type === 'added') {
    const destination = getScreenListActivityDestinationLabel(getScreenListActivityStackStatus(primary, item), section);
    return { action: 'Added', title: `${total} ${plural}`, detail: `to their ${String(destination || 'library').toLowerCase()}` };
  }
...
function getScreenListStackedActivityTypeLabel(type = '', count = 0, group = []) {
  ...
  return `${parts.action} ${parts.title || `${total} title${total === 1 ? '' : 's'}`} ${parts.detail || ''}`.replace(/\s+/g, ' ').trim();
}
...
function buildScreenListStackedActivity(group = [], key = '') {
  ...
  return {
    ...
    stackDisplayDetail: stackDisplay.detail || '',
...
function ensureScreenListStackedActivityPage() {
  ...
  bindScreenListStackedActivityPageZoomGuard(page);
  return page;
}

function bindScreenListStackedActivityPageZoomGuard(page) {
  if (!page || page.dataset.zoomGuardBound === '1') return;
  page.dataset.zoomGuardBound = '1';
  let lastTapAt = 0;
  let lastTapX = 0;
  let lastTapY = 0;
  page.addEventListener('touchend', event => {
    ...
    if (!touch) return;
    const now = Date.now();
    const closeInTime = now - lastTapAt > 0 && now - lastTapAt < 320;
    const closeInSpace = Math.abs(touch.clientX - lastTapX) < 28 && Math.abs(touch.clientY - lastTapY) < 28;
...
function updateActivityInteractionCardState(card, activity = {}) {
  ...
  if (likeIconSlot) likeIconSlot.innerHTML = getActivityFeedLikeIconSvg(!!isLiked);
}
...
function setScreenListActivityLikeButtonVisualState(btnEl, isLiked = false) {
  ...
  if (iconSlot) iconSlot.innerHTML = getActivityFeedLikeIconSvg(!!isLiked);
}
...
function handleScreenListActivityCardOpen(activityId = '', kind = 'activity') {
  const cleanId = String(activityId || '').trim();
  if (!cleanId) return;
  /* Highlight clips open the dedicated full-page reader instead of the generic
     comment sheet — tapping outside the inline embed shows the reader page. */
  const activity = friendActivityClickTargets[cleanId];
  if (activity && isScreenListHighlightActivity(activity) && typeof openHighlightReaderPage === 'function') {
    openHighlightReaderPage(cleanId, activity);
    return;
  }
  const reduceMotion = !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
...
/* ===================================================================
   v11.461: Activity Feed "+" create-post flow.
   The Plus button (which replaces the Import pill on the Activity tab)
   opens this sheet. Only the Highlight Reel post type is active right
   now. It shares the EXACT game-highlight data path used by the
   game-profile editor (commitGameHighlightClip in js/06), so a highlight
   posted here also appears inside that game's detail/Highlights page, and
   a highlight added from the game detail page appears here on the feed.
   =================================================================== */
let activityCreatePostState = null;

function activityCreateCloseIconSvg() {
  return '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6 6 18"/></svg>';
}

function openActivityCreatePost(event) {
  if (event && typeof event.preventDefault === 'function') event.preventDefault();
  if (typeof currentUser === 'undefined' || !currentUser) {
    if (typeof requireShelfdSignedInAction === 'function' && !requireShelfdSignedInAction(event)) return;
  }
  closeActivityCreatePost({ instant: true });
  activityCreatePostState = { type: '', gameKey: '', game: null, saving: false };
  const sheet = document.createElement('div');
  sheet.id = 'activity-create-sheet';
  sheet.className = 'activity-create-sheet';
  sheet.innerHTML = `
    <div class="activity-create-backdrop" onclick="closeActivityCreatePost()"></div>
    <div class="activity-create-panel" role="dialog" aria-modal="true" aria-label="Create a post">
      <div class="activity-create-grabber" aria-hidden="true"></div>
      <div class="activity-create-head">
        <h2 class="activity-create-title">Create</h2>
        <button class="activity-create-close" type="button" onclick="closeActivityCreatePost()" aria-label="Close">${activityCreateCloseIconSvg()}</button>
      </div>
      <div class="activity-create-body" id="activity-create-body">
        ${renderActivityCreateTypeChooser()}
      </div>
    </div>`;
  document.body.appendChild(sheet);
  /* v11.463: kill double-tap + pinch zoom on the sheet (reuses the game-profile
     overlay's zoom guards). Text-zoom on field focus is handled in CSS by sizing
     the inputs at 16px so iOS doesn't auto-zoom. */
  if (typeof installMyListGameProfileZoomGuards === 'function') installMyListGameProfileZoomGuards(sheet);
  installActivityCreateViewportSync();
  requestAnimationFrame(() => sheet.classList.add('is-open'));
}
window.openActivityCreatePost = openActivityCreatePost;

function renderActivityCreateTypeChooser() {
  return `
    <p class="activity-create-sub">What do you want to share?</p>
    <div class="activity-create-type-grid">
      <button class="activity-create-type is-active" type="button" onclick="selectActivityPostType('highlight')">
        <span class="activity-create-type-icon"><svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7L8 5z"/></svg></span>
        <span class="activity-create-type-text">
          <span class="activity-create-type-label">Highlight Reel</span>
          <span class="activity-create-type-desc">Share a Streamable clip from a game</span>
        </span>
        <span class="activity-create-type-go" aria-hidden="true"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg></span>
      </button>
      <button class="activity-create-type is-disabled" type="button" disabled aria-disabled="true">
        <span class="activity-create-type-icon"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"><path d="M12 4l2.4 4.9 5.4.8-3.9 3.8.9 5.3L12 17.3 7.2 18.6l.9-5.3L4.2 9.7l5.4-.8L12 4z"/></svg></span>
        <span class="activity-create-type-text">
          <span class="activity-create-type-label">Review</span>
          <span class="activity-create-type-desc">Coming soon</span>
        </span>
      </button>
      <button class="activity-create-type is-disabled" type="button" disabled aria-disabled="true">
        <span class="activity-create-type-icon"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="8"/><path d="M12 8v4l2.5 2.5" stroke-linecap="round"/></svg></span>
        <span class="activity-create-type-text">
          <span class="activity-create-type-label">Status</span>
          <span class="activity-create-type-desc">Coming soon</span>
        </span>
      </button>
    </div>`;
...
function selectActivityPostType(type) {
  if (type !== 'highlight') return;
  if (!activityCreatePostState) activityCreatePostState = { type: '', gameKey: '', game: null, saving: false };
  activityCreatePostState.type = 'highlight';
  const body = document.getElementById('activity-create-body');
  if (body) body.innerHTML = renderActivityHighlightForm();
  filterActivityHighlightGames('');
  /* v11.463: do NOT auto-focus the search field. Auto-focusing popped the iOS
     keyboard the instant the form opened, which lifted the sheet up and made it
     look shifted/off. The game list is pre-populated, so the user can pick a game
     first and tap to search only when they want to. */
}
window.selectActivityPostType = selectActivityPostType;

function backToActivityCreateTypes() {
  if (activityCreatePostState) activityCreatePostState.type = '';
  const body = document.getElementById('activity-create-body');
  if (body) body.innerHTML = renderActivityCreateTypeChooser();
}
window.backToActivityCreateTypes = backToActivityCreateTypes;

function renderActivityHighlightForm() {
  return `
    <button class="activity-create-back" type="button" onclick="backToActivityCreateTypes()"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 6l-6 6 6 6"/></svg> Post type</button>
    <div class="activity-highlight-form">
      <label class="activity-highlight-label" for="activity-highlight-search-input">Game</label>
      <input id="activity-highlight-search-input" class="activity-highlight-input" type="text" placeholder="Search your games" autocomplete="off" autocorrect="off" spellcheck="false" oninput="filterActivityHighlightGames(this.value)">
      <div id="activity-highlight-results" class="activity-highlight-results"></div>

      <div class="activity-highlight-label-row">
        <label class="activity-highlight-label" for="activity-highlight-url-input">Streamable link</label>
        <button type="button" class="activity-highlight-streamable-upload" onclick="openMyListGameProfileStreamableUpload(event)">Upload on Streamable<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 5h5v5M19 5l-8.5 8.5M11 5H6a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-5"/></svg></button>
      </div>
      <input id="activity-highlight-url-input" class="activity-highlight-input" type="url" inputmode="url" placeholder="https://streamable.com/..." autocomplete="off" autocorrect="off" spellcheck="false" oninput="updateActivityHighlightPreview()">
      <div id="activity-highlight-preview" class="activity-highlight-preview"></div>

      <label class="activity-highlight-label" for="activity-highlight-caption-input">Caption <span class="activity-highlight-label-opt">optional</span></label>
      <textarea id="activity-highlight-caption-input" class="activity-highlight-input activity-highlight-textarea" rows="2" maxlength="180" placeholder="Add a caption" oninput="clearActivityHighlightError()"></textarea>

      <div id="activity-highlight-error" class="activity-highlight-error" aria-live="polite"></div>
      <button id="activity-highlight-submit" class="activity-highlight-submit" type="button" onclick="submitActivityHighlightPost()">Post highlight</button>
    </div>`;
}

function getActivityHighlightLibraryGames() {
  const games = (typeof data !== 'undefined' && Array.isArray(data?.games)) ? data.games : [];
  return games
    .map(g => ({
      g,
      key: (typeof getScreenListGameStableKey === 'function') ? getScreenListGameStableKey(g) : String(g?.id || ''),
      title: (typeof getScreenListActivityItemTitle === 'function') ? getScreenListActivityItemTitle(g) : String(g?.title || g?.name || '')
    }))
    .filter(x => x.key && x.title);
}

function filterActivityHighlightGames(query) {
  const results = document.getElementById('activity-highlight-results');
  if (!results) return;
  const all = getActivityHighlightLibraryGames();
  if (!all.length) {
    results.innerHTML = '<div class="activity-highlight-empty">You have no games in your library yet. Add a game first, then share a highlight.</div>';
    return;
  }
  const q = String(query || '').trim().toLowerCase();
  const matches = (q ? all.filter(x => x.title.toLowerCase().includes(q)) : all).slice(0, 40);
  if (!matches.length) {
    results.innerHTML = `<div class="activity-highlight-empty">No games match "${escHtml(String(query || '').trim())}".</div>`;
    return;
  }
  const selKey = activityCreatePostState?.gameKey || '';
  results.innerHTML = matches.map(({ g, key, title }) => {
    const cover = (typeof getScreenListActivityItemCover === 'function') ? (getScreenListActivityItemCover(g) || '') : '';
    const year = String(g?.year || g?.releaseYear || '').slice(0, 4);
    const selected = key === selKey;
    return `<button class="activity-highlight-result${selected ? ' is-selected' : ''}" type="button" onclick="selectActivityHighlightGame('${escAttr(key)}')" aria-pressed="${selected ? 'true' : 'false'}">
      <span class="activity-highlight-result-cover"${cover ? ` style="background-image:url('${escAttr(cover)}')"` : ''}>${cover ? '' : escHtml(title.slice(0, 1).toUpperCase() || 'G')}</span>
      <span class="activity-highlight-result-main"><span class="activity-highlight-result-title">${escHtml(title)}</span>${year ? `<span class="activity-highlight-result-year">${escHtml(year)}</span>` : ''}</span>
      <span class="activity-highlight-result-check" aria-hidden="true"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></span>
    </button>`;
  }).join('');
}
window.filterActivityHighlightGames = filterActivityHighlightGames;

function selectActivityHighlightGame(key) {
  const k = String(key || '').trim();
  if (!k || !activityCreatePostState) return;
  let game = null;
  if (typeof findMyListGameProfileItem === 'function') game = findMyListGameProfileItem(k)?.item || null;
  if (!game) {
    const all = getActivityHighlightLibraryGames();
    game = all.find(x => x.key === k)?.g || null;
  }
  if (!game) return;
  activityCreatePostState.gameKey = k;
  activityCreatePostState.game = game;
  const input = document.getElementById('activity-highlight-search-input');
  filterActivityHighlightGames(input ? input.value : '');
  clearActivityHighlightError();
}
window.selectActivityHighlightGame = selectActivityHighlightGame;

function updateActivityHighlightPreview() {
  const input = document.getElementById('activity-highlight-url-input');
  const preview = document.getElementById('activity-highlight-preview');
  if (!preview) return;
  const id = (typeof getMyListGameProfileStreamableId === 'function')
    ? (getMyListGameProfileStreamableId(String(input?.value || '').trim()) || '')
    : '';
  if (preview.dataset.previewId === id) return;
  preview.dataset.previewId = id;
  preview.innerHTML = id
    ? `<div class="activity-highlight-preview-frame"><iframe src="https://streamable.com/e/${encodeURIComponent(id)}?loop=1" allow="fullscreen" allowfullscreen loading="lazy" title="Streamable highlight preview"></iframe></div>`
    : '';
  clearActivityHighlightError();
}
window.updateActivityHighlightPreview = updateActivityHighlightPreview;

function showActivityHighlightError(message) {
  const el = document.getElementById('activity-highlight-error');
  if (el) { el.textContent = String(message || ''); el.classList.toggle('is-visible', !!message); }
}
function clearActivityHighlightError() { showActivityHighlightError(''); }
window.clearActivityHighlightError = clearActivityHighlightError;

async function submitActivityHighlightPost() {
  const state = activityCreatePostState;
  if (!state || state.saving) return;
  const btn = document.getElementById('activity-highlight-submit');
  const urlInput = document.getElementById('activity-highlight-url-input');
  const capInput = document.getElementById('activity-highlight-caption-input');
  const game = state.game;
  if (!game) { showActivityHighlightError('Pick a game from your library first.'); return; }
  const clip = (typeof normalizeMyListGameProfileHighlightClip === 'function')
    ? normalizeMyListGameProfileHighlightClip({ url: String(urlInput?.value || '').trim(), caption: capInput?.value || '' })
    : null;
  if (!clip) { showActivityHighlightError('Paste a valid Streamable link.'); urlInput?.focus?.(); return; }
  if (typeof commitGameHighlightClip !== 'function') { showActivityHighlightError('Highlight tools are still loading. Try again.'); return; }

  /* Don't repost a clip that's already on this game's live highlight reel.
     v11.469: the source of truth is the game's CURRENT highlightClips (what the game
     detail page renders) — NOT a separate posted-url ledger that goes stale after a
     delete. So once a Highlight Reel post is deleted (its clip pulled from the game's
     reel by cleanupGameHighlightForDeletedActivity) the same Streamable link is
     postable again. */
  const alreadyPosted = (typeof isGameHighlightClipAlreadyPosted === 'function')
    && isGameHighlightClipAlreadyPosted(game, clip.url);

  state.saving = true;
  clearActivityHighlightError();
  if (btn) { btn.disabled = true; btn.dataset.label = btn.textContent; btn.textContent = alreadyPosted ? 'Saving...' : 'Posting...'; }
  try {
    const existing = (typeof getMyListGameProfileHighlightClips === 'function') ? getMyListGameProfileHighlightClips(game) : [];
    const all = (typeof dedupeMyListGameProfileHighlightClips === 'function')
      ? dedupeMyListGameProfileHighlightClips([...existing, clip])
      : [...existing, clip];
    const itemKey = state.gameKey || ((typeof getScreenListGameStableKey === 'function') ? getScreenListGameStableKey(game) : (game.id || ''));
    const { postId } = await commitGameHighlightClip(game, clip, all, { itemId: itemKey, section: 'games', postToFeed: !alreadyPosted });
    if (typeof showToast === 'function') showToast(postId ? 'Highlight posted' : (alreadyPosted ? 'Highlight already shared' : 'Highlight saved'));
    closeActivityCreatePost();
  } catch (e) {
    console.warn('submitActivityHighlightPost failed:', e);
    showActivityHighlightError('Could not post that highlight. Try again.');
    state.saving = false;
    if (btn) { btn.disabled = false; btn.textContent = btn.dataset.label || 'Post highlight'; }
  }
}
window.submitActivityHighlightPost = submitActivityHighlightPost;

function closeActivityCreatePost(options = {}) {
  uninstallActivityCreateViewportSync();
  const sheet = document.getElementById('activity-create-sheet');
  if (!sheet) { activityCreatePostState = null; return; }
  if (options && options.instant) { sheet.remove(); activityCreatePostState = null; return; }
  sheet.classList.remove('is-open');
  window.setTimeout(() => { try { sheet.remove(); } catch (_) {} }, 200);
  activityCreatePostState = null;
}
window.closeActivityCreatePost = closeActivityCreatePost;

/* Lift the create-post sheet above the iOS keyboard (mirrors the game-profile
   highlight modal's visualViewport keyboard sync). */
function syncActivityCreateViewport() {
  const sheet = document.getElementById('activity-create-sheet');
  if (!sheet) return;
  const vv = window.visualViewport;
  const offset = vv ? Math.max(0, Math.round(window.innerHeight - vv.height - vv.offsetTop)) : 0;
  sheet.style.setProperty('--activity-create-kb', `${offset}px`);
}
function installActivityCreateViewportSync() {
  if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', syncActivityCreateViewport, { passive: true });
    window.visualViewport.addEventListener('scroll', syncActivityCreateViewport, { passive: true });
  }
  window.addEventListener('resize', syncActivityCreateViewport, { passive: true });
  window.requestAnimationFrame(syncActivityCreateViewport);
}
function uninstallActivityCreateViewportSync() {
  if (window.visualViewport) {
    window.visualViewport.removeEventListener('resize', syncActivityCreateViewport);
    window.visualViewport.removeEventListener('scroll', syncActivityCreateViewport);
  }
  window.removeEventListener('resize', syncActivityCreateViewport);
}

function isScreenListHighlightActivity(activity = {}) {
  return String(activity?.type || '').trim() === 'highlight'
    || String(activity?.eventType || '').trim() === 'highlight';
}

function getScreenListActivityTargetById(activityId = '') {
  const cleanId = String(activityId || '').trim();
  if (!cleanId) return null;
  if (friendActivityClickTargets[cleanId]) return friendActivityClickTargets[cleanId];
  const values = Object.values(friendActivityClickTargets || {});
  return values.find(activity => {
    if (!activity) return false;
    return [activity.id, activity.activityId, activity.renderedActivityId, activity.postId, activity.eventKey]
...
function closeScreenListHighlightActionMenu(options = {}) {
  const modal = document.getElementById('screenlist-highlight-action-modal');
  if (!modal) return;
  if (modal.__screenListHighlightClosing) return;
  const instant = !!options.instant;
  if (instant) {
    modal.remove();
    return;
  }
  modal.__screenListHighlightClosing = true;
  modal.classList.add('is-closing');
  modal.classList.remove('open');
  /* v11.468: match the 190ms card transform so the close pop isn't cut short. */
  window.setTimeout(() => modal.remove(), 200);
}

function getScreenListHighlightPostId(activity = {}, fallbackId = '') {
  const candidates = [
    activity.postId,
    activity.id,
    activity.activityId,
  ...
  for (const candidate of candidates) {
    ...
    if (!id) continue;
    id = id.replace(/^feed:/i, '').trim();
    if (id) return id;
  }
  return '';
}

function buildScreenListPostShareUrl(postId = '') {
  const cleanPostId = String(postId || '').trim();
  if (!cleanPostId) return '';
  const shareOrigin = window.SHELFD_SHARE_ORIGIN || 'https://myshelfd.com';
  return new URL(`/post/${encodeURIComponent(cleanPostId)}`, shareOrigin).toString();
}

function getScreenListHighlightSharePayload(activity = {}, fallbackId = '') {
  const item = activity.item || {};
  const gameName = item.title || item.name || '';
  const headline = String(activity?.content?.headline || (gameName ? `Shared a ${gameName} highlight` : 'Shared a highlight')).trim();
  const caption = getScreenListHighlightCaption(activity);
  const postId = getScreenListHighlightPostId(activity, fallbackId);
  const hlUrl = String(activity.highlightUrl || activity?.content?.text || '').trim();
  const hlMatch = hlUrl.match(/streamable\.com\/(?:e\/)?([a-z0-9]+)/i);
  const hlId = hlMatch ? hlMatch[1] : '';
  const shareOrigin = window.SHELFD_SHARE_ORIGIN || 'https://myshelfd.com';
  const uid = activity.uid || '';
  const userEntry = (window.usersMap && window.usersMap[uid]) || {};
  const displayName = userEntry.displayName || userEntry.name || '';
  /* v12.076: use /highlight/{streamableId} as the canonical share URL so iMessage
     gets a proper video thumbnail (Streamable's CDN JPEG) as the og:image and the
     link deep-links back into the app via the AASA /highlight/* path. */
  let highlightShareUrl = '';
  if (hlId) {
    const params = new URLSearchParams();
    if (postId) params.set('postId', postId);
    if (gameName) params.set('game', gameName);
    if (displayName) params.set('user', displayName);
    if (caption) params.set('caption', caption.slice(0, 150));
    params.set('title', headline.slice(0, 100));
    highlightShareUrl = new URL(`/highlight/${hlId}?${params.toString()}`, shareOrigin).toString();
  }
  const fallbackUrl = buildScreenListPostShareUrl(postId) || hlUrl;
  const text = caption ? `${headline}\n${caption}` : headline;
  return {
    title: headline || 'Shelfd highlight',
    text: text || 'Check out this Shelfd highlight.',
    url: highlightShareUrl || fallbackUrl
  };
}

function getScreenListHighlightCaption(activity = {}) {
  const url = String(activity.highlightUrl || '').trim();
  const noteText = String(getScreenListActivityNoteFor(activity) || '').replace(/\s+/g, ' ').trim();
  const candidates = [
    activity.highlightCaption,
    activity.highlightClipCaption,
    activity.gameHighlightCaption,
    activity.caption,
    activity?.content?.caption,
    activity?.content?.highlightCaption,
    activity?.content?.description,
    activity?.content?.text
  ];
  for (const candidate of candidates) {
    const text = String(candidate || '').replace(/\s+/g, ' ').trim();
    if (!text || text === url || text === noteText || /^https?:\/\//i.test(text) || /streamable\.com\/(?:e\/)?[a-z0-9]+/i.test(text)) continue;
    return text;
  }
  return '';
}

async function shareScreenListHighlightActivity(activityId = '') {
  const activity = getScreenListActivityTargetById(activityId);
  if (!isScreenListHighlightActivity(activity)) return;
  closeScreenListHighlightActionMenu();
  const payload = getScreenListHighlightSharePayload(activity, activityId);
  try {
    ...
    const copyText = payload.url || payload.text || payload.title;
    if (navigator.clipboard && copyText) {
      await navigator.clipboard.writeText(copyText);
      if (typeof showToast === 'function') showToast('Highlight link copied');
    }
  } catch (error) {
    if (error && error.name === 'AbortError') return;
    console.warn('[activity] highlight share failed:', error);
    if (typeof showToast === 'function') showToast('Could not share this highlight');
  }
}

function reportScreenListHighlightActivity(activityId = '') {
  const activity = getScreenListActivityTargetById(activityId);
  if (!isScreenListHighlightActivity(activity)) return;
  closeScreenListHighlightActionMenu();
  const item = activity.item || {};
  const title = item.title || item.name || 'highlight';
  if (typeof window.openReportSheet === 'function') {
    window.openReportSheet('highlight', activity.uid || '', activity.postId || activityId, `${title} highlight`);
  }
}

function deleteScreenListHighlightActivity(activityId = '') {
  const activity = getScreenListActivityTargetById(activityId);
  if (!isScreenListHighlightActivity(activity) || !canCurrentUserDeleteActivity(activity)) return;
  closeScreenListHighlightActionMenu();
  const deleteId = String(activity.postId || activity.id || activity.renderedActivityId || activityId || '').trim();
  const collection = activity.postId || activity._collection === 'feed' ? 'feed' : 'activities';
  if (deleteId) openScreenListDeletePostPrompt(deleteId, collection);
}

function openScreenListHighlightActionMenu(activityId = '', event = null) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  }
  const cleanId = String(activityId || '').trim();
  const activity = getScreenListActivityTargetById(cleanId);
  if (!cleanId || !isScreenListHighlightActivity(activity)) return;
  closeScreenListHighlightActionMenu({ instant: true });
  const ownPost = canCurrentUserDeleteActivity(activity);

  /* v12.032: anchor the menu to the 3-dots button (iOS-context-menu feel) so it
     pops out of the button rather than from screen-center. */
  let anchorRect = null;
  try {
    let btn = null;
    if (event) {
      if (event.currentTarget && typeof event.currentTarget.getBoundingClientRect === 'function') {
        ...
        btn = event.target.closest('[data-activity-action="highlight-more"]');
      }
    }
    if (btn && typeof btn.getBoundingClientRect === 'function') anchorRect = btn.getBoundingClientRect();
  } catch (_) {}

  const modal = document.createElement('div');
  modal.id = 'screenlist-highlight-action-modal';
  modal.className = 'screenlist-highlight-action-modal';
  modal.innerHTML = `
    <button class="screenlist-highlight-action-scrim" type="button" onclick="closeScreenListHighlightActionMenu()" aria-label="Close highlight options"></button>
    <div class="screenlist-highlight-action-card" role="dialog" aria-modal="true" aria-label="Highlight options">
      <button class="screenlist-highlight-action-option" type="button" onclick="shareScreenListHighlightActivity('${escAttr(cleanId)}')">Share</button>
      ${ownPost
        ? `<button class="screenlist-highlight-action-option is-destructive" type="button" onclick="deleteScreenListHighlightActivity('${escAttr(cleanId)}')">Delete</button>`
        : `<button class="screenlist-highlight-action-option" type="button" onclick="reportScreenListHighlightActivity('${escAttr(cleanId)}')">Report</button>`}
      <button class="screenlist-highlight-action-cancel" type="button" onclick="closeScreenListHighlightActionMenu()">Cancel</button>
    </div>
  `;
  ...
  if (anchorRect) {
    ...
    const MENU_H = 3 * 49 + 4;   // 3 rows + a little chrome
    const GAP = 8;
    const right = Math.max(10, vw - Math.round(anchorRect.right));
    const openBelow = (anchorRect.bottom + GAP + MENU_H) <= (vh - 12);
    const top = openBelow
      ? Math.round(anchorRect.bottom + GAP)
      : Math.max(12, Math.round(anchorRect.top - GAP - MENU_H));
    modal.style.setProperty('--sl-hl-top', `${top}px`);
    modal.style.setProperty('--sl-hl-right', `${right}px`);
    modal.style.setProperty('--sl-hl-origin', openBelow ? 'top right' : 'bottom right');
...
window.closeScreenListHighlightActionMenu = closeScreenListHighlightActionMenu;
window.openScreenListHighlightActionMenu = openScreenListHighlightActionMenu;
window.shareScreenListHighlightActivity = shareScreenListHighlightActivity;
window.reportScreenListHighlightActivity = reportScreenListHighlightActivity;
window.deleteScreenListHighlightActivity = deleteScreenListHighlightActivity;

function openHighlightShareSheet(activityId, event) {
  if (event) { try { event.preventDefault(); event.stopPropagation(); } catch (_) {} }
  const activity = getScreenListActivityTargetById(activityId);
  if (!activity) return;
  const item = activity.item || {};
  const postId = getScreenListHighlightPostId(activity, activityId);
  const postUrl = buildScreenListPostShareUrl(postId);
  const streamableUrl = String(activity.highlightUrl || activity?.content?.text || '').trim();
  const shareUrl = postUrl || streamableUrl;
  if (!shareUrl) { shareScreenListHighlightActivity(activityId); return; }
  ...
  window.openShelfdNewsShareSheet({
    url: shareUrl,
    title: item.title || item.name || 'Gaming highlight',
    image: item.poster || item.image || '',
    source: 'Shelfd',
    shareKind: 'highlight'
  });
}
window.openHighlightShareSheet = openHighlightShareSheet;

/* v11.469 — HIGHLIGHT REEL DELETE CLEANUP (source-correct, bidirectional).
   Deleting a Highlight Reel activity post used to remove ONLY the feed card. The
   linked clip stayed attached to the connected game (so the game detail page still
   showed it) and the duplicate-check source still flagged the Streamable link as
   posted — so reposting the exact same clip was blocked with "Highlight already
   shared". This pulls the clip from the game's live highlight reel (highlightClips —
   exactly what game detail renders) AND clears the posted-url ledger, then persists
   with highlightSave:true (no generic "updated <game>" card, skips the slow verify).
   Matches by stable game key + normalized Streamable id; never touches unrelated
   highlights/games. Idempotent — returns early once there's nothing left to remove,
   so the optimistic foreground call + the authoritative background call are safe. */
async function cleanupGameHighlightForDeletedActivity(activity = {}) {
  try {
    if (!activity || typeof getMyListGameProfileStreamableId !== 'function') return false;
    const isHighlight = String(activity.type || '') === 'highlight' || String(activity.eventType || '') === 'highlight';
    if (!isHighlight) return false;
    const url = String(activity.highlightUrl || activity?.content?.highlightUrl || activity?.content?.text || '').trim();
    const targetId = getMyListGameProfileStreamableId(url).toLowerCase();
    if (!targetId) return false;
    if (typeof getScreenListGameById !== 'function' || typeof data === 'undefined' || !Array.isArray(data?.games)) return false;

    // Resolve the connected game from the activity's item reference (stable-key first).
    const refItem = activity.item || {};
    const candidateKeys = [refItem.id, refItem.rawgId, refItem.rawg_id, activity.mediaKey, refItem.title]
      .map(v => String(v || '').trim()).filter(Boolean);
    let game = null, gameKey = '';
    for (const cand of candidateKeys) {
      const found = getScreenListGameById(cand);
      if (found && found.item) {
        game = found.item;
        gameKey = String(found.item.id || (typeof getScreenListGameStableKey === 'function' ? getScreenListGameStableKey(found.item) : '') || cand).trim();
        break;
      }
    }
    ...
    // 1) Remove this clip from the game's live highlight reel (the game-detail source).
    const existingClips = (typeof getMyListGameProfileHighlightClips === 'function') ? getMyListGameProfileHighlightClips(game) : [];
    const reduced = existingClips.filter(c => c && !sameId(c.url));
    const clipChanged = reduced.length !== existingClips.length;

    // 2) Clear the duplicate-check ledger entry on the live in-memory game.
    const ledgerBefore = [
      String(game.highlightFeedPostedUrl || '').trim(),
      ...(Array.isArray(game.highlightFeedPostedUrls) ? game.highlightFeedPostedUrls : [])
    ].map(u => String(u || '').trim()).filter(Boolean);
    const ledgerAfter = ledgerBefore.filter(u => !sameId(u));
    game.highlightFeedPostedUrls = ledgerAfter;
    if (sameId(game.highlightFeedPostedUrl || '')) game.highlightFeedPostedUrl = ledgerAfter[0] || '';

    if (!clipChanged && ledgerAfter.length === ledgerBefore.length) return false; // nothing left to clean

    // Mirror the reduced clips onto the live object immediately so an already-open
    // game detail / the next duplicate check sees the removal without a refetch.
    if (clipChanged) {
      game.highlightClips = reduced;
      game.highlights = reduced;
      game.highlightUrl = reduced[0]?.url || '';
      try { if (typeof refreshOpenGameProfileHighlightReel === 'function') refreshOpenGameProfileHighlightReel(gameKey, reduced); } catch (_) {}
    }

    // 3) Persist. highlightSave:true => no generic "updated <game>" activity card.
    if (clipChanged && typeof window.saveScreenListCompetitiveProfile === 'function') {
      await window.saveScreenListCompetitiveProfile({
        itemId: gameKey,
        highlightClips: reduced,
        highlightUrl: reduced[0]?.url || '',
        fetchStats: false,
        highlightSave: true
      });
    } else if (typeof save === 'function') {
      // ledger-only change still needs to stick.
      try { save(); } catch (_) {}
    }
    return true;
  } catch (err) {
    console.warn('[v11.469] highlight delete game cleanup failed (activity post already removed):', err);
    return false;
  }
}
window.cleanupGameHighlightForDeletedActivity = cleanupGameHighlightForDeletedActivity;

/* v11.469 — Direction B: removing a clip from a game's highlight reel (the game
   detail editor) also tears down the connected Activity Feed highlight post so no
   orphan card is left behind. Best-effort + fully guarded: if the post can't be
   found/deleted the clip removal still succeeds (and the new highlightClips-based
   duplicate check already makes the link repostable). Matches by owner uid +
   normalized Streamable id. In-memory caches first; a bounded uid query as fallback. */
async function deleteConnectedHighlightFeedPostForClip(url = '') {
  try {
    if (!currentUser || typeof db === 'undefined' || typeof getMyListGameProfileStreamableId !== 'function') return;
    const targetId = getMyListGameProfileStreamableId(url).toLowerCase();
    if (!targetId) return;
    const myUid = String(currentUser.uid || '');
    const sameId = u => getMyListGameProfileStreamableId(u).toLowerCase() === targetId;
    const isMineHighlight = a => !!a
      && String(a.uid || '') === myUid
      && (String(a.type || '') === 'highlight' || String(a.eventType || '') === 'highlight')
      && sameId(String(a.highlightUrl || a?.content?.highlightUrl || a?.content?.text || '').trim());

    const ids = new Set();
    try {
      Object.values(friendActivityClickTargets || {}).forEach(a => {
        if (isMineHighlight(a)) { const id = getScreenListHighlightPostId(a, ''); if (id) ids.add(id); }
      });
    } catch (_) {}
    try {
      if (Array.isArray(window.feedPosts)) window.feedPosts.forEach(p => { if (isMineHighlight(p) && p.postId) ids.add(String(p.postId)); });
    } catch (_) {}

    // Bounded Firestore fallback (single-field uid index — no composite needed).
    if (!ids.size) {
      try {
        const snap = await db.collection('feed').where('uid', '==', myUid).limit(200).get();
        snap.forEach(doc => { const d = doc.data() || {}; if (isMineHighlight({ ...d, postId: doc.id })) ids.add(doc.id); });
      } catch (e) { console.warn('[v11.469] connected highlight feed lookup skipped:', e && e.message); }
    }
    if (!ids.size) return;

    const idList = Array.from(ids);
    try { if (typeof purgeDeletedActivityFromMemory === 'function') purgeDeletedActivityFromMemory(idList); } catch (_) {}
    try { if (typeof rememberCurrentUserDeletedActivityIds === 'function') rememberCurrentUserDeletedActivityIds(idList); } catch (_) {}
    try {
      document.querySelectorAll('.activity-feed-list [data-activity-card-id], .activity-feed-list [data-activity-id]').forEach(cardEl => {
        const cid = cardEl.getAttribute('data-activity-card-id') || cardEl.getAttribute('data-activity-id') || '';
        if (cid && idList.includes(cid)) cardEl.remove();
      });
    } catch (_) {}
    if (typeof friendActivityCache !== 'undefined' && friendActivityCache) friendActivityCache.timestamp = 0;
    await Promise.all(idList.map(id => db.collection('feed').doc(id).delete().catch(() => {})));
    try { if (typeof persistCurrentUserDeletedActivityIds === 'function') await persistCurrentUserDeletedActivityIds(idList); } catch (_) {}
  } catch (err) {
    console.warn('[v11.469] deleteConnectedHighlightFeedPostForClip failed (clip already removed):', err);
  }
}
window.deleteConnectedHighlightFeedPostForClip = deleteConnectedHighlightFeedPostForClip;
...
async function confirmScreenListDeletePost(postId = '', collection = 'feed') {
  ...
  /* v11.249 — OPTIMISTIC DELETE. Previously the modal only closed AFTER a
     successful Firestore delete inside the try; if the owner check threw or
     Firestore rejected, the catch re-enabled the buttons but LEFT THE MODAL
     OPEN — the "stuck on the popup" bug. Now we:
       1. compute the delete-id set from the in-memory activity (sync),
       2. remove the card from the DOM + memory and CLOSE THE MODAL immediately,
       3. run the real Firestore/persistence delete in the background.
     The modal can never get stuck, and the feed reflects the deletion at once.
     The background write still hard-deletes feed docs / persists the owner's
     activityDeletedIds so it's gone from Firestore and stays gone on reload. */

  /* Resolve the in-memory activity synchronously for the owner check + id set. */
  const memActivity = friendActivityClickTargets[cleanPostId] || {};
  const deleteIds = Array.from(new Set([
    cleanPostId,
    ...getScreenListActivityDeleteCandidates(memActivity, cleanPostId)
  ].map(String).filter(Boolean)));

  /* Optimistically pull it out of the UI + memory and close the modal NOW. */
  const previousScrollY = window.scrollY || window.pageYOffset || 0;
  purgeDeletedActivityFromMemory(deleteIds);
  if (friendActivityCache) friendActivityCache.timestamp = 0;
  rememberCurrentUserDeletedActivityIds(deleteIds);
  try {
    document.querySelectorAll('.activity-feed-list [data-activity-card-id], .activity-feed-list [data-activity-id]').forEach(cardEl => {
      const cid = cardEl.getAttribute('data-activity-card-id') || cardEl.getAttribute('data-activity-id') || '';
  ...
  /* v11.469: if this was a Highlight Reel post, optimistically pull the linked clip
     from the connected game's highlight reel + clear the duplicate-check ledger so
     the game detail no longer shows it and the same Streamable link is repostable.
     Runs off the in-memory activity for instant UI; the background block re-runs it
     from the authoritative feed doc (idempotent) in case the card wasn't cached. */
  if (memActivity && (String(memActivity.type || '') === 'highlight' || String(memActivity.eventType || '') === 'highlight')) {
    cleanupGameHighlightForDeletedActivity(memActivity);
  }
  if (typeof showToast === 'function') showToast('Activity deleted');

  /* Background: actually persist the delete to Firestore. Errors here only
     warn — the UI already updated, and the local deleted-ids list keeps it
     hidden on reload even if the network write is delayed/offline. */
  (async () => {
    try {
      let target = null;
      ...
      const activity = target?.activity || memActivity || {};
      if (!canCurrentUserDeleteActivity(activity)) {
        /* Not the owner per resolved data — keep it hidden locally but don't
           attempt a server write that would be rejected. */
        console.warn('[v11.249] delete: not resolved as owner; hidden locally only.', cleanPostId);
        return;
      }
      const fullIds = Array.from(new Set([
        ...deleteIds,
        target?.id || '',
      ...
      if (target?.collection === 'feed') {
        ...
        /* v11.469: authoritative highlight cleanup off the resolved feed doc (has
           highlightUrl + item even when the card wasn't in the in-memory cache).
           Idempotent with the optimistic foreground call above. */
        try { await cleanupGameHighlightForDeletedActivity(target.activity); } catch (_) {}
        /* also record the id so any synthesized twin stays hidden */
        await persistCurrentUserDeletedActivityIds(fullIds).catch(() => {});
      } else {
        await persistCurrentUserDeletedActivityIds(fullIds);
      }
    } catch (err) {
      console.error('[v11.249] background activity delete failed (UI already updated):', err);
    }
  })();
}
...
function initFeedComposer() {
  ...
  if (photo) {
    ...
    avatar.innerHTML = `<img src="/default-avatar.svg" alt="" decoding="async" style="width:100%;height:100%;object-fit:cover;display:block;">`;
  }
}
...
function getActivityCardUsername(userLike = null, fallback = 'Friend') {
  const handle = String(
    userLike?.usernameHandle ||
    userLike?.userHandle ||
    userLike?.username ||
    userLike?.handle ||
    userLike?.usernameHandleLower ||
    userLike?.handleLower ||
    userLike?.usernameLower ||
    ''
  ).trim().replace(/^@+/, '');
  if (handle) return handle;
  return String(getDisplayName(userLike, fallback) || fallback || 'Friend').trim().replace(/^@+/, '') || 'Friend';
}

function getActivityCardHandle(userLike = null) {
  return String(
    userLike?.usernameHandle ||
    userLike?.userHandle ||
    userLike?.username ||
...
function getActivityCardDisplayName(userLike = null, fallback = 'Friend') {
  const handle = getActivityCardHandle(userLike).toLowerCase();
  const candidates = [
    userLike?.customName,
    userLike?.name,
  ...
  for (const candidate of candidates) {
    ...
    if (handle && normalized.toLowerCase() === handle) continue;
    return normalized;
  }
  return String(getDisplayName(userLike, fallback) || fallback || 'Friend').trim().replace(/^@+/, '') || 'Friend';
}

function renderActivityCardUsernameHTML(userLike = null, fallback = 'Friend', extraClass = '') {
  const classes = ['creator-name'];
  if (extraClass) classes.push(extraClass);
  const displayName = getActivityCardDisplayName(userLike, fallback);
  const handle = getActivityCardHandle(userLike);
  const displayNameHtml = escHtml(displayName);
  const handleHtml = handle ? `<span class="sl-activity-handle">@${escHtml(handle)}</span>` : '';
  const nameHtml = `<span class="sl-activity-identity"><span class="${classes.join(' ')} sl-activity-display-name">${displayNameHtml}</span>${handleHtml}</span>`;
  const creativeTeamTag = (typeof isCreativeTeamUser === 'function' && isCreativeTeamUser(userLike))
    ? renderCreativeTeamTagHTML()
    : '';
  if (typeof isCreatorAdmin === 'function' && isCreatorAdmin(userLike)) {
    if (typeof isOwnerBadgeAccount === 'function' && isOwnerBadgeAccount(userLike)) {
  ...
  if (creativeTeamTag) {
    return `<span class="creator-name-wrap user-badged-name-wrap"><span class="${classes.join(' ')}">${nameHtml}</span>${creativeTeamTag}</span>`;
  }
...
function buildCleanActivityAvatar(a, actor, avatarSrc, initial) {
  ...
  /* v11.658: no initials avatars — a missing/failed photo shows the default
     blank profile picture (real Apple/Google/uploaded photos are unchanged). */
  const def = '/default-avatar.svg';
  if (avatarSrc) {
    return `<div class="sl-av" onclick="${onclick}" role="button" tabindex="0" aria-label="${ariaLabel}"><img class="sl-av-img" src="${src}" alt="" loading="lazy" onerror="this.onerror=null;this.src='${def}'"></div>`;
  }
  return `<div class="sl-av" onclick="${onclick}" role="button" tabindex="0" aria-label="${ariaLabel}"><img class="sl-av-img" src="${def}" alt="" loading="lazy"></div>`;
}
...
function buildFeedPostCardHTML(a, activityId, options = {}) {
  ...
  const initial = getActivityCardUsername(actor, 'F').charAt(0).toUpperCase();
  ...
  const nameHtml = `<span class="activity-card-name" style="cursor:pointer;" onclick="event.stopPropagation(); viewUserFromMap('${escAttr(a.uid)}')">${renderActivityCardUsernameHTML(actor, 'Friend')}</span>`;
  
  let postContentHtml = '';

  /* v11.284: game-highlight post — embed the Streamable clip INLINE (same as
     the competitive game detail page), not a link. Falls back to a link only if
     the URL isn't a parseable Streamable id. Built before the generic
     content.text branch so it owns the body for highlight cards. */
  if (a.type === 'highlight') {
    const hlHeadline = String((content && content.headline) || 'Shared a highlight');
    const hlUrl = String(a.highlightUrl || (content && content.text) || '').trim();
    const hlCaption = getScreenListHighlightCaption(a);
    const hlMatch = hlUrl.match(/streamable\.com\/(?:e\/)?([a-z0-9]+)/i);
    const hlId = hlMatch ? hlMatch[1] : '';
    let hlBody = '';
    if (hlId) {
      const hlSrc = `https://streamable.com/e/${hlId}`;
      hlBody = `<div class="feed-post-highlight-embed"><iframe src="${escAttr(hlSrc)}" allow="fullscreen" allowfullscreen loading="lazy" title="Highlight reel" onclick="event.stopPropagation();"></iframe></div>`;
    } else if (/^https?:\/\//i.test(hlUrl)) {
      hlBody = `<a class="feed-post-highlight-link" href="${escAttr(hlUrl)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();">▶ Watch highlight reel</a>`;
    }
    postContentHtml = `<div class="feed-post-headline">${escHtml(hlHeadline)}</div>${hlCaption ? `<div class="feed-post-highlight-caption">${escHtml(hlCaption)}</div>` : ''}${hlBody}`;
  ...
  const interactionsHtml = `
    <div class="activity-interactions" data-activity-interactions>
      <button class="activity-interaction-btn ${isLiked ? 'liked' : ''}" data-activity-action="like" onclick="event.stopPropagation(); toggleFeedLike('${escAttr(a.postId || a.id)}', this)">
        <span data-like-icon-slot>${getActivityFeedLikeIconSvg(isLiked)}</span><span data-activity-like-count>${likeCount || 0}</span>
      </button>
      <button class="activity-interaction-btn" data-activity-action="reply" onclick="event.stopPropagation(); openFeedPostPage('${escAttr(a.postId || a.id)}')">
        ${getActivityFeedCommentIconSvg()}<span data-activity-reply-count>${replyCount || 0}</span>
  ...
  return `<div class="activity-card feed-post ${meta.topClass}" data-activity-card-id="${escAttr(activityId)}" data-post-id="${escAttr(a.postId || a.id || '')}" onclick="handleScreenListActivityCardOpen('${escAttr(a.postId || a.id || activityId)}','feed')">
    <time class="activity-card-time activity-card-top-time">${escHtml(timeStr)}</time>
...
async function openActivityReplyPage(activityId) {
  ...
  /* v12.075: paint the post card immediately from in-memory cache — eliminates the
     "Loading activity..." flash. Firestore fetch still runs to get authoritative
     interaction IDs and latest counts; post card silently updates after. */
  const memActivity = friendActivityClickTargets[activityId];
  if (memActivity && detailContainer) {
    const memColl = memActivity._collection || 'activities';
    const immediateActivity = { ...memActivity, id: activityId, _collection: memColl };
    currentFeedPostId = activityId;
    currentFeedPostActivityId = activityId;
    currentFeedPostCollection = memColl;
    currentFeedPostActivityData = immediateActivity;
    detailContainer.innerHTML = buildActivityPostDetailHTML(immediateActivity, activityId, memColl);
  ...
  try {
    ...
    if (repliesComposer && repliesComposer.style.display === 'none') {
      repliesComposer.style.display = 'block';
      initReplyComposer();
      window.requestAnimationFrame(syncFeedPostComposerViewport);
    }
...
function buildFeedReplyItemHTML(reply, index = 0, total = 1, depth = 0, childHtml = '') {
  ...
  const avatarHtml = `<img class="feed-reply-avatar-img" src="${escAttr(avatarSrc || '/default-avatar.svg')}" alt="" loading="lazy" onerror="this.onerror=null;this.src='/default-avatar.svg'">`;
  ...
  return `<article class="feed-reply-item x-reply-item ${parentId ? 'feed-reply-threaded' : ''}" data-reply-id="${escAttr(replyId)}" data-parent-reply-id="${escAttr(parentId)}">
    ...
    <div class="feed-reply-content">
      <div class="feed-reply-header">
        <span class="feed-reply-author">${escHtml(getShelfdUsernameHandle(user) || name)}</span>
...
/* v12.039: the comment sheet now sizes itself to the VISUAL viewport every frame.
   The sheet's TOP is pinned to a constant on-screen position (feedSheetTopGapPx,
   captured once on open while the keyboard is down) and its BOTTOM is pinned to
   the visual-viewport bottom (= the top of the keyboard). So when the keyboard
   opens the sheet SHRINKS in place instead of being panned off-screen, and the
   composer — which is the sheet's bottom flex row — sits right on top of the
   keyboard. visualViewport.offsetTop counters iOS's keyboard scroll-pan in pure
   JS, so this works with or without @capacitor/keyboard. */
let feedSheetTopGapPx = 0;
function syncFeedPostComposerViewport() {
  /* v12.074: composer is pinned to bottom:0 (CSS). Lift it by the live keyboard
     height — the exact share-sheet model. keyboardH = layoutH - vv.height - vv.offsetTop
     (offsetTop accounts for iOS's keyboard scroll-pan). Works in BOTH native-resize
     and overlay keyboard modes because bottom:0 is anchored to the layout viewport. */
  const page = document.getElementById('feed-post-page');
  ...
  const keyboardH = vv ? Math.max(0, layoutH - vv.height - vv.offsetTop) : 0;
  composer.style.transform = `translateY(-${Math.round(keyboardH)}px)`;
  if (keyboardH > 80) {
...
/* v12.072: rAF loop that runs syncFeedPostComposerViewport every frame for ~380ms
   on keyboard open AND close. Starts on textarea focus/blur so the composer top
   tracks window.innerHeight per-frame during the iOS keyboard slide animation
   (visualViewport resize only fires once, at the END of the animation). */
let _feedComposerKbRaf = 0;
let _feedComposerKbStop = 0;
function startFeedComposerKeyboardTracking() {
  _feedComposerKbStop = Date.now() + 380;
  if (_feedComposerKbRaf) return; // already running
  function loop() {
    syncFeedPostComposerViewport();
    if (Date.now() < _feedComposerKbStop) {
      _feedComposerKbRaf = requestAnimationFrame(loop);
    } else {
      _feedComposerKbRaf = 0;
      syncFeedPostComposerViewport(); // final snap
    }
  }
  _feedComposerKbRaf = requestAnimationFrame(loop);
}
...
window.openFeedPostPage = openFeedPostPage;

let screenListSharedFeedPostRouteOpening = false;

async function openSharedFeedPostRoute(route = (typeof parseScreenListFeedPostRoute === 'function' ? parseScreenListFeedPostRoute() : null)) {
  const postId = String(route?.postId || '').trim();
  if (!postId || screenListSharedFeedPostRouteOpening) return false;
  screenListSharedFeedPostRouteOpening = true;
  if (typeof prepareSharedMediaRouteView === 'function') prepareSharedMediaRouteView();
  try {
    if (!currentUser && typeof firebase !== 'undefined' && firebase?.auth?.().currentUser) currentUser = firebase.auth().currentUser;
  } catch (_) {}
  try {
    await openFeedPostPage(postId);
    return !!document.getElementById('feed-post-page');
  } catch (error) {
    console.warn('[post-route] open failed:', error);
    if (typeof showToast === 'function') showToast('Could not open post');
    if (!currentUser && typeof showLandingPage === 'function') showLandingPage();
    return false;
  } finally {
    screenListSharedFeedPostRouteOpening = false;
  }
}
window.openSharedFeedPostRoute = openSharedFeedPostRoute;

(function bootResolveSharedFeedPostRouteFromUrl() {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__shelfdBootFeedPostRouteFallbackBound) return;
  window.__shelfdBootFeedPostRouteFallbackBound = true;
  ...
  function readRoute() {
    if (typeof parseScreenListFeedPostRoute === 'function') return parseScreenListFeedPostRoute();
    try {
      const path = String(window.location.pathname || '');
      const hash = String(window.location.hash || '');
      const match = path.match(/^\/post\/([^/?#]+)/i) || hash.match(/^#post\/([^/?#]+)/i);
      return match ? { postId: decodeURIComponent(match[1] || '').trim() } : null;
    } catch (_) {
      return null;
    }
  }
  ...
  function tryOpen() {
    ...
    if (!route?.postId || window.__screenListSharedFeedPostRouteResolved) return;
    if (typeof openSharedFeedPostRoute !== 'function' || typeof db === 'undefined' || !db?.collection) {
      attempts += 1;
      if (attempts <= maxAttempts) schedule();
      return;
    }
    Promise.resolve(openSharedFeedPostRoute(route)).then(ok => {
      if (ok) window.__screenListSharedFeedPostRouteResolved = true;
      else if (++attempts <= maxAttempts) schedule();
    }).catch(() => {
      if (++attempts <= maxAttempts) schedule();
    });
  }
...
function buildFeedPostDetailHTML(post, postId) {
  ...
  let postContentHtml = '';
  if (post.type === 'highlight' || post.eventType === 'highlight') {
    const item = post.item || {};
    const title = item.title || item.name || 'this game';
    const headline = String(content.headline || `Shared a highlight from ${title}`).trim();
    const caption = getScreenListHighlightCaption(post);
    const highlightUrl = String(post.highlightUrl || content.text || '').trim();
    const match = highlightUrl.match(/streamable\.com\/(?:e\/)?([a-z0-9]+)/i);
    const highlightId = match ? match[1] : '';
    let highlightMedia = '';
    if (highlightId) {
      highlightMedia = `<div class="feed-post-highlight-embed"><iframe src="https://streamable.com/e/${escAttr(highlightId)}" allow="fullscreen" allowfullscreen loading="lazy" title="Highlight reel"></iframe></div>`;
    } else if (/^https?:\/\//i.test(highlightUrl)) {
      highlightMedia = `<a class="feed-post-highlight-link" href="${escAttr(highlightUrl)}" target="_blank" rel="noopener noreferrer">Watch highlight reel</a>`;
    }
    postContentHtml += `<div class="feed-post-headline">${escHtml(headline)}</div>${caption ? `<div class="feed-post-highlight-caption">${escHtml(caption)}</div>` : ''}${highlightMedia}`;
  } else if (content.text) {
...
/* v11.393: the comments sheet shows the @username (handle) instead of the
   display name, per spec. Falls back to the display name if no handle exists. */
function getShelfdUsernameHandle(userLike = {}) {
  const u = userLike || {};
  const handle = String(
    u.usernameHandle || u.userHandle || u.handle || u.username
    || u.usernameHandleLower || u.handleLower || u.usernameLower
    || (u.profileData && (u.profileData.usernameHandle || u.profileData.userHandle || u.profileData.handle || u.profileData.username))
    || ''
  ).trim().replace(/^@+/, '');
  return handle;
}
  ...
  return `
    <article class="x-post-detail-card activity-detail-post ${meta.topClass}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}">
      <div class="x-post-main-row">
        ...
        <div class="x-post-body">
          <div class="x-post-header-row">
            <span class="x-post-author" onclick="viewUserFromMap('${escAttr(activity.uid)}')">${escHtml(getShelfdUsernameHandle(actor) || getDisplayName(actor, 'Friend'))}</span>
            <span class="x-post-time">${timeStr}</span>
          </div>
...
function openShelfdFeedPostBottomSheet(page) {
  ...
  /* v12.070: open as a FLEX overlay (bottom-anchored, share-sheet framework).
     Close still sets display:none, which hides it since CSS no longer forces display. */
  page.style.display = 'flex';
  page.style.visibility = 'visible';
  page.classList.remove('is-dragging');
  const inner = page.querySelector('.overlay-page-inner');
  if (inner) inner.style.transform = '';
  /* v12.070: the sheet is now bottom-anchored via flex (share-sheet framework) and
     positioned purely by transform — no top/height px to capture anymore. The
     composer (separate fixed layer) still tracks the keyboard via dvh. */
  feedSheetTopGapPx = 0;
  syncFeedPostComposerViewport();
  lockShelfdFeedSheetBackgroundScroll();
  /* v12.038: stop iOS WKWebView's native scroll-to-input PAN while the comment
     sheet is open. That pan was shoving the whole sheet up out of frame when the
     composer focused. With it disabled the sheet stays put and only the keyboard
     rises (TikTok behaviour); the composer rides above the keyboard via
     --feed-reply-keyboard-offset. Hide the up/down/Done accessory bar too so the
     composer seats directly on the keyboard. Both restored on close. No-op
     without @capacitor/keyboard (same guarantee as the DM chat). */
  try { if (typeof window.shelfdSetKeyboardScrollDisabled === 'function') window.shelfdSetKeyboardScrollDisabled(true); } catch (_) {}
  try { if (typeof window.shelfdSetKeyboardAccessoryBarVisible === 'function') window.shelfdSetKeyboardAccessoryBarVisible(false); } catch (_) {}
...
function closeShelfdFeedPostBottomSheet(page) {
  ...
  /* v12.038: restore native scroll-to-input + accessory bar for the rest of the
     app (they were scoped off while the sheet was open). */
  try { if (typeof window.shelfdSetKeyboardScrollDisabled === 'function') window.shelfdSetKeyboardScrollDisabled(false); } catch (_) {}
  try { if (typeof window.shelfdSetKeyboardAccessoryBarVisible === 'function') window.shelfdSetKeyboardAccessoryBarVisible(true); } catch (_) {}
  closeShelfdFeedMentionPicker();
...
function installShelfdFeedSheetGestures(page) {
  ...
  const writeDragTransform = (inner, dy) => {
    /* v12.070: sheet is now a flex child centered by the overlay (justify-content),
       so no translateX(-50%) — drag is pure vertical translate, matching the
       share-sheet framework. */
    inner.style.setProperty('transform', `translate3d(0, ${dy}px, 0)`, 'important');
    const composer = document.getElementById('feed-post-replies-composer');
    if (composer && dragState && dragState.sheetH) {
      /* v12.064: composer is now position:fixed full-width (left:0;right:0), no
         centering offset — translateY only, not translate3d(-50%,...) */
      composer.style.setProperty('transform', `translateY(${dy}px)`, 'important');
...
/* =============================================================================
   v12.038: @-mention friend picker for the comment composer.
   Typing "@" (optionally followed by a partial handle) at the cursor opens a
   TikTok-style panel that floats directly above the composer / keyboard, listing
   the user's friends (filtered by the partial). Tapping a row inserts
   "@handle " into the composer. On submit, parseShelfdFeedMentions resolves the
   @handles to friend uids so each tagged friend gets a `comment_mention` push.
   The panel tracks the composer/keyboard via the same --feed-reply-keyboard-offset
   / --feed-post-composer-height vars, so it always sits right on top of the
   composer with no extra layout math.
   ========================================================================== */
function getShelfdFeedMentionQuery(input) {
  if (!input) return null;
  const value = String(input.value || '');
  const pos = (typeof input.selectionStart === 'number') ? input.selectionStart : value.length;
  const before = value.slice(0, pos);
  /* an @token at the cursor: @ at string-start or after whitespace, then handle
     chars (letters, digits, _ and . — Shelfd handles allow dots like weatha.man).
     No spaces inside, so a finished "@name " stops matching. */
...
function shelfdFeedMentionCandidates(query) {
  const q = String(query || '').toLowerCase().replace(/^@+/, '');
  const uids = Array.isArray(friends) ? friends : [];
  const rows = [];
  uids.forEach(uid => {
    if (!uid || (currentUser && uid === currentUser.uid)) return;
    const u = (typeof usersMap !== 'undefined' && usersMap) ? usersMap[uid] : null;
    if (!u) return;
    const handle = getShelfdUsernameHandle(u);
    const name = String(u.name || u.displayName || handle || 'User').trim();
    const hay = `${handle} ${name}`.toLowerCase();
    if (q && hay.indexOf(q) === -1) return;
    rows.push({ uid, handle, name, photo: u.photo || u.photoURL || '' });
  });
  rows.sort((a, b) => {
    if (q) {
      const aStarts = a.handle.toLowerCase().startsWith(q) ? 0 : 1;
      const bStarts = b.handle.toLowerCase().startsWith(q) ? 0 : 1;
...
function ensureShelfdFeedMentionPicker() {
  let picker = document.getElementById('feed-mention-picker');
  if (picker) return picker;
  const page = document.getElementById('feed-post-page');
  if (!page) return null;
  picker = document.createElement('div');
  picker.id = 'feed-mention-picker';
  picker.className = 'feed-mention-picker';
  picker.hidden = true;
  picker.innerHTML = '<div class="feed-mention-list" id="feed-mention-list"></div>';
  /* v12.039: live as a flex row INSIDE the sheet, directly above the composer,
     so it rides with the keyboard automatically and pushes the comment list up
     (TikTok-style) instead of being separately positioned. */
  const inner = document.getElementById('feed-post-bottom-sheet-inner');
  /* v12.064: composer is now outside the inner — append picker at end of inner
     (last flex item in the sheet column, replaces scroll content when open). */
  if (inner) inner.appendChild(picker);
  else page.appendChild(picker);
  /* delegated tap → insert (rows are rebuilt on every keystroke). */
  picker.addEventListener('mousedown', (e) => { e.preventDefault(); }); // keep the textarea focused
  picker.addEventListener('click', (e) => {
    const row = e.target && e.target.closest ? e.target.closest('.feed-mention-row') : null;
    if (!row) return;
    insertShelfdFeedMention(row.getAttribute('data-handle') || '');
  });
  return picker;
}

function closeShelfdFeedMentionPicker() {
  const picker = document.getElementById('feed-mention-picker');
  if (picker) { picker.hidden = true; picker.classList.remove('is-open'); }
  const inner = document.getElementById('feed-post-bottom-sheet-inner');
  if (inner) inner.classList.remove('feed-mention-open');
}

function syncShelfdFeedMentionPicker(input) {
  input = input || document.getElementById('feed-reply-input');
  const ctx = input ? getShelfdFeedMentionQuery(input) : null;
  if (!ctx) { closeShelfdFeedMentionPicker(); return; }
  const rows = shelfdFeedMentionCandidates(ctx.query);
  if (!rows.length) { closeShelfdFeedMentionPicker(); return; }
  const picker = ensureShelfdFeedMentionPicker();
  if (!picker) return;
  const list = picker.querySelector('#feed-mention-list');
  list.innerHTML = rows.map(r => `
    <button type="button" class="feed-mention-row" data-uid="${escAttr(r.uid)}" data-handle="${escAttr(r.handle)}">
      <img class="feed-mention-avatar" src="${escAttr(r.photo || '/default-avatar.svg')}" alt="" decoding="async" onerror="this.src='/default-avatar.svg'">
      <span class="feed-mention-meta">
        <span class="feed-mention-name">${escHtml(r.name)}</span>
        ${r.handle ? `<span class="feed-mention-handle">@${escHtml(r.handle)}</span>` : ''}
      </span>
      <span class="feed-mention-pick" aria-hidden="true"></span>
    </button>`).join('');
  list.scrollTop = 0;
  picker.hidden = false;
  /* v12.042: the picker takes over the comment area (the list hides) and slides
     up into place from the top of the sheet, instead of being a strip wedged
     above the composer. */
  const inner = document.getElementById('feed-post-bottom-sheet-inner');
  if (inner) inner.classList.add('feed-mention-open');
  requestAnimationFrame(() => picker.classList.add('is-open'));
}

function insertShelfdFeedMention(handle) {
  const clean = String(handle || '').replace(/^@+/, '').trim();
  const input = document.getElementById('feed-reply-input');
  if (!clean || !input) return;
  const ctx = getShelfdFeedMentionQuery(input);
  const value = String(input.value || '');
  const start = ctx ? ctx.start : value.length;
  const end = ctx ? ctx.end : value.length;
  const insert = `@${clean} `;
  const next = (value.slice(0, start) + insert + value.slice(end)).slice(0, 99);
  input.value = next;
  const caret = Math.min((value.slice(0, start) + insert).length, next.length);
  try { input.setSelectionRange(caret, caret); } catch (_) {}
  closeShelfdFeedMentionPicker();
  input.focus();
  /* re-run autogrow + counter + button-enable without re-opening the picker. */
  try { input.dispatchEvent(new Event('input')); } catch (_) {}
  closeShelfdFeedMentionPicker();
}

/* Resolve @handles in a posted comment to friend uids (only real friends are
   notified — keeps it from being an outbound-spam vector). */
function parseShelfdFeedMentions(text) {
  const found = String(text || '').match(/@([A-Za-z0-9_.]+)/g) || [];
  if (!found.length) return [];
  const wanted = new Set(found.map(h => h.replace(/^@+/, '').toLowerCase()));
  const uids = Array.isArray(friends) ? friends : [];
  const out = [];
  uids.forEach(uid => {
    if (!uid || (currentUser && uid === currentUser.uid)) return;
    const u = (typeof usersMap !== 'undefined' && usersMap) ? usersMap[uid] : null;
    if (!u) return;
    const handle = getShelfdUsernameHandle(u).toLowerCase();
    if (handle && wanted.has(handle) && out.indexOf(uid) === -1) out.push(uid);
  });
  return out;
}

window.closeShelfdFeedMentionPicker = closeShelfdFeedMentionPicker;
window.insertShelfdFeedMention = insertShelfdFeedMention;
...
function initReplyComposer() {
  ...
  const counter = document.getElementById('feed-reply-counter');
  const FEED_REPLY_MAX = 99;
  const updateFeedReplyCounter = function (len) {
    if (!counter) return;
    counter.textContent = `${len}/${FEED_REPLY_MAX}`;
    counter.classList.toggle('is-near-limit', len >= FEED_REPLY_MAX - 10);
  };

  // Auto-expand and enable/disable button
  /* v12.037: grow the composer with the text (up to ~5 lines) instead of being
     pinned to a single line, so the user can see their whole comment. Beyond the
     cap the textarea scrolls internally. */
  const FEED_REPLY_MAX_H = 132;
  const autoGrowFeedReply = function (el) {
    el.style.height = 'auto';
    const next = Math.min(el.scrollHeight, FEED_REPLY_MAX_H);
    el.style.height = next + 'px';
    el.style.overflowY = el.scrollHeight > FEED_REPLY_MAX_H ? 'auto' : 'hidden';
  };
  input.oninput = function() {
    autoGrowFeedReply(this);
    btn.disabled = !this.value.trim();
    updateFeedReplyCounter(this.value.length);
    syncShelfdFeedMentionPicker(this);   // v12.038: @-mention friend picker
    syncFeedPostComposerViewport();
  };
  input.addEventListener('keyup', function () { syncShelfdFeedMentionPicker(this); });
  input.addEventListener('click', function () { syncShelfdFeedMentionPicker(this); });
  input.onfocus = function() {
    ...
    /* v12.072: visualViewport resize fires ONCE when the keyboard is fully open,
       not per-frame. To track the keyboard animation in real-time, run a rAF loop
       that polls window.innerHeight each frame for the duration of the iOS keyboard
       open animation (~350ms). This gives per-frame top updates so the composer
       rises in exact sync with the keyboard, not after it. */
    startFeedComposerKeyboardTracking();
  };
  input.onblur = function() {
    startFeedComposerKeyboardTracking(); // also track the keyboard closing
  };
  ...
  btn.disabled = true;
  updateFeedReplyCounter(0);
  window.requestAnimationFrame(syncFeedPostComposerViewport);
}

function insertFeedReplyEmoji(emoji) {
  const input = document.getElementById('feed-reply-input');
  if (!input) return;
...
async function submitFeedReply() {
  ...
  if (!text) return;
  /* v12.035: hard cap reply length at 99 characters. The textarea
     itself has maxlength="99" (browser-enforced for typing + paste), so
     this submit guard is belt-and-suspenders — protects against any
     programmatic bypass of the textarea attribute. */
  if (text.length > 99) {
    if (typeof showToast === 'function') showToast('Max 99 characters');
    return;
  }
  ...
  try {
    ...
    /* v12.038: @-mention pushes — each friend tagged with @handle in the comment
       gets a dedicated `comment_mention` notification ("tagged you on a comment").
       Skips self and whoever was already notified above (post owner / parent-comment
       author) so nobody is double-pinged. */
    try {
      const commentRecipient = parentReplyId
        ? String(parentReply?.uid || '').trim()
        : getActivityNotificationOwnerUid(latestActivity);
      const mentionedUids = parseShelfdFeedMentions(text).filter(uid => uid && uid !== commentRecipient);
      ...
      console.warn('[mention] notify failed:', mErr && mErr.message ? mErr.message : mErr);
    }
    ...
    input.style.overflowY = 'hidden';
    closeShelfdFeedMentionPicker();
    const _replyCounter = document.getElementById('feed-reply-counter');
    if (_replyCounter) { _replyCounter.textContent = '0/99'; _replyCounter.classList.remove('is-near-limit'); }
    clearFeedReplyParent(false);
...
function buildActivityStoriesHTML(activities) {
  ...
  const items = storyUsers.map(({ activity: a, latestTime }) => {
    ...
    const innerHtml = `<img class="story-avatar" src="${escAttr(avatarSrc || '/default-avatar.svg')}" alt="" loading="lazy" onerror="this.onerror=null;this.src='/default-avatar.svg'">`;
...
function getActivityDisplayAction(eventType = '', item = {}, activity = {}) {
  ...
  if (eventType === 'review' || isActivityMediaReviewPost(activity)) return 'Wrote a Review';
  ...
  if (status === 'planned' || eventType === 'planned') {
    if (isGame) return 'planning to play';
    if (section === 'music') return 'planning to listen';
...
function buildScreenListActivityComposedSentence(eventType = '', item = {}, activity = {}) {
  ...
  function addedToDestinationParts(destinationStatus = status || 'planned') {
    const mediaType = getScreenListActivityMediaTypeLabel(section) || 'title';
    const planningVerb = 'Planning';
    const planningTail = isGame
      ? 'to play'
      : isMusic
        ? 'to listen'
        : isReadingSection(section)
          ? 'to read'
          : 'to watch';
    const planningPreTitle = isMusic ? `to the ${mediaType}` : `the ${mediaType}`;
    return {
      prefix: planningVerb,
      actionTail: planningTail,
      verbContext: '',
      preTitle: planningPreTitle,
      title,
      suffix: '',
      omitMediaTypeDetail: true
    };
  }
  ...
  switch (eventType) {
    ...
    case 'season-finished':
      if (seasonNum > 0) return { prefix: 'Finished watching', verbContext: ` Season ${seasonNum}`, title, suffix: '', seasonFinishedDetail: true };
    ...
    case 'planned':
      return addedToDestinationParts('planned');
...
function getScreenListActivityMediaTypeLabel(section = '') {
  switch (String(section || '').trim().toLowerCase()) {
    case 'movies': return 'movie';
    case 'shows': return 'tv show';
    case 'anime': return 'anime';
    case 'games': return 'video game';
    case 'music': return 'album';
    default: return '';
  }
}

function getScreenListMusicActivityTypeLabel(item = {}) {
  const rawType = [
    item.musicSubtype,
    item.musicType,
    item.albumType,
    item.releaseType,
    item.releaseGroupType,
    item.__shelfdEntity,
    item.__shelfdDeezerType,
    item.type,
    item.kind,
    item.mediaType
  ].map(value => String(value || '').trim().toLowerCase()).find(Boolean) || '';
  if (/\b(track|song|recording)\b/.test(rawType)) return 'song';
  if (/\b(ep|extended play)\b/.test(rawType)) return 'ep';
  return 'album';
}

function getScreenListActivityArtistName(item = {}) {
  const direct = [
    item.artist,
    item.artistName,
    item.albumArtist,
  ...
  if (Array.isArray(item.artists)) {
    const names = item.artists
      .map(artist => typeof artist === 'string' ? artist : (artist?.name || artist?.artistName || ''))
...
function getScreenListActivityReviewHeadlineLines(item = {}, activity = {}, displayTitle = '') {
  const section = String(item.librarySection || item.mediaCategory || activity.mediaCategory || '').trim().toLowerCase();
  const mediaType = getScreenListActivityMediaTypeLabel(section);
  const rawTitle = String(item.title || item.name || displayTitle || 'Untitled').trim() || 'Untitled';
  const resolvedTitle = String(displayTitle || rawTitle).trim() || rawTitle;
  const subtitleLine = mediaType ? `the ${mediaType}` : '';
  const seasonNum = Number(activity?.seasonNum || item?.lastSeasonRatingNum || 0);
  const isSeasonReview = seasonNum > 0 && (section === 'shows' || section === 'anime');
  if (isSeasonReview) {
    return {
      titleLine: `season ${seasonNum} of ${rawTitle}`,
      subtitleLine,
  ...
  return {
    ...
    subtitleLine,
    subtitleBeforeTitle: !!subtitleLine
  };
}

function buildScreenListReviewHeaderHtml(activityId = '', item = {}, activity = {}, displayTitle = '', options = {}) {
  const stackedSummary = !!options.stackedSummary;
  const actionHtml = `<div class="sl-media-review-action"><span class="sl-media-review-action-verb">Wrote</span><span class="sl-media-review-action-rest"> a review for</span></div>`;
  if (stackedSummary) {
    const summaryTitle = String(options.summaryTitle || 'titles').trim() || 'titles';
    return `${actionHtml}<button class="sl-media-review-title" type="button" data-inline-stack-toggle onclick="event.stopPropagation(); toggleScreenListInlineActivityStack('${escAttr(activityId)}', event)">${escHtml(summaryTitle)}</button>`;
  }
  const lines = getScreenListActivityReviewHeadlineLines(item, activity, displayTitle);
  const titleHtml = `<button class="sl-media-review-title" type="button" onclick="event.stopPropagation(); openMediaReviewFromActivityCard('${escAttr(activityId)}')">${escHtml(lines.titleLine)}</button>`;
  const subtitleHtml = lines.subtitleLine ? `<div class="sl-media-review-subtitle">${escHtml(lines.subtitleLine)}</div>` : '';
  return lines.subtitleBeforeTitle
    ? `${actionHtml}${subtitleHtml}${titleHtml}`
    : `${actionHtml}${titleHtml}${subtitleHtml}`;
}

function normalizeScreenListActivityDetailPart(value = '') {
...
function buildScreenListActivitySplitHeadline(eventType = '', item = {}, activity = {}) {
  const stackAction = String(activity?.stackDisplayAction || '').replace(/\s+/g, ' ').trim();
  const stackTitle = String(activity?.stackDisplayTitle || '').replace(/\s+/g, ' ').trim();
  const stackDetail = String(activity?.stackDisplayDetail || '').replace(/\s+/g, ' ').trim();
  if (stackAction || stackTitle || stackDetail) {
    /* v10.996: when this is the music-stacked "Listened to N albums and
       rated them" card, the detail string carries the word "rated" —
       which the user wants colored the same gold as "Listened to".
       Flag detailGold so the renderer adds the `.is-gold-detail` class
       to the detail span. Detection: detail mentions "rated". */
    const detailGold = /\brated\b/i.test(stackDetail);
    return {
      action: stackAction,
      actionTail: '',
      title: stackTitle || String(item.title || item.name || 'Untitled').trim(),
      detail: stackDetail,
      detailGold
    };
  }

  const composed = buildScreenListActivityComposedSentence(eventType, item, activity);
  const action = String(composed.prefix || '').replace(/\s+/g, ' ').trim();
  const title = String(composed.title || item.title || item.name || 'Untitled').trim();
  const detailParts = [];
  const context = normalizeScreenListActivityDetailPart(composed.verbContext || '');
  const preTitle = String(composed.preTitle || '').replace(/\s+/g, ' ').trim();
  const suffix = normalizeScreenListActivityDetailPart(composed.suffix || '');
  const mediaType = getScreenListActivityMediaTypeLabel(item.librarySection || item.mediaCategory || '');
  const activeSection = String(item.librarySection || item.mediaCategory || '').toLowerCase();
  const activeStatus = String(activity?.nextStatus || item?.status || '').toLowerCase();
  const actionTail = String(composed.actionTail || '').replace(/\s+/g, ' ').trim();

  if (eventType === 'rated' && activeSection === 'music') {
    const musicType = getScreenListMusicActivityTypeLabel(item);
    const artist = getScreenListActivityArtistName(item);
    return {
      action: 'Rated',
      actionTail: `the ${musicType}`,
      title,
      detail: artist ? `by ${artist}` : ''
    };
  }

  if ((eventType === 'started' || ((eventType === 'status-changed' || eventType === 'added') && activeStatus === 'watching'))) {
    if (['movies', 'shows', 'anime'].includes(activeSection)) {
      return {
        action: 'Started watching',
        actionTail: '',
        preTitle: mediaType ? `The ${mediaType}` : '',
        title,
        detail: ''
      };
    }
    if (activeSection === 'games') {
      return {
        ...
        actionTail: '',
        title,
        detail: 'the video game'
      };
    }
  }

  if ((eventType === 'completed' || eventType === 'status-changed' || eventType === 'added')
    && activeStatus === 'watched'
    && ['movies', 'shows', 'anime'].includes(activeSection)) {
    return {
  ...
  if (eventType === 'season-finished' && composed.seasonFinishedDetail) {
    const seasonFinishedNum = Number(item?.lastSeasonFinishedNum || activity?.seasonNum || 0);
    return {
      action: 'Finished watching',
      actionTail: '',
      title,
      detail: seasonFinishedNum > 0 ? `Season ${seasonFinishedNum}` : ''
    };
  }

  if (action === 'Added' && actionTail) {
    return {
      action: 'Added',
      actionTail: '',
      preTitle: actionTail,
  ...
  } else if (mediaType && !composed.omitMediaTypeDetail) {
    detailParts.push(mediaType);
  }
  if (suffix) detailParts.push(suffix);
...
function buildImportActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  const avatarSrc = actor.photo || a.photo || '';
  const initial = getActivityCardUsername(actor, 'F').charAt(0).toUpperCase();
  const actorName = renderActivityCardUsernameHTML(actor, 'Friend');
  ...
  return `<article class="shelfd-social-card import-activity-card sl-import-activity-stack-wrap${isExpanded ? ' is-expanded' : ''}" data-import-activity-card="1" data-stacked-activity-id="${escAttr(activityId)}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}">
    <time class="sl-activity-time sl-activity-card-top-time">${escHtml(timeStr)}</time>
    <div class="import-activity-top">
      <div class="sl-activity-avatar-zone">${avatarHtml}</div>
      <div class="import-activity-copy">
        <div class="sl-activity-meta-row"><button class="sl-activity-name" type="button" onclick="event.stopPropagation(); openActivityUserList('${escAttr(a.uid)}','${escAttr(actor.name || actor.customName || a.name || '')}','${escAttr(avatarSrc)}',event.currentTarget)">${actorName}</button></div>
    ...
    <div class="import-activity-bottom">
      <div class="sl-activity-actions activity-interactions import-activity-actions" data-activity-interactions>
        <button class="sl-activity-action-btn activity-interaction-btn ${isLiked ? 'liked' : ''}" data-activity-action="like" onclick="event.stopPropagation(); toggleActivityLike('${escAttr(activityId)}', this)" aria-label="Like activity">
          <span class="sl-activity-icon-slot" data-like-icon-slot>${getActivityFeedLikeIconSvg(isLiked)}</span><span data-activity-like-count>${likeCount || 0}</span>
        </button>
        <button class="sl-activity-action-btn activity-interaction-btn" data-activity-action="reply" onclick="event.stopPropagation(); openActivityReplyPage('${escAttr(activityId)}')" aria-label="Open comments">
          <span class="sl-activity-icon-slot">${getActivityFeedCommentIconSvg()}</span><span data-activity-reply-count>${replyCount || 0}</span>
        </button>${deleteHtml}
      </div>
...
function buildStackedActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  if (a.stackDisplayDetail) primary.stackDisplayDetail = a.stackDisplayDetail;
  ...
  const _stackActionAttr = a.stackDisplayAction ? ` data-stack-action="${escAttr(String(a.stackDisplayAction).toLowerCase().trim())}"` : '';
  html = html.replace('<article ', `<article aria-label="Open grouped activity stack"${_stackActionAttr} `);
  ...
  const hiddenHtml = carouselItems.map((activity, index) => {
    ...
    childHtml = childHtml.replace('<article ', `<article style="--stack-index:${index}; --stack-reverse-index:${Math.max(0, carouselItems.length - index - 1)}" `);
  ...
  /* v10.272: dots + "Show all" controls live below the carousel. The
     ...
     paths. */
  const dotsHtml = carouselItems.map((_, i) => `<span class="sl-activity-stack-carousel-dot${i === 0 ? ' is-active' : ''}" data-dot-index="${i}"></span>`).join('');
  const carouselControlsHtml = carouselItems.length > 1
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  const isMediaReview = isActivityMediaReviewPost(a);
  /* v11.287: game-highlight posts now use the STANDARD activity card layout
     (same avatar / name / plain timestamp as every other card) instead of the
     feed-post layout. Only the body differs (inline Streamable embed). */
  const isHighlight = a.type === 'highlight' || a.eventType === 'highlight';
  ...
  const splitHeadline = buildScreenListActivitySplitHeadline(eventType, item, a);
  ...
  /* v11.287: highlight cards show the inline embed as their media — no side poster. */
  const posterHtml = isHighlight ? '' : options.stackPosterDeckHtml || (mediaCover
  ...
  */
  ...
  const isStackedContext = !!options.renderStackPrimary || !!options.compactStackChild;
  const stackExtraHtml = (!isStackedContext && stackExtraCount > 0) ? `
  ...
  const highlightShareHtml = isHighlight ? `
      <button class="sl-activity-action-btn activity-interaction-btn" data-activity-action="share" onclick="event.stopPropagation(); openHighlightShareSheet('${escAttr(activityId)}', event)" aria-label="Share highlight">
        <span class="sl-activity-icon-slot">${getScreenListShareIconSvg()}</span>
      </button>` : '';
  const highlightMoreHtml = isHighlight ? `
      <button class="sl-activity-action-btn activity-interaction-btn sl-activity-highlight-more" data-activity-action="highlight-more" onclick="openScreenListHighlightActionMenu('${escAttr(activityId)}', event)" aria-label="Highlight options">
        <span class="sl-activity-icon-slot">${getScreenListTrashIconSvg()}</span>
      </button>` : '';
  const deleteHtml = (!isHighlight && canCurrentUserDeleteActivity(a)) ? `
      <button class="sl-activity-action-btn activity-interaction-btn" data-activity-action="delete" onclick="event.stopPropagation(); openScreenListDeletePostPrompt('${escAttr(activityId)}','activities')" aria-label="Delete activity">
        <span class="sl-activity-icon-slot">${getScreenListTrashIconSvg()}</span>
      </button>` : '';
  ...
  /* v11.561: only show the "Full Review" button when the user ACTUALLY wrote a
     review — i.e. the review text has characters. Finished-watching / listened /
     played / rated cards with no typed review no longer show it (a rating alone
     isn't a review). For media-review cards the text lives in content.text. */
  const _reviewTextForBottom = String(
    a?.reviewText ||
    item?.reviewText ||
    (isMediaReview ? (a?.content && (a.content.text || a.content.body)) : '') ||
    ''
  ).trim();
  const _showBottomViewReview = (isMediaReview || _isCompletionRouteBottom) && _reviewTextForBottom.length > 0;
  ...
  const interactionsHtml = options.hideInteractions ? '' : `
    <div class="sl-activity-actions activity-interactions" data-activity-interactions>
      ...
      <button class="sl-activity-action-btn activity-interaction-btn" data-activity-action="reply" onclick="event.stopPropagation(); openActivityReplyPage('${escAttr(activityId)}')" aria-label="Open comments">
        <span class="sl-activity-icon-slot">${getActivityFeedCommentIconSvg()}</span><span data-activity-reply-count>${replyCount || 0}</span>
      </button>${highlightShareHtml}${highlightMoreHtml}${deleteHtml}${bottomViewReviewHtml}
    </div>`;
  ...
  const splitActionText = splitHeadline.action || composedHeadline.prefix || activityAction;
  const splitActionTail = String(splitHeadline.actionTail || '').trim();
  const splitActionHtml = `<div class="sl-activity-split-action"><span class="sl-activity-split-action-main">${escHtml(splitActionText)}</span>${splitActionTail ? `<span class="sl-activity-split-action-tail"> ${escHtml(splitActionTail)}</span>` : ''}</div>`;

  /* v10.996: detailGold flag (set in buildScreenListActivitySplitHeadline
     for music-stacked-rated) maps to the `.is-gold-detail` CSS class so
     the detail line ("and rated them") renders gold. Per spec, gold
     detail applies only on parent activity cards — child / stacked
     compact cards are excluded by user requirement, so the class is
     intentionally omitted from the compactStackChild branch below. */
  const detailClassMain = splitHeadline.detailGold ? 'sl-activity-split-detail is-gold-detail' : 'sl-activity-split-detail';

  /* v11.287: highlight body — "Shared a highlight from {game}" + inline embed. */
  let highlightBodyHtml = '';
  if (isHighlight) {
    const hlHeadline = String((a.content && a.content.headline) || `Shared a highlight from ${title}`);
    /* v11.457: only the action ("Shared a highlight") is gold; the "from {title}"
       part renders white. Split on the first " from ". */
    const hlHeadlineParts = hlHeadline.match(/^([\s\S]*?)\s+(from\s+[\s\S]+)$/i);
    const hlActionText = hlHeadlineParts ? hlHeadlineParts[1] : hlHeadline;
    const hlFromText = hlHeadlineParts ? hlHeadlineParts[2] : '';
    const hlUrl = String(a.highlightUrl || (a.content && a.content.text) || '').trim();
    const hlCaption = getScreenListHighlightCaption(a);
    const hlMatch = hlUrl.match(/streamable\.com\/(?:e\/)?([a-z0-9]+)/i);
    const hlId = hlMatch ? hlMatch[1] : '';
    const hlMedia = hlId
      ? `<div class="feed-post-highlight-embed"><iframe src="https://streamable.com/e/${escAttr(hlId)}" allow="fullscreen" allowfullscreen loading="lazy" title="Highlight reel" onclick="event.stopPropagation();"></iframe></div>`
      : (/^https?:\/\//i.test(hlUrl) ? `<a class="feed-post-highlight-link" href="${escAttr(hlUrl)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();">▶ Watch highlight reel</a>` : '');
    highlightBodyHtml = `
        <div class="sl-activity-action-line sl-activity-action-line-split">
          <div class="sl-activity-split-action"><span class="sl-activity-split-action-main">${escHtml(hlActionText)}</span>${hlFromText ? ` <span class="sl-activity-highlight-from">${escHtml(hlFromText)}</span>` : ''}</div>
          ${hlCaption ? `<div class="sl-activity-highlight-caption">${escHtml(hlCaption)}</div>` : ''}
        </div>
        ${hlMedia}`;
  }

  /* v11.523: render the FULL review text inline on the "Wrote a Review" card —
     username → "Wrote a Review" → title → the whole review → ★ rating. The card
     already grows (grid row 1 is minmax(146px, auto)) with the poster pinned
     top-right at a fixed size, so a one-line review keeps the normal card height
     and a long review extends the card downward. No clamp / no max-height — the
     entire review shows. Same text the Full Page Review reads:
     a.reviewText → content.text/body → item.reviewText. */
  const mediaReviewText = isMediaReview ? String(
    a.reviewText || (a.content && (a.content.text || a.content.body)) || item.reviewText || ''
  ).trim() : '';
  const mediaReviewTextHtml = mediaReviewText
    ? `<div class="sl-media-review-text">"${escHtml(mediaReviewText)}"</div>`
    : '';
  const mediaReviewHeaderHtml = isMediaReview
    ? buildScreenListReviewHeaderHtml(activityId, item, a, displayTitle)
    : '';
  /* v11.545: this media-review branch of bodyHtml now serves ONLY stacked/compact
     review children — the full "Wrote a Review" card has its own dedicated template
     (see the early return near the end of this function). Compact children keep the
     inline action → title → review → ★ layout. */
  const bodyHtml = isHighlight ? highlightBodyHtml : isMediaReview ? `
        ${mediaReviewHeaderHtml}
        ${mediaReviewTextHtml}
        <div class="sl-media-review-rating">${ratingHtml}</div>
        ${userNoteHtml}` : options.compactStackChild ? `
        <div class="sl-activity-action-line sl-activity-action-line-split">
          ${splitActionHtml}
          ${splitHeadline.preTitle ? `<div class="sl-activity-split-pretitle">${escHtml(splitHeadline.preTitle)}</div>` : ''}
          <button class="sl-activity-split-title" type="button" onclick="event.stopPropagation(); handleActivityMediaClick('${escAttr(activityId)}', this)">${escHtml(splitHeadline.title || composedHeadline.title || title)}</button>
          ${splitHeadline.detail ? `<div class="sl-activity-split-detail">${escHtml(splitHeadline.detail)}</div>` : ''}
        </div>
        ${ratingHtml}
        ${userNoteHtml}
        ${commentHtml}` : `
        <div class="sl-activity-action-line sl-activity-action-line-split">
          ...
          ${splitHeadline.preTitle ? `<div class="sl-activity-split-pretitle">${escHtml(splitHeadline.preTitle)}</div>` : ''}
          <button class="sl-activity-split-title" type="button" onclick="event.stopPropagation(); handleActivityMediaClick('${escAttr(activityId)}', this)">${escHtml(splitHeadline.title || composedHeadline.title || title)}</button>
          ${splitHeadline.detail ? `<div class="${detailClassMain}">${escHtml(splitHeadline.detail)}</div>` : ''}
        </div>
        ${ratingHtml}
        ${userNoteHtml}
        ${commentHtml}`;

  const topTimeHtml = options.compactStackChild ? '' : `<time class="sl-activity-time sl-activity-card-top-time">${escHtml(timeStr)}</time>`;

  const episodeActivityClass = (eventType === 'episode-watched' || eventType === 'episode-rated' || a.mergedHadEpisodeWatch)
    ? ' shelfd-episode-activity-card'
    : '';

  /* v11.545: the full (non-stacked) "Wrote a Review" card renders through its OWN
     self-contained template + the .shelfd-media-review-card CSS block — kept fully
     separate from every other activity-card type, so editing this layout can never
     touch the standard cards and vice-versa. Stacked/compact review children fall
     through to the standard path below (they reuse .shelfd-media-review-card for
     the compact styling and get sl-activity-stack-compact-card added afterward). */
  if (isMediaReview && !options.compactStackChild) {
    const isStackedReviewSummary = String(a?.type || '').trim() === 'activity-stack'
      && String(a?.stackEventType || '').trim() === 'review'
      && !!String(a?.stackDisplayTitle || '').trim();
    const stackedReviewTitle = String(a?.stackDisplayTitle || '').trim();
    /* v11.546: the action + title read as ONE sentence — "Wrote a review for the
       {kind} {title}" — with the media title kept inline (white, still clickable)
       rather than on its own line. The {kind} word comes from the section. */
    const reviewHeadlineHtml = isStackedReviewSummary
      ? buildScreenListReviewHeaderHtml(activityId, item, a, displayTitle, { stackedSummary: true, summaryTitle: stackedReviewTitle || 'titles' })
      : mediaReviewHeaderHtml;
    return `<article class="shelfd-social-card ${meta.topClass} shelfd-media-review-card${episodeActivityClass}" data-section="${escAttr(section || '')}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}" data-shelfd-activity-card="v4" onclick="${cardOnclick}">
      ${topTimeHtml}
      <div class="sl-activity-main">
        <div class="sl-activity-avatar-zone">${avatarHtml}</div>
        <div class="sl-activity-copy-zone">
          <div class="sl-activity-meta-row">${nameLine}</div>
          ${reviewHeadlineHtml}
        </div>
      </div>
      ${posterHtml}
      <div class="sl-media-review-body">
        ${mediaReviewTextHtml}
        <div class="sl-media-review-rating">${ratingHtml}</div>
        ${userNoteHtml}
      </div>
  ...
  return `<article class="shelfd-social-card ${meta.topClass}${isMediaReview ? ' shelfd-media-review-card' : ''}${isHighlight ? ' shelfd-highlight-card' : ''}${episodeActivityClass}" data-section="${escAttr(section || '')}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}" data-shelfd-activity-card="v4" onclick="${cardOnclick}">
    ${topTimeHtml}
    <div class="sl-activity-main">
      <div class="sl-activity-avatar-zone">${avatarHtml}</div>
      <div class="sl-activity-copy-zone">
...
function buildActivityFeedHeaderHTML(heading = 'Activity Feed', options = {}) {
  ...
  if (showSectionNav) {
    ...
    const newsActive = activeActivitySubTab === 'news';
    /* v10.285: Activity Feed → Shared Watch pills.
       v11.590: the Notifications pill was removed — Notifications now lives in
       the Friends page tab bar (Friends · Notifications · Add a Friend).
       v11.592: News pill added — editorial entertainment-news feed (its own
       Activity sub-tab, rendered by js/42-news-feed.js). */
    actionButtons.push(`<button type="button" class="activity-shared-watch-pill activity-feed-pill ${feedActive ? 'active' : 'secondary'}" aria-current="${feedActive ? 'true' : 'false'}" onclick="switchActivitySubTab('feed')">Friends Feed<span class="activity-feed-tab-dot" aria-hidden="true" style="display:${(typeof friendActivityUnread !== 'undefined' && friendActivityUnread && !feedActive) ? 'inline-block' : 'none'};"></span></button>`);
    actionButtons.push(`<button type="button" class="activity-shared-watch-pill news-tab-pill ${newsActive ? 'active' : 'secondary'}" aria-current="${newsActive ? 'true' : 'false'}" onclick="switchActivitySubTab('news')">News Feed</button>`);
...
/* v12.078: count-based pagination state for the Friends Feed.
   Scoped to this module — reset on every full re-render, appended by the
   IntersectionObserver sentinel as the user scrolls toward the bottom.
   v12.080: smaller FIRST page (lighter first paint on the iOS WKWebView — each
   card decodes a poster + avatar and some carry embeds), full 30 per page after
   the user starts scrolling (fewer observer fires = smoother mid-scroll). */
const ACTIVITY_FEED_FIRST_PAGE_SIZE = 12;
const ACTIVITY_FEED_PAGE_SIZE = 30;
let _activityFeedRenderItems = [];   // processed, ordered flat list
let _activityFeedRenderCursor = 0;   // index of next item to render
let _activityFeedRenderOpts = null;  // options reference for append calls
let _activityFeedObserver = null;    // current IntersectionObserver
let _activityFeedMinReveal = 0;      // v12.079: cards to re-reveal after a day-window extend (preserves scroll)
...
function renderFriendActivityItems(feed, activities, options = {}) {
  ...
  activities.forEach((activity, index) => {
    ...
    if (renderedActivity.postId) friendActivityClickTargets[String(renderedActivity.postId)] = renderedActivity;
    if (!groups[group]) groups[group] = [];
    groups[group].push({ activity: renderedActivity, id });
  });

  /* Build flat ordered list (Today → Yesterday → This Week → Last Week → Earlier). */
  const orderedItems = [];
  groupOrder.forEach(g => { orderedItems.push(...groups[g]); });

  const storiesHtml = options.hideStories ? '' : buildActivityStoriesHTML(activities);
  const headerHtml = buildActivityFeedHeaderHTML(options.heading || 'Activity Feed', { showSharedWatch: !options.hideSharedWatchPill, showRefresh: !options.hideRefresh, hideHeading: !!options.hideHeading });

  /* v12.078: paginated render for the main Friends Feed (options.showLoadMore).
     All other callers (profile views, stacked, etc.) still get one-shot rendering. */
  if (options.showLoadMore) {
    if (_activityFeedObserver) { _activityFeedObserver.disconnect(); _activityFeedObserver = null; }
    _activityFeedRenderItems = orderedItems;
    _activityFeedRenderCursor = 0;
    _activityFeedRenderOpts = options;

    feed.innerHTML = headerHtml + storiesHtml +
      '<div class="activity-feed-list"></div>' +
      '<div class="activity-feed-sentinel" aria-hidden="true"></div>';

    _appendActivityFeedPage(feed);
    /* v12.079: after a "load older activity" extend re-renders from scratch, keep
       revealing pages until we've re-shown what the user had already scrolled
       through, so their scroll position stays valid (no jump into empty space). */
    while (_activityFeedMinReveal > _activityFeedRenderCursor && _activityFeedRenderCursor < _activityFeedRenderItems.length) {
      _appendActivityFeedPage(feed);
    }
    _activityFeedMinReveal = 0;
    _setupActivityFeedObserver(feed);
    return;
  }

  /* One-shot render (non-paginated callers). */
  let cardsHtml = '';
  orderedItems.forEach(({ activity, id }) => {
    cardsHtml += buildActivityCardHTML(activity, id, options);
  });

  feed.innerHTML = headerHtml + storiesHtml + `<div class="activity-feed-list">${cardsHtml}</div>`;
...
function _appendActivityFeedPage(feed) {
  const list = feed.querySelector('.activity-feed-list');
  const sentinel = feed.querySelector('.activity-feed-sentinel');
  if (!list) return;

  const start = _activityFeedRenderCursor;
  /* v12.080: first page is smaller (lighter first paint), every page after is full. */
  const pageSize = start === 0 ? ACTIVITY_FEED_FIRST_PAGE_SIZE : ACTIVITY_FEED_PAGE_SIZE;
  const end = Math.min(start + pageSize, _activityFeedRenderItems.length);
  const page = _activityFeedRenderItems.slice(start, end);
  _activityFeedRenderCursor = end;

  if (page.length) {
    let cardsHtml = '';
    page.forEach(({ activity, id }) => {
      cardsHtml += buildActivityCardHTML(activity, id, _activityFeedRenderOpts || {});
    });
    const prevCount = list.querySelectorAll('.activity-card, .shelfd-social-card').length;
    list.insertAdjacentHTML('beforeend', cardsHtml);
    Array.from(list.querySelectorAll('.activity-card, .shelfd-social-card')).slice(prevCount).forEach((card, i) => {
      card.style.animationDelay = `${Math.min(i * 45, 360)}ms`;
    });
    hydrateActivityInteractionCounts(list);
    scheduleBackfillActivityGamePosters(list);
  }

  /* v12.079: the auto-scroll sentinel ONLY reveals already-fetched cards. The
     spinner shows ONLY while more CACHED cards remain. When the fetched window is
     exhausted we DO NOT auto-extend the day window from here — doing so re-fetched
     Firestore on every intersect and, for any feed shorter than 30 cards (sentinel
     permanently in view), looped 7→10→13→…→365 days, rebuilding the whole feed
     each step. That runaway re-fetch was the "app keeps refreshing" bug. Older
     activity is now a deliberate tap (loop-proof). */
  const hasMoreItems = _activityFeedRenderCursor < _activityFeedRenderItems.length;
  const canExtendDays = friendActivityDayLimit < FRIEND_ACTIVITY_MAX_DAYS;
  if (sentinel) {
    if (hasMoreItems) {
      sentinel.style.display = 'block';
      sentinel.innerHTML = '<div class="activity-feed-auto-spinner"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="12" cy="12" r="9" stroke-dasharray="56" stroke-dashoffset="14" stroke="rgba(167,139,250,0.7)"/></svg></div>';
    } else {
      /* No more cached cards — stop the auto-observer. Offer a manual "load older"
         affordance only when an older window genuinely exists. */
      if (_activityFeedObserver) { _activityFeedObserver.disconnect(); _activityFeedObserver = null; }
      if (canExtendDays) {
        sentinel.style.display = 'block';
        sentinel.innerHTML = '<div class="activity-feed-load-older-wrap"><button type="button" class="activity-feed-load-older-btn" onclick="loadMoreFriendActivity()">Load older activity</button></div>';
      } else {
        sentinel.innerHTML = '';
...
function _setupActivityFeedObserver(feed) {
  const sentinel = feed.querySelector('.activity-feed-sentinel');
  if (!sentinel || !('IntersectionObserver' in window)) return;
  /* Only fires while CACHED cards remain. The exhausted path is handled by the
     manual button in _appendActivityFeedPage — never an auto server re-fetch. */
  if (_activityFeedRenderCursor >= _activityFeedRenderItems.length) return;
  _activityFeedObserver = new IntersectionObserver(function(entries) {
    if (!entries[0].isIntersecting) return;
    if (_activityFeedRenderCursor < _activityFeedRenderItems.length) {
      _appendActivityFeedPage(feed);
    } else {
      if (_activityFeedObserver) { _activityFeedObserver.disconnect(); _activityFeedObserver = null; }
    }
  }, { rootMargin: '300px' });
  _activityFeedObserver.observe(sentinel);
}
...
async function loadMoreFriendActivity() {
  ...
  /* v11.560: keep the user exactly where they are vertically. loadActivityTabFeed
     re-renders the whole feed (feed.innerHTML), which momentarily collapses the
     page height and snaps window scroll to the top. Capture the scroll position
     before the re-render and restore it after, so extending the day window reveals
     more content below without jumping the user back to the top. */
  const previousScrollY = window.scrollY || window.pageYOffset || 0;
  /* v12.079: remember how many cards were already revealed so the post-extend
     re-render (which resets to page 1) re-reveals at least that many — keeps the
     restored scroll position valid instead of stranding the user past the content. */
  _activityFeedMinReveal = _activityFeedRenderCursor;
  ...
  try {
    ...
    requestAnimationFrame(() => {
      try { window.scrollTo({ top: previousScrollY, left: 0, behavior: 'auto' }); }
      catch (error) { window.scrollTo(0, previousScrollY); }
    });
  } catch (e) {
...
/* v11.467: instant Highlight Reel insertion — no network refetch.
   The old path busted friendActivityCache and relied on a guarded
   loadActivityTabFeed() call that could never run (its isActivityTabVisible
   guard was undefined), so a freshly posted highlight only appeared after the
   120s cache expiry or a manual tab refresh. This inserts the new post straight
   into the authoritative cache (deduped by its `feed:<postId>` eventKey, kept
   fresh) and, when the main Activity Feed is on-screen, re-renders from cache
   immediately — prepending the card with NO skeleton blank and NO Firestore
   read. The eventual cache-expiry refetch rebuilds wholesale and dedupes by the
   same eventKey, so the card is never duplicated. */
function prependActivityFeedHighlightPost(feedPost) {
  if (!feedPost || !feedPost.postId) return;
  try {
    const eventKey = `feed:${feedPost.postId}`;
    const activity = { ...feedPost, timestamp: feedPost.timestamp || Date.now(), eventKey };

    /* Keep the current user resolvable for the card's avatar/name even if the
       feed has not been built yet this session (matches fetchAllFriendActivities). */
    if (currentUser && currentUser.uid) {
      const ownProfile = (typeof userProfile === 'object' && userProfile) ? userProfile : {};
      usersMap[currentUser.uid] = {
        ...(usersMap[currentUser.uid] || {}),
        uid: currentUser.uid,
        name: usersMap[currentUser.uid]?.name || ownProfile.name || currentUser.displayName || 'You',
        photo: usersMap[currentUser.uid]?.photo || ownProfile.photo || currentUser.photoURL || ''
      };
    }

    if (friendActivityCache && Array.isArray(friendActivityCache.activities)) {
      friendActivityCache.activities = [
        activity,
        ...friendActivityCache.activities.filter(a => (a && a.eventKey) !== eventKey)
      ];
      friendActivityCache.timestamp = Date.now();
    }

    const onMainFeed = (typeof activeFriendsTab !== 'undefined' && activeFriendsTab === 'activity')
      && !(typeof isWatchActivitySubTab === 'function' && isWatchActivitySubTab())
      && !(typeof activeActivitySubTab !== 'undefined' && activeActivitySubTab === 'notifications');
    const feed = document.getElementById('friend-activity-feed');
    if (onMainFeed && feed && friendActivityCache && Array.isArray(friendActivityCache.activities)) {
      renderFriendActivityItems(feed, cloneFriendActivityList(friendActivityCache.activities), { showLoadMore: true });
    }
  } catch (e) {
    /* Fallback to the old behavior so the next natural load still refetches. */
    try { if (friendActivityCache) friendActivityCache.timestamp = 0; } catch (_) {}
    console.warn('[activity] instant highlight prepend failed:', e);
  }
}
window.prependActivityFeedHighlightPost = prependActivityFeedHighlightPost;
...
async function fetchAllFriendActivities(dayLimit = 7) {
  ...
  function processUserItems(uid, u, sectionItems, section) {
    for (const item of sectionItems) {
      ...
      /* v11.464: attaching / editing a game highlight reel bumps the game's
         dateModified, but that is NOT a user-facing "updated game" event — the
         Highlight Reel feed post already represents it. Suppress the generic
         dateModified-derived card when the change coincides with a highlight save
         (lastHighlightActivityAt, set in saveScreenListCompetitiveProfile). A real
         later edit bumps dateModified away from this stamp, so its update card
         still fires normally. */
      const highlightActivityAt = item.lastHighlightActivityAt || '';
      const modIsHighlightActivity = !!highlightActivityAt && modTime > 0 &&
        Math.abs(parseFriendActivityTime(highlightActivityAt) - modTime) <= SCREENLIST_MOD_DEDUPE_WINDOW_MS;
      /* Review saves also touch dateModified for sorting/sync, but the
         user-facing activity is the linked "Wrote a review" feed post. Do not
         infer a generic status-changed / finished-watching card from that
         same write. */
      ...
      const modIsReviewActivity = !!reviewActivityAt && modTime > 0 &&
        Math.abs(parseFriendActivityTime(reviewActivityAt) - modTime) <= SCREENLIST_MOD_DEDUPE_WINDOW_MS;
      const ratingOnlySaveAt = item.lastRatingOnlySaveAt || '';
      const modIsRatingOnlySaveActivity = !!ratingOnlySaveAt && modTime > 0 &&
        Math.abs(parseFriendActivityTime(ratingOnlySaveAt) - modTime) <= SCREENLIST_MOD_DEDUPE_WINDOW_MS;
      const modIsRedundant =
        ...
        modIsHighlightActivity ||
        modIsReviewActivity ||
        modIsRatingOnlySaveActivity;
  ...
  const ownUsernameHandle = String(
    ownProfile.usernameHandle ||
    ownProfile.userHandle ||
    ownProfile.username ||
    ownProfile.handle ||
    ''
  ).trim().replace(/^@+/, '');
  const ownUsernameHandleLower = String(
    ownProfile.usernameHandleLower ||
    ownProfile.handleLower ||
    ownProfile.usernameLower ||
    ownUsernameHandle
  ).trim().replace(/^@+/, '').toLowerCase();
  /* v10.990: activity cards render @handle first, but the current-user
     activity cache was seeded with only display name + photo. That left
     own cards without username data and forced a display-name fallback.
     Always merge the current user's handle fields before card rendering. */
  usersMap[currentUser.uid] = {
    ...(usersMap[currentUser.uid] || {}),
    uid: currentUser.uid,
    name: ownName,
    customName: ownProfile.customName || ownName,
    displayName: currentUser.displayName || ownProfile.displayName || ownName,
    photo: ownPhoto,
    usernameHandle: ownUsernameHandle || usersMap[currentUser.uid]?.usernameHandle || '',
    usernameHandleLower: ownUsernameHandleLower || usersMap[currentUser.uid]?.usernameHandleLower || ''
  };
...
async function loadFriendActivity() {
  ...
  } else if (activeFriendsTab === 'notifications') {
    /* v11.590: Notifications is now a Friends-page tab. */
...
/* =============================================================================
   v12.034: Activity-feed video scroll-mute.
   When a user taps play on a highlight clip and then keeps scrolling, the clip
   should KEEP PLAYING but its audio must MUTE once it leaves the frame — so the
   sound doesn't trail the user down the feed. Highlight clips are Streamable
   embeds, which speak the player.js postMessage protocol; sending `mute` does
   NOT reload/restart the video (so the user keeps their place), and muting a
   paused/unstarted player is a harmless no-op — so we simply mute any highlight
   iframe that scrolls out of view. We deliberately do NOT auto-unmute on scroll
   back in: silently re-enabling sound is both surprising and an autoplay-policy
   risk; the user can tap the player to restore audio.
   ========================================================================== */
(function () {
  'use strict';

  function muteStreamableIframe(iframe) {
    if (!iframe || !iframe.contentWindow) return;
    /* player.js spec (what Streamable's embed listens for). The bare {method}
       form is sent too for older/alternate embed builds. */
    try {
      iframe.contentWindow.postMessage(JSON.stringify({
        context: 'player.js', version: '0.0.4', method: 'mute'
      }), '*');
    } catch (_) {}
    try {
  ...
  function getObserver() {
    ...
    io = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting && entry.intersectionRatio >= 0.35) return; // in frame → leave audio alone
        var el = entry.target;
        var iframe = (el && el.tagName === 'IFRAME') ? el : (el && el.querySelector ? el.querySelector('iframe') : null);
        if (iframe) muteStreamableIframe(iframe);
      });
    }, { threshold: [0, 0.35, 0.7] });
    return io;
  }

  function scanActivityFeedVideos() {
    var obs = getObserver();
    if (!obs) return;
    var embeds = document.querySelectorAll('.feed-post-highlight-embed');
    for (var i = 0; i < embeds.length; i++) {
      var el = embeds[i];
      if (el.__shelfdVideoMuteObserved) continue;
      el.__shelfdVideoMuteObserved = true;
      obs.observe(el);
    }
  }
  window.scanActivityFeedVideos = scanActivityFeedVideos;

  function install() {
    scanActivityFeedVideos();
    /* The feed rebuilds its cards on snapshot/hydration, so watch the feed roots
       for newly-mounted highlight embeds and observe them too. */
    try {
      var mo = new MutationObserver(function () { scanActivityFeedVideos(); });
      var roots = [
        document.getElementById('activity-tab-view'),
        document.getElementById('community-view')
      ].filter(Boolean);
      if (!roots.length) roots = [document.body];
      roots.forEach(function (r) { mo.observe(r, { childList: true, subtree: true }); });
    } catch (_) {}
  }
