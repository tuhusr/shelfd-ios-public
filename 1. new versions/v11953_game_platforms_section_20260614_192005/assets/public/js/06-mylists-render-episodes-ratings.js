...
function getGamePlatformsList(details = {}) {
  ...
  if (Array.isArray(igdb.platforms) && igdb.platforms.length && typeof igdb.platforms[0] === 'object') {
    return igdb.platforms
      .map(p => ({ name: cleanGamePlatformName(p.name), logo: String(p.logo || '').trim() }))
      .filter(p => p.name);
  }
  const rawg = Array.isArray(details.platforms)
    ? details.platforms.map(p => p.platform?.name || p.name).filter(Boolean)
    : [];
  return rawg.map(name => ({ name: cleanGamePlatformName(name), logo: '' })).filter(p => p.name);
}
...
function renderGamePlatformsSectionHtml(details = {}) {
  ...
  return `<div class="discover-media-section game-platforms-section" id="game-media-platforms-section"${platforms.length ? '' : ' hidden'}>
    <h3>Platforms</h3>
    <div class="game-platforms-row">${platforms.map(p => `<div class="game-platform-chip">${p.logo ? `<span class="game-platform-logo-wrap"><img class="game-platform-logo" src="${escAttr(p.logo)}" alt="" loading="lazy" decoding="async"></span>` : ''}<span class="game-platform-name">${escHtml(p.name)}</span></div>`).join('')}</div>
  </div>`;
}
