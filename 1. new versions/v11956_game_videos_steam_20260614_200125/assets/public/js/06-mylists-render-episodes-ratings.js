...
async function fetchGameInfoFromIgdb(details = {}) {
  ...
  // so the browser HTTP cache doesn't replay a stale-shape response (e.g. the
  // pre-logo platforms format).
  params.set('iv', '2');
  try {
...
function playGameVideo(btn) {
  ...
  const src = `https://www.youtube-nocookie.com/embed/${vid}?playsinline=1&rel=0&modestbranding=1&iv_load_policy=3&autoplay=1&enablejsapi=1${origin}&controls=1&fs=1`;
