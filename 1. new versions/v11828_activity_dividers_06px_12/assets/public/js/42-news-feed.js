...
(function () {
  ...
  function preciseIdentityKeys(item) {
    ...
    for (var i = 0; i < all.length; i++) { if (/^(u:|id:|pid:|ev:|yt:)/.test(all[i])) out.push(all[i]); }
    return out;
  }
  ...
  function recordOpen(url) {
    ...
    try { recordConsumed(url, 'open'); } catch (e) {}
  }
  ...
  /* v11.693: HARD-HIDE consumed (liked / shared / opened) articles out of the pool
     BEFORE ranking, so the exact item the user engaged with does not resurface as
     a normal feed card in ANY tab (All / Movies&TV / Anime / Games / Music /
     Following). Safety valves: (1) Following is finite — hide always (an exhausted
     follow set honestly shows "nothing new"); (2) the endless categories must
     never go blank purely from suppression, so if hiding would leave fewer than a
     page of fresh items we re-admit the consumed ones at the END (still buried by
     the score) to keep infinite scroll alive — you only ever re-see consumed
     content once everything fresh is genuinely exhausted. */
  function suppressConsumedPool(pool) {
    var keep = [], hidden = [], map = readConsumed(), now = Date.now();   // read the store ONCE per order build
    for (var i = 0; i < pool.length; i++) {
      if (isConsumedInMap(pool[i], map, now)) hidden.push(pool[i]); else keep.push(pool[i]);
    }
    if (!hidden.length) return pool;
    if (state.category === 'following') return keep;
    if (keep.length >= PAGE_SIZE) return keep;
    return keep.concat(hidden);
  }
  ...
  function openNewsYouTube(id, meta) {
    ...
    try { if (meta && meta.url) recordOpen(meta.url); } catch (e) {}    // mark watched
  ...
  function toggleInlineNewsVideoMute(card) {
    ...
    if (!nextMuted) {
      ...
      try { if (meta.url) recordOpen(meta.url); } catch (e) {}
  ...
  function mountAutoplayVideo(card) {
    ...
    /* v11.708: ALWAYS autoplay MUTED (muted autoplay is universally allowed →
       guaranteed to start), then unMute via the YT API when sound is wanted +
       capable (native flag present). This replaces the v11.706 gate that mounted
       unmuted ONLY if a silent-audio probe passed — that probe false-negatived in
       WKWebView and kept videos muted even with the native flag set. Unmuting an
       already-playing video is reliable on the flagged build and harmlessly
       ignored elsewhere. */
    var wantSound = newsSoundPrefOn() && newsSoundCapable();
    _mountNewsVideo(card, meta, youtubeVideoId(meta.url), true, true);
    state.inlineVideoWantSound = wantSound;
    if (wantSound) {
      var media = state.inlineVideoMedia;
      [300, 700, 1300, 2200].forEach(function (d) {           // unmute ASAP; retries cover a missed onReady
  ...
  window.shelfdMountNewsInlineVideo = function (media, meta, opts) {
    ...
    _mountNewsVideo(shimCard, meta, ytId, opts.muted !== false, true);
    if (opts.wantSound && opts.muted !== false) {
      state.inlineVideoWantSound = newsSoundPrefOn() && newsSoundCapable();
      if (state.inlineVideoWantSound) {
        [300, 700, 1300, 2200].forEach(function (d) {
          setTimeout(function () { applyAutoplayUnmute(media); }, d);
        });
      }
    }
  ...
  function openCardArticle(card) {
    ...
    if (isExternalOnlyNewsUrl(meta.url)) {
      try { recordOpen(meta.url); recordTaste(meta, 0.8); } catch (e) {}
