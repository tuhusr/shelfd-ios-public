...
function buildActivityNotificationRowHTML(notification = {}) {
  ...
  const incomingRequestList = Array.isArray(window.incomingRequests)
    ? window.incomingRequests
    : (typeof incomingRequests !== 'undefined' && Array.isArray(incomingRequests) ? incomingRequests : []);
  const isActionableFriendRequest = type === 'friend_request'
    && targetKind !== 'creator_auto_add'
    && actorUid
    && incomingRequestList.includes(actorUid);
  const name = isActionableFriendRequest
  ...
  const avatarHtml = photo
    ...
    : `<span>${h((name.charAt(0) || 'S').toUpperCase())}</span>`;
  ...
  if (isActionableFriendRequest) {
    ...
    <span class="shelfd-notif-request-actions">
      <button type="button" class="shelfd-notif-request-btn primary" onclick="event.stopPropagation(); handleShelfdNotificationFriendRequest('${a(actorUid)}','accept','${a(id)}')">Accept</button>
      <button type="button" class="shelfd-notif-request-btn secondary" onclick="event.stopPropagation(); handleShelfdNotificationFriendRequest('${a(actorUid)}','deny','${a(id)}')">Deny</button>
    </span>
    ${dotHtml}
  </div>`;
  }
...
async function handleShelfdNotificationFriendRequest(actorUid = '', action = 'accept', notificationId = '') {
  ...
  if (typeof renderActivityNotificationsList === 'function') renderActivityNotificationsList();
}
...
function getScreenListHighlightSharePayload(activity = {}, fallbackId = '') {
  ...
  const headline = String(activity?.content?.headline || `Shared a highlight from ${title}`).trim();
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  */
  ...
  const highlightMoreHtml = isHighlight ? `
      <button class="sl-activity-action-btn activity-interaction-btn" data-activity-action="highlight-more" onclick="openScreenListHighlightActionMenu('${escAttr(activityId)}', event)" aria-label="Highlight options">
  ...
  const interactionsHtml = options.hideInteractions ? '' : `
    <div class="sl-activity-actions activity-interactions" data-activity-interactions>
      ...
      </button>${highlightMoreHtml}${deleteHtml}${bottomViewReviewHtml}
    </div>`;
...
async function loadMoreFriendActivity() {
  ...
  /* v11.560: keep the user exactly where they are vertically. loadActivityTabFeed
     ...
     before the re-render and restore it after, so "Load more" just reveals the
     next batch below instead of jumping the user back to the top. */
  const previousScrollY = window.scrollY || window.pageYOffset || 0;
