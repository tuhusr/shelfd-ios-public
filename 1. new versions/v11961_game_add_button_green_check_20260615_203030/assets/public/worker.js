...
const SCREENLIST_IMDB_RATING_CACHE_VERSION = "v4";
...
async function serveGameProfileShareHtml(request, env, url) {
  ...
  /* Render the share-card preview (cover + ranks + KD) rather than the bare
     cover, so the link preview mirrors the in-app competitive profile. */
  const ogImage = new URL("/game-profile-og.svg", url.origin);
...
function serveGameProfileOgSvg(url) {
  ...
  const titleMarkup = titleLines.map((line, idx) =>
    `<text x="324" y="${224 + idx * 64}" fill="#FFFFFF" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="60" font-weight="700">${escapeHtmlMeta(line)}</text>`
  ).join("");
  const metaY = 224 + titleLines.length * 64 - 12;
  const yearMarkup = year
    ? `<text x="324" y="${metaY}" fill="#FFFFFF" fill-opacity="0.55" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="30" font-weight="300">${escapeHtmlMeta(year)}</text>`
    : "";
  const genreMarkup = genre
    ? `<text x="324" y="${metaY + 44}" fill="#FFFFFF" fill-opacity="0.55" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="26" font-weight="300">${escapeHtmlMeta(genre)}</text>`
    : "";
  const posterMarkup = safePoster
    ? `<image href="${safePoster}" x="80" y="92" width="200" height="200" preserveAspectRatio="xMidYMid slice" clip-path="url(#gpCoverClip)"/>`
    : `<rect x="80" y="92" width="200" height="200" rx="28" fill="#241B3D"/><text x="180" y="218" fill="#C7B7FF" font-size="96" font-weight="600" text-anchor="middle">${initial}</text>`;
  const statCell = (x, y, label, value, accent) => {
    const big = String(value || "-").length > 9;
    return `<rect x="${x}" y="${y}" width="500" height="118" rx="24" fill="#FFFFFF" fill-opacity="0.04" stroke="#FFFFFF" stroke-opacity="0.10"/>`
      + `<text x="${x + 34}" y="${y + 44}" fill="${accent}" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="22" font-weight="600" letter-spacing="1.5">${escapeHtmlMeta(label)}</text>`
      + `<text x="${x + 34}" y="${y + 92}" fill="#FFFFFF" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="${big ? 36 : 44}" font-weight="700">${escapeHtmlMeta(value || "-")}</text>`;
  };
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <defs>
    ...
    <radialGradient id="gpglow" cx="22%" cy="16%" r="70%">
  ...
  <rect x="40" y="40" width="1120" height="550" rx="48" fill="url(#gpglow)"/>
  ${posterMarkup}
  <rect x="324" y="104" width="218" height="52" rx="26" fill="#7C4DFF" fill-opacity="0.16" stroke="#A78BFA" stroke-opacity="0.42"/>
  <text x="356" y="138" fill="#C7B7FF" font-family="system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" font-size="24" font-weight="700" letter-spacing="2">COMPETITIVE</text>
  ${titleMarkup}
  ${yearMarkup}
  ${genreMarkup}
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/game-profile-og.svg" && request.method === "GET") {
      return serveGameProfileOgSvg(url);
    }
