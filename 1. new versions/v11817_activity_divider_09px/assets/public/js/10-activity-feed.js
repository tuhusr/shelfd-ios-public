...
function buildScreenListActivitySplitHeadline(eventType = '', item = {}, activity = {}) {
  ...
  return {
    action,
    actionTail: String(composed.actionTail || '').replace(/\s+/g, ' ').trim(),
    title,
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  */
  ...
  if (isMediaReview && !options.compactStackChild) {
    ...
    const stackedReviewActionLead = String(a?.stackDisplayAction || 'Wrote a review for').trim();
    ...
    const reviewMediaWord = ({ movies: 'movie', shows: 'show', anime: 'anime', games: 'game', music: 'album', books: 'book', manga: 'manga' })[String(section || '').toLowerCase()] || '';
    const reviewActionLead = reviewMediaWord ? `Wrote a review for the ${escHtml(reviewMediaWord)} ` : 'Wrote a review for ';
    const reviewHeadlineHtml = isStackedReviewSummary
      ? `${escHtml(stackedReviewActionLead)} <button class="sl-media-review-title" type="button" data-inline-stack-toggle onclick="event.stopPropagation(); toggleScreenListInlineActivityStack('${escAttr(activityId)}', event)">${escHtml(stackedReviewTitle || 'titles')}</button>`
      : `${reviewActionLead}<button class="sl-media-review-title" type="button" onclick="event.stopPropagation(); openMediaReviewFromActivityCard('${escAttr(activityId)}')">${escHtml(displayTitle)}</button>`;
    return `<article class="shelfd-social-card ${meta.topClass} shelfd-media-review-card${episodeActivityClass}" data-section="${escAttr(section || '')}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}" data-shelfd-activity-card="v4" onclick="${cardOnclick}">
      ...
      <div class="sl-activity-main">
        ...
        <div class="sl-activity-copy-zone">
          ...
          <div class="sl-media-review-action">${reviewHeadlineHtml}</div>
        </div>
      </div>
      ${posterHtml}
