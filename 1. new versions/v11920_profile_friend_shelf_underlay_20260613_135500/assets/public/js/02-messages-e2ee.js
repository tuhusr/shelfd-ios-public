...
function getDirectMessageMediaHtml(payload = {}) {
  ...
  const shareHtml = normalizedShare && typeof renderDirectMessageShareCard === 'function'
    ...
    : '';
  const imageHtml = payload.imageData ? `<button class="dm-photo-message-button" type="button" onclick="openDirectMessagePhotoViewer(this.querySelector('.dm-photo-message'))" aria-label="Open photo full screen"><img class="dm-photo-message" src="${escAttr(payload.imageData)}" alt="${escAttr(payload.imageName || 'Photo message')}" loading="lazy" decoding="sync" draggable="false"></button>` : '';
  return shareHtml + imageHtml;
}

function getDirectMessageTextHtml(payload = {}) {
  return payload.text ? `<span class="dm-message-text">${escHtml(payload.text || '')}</span>` : '';
}
