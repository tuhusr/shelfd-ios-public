...
function getGameMediaInfoRows(details = {}) {
  ...
  const rawgPlatforms = Array.isArray(details.platforms)
  ...
  const releaseDate = igdb.releaseDate ? formatGameReleaseDate(igdb.releaseDate) : formatGameReleaseDate(details.released);
  ...
  const platforms = (igdb.platforms && igdb.platforms.length) ? igdb.platforms : rawgPlatforms;
...
async function hydrateGameMediaProfileInfo(overlay, details = {}) {
  try {
    ...
    const section = root && root.querySelector ? root.querySelector('#game-media-info-section') : null;
    if (!section) return;
    const tmp = document.createElement('div');
    tmp.innerHTML = renderGameMediaInfoSectionHtml(details);
