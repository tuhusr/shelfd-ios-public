...
function isCreativeTeamUser(userLike = null) {
  ...
  const email = getUserAccountEmail(userLike);
  if (email && typeof CREATIVE_TEAM_EMAILS !== 'undefined' && CREATIVE_TEAM_EMAILS.has(email)) return true;
  const handle = String(
  ...
  if (handle && typeof CREATIVE_TEAM_HANDLES !== 'undefined' && CREATIVE_TEAM_HANDLES.has(handle)) return true;
  const candidates = [
    userLike.name,
    userLike.customName,
...
function renderDisplayNameHTML(userLike = null, fallback = 'Unknown User', extraClass = '', options = {}) {
  ...
  const suppressCreativeTeamTag = !!(options && options.suppressCreativeTeamTag);
  const creativeTeamTag = !suppressCreativeTeamTag && isCreativeTeamUser(userLike) ? renderCreativeTeamTagHTML() : '';
...
/* v11.989: standalone creator/owner "C" badge — the exact red owner badge the
   activity feed shows — for appending after a display name OR @handle ANYWHERE
   the dev/creator account is shown (shelf banner, FFM rows, friends list,
   add-a-friend search, DM, etc.). Returns '' for every other account. */
function renderShelfdOwnerBadgeHTML(userLike = null) {
  if (typeof isOwnerBadgeAccount !== 'function' || !isOwnerBadgeAccount(userLike)) return '';
  return `<button type="button" class="creator-role creator-role-compact creator-role-owner shelfd-inline-owner-badge" onclick="event.stopPropagation();toggleCreatorRoleTooltip(this)" aria-label="Creator and Developer"><img class="creator-owner-badge-img" src="/badges/owner-badge.svg" width="12" height="15" alt="" decoding="async"><span class="creator-role-tooltip creator-owner-tooltip" role="status" aria-hidden="true">Creator and Developer</span></button>`;
}
window.renderShelfdOwnerBadgeHTML = renderShelfdOwnerBadgeHTML;
...
function normalizeVisibleMyListStatusTab(tab = activeTab, section = activeSection) {
  if (section === 'games' && (tab === 'live' || tab === 'competitive' || tab === 'party')) return 'watching';
...
function getMyListStatusLabel(status = '', section = activeSection) {
  ...
  if (status === 'party') return 'Party Games';
...
function getMyListStatusButtonConfigs(section = activeSection) {
  const statuses = section === 'games'
    ? ['watching', 'live', 'competitive', 'party', 'planned', 'watched', 'wishlist']
