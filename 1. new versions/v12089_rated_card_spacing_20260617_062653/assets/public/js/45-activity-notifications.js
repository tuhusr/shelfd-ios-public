...
const SHELFD_NOTIFICATIONS_VALID_TYPES = ['activity_like', 'activity_comment', 'comment_mention', 'friend_review_posted', 'friend_highlight_posted', 'friend_request', 'friend_accept', 'shared_watch_request'];
...
function buildShelfdNotificationDocId(options = {}) {
  ...
  if (type === 'comment_mention') {
    const comment = sanitizeShelfdNotifIdPart(options.targetCommentId || options.actorUid || '');
    return `comment_mention:${activity}:${comment}`;
  }
...
/* v11.978: remove a notification entirely — drop it from the in-memory list +
   the DOM immediately (so Deny/Delete clears the whole card, not just its
   buttons), then delete the Firestore doc so it never re-renders. */
async function deleteActivityNotification(notificationId = '') {
  const id = String(notificationId || '').trim();
  if (!id) return;
  activityNotificationsList = activityNotificationsList.filter(n => String(n.notificationId || '') !== id);
  try {
    const sel = (window.CSS && CSS.escape) ? CSS.escape(id) : id;
    document.querySelectorAll(`.shelfd-notif[data-notification-id="${sel}"]`).forEach(el => el.remove());
  } catch (_) {}
  try { updateShelfdNotificationsUnreadBadge(); } catch (_) {}
  if (!currentUser || !db) return;
  try {
    await db.collection('notifications').doc(currentUser.uid).collection('items').doc(id).delete();
  } catch (error) {
    console.warn('[shelfd notifications] delete failed:', error && error.message ? error.message : error);
  }
}
window.deleteActivityNotification = deleteActivityNotification;

/* v11.978: delete the friend_request notification from a given actor (used when a
   request is declined from the shelf banner, which only knows the uid). Looks the
   card up in the live list and falls back to the deterministic doc id. */
function deleteFriendRequestNotificationForActor(actorUid = '') {
  const u = String(actorUid || '').trim();
  if (!u) return;
  const match = activityNotificationsList.find(n =>
    n && String(n.type || '') === 'friend_request' && String(n.actorUid || '') === u);
  const id = match ? String(match.notificationId || '') : '';
  deleteActivityNotification(id || ('friend_request:' + u));
}
window.deleteFriendRequestNotificationForActor = deleteFriendRequestNotificationForActor;
...
function getShelfdNotificationPushBody(notification = {}) {
  ...
  if (type === 'comment_mention') return snippet ? `Tagged you in a comment: ${snippet}` : 'Tagged you in a comment';
  if (type === 'friend_review_posted') return `Posted a review${ofMedia}`;
  if (type === 'friend_highlight_posted') return mediaTitle ? `Posted a gaming clip\n${mediaTitle}` : 'Posted a gaming clip';
...
function getShelfdNotificationActionLine(notification = {}) {
  ...
  if (type === 'comment_mention') return 'Tagged you in a comment';
  if (type === 'friend_review_posted') return withMedia('Posted a review');
  if (type === 'friend_highlight_posted') return withMedia('Posted a gaming clip');
  if (type === 'friend_request' && targetKind === 'creator_auto_add') return 'Added you as a friend';
  if (type === 'friend_request') return 'requested to add you as a friend';
  if (type === 'friend_accept') return 'Accepted your friend request';
  if (type === 'shared_watch_request') {
...
async function handleShelfdNotificationFriendRequest(actorUid = '', action = 'accept', notificationId = '') {
  ...
  /* v11.978: Deny removes the ENTIRE card (and deletes the notification so it
     never comes back) — not just its buttons. Accept keeps the card and flips its
     slot to the follow-back button (Follow → Requested → Following). */
  if (action === 'deny') {
    deleteActivityNotification(notificationId || ('friend_request:' + uid));
    return;
  }
