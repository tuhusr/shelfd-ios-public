...
(function() {
  ...
  let activeDmThreadDesiredId = '';
  let activeDmThreadHeartbeatTimer = null;
  const ACTIVE_DM_THREAD_HEARTBEAT_MS = 12000;
  ...
  async function postRegisterToWorker(uid, token) {
    try {
      ...
      syncActiveDmThreadFromWindow('token-registered');
    } catch (e) {
  ...
  function getCurrentPushToken() {
    return String(lastTokenSeen || pendingTokenAwaitingUid || '').trim();
  }

  function getActiveDmThreadFromWindow() {
    try {
      if (typeof window.__shelfdGetVisibleActiveDmThreadId === 'function') {
        return String(window.__shelfdGetVisibleActiveDmThreadId() || '').trim();
      }
    } catch (_) {}
    try {
      return String(window.__shelfdActiveDmThreadId || '').trim();
    } catch (_) {
      return '';
    }
  }

  function stopActiveDmThreadHeartbeat() {
    if (activeDmThreadHeartbeatTimer) {
      clearInterval(activeDmThreadHeartbeatTimer);
      activeDmThreadHeartbeatTimer = null;
    }
  }

  function startActiveDmThreadHeartbeat() {
    stopActiveDmThreadHeartbeat();
    if (!activeDmThreadDesiredId) return;
    activeDmThreadHeartbeatTimer = setInterval(() => {
      const freshThreadId = getActiveDmThreadFromWindow();
      if (!freshThreadId) {
        setActiveDmThreadForPush('', 'heartbeat-no-active-thread');
        return;
      }
      postActiveDmThreadToWorker(freshThreadId, 'heartbeat', false);
    }, ACTIVE_DM_THREAD_HEARTBEAT_MS);
  }

  async function postActiveDmThreadToWorker(threadId, reason, preferBeacon) {
    ...
    try {
      if (!activeThreadId && preferBeacon && navigator && typeof navigator.sendBeacon === 'function') {
        const blob = new Blob([body], { type: 'application/json' });
        if (navigator.sendBeacon('/api/push/active-dm-thread', blob)) return true;
      }
    } catch (_) {}
    try {
      const r = await fetch('/api/push/active-dm-thread', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'omit',
        cache: 'no-store',
        keepalive: !activeThreadId,
        body
      });
      const j = await r.json().catch(() => ({}));
      LOG('active DM thread state reason=' + String(reason || 'unknown') + ' active=' + (activeThreadId || '(none)') + ' status=' + r.status, j);
      return !!(j && j.ok);
    } catch (e) {
      WARN('/api/push/active-dm-thread failed:', e && e.message ? e.message : e);
      return false;
    }
  }

  function setActiveDmThreadForPush(threadId, reason) {
    const activeThreadId = String(threadId || '').trim();
    activeDmThreadDesiredId = activeThreadId;
    try {
      window.__shelfdActiveDmThreadId = activeThreadId;
      window.__shelfdActiveDmThreadSyncedAtMs = Date.now();
    } catch (_) {}
    if (activeThreadId) startActiveDmThreadHeartbeat();
    else stopActiveDmThreadHeartbeat();
    postActiveDmThreadToWorker(activeThreadId, reason || (activeThreadId ? 'active-thread' : 'clear-active-thread'), !activeThreadId);
  }

  function syncActiveDmThreadFromWindow(reason) {
    setActiveDmThreadForPush(getActiveDmThreadFromWindow(), reason || 'sync-active-thread');
  }
    ...
    Push.addListener('registration', async (event) => {
      try {
        ...
        if (token === lastTokenSeen) {
          LOG('token unchanged from last seen, skipping POST');
          syncActiveDmThreadFromWindow('token-unchanged');
          return;
        }
        lastTokenSeen = token;
        const uid = resolveCurrentUid();
  ...
  function start() {
    const wireAuthHook = () => {
      ...
      firebase.auth().onAuthStateChanged(async (user) => {
        if (!user) {
          setActiveDmThreadForPush('', 'auth-sign-out');
          registeredUid = '';
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
      setActiveDmThreadForPush('', 'visibility-hidden');
      return;
    }
    syncActiveDmThreadFromWindow('visibility-visible');
    ...
    try {
      ...
      App.addListener('appStateChange', (event) => {
        if (event && event.isActive === false) {
          setActiveDmThreadForPush('', 'app-background');
          return;
        }
        if (event && event.isActive) {
          syncActiveDmThreadFromWindow('app-foreground');
  ...
  window.__shelfdPush = {
    ...
    getPlugin,
    available: true,
    setActiveDmThread: setActiveDmThreadForPush,
    syncActiveDmThread: syncActiveDmThreadFromWindow,
    clearActiveDmThread: (reason) => setActiveDmThreadForPush('', reason || 'manual-clear'),
    state: () => ({ registeredUid, lastTokenTail: String(lastTokenSeen).slice(-6), listenersBound, isRegistering, pluginMissingRetryCount, activeDmThreadId: activeDmThreadDesiredId }),
    nativeDiagnostic,
    diagnose: async function() {
      const uid = resolveCurrentUid();
      const r = await fetch('/api/push/diagnose?uid=' + encodeURIComponent(uid), { cache: 'no-store' });
  ...
  syncActiveDmThreadFromWindow('push-helper-ready');
