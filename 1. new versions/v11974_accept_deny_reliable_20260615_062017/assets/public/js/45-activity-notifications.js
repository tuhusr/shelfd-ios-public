...
/* v11.974: action button(s) for a friend_request notification card. The card
   ITSELF is proof the actor requested me, so Accept/Deny is the DEFAULT — we no
   longer gate on incomingRequests (a cross-user array-contains query that lags on
   iOS, which is why the buttons were missing). We only show a different state
   when reliable, persisted MY-OWN-doc data says the request is already resolved:
     • we're mutual friends                       → Following (tap = unfollow sheet)
     • I accepted it (they now follow me)         → Follow / Requested (follow back)
     • I declined it                              → no buttons
     • otherwise (unresolved incoming request)    → Accept / Deny
...
function buildNotifFriendRequestActionsHTML(actorUid, notificationId) {
  ...
  /* I accepted their request (they follow me) → offer to follow them back, showing
     my live follow status toward them (Requested if I've sent one, else Follow). */
  ...
  /* Default: an unresolved incoming follow request → Accept / Deny. */
...
/* v11.974: shared "does this user have an unresolved follow request to me?"
   determiner for the shelf Confirm/Delete banner. Uses the SAME reliable signals
   — a friend_request notification from them (or the derived incoming list) gated
   by my own persisted accepted/rejected/friends state — so the shelf banner can't
   silently miss a request just because the derived query lagged. */
function shelfdActorRequestedMeUnresolved(uid) {
  const u = String(uid || '').trim();
  if (!u || !currentUser || u === currentUser.uid) return false;
  if (notifFriendsList().includes(u)) return false;
