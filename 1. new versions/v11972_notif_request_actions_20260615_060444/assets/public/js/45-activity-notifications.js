...
/* v11.972: action button(s) for a friend_request notification card, reflecting
   the live two-way-follow relationship with the actor:
     • they have a pending request to me        → Accept / Deny
     • I accepted (now in my follow-back queue)  → Follow  (follow them back)
     • I've requested them, awaiting their accept → Requested (tap to cancel)
     • we're mutual friends                       → Following (tap = unfollow sheet)
     • declined / relationship not loaded yet     → no buttons
...
function buildNotifFriendRequestActionsHTML(actorUid, notificationId) {
  ...
  if (notifIncomingRequestsList().includes(u)) {
  ...
  if (notifPendingFollowBackList().includes(u)) {
