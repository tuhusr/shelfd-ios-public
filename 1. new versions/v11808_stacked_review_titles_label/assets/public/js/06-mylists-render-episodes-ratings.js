...
function renderMyListGameProfileTrackerShortcut(item = {}) {
  const href = getMyListGameProfileTrackerHomeUrl(item);
  if (!href) return '';
  const title = item.title || item.name || 'this game';
  return `
    <a class="mylist-game-profile-tracker-shortcut" href="${escAttr(href)}" target="_blank" rel="noopener" onclick="event.stopPropagation()" aria-label="Open ${escAttr(title)} on Tracker.gg">
      <span class="mylist-game-profile-tracker-mark" aria-hidden="true">T</span>
      <svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path d="M7 5h8v8"></path><path d="M15 5 6 14"></path></svg>
    </a>
  `;
}
...
function openMyListGameProfilePage(itemId = '', options = {}) {
  ...
  const trackerHomeUrl = (typeof getMyListGameProfileTrackerHomeUrl === 'function'
    ...
    : '') || 'https://tracker.gg';
  const trackerShortcutHtml = renderMyListGameProfileTrackerShortcut(item);
        ...
        <section class="mylist-game-profile-hero"${poster ? ` style="--game-profile-cover:url('${escAttr(poster)}')"` : ''}>
          ...
          <div class="mylist-game-profile-hero-card">
            ...
            <div class="mylist-game-profile-identity">
              ${isCompetitive ? `<div class="mylist-game-profile-eyebrow-row">
...
function renderCard(item, isDraggable) {
  ...
  const isGamePlayingProfileCard = isGameCard
    && !isGamesWishlistCard
    && ['watching', 'live', 'competitive'].includes(itemStatus.toLowerCase());
    ...
    <div class="discover-media-body discover-media-body-cinema">
      ${(facts.length || credits.length || directTrailer) ? `<div class="discover-media-detail-grid${directTrailer ? ' has-trailer' : ''}">
        ${(facts.length || credits.length) ? `<div class="discover-media-detail-stack">
          ${facts.length ? `<div class="discover-media-facts">${facts.map(fact => `<div class="${getGameMediaFactClass(fact)}"><strong>${escHtml(fact.value)}</strong><span>${escHtml(fact.label)}</span></div>`).join('')}</div>` : ''}
