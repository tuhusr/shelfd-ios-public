...
(function initScreenListDeployAutoRefresh() {
  ...
  /* v11.699: RESUME path — fetch the live build version (uncached) and, if a
     newer one is live and the screen is safe, reload to pick up the new assets.
     Called only when the app returns from the background (visibilitychange /
     pageshow-persisted / Capacitor appStateChange), so the user is RETURNING to
     the app — never mid-interaction. This is what restores reliable update
     delivery to the native WKWebView without a manual force-quit. */
  async function checkAndMaybeAutoReloadOnResume() {
    try {
      const res = await fetch('/index.html?screenlist_update_check=' + Date.now(), {
        cache: 'no-store',
        headers: { 'Cache-Control': 'no-store' }
      });
      if (!res.ok) return;
      const nextVersion = readVersionFromHtml(await res.text());
      if (!nextVersion || nextVersion === currentVersion) return;
      if (!shouldSurfaceUpdateNotice(nextVersion)) return;   // dev-preview builds gated to the creator account
      if (isLiveUpdateReloadUnsafe()) return;                // protect drafts — retry on the next clean resume
      applyLiveUpdateReload(nextVersion);
    } catch (_) {}
  }
