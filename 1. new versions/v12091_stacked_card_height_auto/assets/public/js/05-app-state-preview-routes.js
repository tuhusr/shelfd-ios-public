...
function normalizeSharedMediaPayload(payload = {}) {
  ...
  /* v12.048: gaming highlight shares. The url is a myshelfd.com/post/{id} link;
     preserve it as-is so the DM card can open the feed post page on tap. */
  if (String(payload.kind || '').toLowerCase() === 'highlight') {
    const postUrl = String(payload.url || '').trim();
    if (!/^https?:\/\//i.test(postUrl)) return null;
    return {
      kind: 'highlight',
      id: String(payload.id || '').trim() || postUrl,
      title: String(payload.title || '').trim() || 'Gaming Highlight',
...
async function openSharedMediaProfileRoute(route = parseScreenListMediaRoute(), opts = {}) {
  ...
  try {
    if (route.kind === 'game') {
      ...
      const sharedGameIdStr = String(route.id || '');
      const sharedIgdbMatch = sharedGameIdStr.match(/^igdb:(\d+)$/i);
      /* v11.991: seed the title (so resolveRawgIdForGameSeed can find the RAWG
         ...
         match for the title).
         v11.998: ALSO seed a stable identity (id + gameIdentityKey, plus igdbId/
         source/sourceId for igdb: ids). Without it the FPMP "+" add button rendered
         with an EMPTY data-discover-id when RAWG didn't resolve the game, so the
         add tap silently no-op'd (the click handler bails on !id) — the reported
         "clicking + does nothing" bug for shared games. */
      const gameSeed = {
        id: sharedGameIdStr,
        gameIdentityKey: sharedGameIdStr,
        rawgId: numericRawg,
        title: sharedGameTitle,
        name: sharedGameTitle
      };
      if (/^https?:\/\//i.test(sharedGamePoster)) {
        ...
        gameSeed.igdbCoverUrl = sharedGamePoster;
      }
      if (sharedIgdbMatch) {
        gameSeed.igdbId = sharedIgdbMatch[1];
        gameSeed.sourceId = sharedIgdbMatch[1];
        gameSeed.source = 'igdb';
      }
