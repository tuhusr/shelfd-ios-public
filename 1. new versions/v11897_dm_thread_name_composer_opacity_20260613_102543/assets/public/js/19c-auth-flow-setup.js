...
(function() {
  ...
  /* v11.070: seed a username suggestion for Sign in with Apple users from the
     Apple-provided name so the username step doesn't start blank. We never use
     the email local-part (Hide-My-Email relays are random), and leave it empty
     if Apple withheld the name — the user just types one. */
  function suggestUsernameFromAppleUser(user) {
    try {
      return sanitizeUsernameInput(String((user && user.displayName) || '').replace(/\s+/g, ''));
    } catch (_) { return ''; }
  }
  ...
  /* ───────── Slide-up panel open/close ─────────
     ...
     v834 — the REAL fix:
     ────────────────────
     When ANY auth panel opens we now hide `#login-screen` and
     `#app-container` outright via `display: none`. The verify and
     setup panels already did this; sign-in / sign-up / reset did
     not — and that's exactly what's been blocking the keyboard on
     installed PWA standalone (iOS).

     Cause: the landing screen contains the Google Identity Services
     iframe (#gis-signin-target → an iframe loaded from
     accounts.google.com). In iOS WKWebView standalone mode, GIS's
     third-party iframe creates its own touch-hit-testing region.
     Even when the auth panel is rendered visually on top with
     z-index: 5050, iOS routes taps on the input rectangles to the
     iframe's hit region (because the WebKit hit-tester walks the
     compositor tree, not the z-index, for cross-origin iframes).
     Buttons still work because their `onclick` handlers fire from
     the JS event system, but <input> focus is iOS-native and gets
     swallowed by the iframe.

     Hiding the entire landing screen guarantees the iframe is
     removed from the hit-test tree while the auth panel is open.
     `panelOwnedBackdrops` tracks which elements we hid so we can
     reliably restore them on close. We also track which one was
     visible originally (login-screen vs app-container) so we never
     reveal the wrong one — e.g. opening Reset while signed in must
     restore app-container, not login-screen. */
  let panelOwnedBackdrops = null;

  function hideBackdropsForAuthPanel() {
    if (panelOwnedBackdrops) return; // already hidden by an earlier panel in the stack
    const loginEl = document.getElementById('login-screen');
    const appEl   = document.getElementById('app-container');
    const snapshot = {
      login: loginEl ? loginEl.style.display : null,
      app:   appEl   ? appEl.style.display   : null
    };
    if (loginEl) loginEl.style.display = 'none';
    if (appEl)   appEl.style.display   = 'none';
    panelOwnedBackdrops = snapshot;
  }

  function restoreBackdropsAfterAuthPanel() {
    if (!panelOwnedBackdrops) return;
    const loginEl = document.getElementById('login-screen');
    const appEl   = document.getElementById('app-container');
    if (loginEl) loginEl.style.display = panelOwnedBackdrops.login || '';
    if (appEl)   appEl.style.display   = panelOwnedBackdrops.app   || '';
    panelOwnedBackdrops = null;
  }
  ...
  function closePanel(id) {
    ...
    if (!OPEN_PANEL_STACK.length) {
      ...
      /* v834: restore whichever shell was visible before any auth panel opened. */
      restoreBackdropsAfterAuthPanel();
    }
  }
  ...
  function resetShelfdSetupDraftFields(defaults = {}) {
    ...
    const usernameDefault = String(defaults.usernameDefault || '').trim().replace(/^@+/, '').toLowerCase();
    const displayNameDefault = String(defaults.displayNameDefault || '').trim();

    if (usernameField) {
      usernameField.value = usernameDefault;
      try { usernameField.defaultValue = usernameDefault; } catch (_) {}
    }
    if (displayNameField) {
      displayNameField.value = displayNameDefault;
      try { displayNameField.defaultValue = displayNameDefault; } catch (_) {}
    }
    if (usernameHint) {
      usernameHint.textContent = '1-30 characters. Letters, numbers, periods, and underscores only.';
      usernameHint.dataset.kind = '';
    }
  ...
  function isTextEntryActiveInside(panel) {
    const active = document.activeElement;
    if (!active || !panel || !panel.contains(active)) return false;
    return /^(INPUT|TEXTAREA|SELECT)$/i.test(active.tagName || '');
  }

  function stabilizeAuthPanelInitialFrame(panelOrId) {
    const panel = typeof panelOrId === 'string' ? $(panelOrId) : panelOrId;
    if (!panel) return;
    try { document.activeElement?.blur?.(); } catch (_) {}
    const resetIfSafe = () => {
      if (isTextEntryActiveInside(panel)) return;
      resetAuthPanelScroll(panel);
    };
    resetIfSafe();
    requestAnimationFrame(resetIfSafe);
    setTimeout(resetIfSafe, 80);
    setTimeout(resetIfSafe, 240);

    const vv = window.visualViewport;
    if (!vv || typeof vv.addEventListener !== 'function') return;
    const onViewportSettle = () => resetIfSafe();
    try { vv.addEventListener('resize', onViewportSettle, { passive: true }); } catch (_) {}
    setTimeout(() => {
      try { vv.removeEventListener('resize', onViewportSettle); } catch (_) {}
    }, 700);
  }
  ...
  /* ───────── Username live hint (v816: setup step 1 field) ─────────
     Pure local format check: letters/numbers/periods/underscores, 1-30 chars.
  ...
  function bindUsernameHint() {
    ...
    field.addEventListener('input', () => {
      ...
      if (!v) {
        hint.textContent = '1-30 characters. Letters, numbers, periods, and underscores only.';
        hint.dataset.kind = '';
  ...
  function setBusy(which, state) {
    ...
    const submitId =
      ...
      which === 'setupFinish'    ? 'shelfd-setup-finish-btn' :
                                   null;
  ...
  function openShelfdSignUpPage() {
    ...
    /* Do not auto-focus the email field here. On iOS/TestFlight, opening the
       keyboard during the slide-in can resize the visual viewport before the
       fixed panel settles, leaving the Create Account form vertically out of
       frame. The user can tap the field once the page is stable. */
    stabilizeAuthPanelInitialFrame('shelfd-signup-page');
  }
  ...
  function openShelfdVerifyPage(opts) {
    ...
    /* v834: openPanel now handles backdrop hiding via the shared
       hideBackdropsForAuthPanel helper (which also snapshots the prior
       display state for restore on close). We no longer manually hide
       login + app shell here. */
  ...
  function openShelfdSetupPage(step = 1, opts = {}) {
    const settings = opts || {};
    /* v834: openPanel handles backdrop hiding via the shared
       hideBackdropsForAuthPanel helper (snapshots prior display state
       for accurate restore on close). No manual hiding here. */
    ...
    /* v11.022: always reset onboarding draft fields on open. The previous
       flow only filled defaults when the DOM input was empty, so after a
       delete-account return to landing, iOS could keep the old username/name
       in the live input nodes and prefill the next Apple onboarding. Defaults
       are only supplied when resuming a real unfinished Firestore setup doc. */
    resetShelfdSetupDraftFields(settings);

    setSetupStep(step);
    openPanel('shelfd-setup-page');
    resetAuthPanelScroll(root);
    requestAnimationFrame(() => resetAuthPanelScroll(root));
    setTimeout(() => resetShelfdSetupDraftFields(settings), 40);
    setTimeout(() => {
      resetShelfdSetupDraftFields(settings);
      resetAuthPanelScroll(root);
  ...
  async function handleShelfdUsernameSetupNext() {
    ...
    if (!isValidUsername(username)) {
      setBanner($('shelfd-setup-username-error'), 'Username must be 1-30 characters and can only use letters, numbers, periods, and underscores. No spaces or special characters.', 'error');
      return;
    }
    if (!validateUsernameModeration(username).allowed) {
      setBanner($('shelfd-setup-username-error'), usernameModerationMessage(), 'error');
      return;
    }
    ...
    /* v11.070 — Sign in with Apple compliance (App Store Guideline 4).
       Apple already provides the user's name via the Authentication Services
       framework, so Apple users must NOT be asked to type it again. For Apple
       users we seed the display name from the Apple name (falling back to the
       username only if Apple withheld it) and SKIP the display-name step
       entirely — they go username → photo → welcome. The email comes from the
       Apple credential (incl. Hide-My-Email relays) and is stored below; it's
       never requested. Email-signup users keep the full 4-step flow. */
    const appleUser = isAppleUser(user);
    const appleName = String((user.displayName || '')).trim();
    const initialDisplayName = appleUser ? (appleName || username) : username;
    const initialDisplayNameLower = initialDisplayName.toLowerCase();
    const nextSetupStep = appleUser ? 3 : 2;
    ...
    try {
      await userRef.set({
        ...
        nameLower: initialDisplayNameLower,
        customName: initialDisplayName,
        customNameLower: initialDisplayNameLower,
        displayName: initialDisplayName,
        /* v813 — body.true-dark-mode → modern Shelfd UI. */
        themeMode: 'true-dark',
        onboardingComplete: false,
        onboardingStep: nextSetupStep,
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
    ...
    /* v11.070: seed the auth + local display name. For Apple users this is the
       Apple-provided name (so the name step can be skipped); for everyone else
       it's the username placeholder they'll replace on the display-name step. */
    try { await user.updateProfile({ displayName: initialDisplayName }); } catch (_) {}
    try {
    ...
    setBusy('setupUsername', false);
    /* v11.070: Apple users jump straight to the photo step (3); the display-name
       entry step (2) is skipped because Apple already supplied the name. */
  ...
  async function shelfdSetupFinish() {
    ...
    if (typeof firebase === 'undefined' || !firebase.auth) {
      if (typeof showToast === 'function') showToast('Service is loading. Try again in a moment.', { durationMs: 3500 });
      return;
    }
    const user = firebase.auth().currentUser;
    if (!user) {
      if (typeof showToast === 'function') showToast('Please sign in again.', { durationMs: 3500 });
      try { closeShelfdSetupPage(); } catch (_) {}
      return;
    }
  ...
  /* ════════════════════════════════════════════════════════════════════════
     ...
     v816 routing matrix:
       ...
       Google user, no user doc → setup step 1 (v11.875: full onboarding)
  ...
  async function authOnboardingGate(user) {
    ...
    try {
      ...
      if (!data) {
        if (isEmailPasswordUser(user) || isAppleUser(user) || isGoogleUser(user)) {
          /* v11.070: Apple users start at the username step with a suggestion
             seeded from their Apple name (the display-name step is skipped in
             the username handler per Sign in with Apple design rules).
             v11.875: Google users ALSO run the full onboarding now — they
             enter at the username step with a suggestion seeded from their
             Google name, then continue through the normal display-name →
             photo → welcome steps (no Apple-style display-name skip). */
          const setupOpts = (isAppleUser(user) || isGoogleUser(user))
            ? { usernameDefault: suggestUsernameFromAppleUser(user) }
            : {};
          openShelfdSetupPage(1, setupOpts);
          return true;
        }
        return false;
      }
      ...
      if (data.onboardingComplete === false) {
        let step = (data.onboardingStep === 2 || data.onboardingStep === 3 || data.onboardingStep === 4) ? data.onboardingStep : 1;
        /* v11.070: Apple users never see the display-name entry step — if a
           legacy / mid-flow doc lands them on step 2, jump straight to photo. */
        if (isAppleUser(user) && step === 2) step = 3;
  ...
  /* ════════════════════════════════════════════════════════════════════════
     v835 — PWA-safe inline sign-in form (lives directly on landing page,
     bypasses every slide-up panel mechanism). The iOS PWA WKWebView bug
     that suppressed the keyboard on the slide-up panels cannot affect
     this form because it sits in the normal document flow with no
  ...
  let inlineSigninBusy = false;
  async function handleShelfdLandingInlineSignin(e) {
    if (e && typeof e.preventDefault === 'function') e.preventDefault();
    if (inlineSigninBusy) return false;
    if (typeof firebase === 'undefined' || !firebase.auth) {
      setBanner($('shelfd-landing-inline-error'), 'Sign-in service is loading — try again in a moment.', 'error');
      return false;
    }
    const emailEl = $('shelfd-landing-inline-email');
    const pwEl    = $('shelfd-landing-inline-password');
    const submit  = $('shelfd-landing-inline-submit');
    const email   = String(emailEl && emailEl.value || '').trim();
    const password = String(pwEl && pwEl.value || '');
    if (!isValidEmail(email)) {
      setBanner($('shelfd-landing-inline-error'), 'Please enter a valid email address.', 'error');
      return false;
    }
    if (!password) {
      setBanner($('shelfd-landing-inline-error'), 'Please enter your password.', 'error');
      return false;
    }
    inlineSigninBusy = true;
    ...
    try {
      ...
      console.warn('[shelfd-auth] inline sign-in failed:', err && err.code);
      setBanner($('shelfd-landing-inline-error'), mapAuthError(err, 'Could not sign in. Try again.'), 'error');
      inlineSigninBusy = false;
  ...
  /* ════════════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════════════ */
  window.toggleShelfdLandingInlineSignin = toggleShelfdLandingInlineSignin;
  window.handleShelfdLandingInlineSignin = handleShelfdLandingInlineSignin;
  ...
  window.resetShelfdSetupDraftFieldsForAccountDeletion = () => resetShelfdSetupDraftFields({});
  window.closeShelfdAuthPanelsForAccountDeletion = closeAllPanels;
