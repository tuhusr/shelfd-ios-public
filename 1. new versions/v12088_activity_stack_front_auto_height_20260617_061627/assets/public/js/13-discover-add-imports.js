...
/* v12.021: additive Steam wishlist sync. Pulls the linked account's Steam
   wishlist and ADDS any game that isn't already in the games library (matched
   by Steam App ID, then title) as a Shelfd 'wishlist' status item. Never
   overwrites or removes anything — a game already in any status is left alone,
   so owned/played/manually-added games are never duplicated or downgraded. */
async function mergeSteamWishlistIntoOwnGames(steamId) {
  const games = getOwnGamesArrayForSteamSync();
  if (!games || !steamId) return { added: 0, fetched: 0 };
  let payload;
  try {
    payload = await fetchSteamProxyJson('/api/steam/wishlist', { steamId });
  } catch (e) {
    console.warn('Steam wishlist fetch failed:', e);
    return { added: 0, fetched: 0 };
  }
  const items = Array.isArray(payload.items) ? payload.items : [];
  // Diagnostic snapshot so a sync that adds nothing can be told apart from a
  // private/empty wishlist when debugging on-device.
  try {
    window.__shelfdSteamWishlistDiag = {
      fetched: items.length,
      sample: items.slice(0, 5).map(i => ({ appId: i.appId, name: i.name })),
      at: new Date().toISOString()
    };
  } catch (_) {}
  if (!items.length) return { added: 0, fetched: 0 };

  // 1) Drop anything already in the library by Steam App ID — no network, and
  //    on a resync this skips everything already imported. Match by App ID ONLY
  //    (a blank-title compare would false-match library games with no title).
  const inLibraryAppIds = new Set(
    games.map(g => String((g && (g.steamAppId || g.appId)) || '').trim()).filter(Boolean)
  );
  const MAX_NEW_WISHLIST_PER_SYNC = 300;
  const ordered = items
    .slice()
    .sort((a, b) => Number(b.dateAdded || 0) - Number(a.dateAdded || 0)) // newest-wishlisted first
    .filter(w => {
      const id = String(w.appId || w.appid || '').trim();
      return id && !inLibraryAppIds.has(id);
    });
  const candidates = ordered.slice(0, MAX_NEW_WISHLIST_PER_SYNC);
  if (ordered.length > candidates.length) {
    console.warn(`Steam wishlist: ${ordered.length - candidates.length} extra new game(s) will import on the next resync (per-sync cap of ${MAX_NEW_WISHLIST_PER_SYNC}).`);
  }
  if (!candidates.length) return { added: 0, fetched: items.length };

  // 2) Hydrate the name (the keyed Steam API returns none) and build each
  //    candidate, in parallel with bounded concurrency so 50+ items don't run
  //    as 50 slow sequential round-trips. Building is side-effect-free here —
  //    results are pushed in step 3 to avoid races on the shared games array.
  const built = [];
  let idx = 0;
  const CONCURRENCY = 5;
  async function worker() {
    while (idx < candidates.length) {
      ...
      const appId = String(w.appId || w.appid || '').trim();
      if (!appId) continue;
      let title = String(w.name || '').trim();
      if (!title) {
        try {
          const d = await fetchSteamProxyJson('/api/steam/app-details', { appId });
          title = String(d?.data?.name || d?.name || '').trim();
        } catch (_) {}
      }
      if (!title) continue;
      try {
        const made = await buildSteamImportItems({
          source: 'steam', title, year: '', status: 'wishlist', rating: 0, typeHint: 'game',
          steamId: String(steamId), steamAppId: appId,
          steamUrl: `https://store.steampowered.com/app/${appId}`,
          playtimeMinutes: 0, playtimeHours: 0, lastPlayedAt: '', cover: '', igdbCoverUrl: ''
        });
        (Array.isArray(made) ? made : []).forEach(it => { if (it) built.push({ it, appId, title }); });
      } catch (e) { console.warn('Steam wishlist build failed:', title, e); }
    }
  }
  await Promise.all(Array.from({ length: Math.min(CONCURRENCY, candidates.length) }, worker));

  // 3) Single-threaded dedup + push. Re-checking against games as we go catches
  //    title-level duplicates (manual adds) and any intra-wishlist repeats.
  let added = 0;
  for (const { it, appId, title } of built) {
    if (findExistingSteamGameInLibrary(games, appId, title)) continue;
    it.status = 'wishlist'; // force wishlist regardless of any builder default
    it.importSource = it.importSource || 'steam';
    it.importSourceLabel = it.importSourceLabel || 'Steam';
    it.importedAt = it.importedAt || new Date().toISOString();
    it.steamWishlist = true;
    games.push(it);
    added++;
  }
  try { if (window.__shelfdSteamWishlistDiag) window.__shelfdSteamWishlistDiag.added = added; } catch (_) {}
...
async function quickSyncSteamLibrary() {
  ...
  try {
    ...
    // v12.021: additively pull the Steam wishlist into the Shelfd games Wishlist.
    let wishlistAdded = 0;
    let wishlistFetched = 0;
    try {
      const wl = await mergeSteamWishlistIntoOwnGames(connection.steamId);
      wishlistAdded = Number(wl?.added || 0);
      wishlistFetched = Number(wl?.fetched || 0);
    } catch (e) { console.warn('Steam wishlist sync failed:', e); }
    try {
      await saveSteamConnectionPatch({
        steamId: connection.steamId,
        personaName: payload.player?.personaName || connection.personaName,
    ...
    if (typeof showToast === 'function') {
      ...
      if (wishlistAdded) bits.push(`${wishlistAdded} wishlist`);
      if (updated) bits.push(`${updated} updated`);
      if (bits.length) {
        showToast(`Steam synced — ${bits.join(', ')}. Refreshing achievements…`);
      } else if (wishlistFetched === 0) {
        // Nothing new in the library AND the wishlist came back empty — almost
        // always the "Game details" privacy setting (wishlist not public).
        showToast('Steam library up to date. Wishlist came back empty — set Steam privacy "Game details" to Public to sync it.');
      } else {
        showToast('Steam library and wishlist are up to date. Refreshing achievements…');
      }
    }
...
function getCompactEpisodeStats(item = {}) {
  ...
  if (episodes.length) {
    /* watched = only episodes that have actually aired AND are marked watched.
       total = all episodes in the season (including unaired) so the display
       reads "9/10" when 9 of 10 are out and watched, not "9/9". */
