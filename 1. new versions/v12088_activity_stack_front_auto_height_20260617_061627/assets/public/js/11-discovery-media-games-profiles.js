...
function getDiscoverSocialStatusLabel(status, section) {
  ...
  if (section === 'games') {
    ...
    if (normalized === 'party') return 'Party Games';
  }
...
function hydrateDiscoverHeroTrailerPreview(overlay = document.getElementById('discover-media-profile')) {
  ...
  iframe.src = `https://www.youtube.com/embed/${encodeURIComponent(trailerKey)}?${params.toString()}`;   // v11.968: youtube.com (not nocookie) so the embed carries trust cookies → no bot wall
...
// v11.951: pull a game's "more like this" from IGDB's curated similar_games list
// (via /api/igdb/game-info), mapped into the shared More Like This card shape.
// IGDB total_rating (0-100) is placed on raw.metacritic so the existing card
// rating logic renders it on a 5-star scale. Cards are tagged source 'igdb' so
// the tap handler opens them by seed (title + IGDB cover), not as a RAWG id.
async function fetchIgdbSimilarGamesForMoreLikeThis(context = {}, details = {}) {
  const title = String(context.title || details.title || details.name || '').trim();
  if (!title) return [];
  const params = new URLSearchParams({ title });
  const year = String(context.year || details.released || details.year || '').slice(0, 4);
  if (year) params.set('year', year);
  params.set('iv', '3');
  const res = await fetch('/api/igdb/game-info?' + params.toString(), { cache: 'default' });
  if (!res.ok) return [];
  const data = await res.json();
  const similar = (data && data.ok && Array.isArray(data.similarGames)) ? data.similarGames : [];
  return similar.map(g => ({
    ...
    source: 'igdb',
    raw: {
      metacritic: Math.round(Number(g.rating || 0)) || 0,
...
async function fetchSourceFallbackMoreLikeThis(type, details = {}) {
  ...
  if (type === 'game') {
    /* v11.951: IGDB similar_games is the PRIMARY source for game More Like This.
       RAWG's free key hits a monthly request cap (returns {"error":"The monthly
       API limit reached"}), which made the RAWG-only query return nothing → the
       section never loaded. IGDB (Twitch) is not capped the same way and returns
       a curated similar-games list. RAWG stays as the fallback below. */
    try {
      const igdbSimilar = await fetchIgdbSimilarGamesForMoreLikeThis(context, details);
      if (igdbSimilar.length) recommendations = igdbSimilar;
    } catch (igdbError) {
      console.warn('IGDB more-like-this failed; falling back to RAWG.', igdbError);
    }
...
async function openDeepSeekMoreLikeThisProfile(event, button) {
  ...
  try {
    ...
    if (type === 'game') {
      // v11.951: IGDB-sourced similar games carry an IGDB id (NOT a RAWG id), so
      // open them by seed (title + IGDB cover) and let openGameMediaProfile resolve
      // the RAWG id by title — never pass the IGDB id as a rawgId.
      if (sourceKind === 'igdb') {
        const seed = { title, name: title, released: year ? `${year}-01-01` : '', year, background_image: sourceImage, cover: sourceImage, rawgId: '' };
        openGameMediaProfile(event, '', seed);
        return;
      }
      if (sourceId) {
...
function playDmInlineVideo(event, el) {
  ...
  try {
    ...
    const src = 'https://www.youtube.com/embed/' + encodeURIComponent(vid) + '?autoplay=1&playsinline=1&rel=0&modestbranding=1';   // v11.968: youtube.com (not nocookie) so the embed carries trust cookies → no bot wall
...
function openSharedHighlightFromDm(event, el) {
  if (event && event.preventDefault) event.preventDefault();
  if (event && event.stopPropagation) event.stopPropagation();
  var postUrl = '';
  try { postUrl = el && el.getAttribute ? (el.getAttribute('data-post-url') || el.getAttribute('href') || '') : ''; } catch (e) {}
  if (!postUrl) return false;
  try { if (typeof closeDirectMessagesPage === 'function') closeDirectMessagesPage(true); } catch (e) {}
  var route = typeof parseScreenListFeedPostRoute === 'function' ? parseScreenListFeedPostRoute(postUrl) : null;
  if (route && route.postId && typeof openSharedFeedPostRoute === 'function') {
    openSharedFeedPostRoute(route);
  }
  return false;
}
window.openSharedHighlightFromDm = openSharedHighlightFromDm;

function renderDirectMessageShareCard(message = {}) {
  const media = normalizeSharedMediaPayload(message.shareMedia);
  if (!media || !media.url) return '';
  /* v12.048: gaming highlight card — taps open the feed post page, not the news reader. */
  if (media.kind === 'highlight') {
    return `<a class="dm-shared-media-card dm-shared-highlight-card" href="${escAttr(media.url)}" data-post-url="${escAttr(media.url)}" onclick="return openSharedHighlightFromDm(event, this)" title="View gaming highlight">
      ${media.poster ? `<img src="${escAttr(media.poster)}" alt="" loading="lazy">` : ''}
      <span><strong>${escHtml(media.title || 'Gaming Highlight')}</strong><em>Gaming highlight</em></span>
    </a>`;
  }
  ...
  return `<section class="discover-media-page discover-person-page${movieTvDesktopSource ? ' discover-person-page-desktop-cinema' : ''}" role="dialog" aria-modal="true" aria-label="${escAttr(name)} details">
    ...
    <div class="discover-media-hero discover-person-hero" style="${photo ? `background-image:url('${escAttr(photo)}')` : ''}">
      ...
      <div class="discover-media-hero-content">
        ...
        <p class="discover-person-bio" data-person-bio tabindex="0" role="button" aria-expanded="false" onclick="this.classList.toggle('is-expanded'); this.setAttribute('aria-expanded', this.classList.contains('is-expanded') ? 'true' : 'false');">${escHtml(biography)}</p>
      </div>
    </div>
...
function shouldAlwaysShowDiscoverCategoryRatingStar(gridId = '') {
  const id = String(gridId || '').trim();
  if (id === 'discover-movie-anticipated-grid' || id === 'discover-tv-anticipated-grid') {
    return false;
  }
...
function openGameDiscoverAddSheet(rawgId, title, poster, seedOverride = null) {
  ...
  /* v11.957: the carried seed lock can be STALE relative to the game we actually
     built into selectedTmdb — e.g. a universal-search seed that guessed a RAWG id
     for an IGDB-only game (Empulse). If it doesn't match, drop it and rebuild the
     lock from selectedTmdb, otherwise pickGameDiscoverStatus throws a false
     "Game identity changed before saving" and the add silently fails. */
  let lock = seed.shelfdGameIdentityLock || null;
  if (lock && typeof isShelfdGameIdentityMatch === 'function' && !isShelfdGameIdentityMatch(lock, window.selectedTmdb)) lock = null;
  if (!lock && typeof createShelfdGameIdentityLock === 'function') lock = createShelfdGameIdentityLock(window.selectedTmdb, '2 discovery game result tapped');
    ...
    return `
      ...
      <div class="sas-body">
        ...
        <div class="sas-statuses">
          ...
          <button class="sas-status-btn" type="button" onclick="pickGameDiscoverStatus('party')">Party Games</button>
        </div>
      </div>`;
  }
  return `
  ...
  try {
    if (typeof submitModal === 'function') {
      ...
      if (result?.ok) {
        if (typeof playLibraryAddPopSound === 'function') playLibraryAddPopSound();
        if (typeof showToast === 'function') showToast(result.message || 'Added to your shelf!');
        /* v11.961: flip the open profile's (+) button to the green check
           immediately. The game add flow goes through submitModal (not the dock's
           addDiscoveryTitle), so nothing was updating the button — it only turned
           green after closing + reopening the profile. */
        try {
          const profileOverlay = document.getElementById('discover-media-profile');
          const addBtn = profileOverlay && profileOverlay.querySelector
            ? profileOverlay.querySelector('.discover-media-add-floating[data-discover-type="game"]')
            : null;
          if (addBtn && typeof markShelfLogMediaProfileButtonAdded === 'function') {
            markShelfLogMediaProfileButtonAdded(addBtn);
          }
        } catch (_) {}
      } else if (!result?.duplicate) {
