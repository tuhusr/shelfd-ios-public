...
function getScreenListActivityDestinationLabel(status = '', section = '') {
  ...
  if (section === 'music') {
    if (clean === 'planned') return 'Planned';
...
function buildScreenListActivityComposedSentence(eventType = '', item = {}, activity = {}) {
  ...
  function addedToDestinationParts(destinationStatus = status || 'planned') {
    ...
    const destination = String(getScreenListActivityDestinationLabel(destinationStatus, section) || 'library')
      .replace(/\s+/g, ' ')
      .trim()
      .toLowerCase();
    return {
      prefix: 'Added',
      actionTail: `the ${mediaType}`,
      verbContext: '',
      title,
      suffix: `to their ${destination}`,
