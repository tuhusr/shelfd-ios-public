...
(function () {
  ...
  function deepLinkFor(meta) {
    /* v11.676: a media-profile share (movie/tv/anime/game) carries its own canonical
       /media/{kind}/{id} deep link in meta.url — use it directly. Articles build a
       /article/{token} link from their raw source url. */
    if (meta && meta.shareKind === 'media') return String(meta.url || '');
  ...
  function openSheet(meta) {
    ...
    state.meta = {
      ...
      shareKind: meta.shareKind === 'media' ? 'media' : 'article',
      mediaKind: String(meta.mediaKind || ''),
      mediaId: String(meta.mediaId || '')
    };
    /* News-share taste is an ARTICLE ranking signal only — media-profile shares
       must NOT feed it (they have no place in the article feed algorithm). */
    if (state.meta.shareKind !== 'media') {
      try {
  ...
  function wireEvents(overlay) {
    ...
    if (native) native.addEventListener('click', function () {
      var meta = state.meta;
      closeSheet();
      if (meta.shareKind === 'media' && typeof window.shelfdShareMediaProfileNative === 'function') {
