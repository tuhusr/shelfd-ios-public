...
function buildDirectMessageListHTML(thread, threadId = activeDmThreadId) {
  ...
  return messages.length ? messages.map((message, idx) => {
    ...
    return `${dividerHtml}<div class="${rowClass}">${avatarHtml}<div class="dm-v2-bubble-stack">${stackInner}</div></div>${seenHtml}`;
...
function renderDirectMessageThread(threadId = activeDmThreadId) {
  ...
  const enterStyle = shouldEnter ? ' style="transform: translate3d(100vw, 0, 0); -webkit-transform: translate3d(100vw, 0, 0); transition: none; -webkit-transition: none; will-change: transform;"' : '';
