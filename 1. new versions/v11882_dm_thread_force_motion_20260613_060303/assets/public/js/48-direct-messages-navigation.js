...
function startDirectMessageThreadOpenAnimation(shell = document.getElementById('dm-fullscreen-shell')) {
  ...
  const startTransform = 'translate3d(' + offRightPx + 'px, 0, 0)';
  panel.style.transform = startTransform;   // start off-screen RIGHT
  panel.style.webkitTransform = startTransform;
  ...
  const startThreadOpenTransition = () => {
    if (!panel.isConnected || dmThreadOpenAnimationState?.token !== token) { finish(); return; }
    try {
      panel.addEventListener('transitionend', event => {
        if (!event || event.target === panel) finish();
      }, { once: true });
    } catch (_) {}
    const transition = `transform ${duration}ms ${easing}, -webkit-transform ${duration}ms ${easing}`;
    panel.style.transition = transition;
    panel.style.webkitTransition = transition;
    panel.style.transform = 'translate3d(0, 0, 0)';
    panel.style.webkitTransform = 'translate3d(0, 0, 0)';
    state.timer = window.setTimeout(finish, duration + 120);
  };

  requestAnimationFrame(() => requestAnimationFrame(startThreadOpenTransition));
  return;
