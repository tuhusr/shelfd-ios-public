...
async function loadActivityNotifications() {
  ...
  const snap = await db.collection('notifications')
    ...
    .limit(60)
    .get();
  const notifications = snap.docs.map(doc => ({ notificationId: doc.id, ...(doc.data() || {}) }));
  activityNotificationsUnreadCount = notifications.filter(item => item.read !== true).length;
...
function renderActivityNotificationsList(notifications = [], options = {}) {
  ...
  feed.innerHTML = `${headerHtml}<div class="activity-notifications-shell">${notifications.map(buildActivityNotificationRowHTML).join('')}</div>`;
  if (options.markExistingRead) {
    const cutoff = Number(options.openedAtMs || activityNotificationsOpenedAtMs || Date.now()) || Date.now();
    const existingUnread = notifications.filter(item => item.read !== true && Number(item.createdAtMs || item.updatedAtMs || 0) <= cutoff);
    window.setTimeout(() => markActivityNotificationsRead(existingUnread), 650);
  }
}
    ...
    activityNotificationsUnsubscribe = db.collection('notifications')
      ...
      .onSnapshot(snapshot => {
        ...
        if (activeFriendsTab === 'activity' && activeActivitySubTab === 'notifications') {
          renderActivityNotificationsList(notifications, { markExistingRead: false });
        }
      }, error => {
...
async function renderActivityNotificationsPage() {
  ...
  try {
    ...
    const notifications = await loadActivityNotifications();
    renderActivityNotificationsList(notifications, { markExistingRead: true, openedAtMs: activityNotificationsOpenedAtMs });
  } catch (error) {
