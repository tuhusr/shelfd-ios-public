...
function renderDiscoverCards(type, items, gridId) {
  ...
  grid.innerHTML = items.map(item => {
    ...
    const ratingInline = tmdbRating > 0
      ? `<span class="dc-rating" aria-label="Rating ${tmdbRating.toFixed(1)}"><span class="dc-rating-star" aria-hidden="true">★</span>${tmdbRating.toFixed(1)}</span>`
      : '';
...
function renderRankedDiscoverCards(type, items, gridId) {
  ...
  grid.innerHTML = items.map((item, index) => {
    ...
    const ratingInline = tmdbRating > 0
      ? `<span class="dc-rating" aria-label="Rating ${tmdbRating.toFixed(1)}"><span class="dc-rating-star" aria-hidden="true">★</span>${tmdbRating.toFixed(1)}</span>`
      : '';
    const cardReleaseLine = getDiscoverCardReleaseLine(item);
