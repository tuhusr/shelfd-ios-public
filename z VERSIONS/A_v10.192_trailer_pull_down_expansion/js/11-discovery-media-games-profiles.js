...
function closeDiscoverMediaProfile(reasonOrOptions = null) {
  ...
  resetDiscoverTrailerExpansion(overlay);
...
function openDiscoverHeroTrailer(event, button) {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  const overlay = button?.closest?.('.discover-media-profile-overlay') || document.getElementById('discover-media-profile');
  if (overlay?._shelfdSuppressTrailerClickUntil && Date.now() < overlay._shelfdSuppressTrailerClickUntil) return;
  if (isDiscoverTrailerExpansionFullscreen(overlay)) return;
  ...
  destroyDiscoverHeroTrailerPreview(document.getElementById('discover-media-profile'));
...
function renderDiscoverHeroTrailerPreview(trailer, title = '') {
  ...
  return `<button class="discover-media-hero-preview" type="button" data-discover-hero-trailer-preview data-trailer-key="${escAttr(trailerKey)}" data-trailer-title="${escAttr(title)}" onclick="openDiscoverHeroTrailer(event, this)" aria-label="${escAttr(`Play trailer for ${title || 'this title'}`)}">
...
function getDiscoverHeroTrailerPreviewIframe(overlay = document.getElementById('discover-media-profile')) {
  return overlay?.querySelector?.('[data-discover-trailer-preview-frame] iframe') || null;
}

function hasExpandableDiscoverHeroTrailer(overlay = document.getElementById('discover-media-profile')) {
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
  const iframe = getDiscoverHeroTrailerPreviewIframe(overlay);
  return !!(preview && iframe && String(preview.dataset?.trailerKey || '').trim());
}

function getDiscoverTrailerExpansionState(overlay) {
  if (!overlay) return null;
  if (!overlay._shelfdTrailerExpansion) {
    overlay._shelfdTrailerExpansion = {
      state: 'normal',
      rect: null,
      progress: 0,
      raf: 0
  ...
  return overlay._shelfdTrailerExpansion;
}

function setDiscoverHeroTrailerCommand(overlay, command, args = []) {
  const iframe = getDiscoverHeroTrailerPreviewIframe(overlay);
...
function captureDiscoverTrailerExpansionRect(overlay) {
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
...
function applyDiscoverTrailerExpansionProgress(overlay, progress = 0, options = {}) {
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
  const state = getDiscoverTrailerExpansionState(overlay);
  if (!overlay || !preview || !state) return false;
  if (!state.rect || options.captureRect) state.rect = captureDiscoverTrailerExpansionRect(overlay);
  if (!state.rect) return false;

  const clamped = Math.max(0, Math.min(1, Number(progress) || 0));
  const viewportW = Math.max(1, window.innerWidth || document.documentElement.clientWidth || 390);
  const viewportH = Math.max(1, window.innerHeight || document.documentElement.clientHeight || 844);
  const rect = state.rect;
  const tx = (0 - rect.left) * clamped;
  const ty = (0 - rect.top) * clamped;
  const sx = 1 + ((viewportW / Math.max(1, rect.width)) - 1) * clamped;
  const sy = 1 + ((viewportH / Math.max(1, rect.height)) - 1) * clamped;
  const radius = Math.max(0, 18 * (1 - clamped));

  state.progress = clamped;
  preview.style.setProperty('--trailer-origin-left', `${rect.left}px`);
  preview.style.setProperty('--trailer-origin-top', `${rect.top}px`);
  preview.style.setProperty('--trailer-origin-width', `${rect.width}px`);
  preview.style.setProperty('--trailer-origin-height', `${rect.height}px`);
  preview.style.setProperty('--trailer-expand-tx', `${tx}px`);
  preview.style.setProperty('--trailer-expand-ty', `${ty}px`);
  preview.style.setProperty('--trailer-expand-sx', String(sx));
  preview.style.setProperty('--trailer-expand-sy', String(sy));
  preview.style.setProperty('--trailer-expand-progress', String(clamped));
  preview.style.setProperty('--trailer-expand-radius', `${radius}px`);
  preview.style.setProperty('--trailer-overlay-opacity', String(Math.max(0, 1 - clamped * 0.86)));
  preview.style.setProperty('--trailer-vignette-opacity', String(Math.max(0, 1 - clamped * 0.72)));
  preview.style.setProperty('--trailer-frame-scale', String(1.22 - clamped * 0.10));
  overlay.style.setProperty('--trailer-profile-opacity', String(Math.max(0, 1 - clamped)));

  overlay.classList.add('media-profile-trailer-expanding');
  overlay.classList.toggle('media-profile-trailer-fullscreen', clamped >= 0.995);
  document.body.classList.toggle('media-profile-trailer-expanded', clamped >= 0.995);
  return true;
}

function animateDiscoverTrailerExpansionTo(overlay, targetProgress = 0, options = {}) {
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
  const state = getDiscoverTrailerExpansionState(overlay);
  if (!overlay || !preview || !state) return;
  const duration = Number(options.duration || 260);
  preview.style.transition = `transform ${duration}ms cubic-bezier(0.2, 1, 0.22, 1), border-radius ${duration}ms ease`;
  applyDiscoverTrailerExpansionProgress(overlay, targetProgress, { captureRect: !state.rect });
  window.setTimeout(() => {
    preview.style.transition = '';
    if (targetProgress >= 1) {
      state.state = 'fullscreen';
      applyDiscoverTrailerExpansionProgress(overlay, 1);
      setDiscoverHeroTrailerCommand(overlay, 'playVideo');
      return;
    }
    resetDiscoverTrailerExpansion(overlay);
  }, duration + 24);
...
function resetDiscoverTrailerExpansion(overlay = document.getElementById('discover-media-profile')) {
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
  const state = getDiscoverTrailerExpansionState(overlay);
  if (state?.raf) cancelAnimationFrame(state.raf);
  if (preview) {
    preview.style.removeProperty('--trailer-origin-left');
    preview.style.removeProperty('--trailer-origin-top');
    preview.style.removeProperty('--trailer-origin-width');
    preview.style.removeProperty('--trailer-origin-height');
    preview.style.removeProperty('--trailer-expand-tx');
    preview.style.removeProperty('--trailer-expand-ty');
    preview.style.removeProperty('--trailer-expand-sx');
    preview.style.removeProperty('--trailer-expand-sy');
    preview.style.removeProperty('--trailer-expand-progress');
    preview.style.removeProperty('--trailer-expand-radius');
    preview.style.removeProperty('--trailer-overlay-opacity');
    preview.style.removeProperty('--trailer-vignette-opacity');
    preview.style.removeProperty('--trailer-frame-scale');
    preview.style.transition = '';
  }
  if (overlay) {
    overlay.style.removeProperty('--trailer-profile-opacity');
    overlay.classList.remove('media-profile-trailer-expanding', 'media-profile-trailer-fullscreen');
    overlay._shelfdTrailerExpansion = { state: 'normal', rect: null, progress: 0, raf: 0 };
  }
  document.body.classList.remove('media-profile-trailer-expanded');
}

function commitDiscoverTrailerExpansion(overlay) {
  const state = getDiscoverTrailerExpansionState(overlay);
  if (!state) return;
  state.state = 'settling-fullscreen';
  animateDiscoverTrailerExpansionTo(overlay, 1, { duration: 260 });
}

function collapseDiscoverTrailerExpansion(overlay) {
  const state = getDiscoverTrailerExpansionState(overlay);
  if (!state) return;
  state.state = 'collapsing';
  animateDiscoverTrailerExpansionTo(overlay, 0, { duration: 240 });
}

function isDiscoverTrailerExpansionFullscreen(overlay = document.getElementById('discover-media-profile')) {
  return !!overlay?.classList?.contains('media-profile-trailer-fullscreen');
}

function updateDiscoverTrailerExpansionDuringGesture(overlay, progress = 0, captureRect = false) {
  const state = getDiscoverTrailerExpansionState(overlay);
  if (!state) return;
  if (state.raf) cancelAnimationFrame(state.raf);
  state.raf = requestAnimationFrame(() => {
    state.raf = 0;
    applyDiscoverTrailerExpansionProgress(overlay, progress, { captureRect });
  });
}

function isDiscoverTrailerGestureStartTarget(event, overlay) {
  if (!hasExpandableDiscoverHeroTrailer(overlay)) return false;
  if (isDiscoverTrailerExpansionFullscreen(overlay)) return true;
  const target = event?.target;
  const hero = overlay?.querySelector?.('.discover-media-hero.has-trailer-preview');
  const preview = getDiscoverHeroTrailerPreviewElement(overlay);
  return !!(target && (target.closest?.('[data-discover-hero-trailer-preview]') || (hero && hero.contains(target))));
}
...
function bindDiscoverMediaProfileSwipeBack(overlay) {
  ...
  let canExpandTrailer = false;
  let canCollapseTrailer = false;
  let gestureMode = '';
  ...
  const resetGestureStyles = () => {
    ...
    canExpandTrailer = false;
    canCollapseTrailer = false;
  ...
  const prepareTrailerExpandGesture = () => {
    clearGestureFrame();
    page.classList.add('media-profile-trailer-gesture-active');
    page.style.touchAction = 'none';
    overlay.style.transition = 'none';
    applyDiscoverTrailerExpansionProgress(overlay, isDiscoverTrailerExpansionFullscreen(overlay) ? 1 : 0, { captureRect: !isDiscoverTrailerExpansionFullscreen(overlay) });
  };
  ...
  const snapTrailerExpansionBack = () => {
    overlay._shelfdSuppressTrailerClickUntil = Date.now() + 520;
    page.classList.remove('media-profile-trailer-gesture-active');
    animateDiscoverTrailerExpansionTo(overlay, 0, { duration: 220 });
    window.setTimeout(() => {
      if (!isDiscoverTrailerExpansionFullscreen(overlay)) resetGestureStyles();
    }, 244);
  };

  const restoreTrailerFullscreen = () => {
    overlay._shelfdSuppressTrailerClickUntil = Date.now() + 520;
    page.classList.remove('media-profile-trailer-gesture-active');
    animateDiscoverTrailerExpansionTo(overlay, 1, { duration: 200 });
    window.setTimeout(() => {
      gestureMode = '';
      canExpandTrailer = false;
      canCollapseTrailer = false;
  ...
  const finishTrailerCollapse = () => {
    overlay._shelfdSuppressTrailerClickUntil = Date.now() + 520;
    page.classList.remove('media-profile-trailer-gesture-active');
    collapseDiscoverTrailerExpansion(overlay);
    window.setTimeout(resetGestureStyles, 270);
  };
  ...
  const handleGestureStart = (event) => {
    ...
    const trailerFullscreen = isDiscoverTrailerExpansionFullscreen(overlay);
    const targetIsPreview = !!event.target.closest?.('[data-discover-hero-trailer-preview]');
    if (event.target.closest('.discover-media-back, .discover-media-cast, .discover-media-similar, .discover-media-library-dock, .discover-media-add-floating, a, input, textarea, select')) return;
    if (!targetIsPreview && event.target.closest('button')) return;
    ...
    const hasHeroTrailer = hasExpandableDiscoverHeroTrailer(overlay);
    canCollapseTrailer = trailerFullscreen;
    canExpandTrailer = !trailerFullscreen && page.scrollTop <= 2 && hasHeroTrailer && (isDiscoverTrailerGestureStartTarget(event, overlay) || startY < viewportH * 0.56);
    canSwipeBack = !trailerFullscreen && startX <= 48;
    canPullDown = !hasHeroTrailer && !trailerFullscreen && page.scrollTop <= 2;
    gestureMode = '';
    activePointerId = event.pointerId ?? null;
  };

  const handleGestureMove = (event) => {
    if (!canSwipeBack && !canPullDown && !canExpandTrailer && !canCollapseTrailer) return;
    const point = event.touches?.[0] || event;
    if (!point) return;
    ...
    if (!gestureMode) {
      if (canCollapseTrailer && dy < -14 && absDy > absDx * 1.15) {
        gestureMode = 'trailer-collapse';
        prepareTrailerExpandGesture();
      } else if (canExpandTrailer && dy > 14 && absDy > absDx * 1.12) {
        gestureMode = 'trailer-expand';
        prepareTrailerExpandGesture();
      } else if (canSwipeBack && dx > 14 && absDx > absDy * 1.35) {
    ...
    if (gestureMode === 'trailer-expand') {
      ...
      const progress = Math.min(1, Math.max(0, dy) / Math.max(1, viewportH * 0.42));
      pendingProgress = progress;
      updateDiscoverTrailerExpansionDuringGesture(overlay, progress, false);
      return;
    }

    if (gestureMode === 'trailer-collapse') {
      if (event.cancelable) event.preventDefault();
      const progress = 1 - Math.min(1, Math.max(0, -dy) / Math.max(1, viewportH * 0.36));
      pendingProgress = progress;
      updateDiscoverTrailerExpansionDuringGesture(overlay, progress, false);
      return;
    }
  ...
  const handleGestureEnd = (event) => {
    if (!canSwipeBack && !canPullDown && !canExpandTrailer && !canCollapseTrailer && !gestureMode) return;
    ...
    if (mode === 'trailer-expand') {
      overlay._shelfdSuppressTrailerClickUntil = Date.now() + 520;
      const progress = Math.min(1, Math.max(0, dy) / Math.max(1, viewportH * 0.42));
      if (progress >= 0.52 || dy > 190) commitDiscoverTrailerExpansion(overlay);
      else snapTrailerExpansionBack();
      gestureMode = '';
      return;
    }
    ...
    if (mode === 'trailer-collapse') {
      const progress = 1 - Math.min(1, Math.max(0, -dy) / Math.max(1, viewportH * 0.36));
      if (progress <= 0.56 || dy < -150) finishTrailerCollapse();
      else restoreTrailerFullscreen();
      gestureMode = '';
      return;
    }
  ...
  const handleGestureCancel = () => {
    if (gestureMode === 'trailer-expand') {
      snapTrailerExpansionBack();
      gestureMode = '';
      return;
    }
    if (gestureMode === 'trailer-collapse') {
      restoreTrailerFullscreen();
      gestureMode = '';
      return;
    }
    resetGestureStyles();
