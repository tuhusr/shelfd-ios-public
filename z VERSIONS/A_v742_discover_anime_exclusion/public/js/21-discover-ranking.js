...
(function() {
  ...
  /* v741: engagement-sum scoring. Used by Trending + Popular categories.
     popularity = imdbVotes + tmdbVoteCount  (pure flat sum of engagement
     counts — IMDb users who rated + TMDB users who rated). No rating
     values, no trending decimal field, no multipliers. Log-normalized to
     0–1 so the framework's sort + ×100 calculatedScore stays well-formed.
     log10(10M) ≈ 7, so 7 is a generous max-engagement ceiling.

     `tmdbVoteCount` is preserved by enrichItemsWithImdbRatings BEFORE it
     overwrites vote_count with imdbVotes. For unenriched items (OMDb miss)
     we fall back to vote_count which is still the original TMDB value. */
  function getEngagementSum(item) {
    const imdbVotes = safeNumber(item && item.imdbVotes);
    const tmdbVotes = ('tmdbVoteCount' in (item || {}))
      ? safeNumber(item.tmdbVoteCount)
      : safeNumber(item && item.vote_count);
    return imdbVotes + tmdbVotes;
  }
  ...
  function scoreTrending(item, ctx) {
    /* v741: pure engagement-sum sort. The trending pool itself comes from
       TMDB /trending/{type}/week — the time window is enforced at fetch
       time, not in the score. Within that pool, the most-engaged title
       wins. No more order-position weighting, popularity blending, or
       rating-value contamination. */
