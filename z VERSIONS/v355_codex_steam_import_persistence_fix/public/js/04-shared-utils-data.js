...
async function verifyOwnDataDirectWrite(expectedData) {
  ...
  if (!snap.exists) throw new Error("Library save verification could not find the saved document.");
  const storedData = parseOwnFirestoreData(snap.data() || {});
...
async function writeOwnDataDirect(nextData, options = {}) {
  ...
  if (DOC_REF) {
    await DOC_REF.set(getOwnDataFirestorePayload(safeData), { merge: true });
