...
(function initShelfdMovieRatingDuel() {
  ...
  function readCachedTierList(mediaKey = duelMediaKey) {
    ...
    return movieDuelTierListCache[key]
      ? normalizeSavedTierList(movieDuelTierListCache[key], mediaKey)
      : null;
  }

  function writeCachedTierList(mediaKey = duelMediaKey, tierList = null) {
    const key = getTierListCacheKey(mediaKey);
    movieDuelTierListCache[key] = normalizeSavedTierList(tierList, mediaKey);
    return normalizeSavedTierList(movieDuelTierListCache[key], mediaKey);
  }
  ...
  function getSavedTierList(mediaKey = duelMediaKey) {
    ...
    return isMovieDuelReadOnly()
      ...
      : seedTierListFromLibrary(mediaKey, cachedTierList || savedTierList);
  }
  ...
  async function saveTierList(mediaKey = duelMediaKey, tierList = null) {
    const config = getMediaConfig(mediaKey);
    const normalized = writeCachedTierList(mediaKey, tierList);
    const payload = {
      version: 2,
