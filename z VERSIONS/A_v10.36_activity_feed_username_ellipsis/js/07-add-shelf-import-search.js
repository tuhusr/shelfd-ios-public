...
function getAddShelfModalRatingValue(score = pendingModalRatingSelection, section = resolveAddShelfSelectedSection()) {
  ...
  picker.innerHTML = `
    ...
    <div class="modal-status-confirm">
      ...
      <div class="modal-status-confirm-actions">
        <button class="btn-secondary modal-status-confirm-back" onclick="handleAddShelfConfirmBack()">Back</button>
...
function showModalRatingPrompt(selectedRating = pendingModalRatingSelection) {
  ...
  picker.innerHTML = `
    ...
    <div class="discover-rating-prompt add-shelf-rating-prompt">
      <div class="modal-status-confirm-title">${escHtml(selectedTmdb.title || 'This title')}</div>
      <div class="discover-add-desc">Choose one rating now. The final confirm will keep that selection.</div>
      ${stars}
      <div class="modal-status-confirm-actions">
        <button class="btn-secondary modal-status-confirm-back" onclick="showModalStatusPicker()">Back</button>
        <button class="btn-secondary" onclick="skipAddShelfModalRating()">Skip</button>
        <button class="btn-primary modal-status-confirm-submit" onclick="confirmAddShelfModalRating()" ${Number(selectedRating || 0) > 0 ? '' : 'disabled'}>Confirm</button>
      </div>
    </div>
  `;
  picker.style.display = 'flex';
...
async function confirmModalAdd() {
  ...
  try {
    const rating = pendingModalStatusSelection === 'watched'
      ? (Number(pendingModalRatingSelection || 0) || 0)
      : 0;
    const result = await submitModal(pendingModalStatusSelection, rating);
    if (!result?.ok) {
...
async function submitModal(status, rating = 0) {
  ...
  const item = {
    ...
    nextEpisodeAirDate: selectedTmdb.nextEpisodeAirDate || '',
    next_episode_to_air: selectedTmdb.next_episode_to_air || null,
  };
