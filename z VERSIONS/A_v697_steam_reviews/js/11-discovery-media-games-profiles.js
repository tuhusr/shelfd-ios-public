...
/* =========================================================================
   v697: Steam review data for Discover > Games cards (Row 4).
   Uses the Worker proxy (/api/steam/appreviews/{id}) to bypass CORS.
   Caches in-memory for 24 h so the same app isn't refetched each load.
   ========================================================================= */
const _steamRevCache    = new Map();   /* key: steamAppId, value: result + ts */
const _steamRevFetching = new Set();   /* dedup concurrent fetches */
const _STEAM_REV_TTL    = 86_400_000; /* 24 h in ms */

function _getSteamId(item) {
  const stores = Array.isArray(item?.stores) ? item.stores : [];
  const s = stores.find(st => st.store?.id === 1 || /steam/i.test(st.store?.name || ''));
  if (!s) return '';
  const m = String(s.url || '').match(/\/app\/(\d+)/);
  return m ? m[1] : '';
}
...
function _backfillSteamReviews(grid, items) {
  ...
  items.forEach((item, idx) => {
    const steamId = _getSteamId(item);
    if (!steamId) return;
    /* Stagger requests 120 ms apart so we don't hammer Steam/Worker
       all at once for a 24-item grid. */
    setTimeout(async () => {
      const rev = await _fetchSteamReview(steamId);
      if (!rev) return;
      /* Locate the rating cell by the game's RAWG id stored on the poster. */
      const poster = grid.querySelector(`.discover-poster[data-rawg-id="${CSS.escape(String(item.id || ''))}"]`);
...
function renderGamesDiscoverCards(items, gridId) {
  ...
  grid.innerHTML = items.map(item => {
    ...
    /* v696: Restructured game card — 5-row info block below the poster.
       Row 1: title  2: year  3: genres (max 3)  4: Rating placeholder  5: Add btn */
    return `<div class="discover-card games-discover-card">
      ...
      <div class="discover-card-body games-dc-body">
        ...
        <div class="games-dc-rating">Rating</div>
