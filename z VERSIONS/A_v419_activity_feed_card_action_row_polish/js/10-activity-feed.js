...
function buildStackedActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  html = html.replace(/onclick="handleScreenListActivityCardOpen\('[^']*','activity'\)"/, `onclick="openScreenListStackedActivityPage('${escAttr(activityId)}')"`);
  return `
    <div class="sl-activity-stack-wrap" data-stacked-activity-id="${escAttr(activityId)}">
      <button class="sl-activity-stack-layer sl-activity-stack-layer-one" type="button" onclick="openScreenListStackedActivityPage('${escAttr(activityId)}')" aria-label="Open grouped activity stack"></button>
      <button class="sl-activity-stack-layer sl-activity-stack-layer-two" type="button" onclick="openScreenListStackedActivityPage('${escAttr(activityId)}')" aria-label="Open grouped activity stack"></button>
      ${html}
    </div>`;
}
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  const stackExtraHtml = stackExtraCount > 0 ? `
      <button class="sl-activity-action-btn activity-interaction-btn sl-activity-stack-action-count" data-activity-action="stack" onclick="event.stopPropagation(); openScreenListStackedActivityPage('${escAttr(stackActivityId)}')" aria-label="Open ${stackExtraCount} more grouped activities">+${stackExtraCount}</button>` : '';
  ...
  const interactionsHtml = options.hideInteractions ? '' : `
    <div class="sl-activity-actions activity-interactions" data-activity-interactions>
      ...
      <button class="sl-activity-action-btn activity-interaction-btn ${isLiked ? 'liked' : ''}" data-activity-action="like" onclick="event.stopPropagation(); toggleActivityLike('${escAttr(activityId)}', this)" aria-label="Like activity">
        <span data-like-icon-slot>${getScreenListHeartIconSvg(isLiked)}</span>
        <span data-activity-like-count>${likeCount}</span>
      </button>${deleteHtml}${stackExtraHtml}
    </div>`;
