...
(function() {
  ...
  const PRESET_GAME_GENRE_OPTIONS = [
    ...
    { label: 'Horror', slug: 'horror', tag: 'horror' },
  ...
  const PRESET_GAME_PLATFORM_OPTIONS = [
    { label: 'Any', platformId: '' },
    { label: 'PC', platformId: '4' },
    { label: 'PlayStation', platformId: '187' },
    { label: 'Xbox', platformId: '1' },
    { label: 'Nintendo Switch', platformId: '7' },
    { label: 'iOS', platformId: '3' },
    { label: 'Android', platformId: '21' }
  ];
  ...
  function normalizePresetGameItems(items = []) {
    ...
    return (Array.isArray(items) ? items : [])
      .filter(item => item?.id && item?.name && item?.background_image)
      .map(item => ({ ...item, source: 'rawg', kind: 'game', rawgId: String(item.rawgId || item.rawg_id || item.id || '') }))
      .filter(item => {
        const id = String(item.rawgId || item.id || '').trim();
  ...
  function getPresetGameEngagement(item = {}) {
    return Number(item.added || 0) + Number(item.ratings_count || 0) + Number(item.reviews_count || 0);
  }
  ...
  function rankPresetGameItems(items = [], preset = 'popular') {
    ...
    return list.sort((a, b) => {
      ...
      const aEngagement = getPresetGameEngagement(a);
      const bEngagement = getPresetGameEngagement(b);
      const aQuality = getPresetGameQuality(a);
      const bQuality = getPresetGameQuality(b);
      const aScore = preset === 'rated'
        ? (aQuality * 1000000) + Math.log10(aEngagement + 1) * 100000
        : (aEngagement * 10) + (aQuality * 1000);
      const bScore = preset === 'rated'
        ? (bQuality * 1000000) + Math.log10(bEngagement + 1) * 100000
        : (bEngagement * 10) + (bQuality * 1000);
      if (bScore !== aScore) return bScore - aScore;

      if (Number.isFinite(aRelease) && Number.isFinite(bRelease) && bRelease !== aRelease) return bRelease - aRelease;
      return String(a.name || '').localeCompare(String(b.name || ''), undefined, { sensitivity: 'base' });
    }).slice(0, PRESET_RESULT_LIMIT);
  }
  ...
  async function fetchGamePresetDirect(config = {}) {
    ...
    if (preset === 'game-genre') {
      ...
      return rankPresetGameItems(await fetchPresetRawgPages({
        ...baseParams,
        ...(genre?.tag ? { tags: genre.tag } : { genres: String(config.gameGenre || '') })
      }, PRESET_POPULAR_PAGE_COUNT), 'popular');
    }
    if (preset === 'game-platform') {
      return rankPresetGameItems(await fetchPresetRawgPages({
        ...baseParams,
        platforms: String(config.platformId || '')
      }, PRESET_POPULAR_PAGE_COUNT), 'popular');
    }
    if (preset === 'rated') {
      return rankPresetGameItems(await fetchPresetRawgPages({
        ordering: '-rating',
        metacritic: '70,100'
      }, PRESET_POPULAR_PAGE_COUNT), 'rated');
    }
    if (preset === 'anticipated' || preset === 'upcoming') {
      return rankPresetGameItems(await fetchPresetRawgPages({
        ordering: '-added',
        dates: `${getPresetDate(1)},${getPresetDate(365)}`
      }, PRESET_POPULAR_PAGE_COUNT), 'anticipated');
    }
    if (preset === 'decade') {
    ...
    return rankPresetGameItems(await fetchPresetRawgPages(baseParams, PRESET_POPULAR_PAGE_COUNT), 'popular');
  }
  ...
  function renderPresetGameResultCard(item = {}) {
    ...
    if (rawgId) {
      presetGameResultSeeds.set(rawgId, {
        id: rawgId,
        rawgId,
        title,
        name: title,
    ...
    return `
      <button type="button" class="shelfd-search-preset-title-card" onclick="handleSearchPresetGameClick(event, '${escAttr(rawgId)}')" title="${escAttr(title)}">
        <span class="shelfd-search-preset-poster">
          ${poster ? `<img src="${escAttr(poster)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">` : ''}
          ${rating ? `<span class="shelfd-search-preset-rating"><span aria-hidden="true">★</span>${escHtml(rating)}</span>` : ''}
        </span>
  ...
  window.handleSearchPresetGameClick = function(event, rawgId) {
    const id = String(rawgId || '').trim();
    if (!id || typeof window.openGameMediaProfile !== 'function') return;
    const seed = presetGameResultSeeds.get(id) || { id, rawgId: id, source: 'rawg' };
    try {
