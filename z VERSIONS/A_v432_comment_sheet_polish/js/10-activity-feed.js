...
function installShelfdFeedSheetGestures(page) {
  ...
  // v432: applies a soft resistance once the user drags past ~60% of the sheet
  // so the gesture feels weighted instead of running away under your finger.
  const applyResistance = (dy, sheetH) => {
    const knee = sheetH * 0.6;
    if (dy <= knee) return dy;
    const over = dy - knee;
    return knee + over / (1 + over / 80);
  };
  ...
  const onPointerDown = (event) => {
    ...
    // Don't preventDefault yet — only once we've actually started dragging,
    // so a tap on the handle doesn't block native scroll/click chains elsewhere.
  };
  const onPointerMove = (event) => {
    ...
    dragState.lastT = Date.now();
    inner.style.transform = `translate3d(-50%, ${dy}px, 0)`;
  ...
  const onPointerUp = () => {
    ...
    const elapsed = Math.max(1, Date.now() - dragState.lastT + 1);
    // recent velocity over the last move window (px/ms)
    const velocity = dy / Math.max(1, Date.now() - dragState.startedAt);
    const shouldClose = dy > 90 || dy / sheetH > 0.28 || velocity > 0.55;
    dragState = null;
    // Re-enable transition BEFORE removing inline transform so the snap-back/close
    // is a single smooth interpolated frame instead of an instant jump.
    page.classList.remove('is-dragging');
    if (!inner) return;
    if (shouldClose) {
      // Trigger close: removing .is-open changes the CSS target to translateY(100%).
      // We then clear the inline transform so the active transition interpolates
      // from the user's last drag position straight to fully-closed.
      page.classList.remove('is-open');
      requestAnimationFrame(() => {
        inner.style.transform = '';
      });
      // Mirror the rest of closeFeedPostPage's cleanup work without re-triggering
      // a second close animation.
      const reduceMotion = !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
      const hideMs = reduceMotion ? 0 : 380;
      if (_shelfdFeedSheetCloseTimer) clearTimeout(_shelfdFeedSheetCloseTimer);
      _shelfdFeedSheetCloseTimer = setTimeout(() => {
      ...
      // Snap back: clear inline transform → CSS .is-open rule animates back to 0.
      inner.style.transform = '';
    }
  };
