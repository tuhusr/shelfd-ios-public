...
/* v528: 4th bottom nav tab "Friends" — navigates to community-view but
   auto-switches to the friends inner tab and marks the body so CSS can
   hide the Activity inner tab and show only Friends + Requests.
   v549: tighter exclusivity: only one of (Activity, Friends) is ever
...
/* v549: Activity bottom-nav explicit handler — guarantees shelfd-friends-list-mode
   is cleared before the view re-renders, so the Activity feed is what the user
   actually sees (not lingering Friends UI). */
