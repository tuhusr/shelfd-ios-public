...
function setScreenListActivityNoteFor(activity = {}, text = '') {
  ...
  if (!trimmed) {
    ...
    data.shelfdActivityNotes[key] = { text: trimmed, at: new Date().toISOString() };
  }
}
...
function getActivityDisplayAction(eventType = '', item = {}, activity = {}) {
  ...
  /* v553/555: completion verb.
     - Movies → "watched"
     - Games  → "played"
     - Shows/anime → "Finished watching" (or "Finished watching Season N") */
  if (status === 'watched' || eventType === 'completed') {
    if (isGame) return 'played';
    if (isMovie) return 'watched';
    const num = Number(item?.lastSeasonFinishedNum || 0);
    if (num > 1) return `Finished watching Season ${num}`;
    return 'Finished watching';
  }
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  /* v554/557: rating chip selector
     - Merged "watched + rated" card uses compact "EP rated ★ N" with the
       episode's rating.
     - Everything else (show rating / season / movie / game) uses the
       full 5-star chip with item.rating.
     v557 retroactive: if item.lastEpisodeRatingValue isn't set (older data
     before tracking landed), fall back to scanning item.episodes for any
     ep with a rating, taking the highest one. */
  const isMergedWatchRate = !!a.mergedHadEpisodeWatch && a.eventType === 'rated';
  let epRating = Number(item.lastEpisodeRatingValue || 0);
  if (isMergedWatchRate && !epRating && Array.isArray(item.episodes)) {
    let best = 0;
