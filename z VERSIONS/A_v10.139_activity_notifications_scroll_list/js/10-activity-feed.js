...
let activityNotificationsSourceSyncTimer = null;
...
function stopActivityNotificationsLiveListener() {
  ...
  if (activityNotificationsSourceSyncTimer) {
    window.clearInterval(activityNotificationsSourceSyncTimer);
    activityNotificationsSourceSyncTimer = null;
  }
  activityNotificationsCache = new Map();
}
...
function startActivityNotificationsLiveListener() {
  ...
  if (!activityNotificationsSourceSyncTimer) {
    activityNotificationsSourceSyncTimer = window.setInterval(async () => {
      if (activeFriendsTab !== 'activity' || activeActivitySubTab !== 'notifications') return;
      const created = await syncActivityNotificationsFromSources();
      if (created > 0) {
        const notifications = await loadActivityNotifications();
        renderActivityNotificationsList(notifications);
      }
    }, 7000);
  }
...
async function renderActivityNotificationsPage() {
  ...
  try {
    ...
    await syncActivityNotificationsFromSources();
    const notifications = await loadActivityNotifications();
    renderActivityNotificationsList(notifications);
  } catch (error) {
