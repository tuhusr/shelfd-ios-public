...
function isPwaCriticalAssetPath(pathname = "") {
  const path = String(pathname || "").trim();
  if (!path) return false;
  if (path === "/sw.js" || path === "/script.js" || path === "/site.webmanifest") return true;
  if (path === "/index.html" || path === "/") return true;
  if (path === "/js/00-live-update-pwa.js") return true;
  return false;
}

function withPwaCriticalNoStoreHeaders(response, pathname = "") {
  if (!isPwaCriticalAssetPath(pathname)) return response;
  const headers = new Headers(response.headers);
  headers.set("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
  headers.set("CDN-Cache-Control", "no-store");
  headers.set("Cloudflare-CDN-Cache-Control", "no-store");
...
export default {
  async fetch(request, env, ctx) {
    ...
    response = withPwaCriticalNoStoreHeaders(response, url.pathname);
