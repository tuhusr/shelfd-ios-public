...
function buildStackedActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  let html = buildActivityCardHTML(primary, activityId, { ...options, renderStackPrimary: true });
  ...
  return `
    <div class="sl-activity-stack-wrap" data-stacked-activity-id="${escAttr(activityId)}">
      ...
      ${html}
      ${extraCount > 0 ? `<button class="sl-activity-stack-extra-count" type="button" onclick="event.stopPropagation(); openScreenListStackedActivityPage('${escAttr(activityId)}')" aria-label="Open ${extraCount} more grouped activities">+${extraCount}</button>` : ''}
    </div>`;
}
