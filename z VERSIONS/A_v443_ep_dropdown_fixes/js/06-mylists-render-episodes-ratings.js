...
function updateMyListUpcomingReleaseDateElement(item = {}, section = activeSection) {
  // v434: this function now drives the full Watch List availability line — release
  // date / "In theaters" / "Where to watch:" + provider logos. The element is
  // identified by .card-availability-line so we can replace its inner HTML in place
    ...
    <div class="card ${type === "show" ? "show-card" : ""}${isGameCard ? " game-library-card" : ""} ${viewingUser ? "friend-view-card" : ""}${isDraggable ? ' card-draggable' : ''}" id="card-${item.id}" ${dragAttrs}>
      <div class="card-header">
        ...
        <div class="card-info">
          ...
          ${(activeTab === 'planned' && isScreenListMovieTvAnimeSection(activeSection)) ? renderMyListWatchListAvailabilityHtml(item, activeSection) : ''}
      ...
      <div class="card-action-row${bottomExternalBadgeHtml ? ' has-bottom-export' : ''}">
        ...
        <div class="card-footer-actions">
          ${isGameCard && !viewingUser ? `<button class="game-card-cover-btn" type="button" onclick="event.stopPropagation();openScreenListGameCoverPickerForItem('${escAttr(item.id)}')" aria-label="Choose game cover">Change Cover</button>` : ''}
...
function toggleSeason(itemId, sNum) {
  ...
  // v443 Issue 3: when a season is opened, scroll its header to the top of the
  // ep-scroll container so the user sees the episodes without scrolling.
  // Only on open (not close) to avoid disorienting auto-scroll on close.
  if (!open) {
    requestAnimationFrame(() => {
      const epScroll = document.querySelector('#ep-list-' + itemId + ' .ep-scroll');
      // Find the season-header that precedes this season's episodes.
      // Structure: .season-block > .season-header + .season-eps#s-eps-*
      const seasonEps = el; // the element we just showed
      const seasonBlock = seasonEps.parentElement; // .season-block
      ...
      // Scroll ep-scroll so header sits at the very top of the scroll area.
      const headerTop = header.getBoundingClientRect().top;
      const scrollTop = epScroll.getBoundingClientRect().top;
      const target = epScroll.scrollTop + (headerTop - scrollTop);
      epScroll.scrollTo({ top: Math.max(0, target), behavior: 'smooth' });
    });
  }
}
...
function enforceScreenListGamePosterPlaceholder(posterEl) {
  if (!posterEl) return;
  const img = posterEl.matches?.('img') ? posterEl : posterEl.querySelector?.('img');
  if (img) img.removeAttribute('src');
  if (!posterEl.matches?.('img')) {
    posterEl.style.removeProperty('background-image');
    posterEl.classList.add('no-img', 'screenlist-game-cover-pending');
    if (!posterEl.textContent.trim() && !posterEl.querySelector('span')) {
      posterEl.innerHTML = `<span>${SCREENLIST_GAME_COVER_PLACEHOLDER_TEXT}</span>`;
    }
  }
  posterEl.dataset.igdbCoverApplied = '0';
  posterEl.dataset.poster = '';
}
...
function setEpisodesExpanded(list, shouldOpen, immediate) {
  // v443: reworked to avoid multiple forced-layout reads in the same frame,
  // which were the main cause of the title card shake/jitter. We now use a
  // single scrollHeight measurement inside a rAF so the measurement and the
  // height change happen in different frames and the browser can batch its
  // layout work without disrupting the composited transform/opacity animation.
  if (!list) return;
  ...
  if (shouldOpen) {
    ...
    if (immediate) { list.style.height = 'auto'; return; }
    list.style.height = '0px';
    requestAnimationFrame(() => {
      const content = list.querySelector('.ep-list-inner');
      const target = content ? content.scrollHeight : list.scrollHeight;
...
function toggleEpisodes(id) {
  ...
  if (!open) {
    ...
    // Wait one rAF so the card DOM is stable before measuring.
    requestAnimationFrame(() => {
      ...
      // Aim just below the sticky header/safe-area.  A value of 72px covers
      // the PWA/mobile header + a small breathing gap.
