...
function updateMyListNextEpisodeElement(item = {}, section = activeSection) {
  ...
  const html = renderMyListNextEpisodeHtml(item, section);
  if (!html) {
  ...
  if (!info) return;
  const progress = info.querySelector('.progress-area');
  const rating = info.querySelector('.rating-area');
  if (progress?.parentNode === info) progress.insertAdjacentElement('afterend', node);
  else if (rating?.parentNode === info) info.insertBefore(node, rating);
...
function renderCard(item, isDraggable) {
  ...
  return `
    <div class="card ${type === "show" ? "show-card" : ""}${isGameCard ? " game-library-card" : ""} ${viewingUser ? "friend-view-card" : ""}${isDraggable ? ' card-draggable' : ''}" id="card-${item.id}" ${isGameCard ? `onclick="handleScreenListGameCardSurfaceClick(event,'${itemIdAttr}')"` : ''} ${dragAttrs}>
      <div class="card-header">
        ...
        <div class="card-info">
          ...
          ${(() => {
            ...
            return renderMyListNextEpisodeHtml(item, activeSection);
          })()}
      ...
      <div class="card-action-row${bottomExternalBadgeHtml ? ' has-bottom-export' : ''}${watchlistPriorityHtml ? ' has-watch-priority' : ''}">
...
function setWatchlistPriority(itemId = '', rawValue = '') {
  ...
  if (Number.isFinite(parsed) && parsed > 0) {
    item.watchPriority = Math.max(1, Math.floor(parsed));
  } else {
    delete item.watchPriority;
