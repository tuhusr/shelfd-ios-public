...
(function initShelfdMovieRatingDuel() {
  ...
  function getSavedTierList(mediaKey = duelMediaKey) {
    ...
    return isMovieDuelReadOnly()
      ? normalizeSavedTierList(savedTierList, mediaKey)
      : seedTierListFromLibrary(mediaKey, savedTierList);
  }
          ...
          return `
            <section class="movie-duel-tier-row movie-duel-tier-${attr(tier.key.toLowerCase())}">
              ...
              <div class="movie-duel-tier-scroll-wrap" data-movie-duel-tier-row="${attr(tier.key)}">
                <div class="movie-duel-tier-scroll" data-movie-duel-tier-scroll="${attr(tier.key)}" onscroll="syncMovieRatingDuelTierRowArrows()" aria-label="${attr(tier.label)}">
                ...
                <button type="button" class="movie-duel-tier-scroll-btn movie-duel-tier-scroll-btn-next" data-movie-duel-tier-next="${attr(tier.key)}" onclick="scrollMovieRatingDuelTierRow('${attr(tier.key)}', 1)" aria-label="Scroll ${attr(tier.label)} row to the right">
                  <svg viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M9 5l7 7-7 7" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path>
                  </svg>
                </button>
              </div>
            </section>
  ...
  function loadMoreMovieRatingDuelTier(tierKey = '') {
    ...
    requestAnimationFrame(syncMovieRatingDuelTierRowArrows);
  }

  function syncMovieRatingDuelTierRowArrows() {
    document.querySelectorAll('[data-movie-duel-tier-row]').forEach(row => {
      const tierKey = cleanText(row.getAttribute('data-movie-duel-tier-row') || '').toUpperCase();
      const scroller = row.querySelector(`[data-movie-duel-tier-scroll="${tierKey}"]`);
      const nextBtn = row.querySelector(`[data-movie-duel-tier-next="${tierKey}"]`);
      if (!scroller || !nextBtn) return;
      const canOverflow = scroller.scrollWidth - scroller.clientWidth > 6;
      const canScrollRight = scroller.scrollLeft + scroller.clientWidth < scroller.scrollWidth - 6;
      nextBtn.hidden = !canOverflow;
      nextBtn.disabled = !canScrollRight;
      nextBtn.classList.toggle('is-disabled', !canScrollRight);
    });
  }

  function scrollMovieRatingDuelTierRow(tierKey = '', direction = 1) {
    const key = cleanText(tierKey).toUpperCase();
    if (!key) return;
    const scroller = document.querySelector(`[data-movie-duel-tier-scroll="${key}"]`);
    if (!scroller) return;
    const step = Math.max(240, Math.round(scroller.clientWidth * 0.78));
    scroller.scrollBy({ left: step * (Number(direction || 1) >= 0 ? 1 : -1), behavior: 'smooth' });
    window.setTimeout(syncMovieRatingDuelTierRowArrows, 260);
  }
  ...
  async function refreshViewingUserTierListProfile() {
    ...
    if (!uid || !db?.collection) return viewingUser || null;
    try {
  ...
  window.scrollMovieRatingDuelTierRow = scrollMovieRatingDuelTierRow;
  window.syncMovieRatingDuelTierRowArrows = syncMovieRatingDuelTierRowArrows;
})();
