...
function updateDirectMessageKeyboardLift() {
  ...
  const typing = isDirectMessageTypingActive();
  const isMobile = isDirectMessagesMobileViewport();
  /* v718: Always read the actual viewport lift — don't gate on focus state.
     This ensures the bar tracks the keyboard fully back down even if the
     input loses focus before the keyboard animation completes. */
  const actualLift = isMobile ? getDirectMessageKeyboardBottom() : 0;
  const lift = actualLift;
  page.classList.toggle('dm-keyboard-active', isMobile && lift > 0);
...
function resetDirectMessageKeyboardLift() {
  const page = document.getElementById('direct-messages-page');
  if (!page) return;
  /* v718: add a settle class so CSS can apply a smooth close transition,
     then keep polling until the keyboard is fully gone (covers the full
     ~350ms iOS keyboard close animation). */
  page.classList.add('dm-compose-settling');
  const finish = () => {
    page.classList.remove('dm-keyboard-active', 'dm-compose-settling');
    page.style.setProperty('--dm-keyboard-lift', '0px');
    page.style.setProperty('--dm-keyboard-bottom', '0px');
  };
  /* Poll through the keyboard close window */
  [60, 130, 200, 300, 400, 500].forEach(t => window.setTimeout(updateDirectMessageKeyboardLift, t));
