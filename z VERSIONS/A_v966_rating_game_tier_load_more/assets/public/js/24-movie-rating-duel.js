/* v966: Movies-only matchup game keeps saved tier posters paged per row. */
(function initShelfdMovieRatingDuel() {
  if (window.__shelfdMovieRatingDuelV966) return;
  window.__shelfdMovieRatingDuelV966 = true;
  ...
  function seedTierListFromLibrary(tierList = null) {
    ...
    getOwnWatchedMoviesForDuel().forEach(item => {
      ...
      if (savedEntry) {
        ...
        if ((!savedEntry.title || savedEntry.title === 'Untitled movie') && entry.title) savedEntry.title = entry.title;
  ...
  function getMovieDuelTierVisibleCount(tierKey = '', total = 0) {
    if (!total) return 0;
    ...
    return `
      <div class="movie-duel-tier-board">
        ${TIER_ROWS.map(tier => {
          ...
          const visibleCount = getMovieDuelTierVisibleCount(tier.key, entries.length);
  ...
  function loadMoreMovieRatingDuelTier(tierKey = '') {
    ...
    const normalized = normalizeSavedTierList(movieDuelVisibleTierList);
    ...
    if (!board) return;
    board.outerHTML = renderMovieDuelTierList(normalized, movieDuelVisibleTierActiveId, { preserveVisibleCounts: true });
  }
    ...
    overlay.querySelector('.movie-duel-sheet').innerHTML = `
      <div class="movie-duel-header">
        <div>
          ...
          <h2>Your Movie Tier List</h2>
          <p>Shows 12 posters per tier first. Use Load More to reveal the rest without overloading the page.</p>
        </div>
        <button type="button" class="movie-duel-close" onclick="closeMovieRatingDuel()" aria-label="Close">Ã—</button>
      </div>
      ${renderMovieDuelTierList(getSavedMovieTierList(), '')}
      <div class="movie-duel-actions movie-duel-actions-result">
        <button type="button" class="movie-duel-secondary" onclick="openMovieRatingDuelLauncher()">Back</button>
        <button type="button" class="movie-duel-primary" onclick="closeMovieRatingDuel()">Done</button>
      </div>
    `;
    lockMovieDuelScroll();
