  ...
  sheet.innerHTML = `
    ...
    <div class="sas-body">
      ...
      <div class="sas-statuses">
        ...
        <button class="sas-status-btn" type="button" onclick="pickSeasonalStatus('dropped')">Dropped</button>
      </div>
    </div>`;
  document.body.appendChild(sheet);
...
async function pickSeasonalStatus(status) {
  if (!window.selectedTmdb) {
    closeSeasonalAddSheet();
    return;
  }
  closeSeasonalAddSheet();
  try {
    /* submitModal lives in 07-add-shelf-import-search.js and uses
       selectedTmdb.librarySection ('anime') to target the correct list. */
