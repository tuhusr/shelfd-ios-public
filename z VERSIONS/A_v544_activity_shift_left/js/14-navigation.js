...
async function goToFriendsList() {
  ...
  // Apply the mode class AFTER switchMainNav (which clears it on the
  // 'community' branch) so this state sticks.
  document.body.classList.add('shelfd-friends-list-mode');
  activeFriendsTab = 'friends';
  if (typeof switchFriendsTab === 'function') {
    switchFriendsTab('friends');
  }
  // Bottom nav active states — Friends button on, others off.
