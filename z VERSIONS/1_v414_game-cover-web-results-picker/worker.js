...
function normalizeGameWebCoverQuery(title = "") {
  ...
  return cleanTitle ? `${cleanTitle} video game cover official` : "";
}
...
function normalizeTavilyImageCandidate(item = {}, fallbackSource = {}) {
  ...
  const coverUrl = String(item?.url || item?.image_url || item?.src || "").trim();
  if (!coverUrl) return null;
  return {
    coverUrl,
    title: String(item?.title || item?.description || fallbackSource.title || "").trim(),
    sourceUrl: String(item?.source_url || item?.source || fallbackSource.url || "").trim(),
