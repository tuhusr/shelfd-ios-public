...
function updateMyListUpcomingReleaseDateElement(item = {}, section = activeSection) {
  const label = getMyListUpcomingReleaseLabel(item, section);
  const node = document.querySelector(`#card-${CSS.escape(String(item?.id || ''))} .mylist-upcoming-release-date`);
  if (node) {
  ...
  if (!label) return;
  const info = document.querySelector(`#card-${CSS.escape(String(item?.id || ''))} .card-info`);
  const status = document.querySelector(`#card-${CSS.escape(String(item?.id || ''))} .status-pills`);
  if (!info) return;
  const div = document.createElement('div');
  div.className = 'card-release-date mylist-upcoming-release-date';
  div.textContent = label;
  if (status && status.parentNode === info) info.insertBefore(div, status);
  else info.appendChild(div);
}
...
function queueMyListUpcomingReleaseDateHydration(item = {}, section = activeSection) {
  ...
  if (known) {
    if (isScreenListFutureReleaseDate(known)) updateMyListUpcomingReleaseDateElement(item, section);
    return;
  }
  const key = `${section}:${item?.id || ''}:${item?.tmdbId || ''}:${item?.title || ''}`;
  if (SCREENLIST_UPCOMING_RELEASE_LOOKUP_INFLIGHT.has(key) || SCREENLIST_UPCOMING_RELEASE_LOOKUP_DONE.has(key)) return;
  SCREENLIST_UPCOMING_RELEASE_LOOKUP_INFLIGHT.add(key);
  setTimeout(async () => {
    try {
      ...
      SCREENLIST_UPCOMING_RELEASE_LOOKUP_INFLIGHT.delete(key);
      SCREENLIST_UPCOMING_RELEASE_LOOKUP_DONE.add(key);
    }
  }, 120);
}
    ...
    <div class="card ${type === "show" ? "show-card" : ""}${isGameCard ? " game-library-card" : ""} ${viewingUser ? "friend-view-card" : ""}${isDraggable ? ' card-draggable' : ''}" id="card-${item.id}" ${dragAttrs}>
      <div class="card-header">
        ...
        <div class="card-info">
          ...
          ${getMyListUpcomingReleaseLabel(item, activeSection) ? `<div class="card-release-date mylist-upcoming-release-date">${escHtml(getMyListUpcomingReleaseLabel(item, activeSection))}</div>` : ''}
...
function openScreenListGameCoverPicker(context = {}) {
  ...
  if (input) input.value = title ? `${title} video game cover official` : '';
...
function renderScreenListGameCoverPickerResults(groups = []) {
  ...
  grid.innerHTML = safeGroups.map(group => {
    const cards = group.results.map(result => {
...
async function loadScreenListGameCoverPickerResults(query = '') {
  ...
  try {
    ...
    const [igdbRes, webRes] = await Promise.all([
      fetch('/api/igdb/covers?' + params.toString(), { cache: 'no-store' }),
      fetch('/api/game/web-covers?' + webParams.toString(), { cache: 'no-store' })
    ]);
    const json = igdbRes.ok ? await igdbRes.json() : null;
    const webJson = webRes.ok ? await webRes.json() : null;
    ...
    renderScreenListGameCoverPickerResults([
      ...
      { title: 'Web Cover Results', description: 'Top automatic web image matches for "title video game cover official."', results: webResults }
    ]);
  } catch (error) {
