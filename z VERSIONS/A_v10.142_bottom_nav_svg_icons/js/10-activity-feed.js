...
let activityNotificationsSourceSyncPromise = null;
let activityNotificationsOpenedAtMs = 0;
let activityNotificationsCache = new Map();
let activityNotificationsHasLoadedOnce = false;
let activityNotificationsInitialSyncComplete = false;
let activityNotificationsInitialSyncPromise = null;
let activityNotificationsCacheOwnerUid = '';
let activityNotificationsListenerUid = '';
let activityNotificationsHydrating = false;
let activityNotificationsHydratingFromEmpty = false;
let activityNotificationsLastRenderSignature = '';

function sanitizeActivityNotificationIdPart(value = '') {
  const clean = String(value || '').trim().replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '');
  return clean ? clean.slice(0, 72) : 'target';
}

function buildActivityNotificationDocId(options = {}) {
  const dedupeCommentId = Object.prototype.hasOwnProperty.call(options, 'dedupeTargetCommentId')
    ? options.dedupeTargetCommentId
    : options.targetCommentId;
  return [
    options.recipientUid,
    options.actorUid,
    options.type,
    options.targetActivityId,
    dedupeCommentId || ''
  ].map(sanitizeActivityNotificationIdPart).join('__');
}

function getActivityNotificationActorProfile(actorUid = '') {
  const uid = String(actorUid || currentUser?.uid || '').trim();
  const actor = usersMap?.[uid] || (currentUser?.uid === uid ? (userProfile || currentUser || {}) : {}) || {};
  const actorName = getDisplayName(actor, actor.displayName || actor.name || currentUser?.displayName || 'Shelfd user');
  return {
    actorName,
    actorPhoto: actor.photo || actor.photoURL || currentUser?.photoURL || ''
  };
}

function getActivityNotificationMedia(activity = {}) {
  const item = activity.item || {};
  ...
  const poster = getScreenListActivityItemCover(item)
    || content.trailerCover
    || content.poster
    || content.thumbnail
    || '';
  return {
    mediaTitle: title || 'Activity post',
    mediaPoster: poster || '',
    mediaKey: activity.mediaKey || getScreenListActivityPostMediaKey(item) || ''
  };
}
...
async function createActivityNotification(options = {}) {
  ...
  const targetActivityId = String(options.targetActivityId || '').trim();
  if (!recipientUid || !actorUid || !type || !targetActivityId) return false;
  if (recipientUid === actorUid) return false;

  const activity = options.activity || {};
  const media = { ...getActivityNotificationMedia(activity), ...(options.media || {}) };
  const actor = getActivityNotificationActorProfile(actorUid);
  const notificationId = buildActivityNotificationDocId({
    ...options,
    recipientUid,
    actorUid,
    type,
  ...
  const eventMs = Number(options.createdAtMs || options.timestamp || nowMs) || nowMs;
  ...
  if (options.skipIfExists) {
    try {
      ...
      console.warn('Notification duplicate check failed:', error);
    }
  }
  const payload = {
    notificationId,
    ...
    type,
    targetActivityId,
    targetKind: String(options.targetKind || 'activity').trim(),
    targetCollection: String(options.targetCollection || '').trim(),
    targetCommentId: String(options.targetCommentId || '').trim(),
    targetReplyId: String(options.targetReplyId || '').trim(),
    targetUid: String(options.targetUid || '').trim(),
    mediaTitle: String(media.mediaTitle || 'Activity post').trim(),
    mediaPoster: String(media.mediaPoster || '').trim(),
    mediaKey: String(media.mediaKey || '').trim(),
  ...
  try {
    ...
    console.error('Activity notification write failed:', error);
    return false;
  }
}
...
async function markActivityNotificationsRead(notifications = []) {
  if (!currentUser || !Array.isArray(notifications) || !notifications.length) return;
  const unread = notifications.filter(item => item && item.notificationId && item.read !== true).slice(0, 100);
  if (!unread.length) return;
  try {
    const batch = db.batch();
    unread.forEach(item => {
      const ref = db.collection('notifications').doc(currentUser.uid).collection('items').doc(item.notificationId);
      batch.set(ref, {
        read: true,
        readAtMs: Date.now(),
        readAt: firebase.firestore.FieldValue.serverTimestamp()
    ...
    await batch.commit();
    activityNotificationsUnreadCount = 0;
    window.activityNotificationsUnreadCount = 0;
...
function getActivityNotificationText(notification = {}) {
  const actor = notification.actorName || 'Someone';
  const title = notification.mediaTitle || 'your activity';
  if (notification.type === 'feed_like') return `${actor} liked your post`;
  if (notification.type === 'activity_like') return `${actor} liked your activity on ${title}`;
  if (notification.type === 'activity_comment') return `${actor} commented on ${title}`;
  if (notification.type === 'comment_reply') return `${actor} replied to your comment on ${title}`;
  if (notification.type === 'comment_like') return `${actor} liked your comment on ${title}`;
  ...
  if (notification.type === 'shared_watch_request') return `${actor} sent a Shared Watch request for ${title}`;
  return `${actor} interacted with ${title}`;
}

function getActivityNotificationAccentClass(type = '') {
...
function buildActivityNotificationRowHTML(notification = {}) {
  ...
  const actorPhoto = notification.actorPhoto || '';
  const mediaTitle = notification.mediaTitle || '';
  const mediaPoster = notification.mediaPoster || '';
  const time = relativeTime(notification.createdAtMs || notification.updatedAtMs || notification.createdAt);
  const unread = notification.read !== true;
  const accent = getActivityNotificationAccentClass(notification.type || '');
  const avatarHtml = actorPhoto
    ? `<img src="${escAttr(actorPhoto)}" alt="" loading="lazy" decoding="async">`
    : `<span>${escHtml(actorInitial)}</span>`;
  const posterHtml = mediaPoster
    ? `<div class="activity-notification-thumb"><img src="${escAttr(mediaPoster)}" alt="" loading="lazy" decoding="async"></div>`
    : `<div class="activity-notification-thumb fallback">${escHtml((mediaTitle || 'S').charAt(0).toUpperCase())}</div>`;
  return `<button class="activity-notification-row ${unread ? 'unread' : 'read'} accent-${accent}" type="button" onclick="openActivityNotificationTarget('${escAttr(id)}')" data-notification-id="${escAttr(id)}">
    <div class="activity-notification-avatar">${avatarHtml}</div>
    <div class="activity-notification-copy">
      <div class="activity-notification-text">${escHtml(getActivityNotificationText(notification))}</div>
      ${notification.textSnippet ? `<div class="activity-notification-snippet">${escHtml(notification.textSnippet)}</div>` : ''}
      <time class="activity-notification-time">${escHtml(time)}</time>
    </div>
    ${posterHtml}
  </button>`;
}
...
async function loadActivityNotifications() {
  ...
  const snap = await db.collection('notifications')
    ...
    .limit(60)
    .get();
  return setActivityNotificationsCache(snap.docs.map(doc => ({ notificationId: doc.id, ...(doc.data() || {}) })));
}

let activityNotificationsBackfillPromise = null;

function getActivityNotificationEventMs(value, fallback = Date.now()) {
  const parsed = parseFriendActivityTime(value);
  return parsed || fallback || Date.now();
}

function getBackfillReplyId(reply = {}, index = 0) {
  return getFeedReplyStableId(reply, index);
}

async function backfillReplyNotificationsForCurrentUser(activity = {}, replies = [], options = {}) {
  if (!currentUser || !Array.isArray(replies) || !replies.length) return 0;
  const targetActivityId = String(options.targetActivityId || activity.id || activity.postId || '').trim();
  if (!targetActivityId) return 0;
  const collection = String(options.targetCollection || activity._collection || '').trim();
  const targetKind = options.targetKind || (collection === 'feed' ? 'feed' : 'activity');
  const normalized = replies.map((reply, index) => ({ ...(reply || {}), id: getBackfillReplyId(reply, index) }));
  const byId = new Map(normalized.map(reply => [String(reply.id), reply]));
  let created = 0;
  for (const reply of normalized) {
    const actorUid = String(reply.uid || '').trim();
    if (!actorUid) continue;
    const parentReplyId = getFeedReplyParentId(reply);
    const eventMs = getActivityNotificationEventMs(reply.timestamp, getActivityNotificationEventMs(activity.timestamp || activity.createdAt || activity.updatedAt));
    if (parentReplyId) {
      const parent = byId.get(parentReplyId);
      if (String(parent?.uid || '') === currentUser.uid && actorUid !== currentUser.uid) {
        if (await createActivityNotification({
          ...
          targetCommentId: parentReplyId,
          targetReplyId: reply.id,
          dedupeTargetCommentId: parentReplyId,
          activity,
    ...
    } else if (getActivityNotificationOwnerUid(activity) === currentUser.uid && actorUid !== currentUser.uid) {
      if (await createActivityNotification({
        recipientUid: currentUser.uid,
        actorUid,
    ...
    if (actorUid === currentUser.uid && likes.length) {
      for (const likeUid of likes) {
        const cleanLikeUid = String(likeUid || '').trim();
        if (!cleanLikeUid || cleanLikeUid === currentUser.uid) continue;
        if (await createActivityNotification({
          recipientUid: currentUser.uid,
          actorUid: cleanLikeUid,
          type: 'comment_like',
          targetActivityId,
          targetKind,
          targetCollection: collection,
...
async function backfillOwnedFeedNotificationsForCurrentUser() {
  ...
  try {
    ...
    for (const doc of snap.docs) {
      ...
      const targetActivityId = String(post.postId || post.id || doc.id);
      const fallbackMs = getActivityNotificationEventMs(post.timestamp || post.createdAt || post.updatedAt);
      const likes = Array.isArray(post.likes) ? post.likes : [];
      for (const likeUid of likes) {
        const actorUid = String(likeUid || '').trim();
        if (!actorUid || actorUid === currentUser.uid) continue;
        if (await createActivityNotification({
          recipientUid: currentUser.uid,
          actorUid,
          type: 'feed_like',
      ...
      created += await backfillReplyNotificationsForCurrentUser(post, Array.isArray(post.replies) ? post.replies : [], {
        targetActivityId,
        targetCollection: 'feed',
        targetKind: 'feed'
      });
    ...
    console.warn('Feed notification backfill skipped:', error);
  }
  return created;
}

function getCachedOwnedActivityBackfillCandidates() {
  const byId = new Map();
  const push = (activity = {}, fallbackId = '') => {
    if (!activity || String(activity.uid || '') !== currentUser?.uid) return;
    if (activity.type === 'post' || activity.type === 'trailer' || activity.type === 'card-comment') return;
    const id = String(activity.renderedActivityId || activity.activityId || activity.originalActivityId || getStableActivityDocId(activity, fallbackId) || fallbackId || '').trim();
    if (!id) return;
    byId.set(id, { ...activity, id });
  };
  (friendActivityCache?.activities || []).forEach((activity, index) => push(activity, `owned-${index}`));
  Object.entries(friendActivityClickTargets || {}).forEach(([id, activity]) => push(activity, id));
  return [...byId.values()]
    .sort((a, b) => getActivityNotificationEventMs(b.timestamp || b.item?.dateAdded, 0) - getActivityNotificationEventMs(a.timestamp || a.item?.dateAdded, 0))
    .slice(0, 80);
}

async function backfillOwnedLibraryActivityNotificationsForCurrentUser() {
  if (!currentUser || !db) return 0;
  let created = 0;
  const candidates = getCachedOwnedActivityBackfillCandidates();
  for (const activity of candidates) {
    const targetActivityId = String(activity.id || activity.renderedActivityId || '').trim();
    if (!targetActivityId) continue;
    const stableId = getStableActivityDocId(activity, targetActivityId) || targetActivityId;
    const interactionDocId = getActivityInteractionMetaDocId(stableId);
    try {
      const snap = await db.collection('meta').doc(interactionDocId).get();
      if (!snap.exists) continue;
      ...
      const fallbackMs = getActivityNotificationEventMs(activity.timestamp || activity.item?.dateModified || activity.item?.dateAdded);
      const likes = Array.isArray(interaction.likes) ? interaction.likes : [];
      for (const likeUid of likes) {
        const actorUid = String(likeUid || '').trim();
        if (!actorUid || actorUid === currentUser.uid) continue;
        if (await createActivityNotification({
      ...
      created += await backfillReplyNotificationsForCurrentUser(merged, Array.isArray(interaction.replies) ? interaction.replies : [], {
        targetActivityId,
        targetCollection: 'meta',
        targetKind: 'activity'
      });
    } catch (error) {
      console.warn('Activity interaction notification backfill skipped:', stableId, error);
    }
  }
  return created;
}

async function backfillSocialRequestNotificationsForCurrentUser() {
  ...
  for (const uid of incoming) {
    ...
    if (await createActivityNotification({
      ...
      actorUid,
      type: 'friend_request',
      targetActivityId: `friend-request-${actorUid}`,
      targetKind: 'friend_request',
      targetUid: actorUid,
      media: { mediaTitle: 'Friend request', mediaPoster: '', mediaKey: `friend:${actorUid}` },
      createdAtMs: nowMs,
      backfilled: true,
      skipIfExists: true
    })) created += 1;
  }
  const accepted = Array.isArray(acceptedFriendSourceUids) ? acceptedFriendSourceUids : [];
  for (const uid of accepted) {
    const actorUid = String(uid || '').trim();
    if (!actorUid || actorUid === currentUser.uid || !friends.includes(actorUid)) continue;
    if (await createActivityNotification({
      recipientUid: currentUser.uid,
      actorUid,
      type: 'friend_accept',
      targetActivityId: `friend-accept-${actorUid}`,
      targetKind: 'friend_request',
      targetUid: actorUid,
      media: { mediaTitle: 'Friend accepted', mediaPoster: '', mediaKey: `friend:${actorUid}` },
      createdAtMs: nowMs,
      backfilled: true,
      skipIfExists: true
    })) created += 1;
  ...
  const sharedRequests = Array.isArray(watchTogetherIncomingRequests) ? watchTogetherIncomingRequests : [];
  for (const group of sharedRequests) {
    const actorUid = String(group?.ownerUid || '').trim();
    const groupId = String(group?.id || '').trim();
    if (!actorUid || actorUid === currentUser.uid || !groupId) continue;
    if (await createActivityNotification({
      recipientUid: currentUser.uid,
      actorUid,
      type: 'shared_watch_request',
      targetActivityId: groupId,
      targetKind: 'shared_watch',
      targetCollection: 'users',
      activity: {
        uid: actorUid,
        item: { title: group.title || 'Shared Watch', cover: group.cover || '', librarySection: group.section || group.librarySection || '' },
        mediaKey: group.mediaKey || group.titleKey || groupId
      },
      media: { mediaTitle: group.title || 'Shared Watch', mediaPoster: group.cover || '', mediaKey: group.mediaKey || group.titleKey || groupId },
      createdAtMs: Number(group.createdAtMs || group.updatedAtMs || nowMs) || nowMs,
      backfilled: true,
      skipIfExists: true
    })) created += 1;
  }
...
async function backfillExistingActivityNotificationsForCurrentUser() {
  if (!currentUser || !db) return 0;
  if (activityNotificationsBackfillPromise) return activityNotificationsBackfillPromise;
  activityNotificationsBackfillPromise = (async () => {
    let created = 0;
    created += await backfillOwnedFeedNotificationsForCurrentUser();
    created += await backfillOwnedLibraryActivityNotificationsForCurrentUser();
    created += await backfillSocialRequestNotificationsForCurrentUser();
    return created;
  })().finally(() => {
    activityNotificationsBackfillPromise = null;
  });
  return activityNotificationsBackfillPromise;
}

async function syncActivityNotificationsFromSources() {
  if (!currentUser || !db) return 0;
  if (activityNotificationsSourceSyncPromise) return activityNotificationsSourceSyncPromise;
  activityNotificationsSourceSyncPromise = backfillExistingActivityNotificationsForCurrentUser()
    .catch(error => {
      console.warn('Activity notification source sync failed:', error);
      return 0;
    })
    .finally(() => {
      activityNotificationsSourceSyncPromise = null;
    });
  return activityNotificationsSourceSyncPromise;
}

function getActivityNotificationSortMs(notification = {}) {
  return Number(notification.createdAtMs || notification.updatedAtMs || 0) || getActivityNotificationEventMs(notification.createdAt || notification.updatedAt, 0);
}

function setActivityNotificationsCache(notifications = []) {
  activityNotificationsCache = new Map();
  (Array.isArray(notifications) ? notifications : []).forEach(item => {
    const id = String(item?.notificationId || '').trim();
    if (id) activityNotificationsCache.set(id, item);
  });
  const list = getCachedActivityNotificationsList();
  activityNotificationsUnreadCount = list.filter(item => item.read !== true).length;
  window.activityNotificationsUnreadCount = activityNotificationsUnreadCount;
  if (typeof updateRequestsBadges === 'function') updateRequestsBadges();
  return list;
}

function mergeActivityNotificationsCache(notifications = []) {
  (Array.isArray(notifications) ? notifications : []).forEach(item => {
    const id = String(item?.notificationId || '').trim();
    if (!id) return;
    activityNotificationsCache.set(id, {
      ...(activityNotificationsCache.get(id) || {}),
      ...item,
      notificationId: id
    });
  });
...
function getCachedActivityNotificationsList() {
  return [...activityNotificationsCache.values()]
    .sort((a, b) => getActivityNotificationSortMs(b) - getActivityNotificationSortMs(a))
    .slice(0, 80);
}

function ensureActivityNotificationsCacheOwner() {
  const nextOwnerUid = String(currentUser?.uid || '').trim();
  if (activityNotificationsCacheOwnerUid === nextOwnerUid) return;
  activityNotificationsCacheOwnerUid = nextOwnerUid;
  activityNotificationsCache = new Map();
  activityNotificationsHasLoadedOnce = false;
  activityNotificationsInitialSyncComplete = false;
  activityNotificationsInitialSyncPromise = null;
  activityNotificationsHydrating = false;
  activityNotificationsHydratingFromEmpty = false;
  activityNotificationsLastRenderSignature = '';
  activityNotificationsUnreadCount = 0;
  window.activityNotificationsUnreadCount = 0;
  if (typeof updateRequestsBadges === 'function') updateRequestsBadges();
}

function getActivityNotificationsRenderSignature(mode = 'list', list = []) {
  if (mode !== 'list') return `${mode}:${activityNotificationsCacheOwnerUid}:${currentUser?.uid || ''}`;
  const ids = (Array.isArray(list) ? list : []).map(item => `${item.notificationId || ''}:${item.read === true ? 1 : 0}:${getActivityNotificationSortMs(item)}`).join('|');
  return `list:${activityNotificationsCacheOwnerUid}:${ids}`;
}

function renderActivityNotificationsLoading() {
  const feed = document.getElementById('activity-notifications-feed');
  if (!feed) return;
  const signature = getActivityNotificationsRenderSignature('loading');
  if (activityNotificationsLastRenderSignature === signature) return;
  activityNotificationsLastRenderSignature = signature;
  const headerHtml = buildActivityFeedHeaderHTML('Notifications');
...
function renderActivityNotificationsList(notifications = null) {
  const feed = document.getElementById('activity-notifications-feed');
  if (!feed) return;
  const headerHtml = buildActivityFeedHeaderHTML('Notifications');
  const list = Array.isArray(notifications) ? notifications : getCachedActivityNotificationsList();
  const shouldShowLoading = !list.length && (
    !activityNotificationsHasLoadedOnce
    || (activityNotificationsHydrating && activityNotificationsHydratingFromEmpty)
    || (!activityNotificationsInitialSyncComplete && activityNotificationsInitialSyncPromise)
  );
  if (shouldShowLoading) {
    renderActivityNotificationsLoading();
    return;
  }
  if (!list.length) {
    const signature = getActivityNotificationsRenderSignature('empty');
    if (activityNotificationsLastRenderSignature === signature) return;
    activityNotificationsLastRenderSignature = signature;
    feed.innerHTML = `${headerHtml}<div class="activity-notifications-empty"><strong>No notifications yet</strong><span>Likes, comments, replies, and shared activity will show up here.</span></div>`;
    return;
  }
  const signature = getActivityNotificationsRenderSignature('list', list);
  if (activityNotificationsLastRenderSignature === signature) return;
  activityNotificationsLastRenderSignature = signature;
  feed.innerHTML = `${headerHtml}<div class="activity-notifications-shell">${list.map(buildActivityNotificationRowHTML).join('')}</div>`;
}

function stopActivityNotificationsLiveListener() {
  if (activityNotificationsUnsubscribe) {
    try { activityNotificationsUnsubscribe(); } catch (error) {}
    activityNotificationsUnsubscribe = null;
  }
  activityNotificationsListenerUid = '';
}

function ensureActivityNotificationsInitialSync() {
  if (!currentUser || !db) return Promise.resolve(0);
  if (activityNotificationsInitialSyncComplete) return Promise.resolve(0);
  if (activityNotificationsInitialSyncPromise) return activityNotificationsInitialSyncPromise;
  activityNotificationsHydratingFromEmpty = getCachedActivityNotificationsList().length === 0;
  activityNotificationsHydrating = true;
  activityNotificationsInitialSyncPromise = (async () => {
    let created = 0;
    try {
      created = await syncActivityNotificationsFromSources();
    } catch (error) {
      console.warn('Activity notification initial sync failed:', error);
    }
    try {
      await loadActivityNotifications();
    } catch (error) {
      console.warn('Activity notification refresh after sync failed:', error);
    }
    return created;
  })().finally(() => {
    activityNotificationsHydrating = false;
    activityNotificationsHydratingFromEmpty = false;
    activityNotificationsInitialSyncComplete = true;
    activityNotificationsInitialSyncPromise = null;
    if (activeFriendsTab === 'activity' && activeActivitySubTab === 'notifications') {
      renderActivityNotificationsList();
    }
  });
  return activityNotificationsInitialSyncPromise;
}

function startActivityNotificationsLiveListener() {
  if (!currentUser || !db) return;
  ensureActivityNotificationsCacheOwner();
  const currentUid = String(currentUser.uid || '').trim();
  if (activityNotificationsUnsubscribe && activityNotificationsListenerUid && activityNotificationsListenerUid !== currentUid) {
    stopActivityNotificationsLiveListener();
  }
  if (!activityNotificationsUnsubscribe) {
    activityNotificationsListenerUid = currentUid;
    activityNotificationsUnsubscribe = db.collection('notifications')
      ...
      .limit(60)
      .onSnapshot(snapshot => {
        activityNotificationsHasLoadedOnce = true;
        const notifications = setActivityNotificationsCache(snapshot.docs.map(doc => ({ notificationId: doc.id, ...(doc.data() || {}) })));
        if (activeFriendsTab === 'activity' && activeActivitySubTab === 'notifications') {
          if (!(activityNotificationsHydrating && activityNotificationsHydratingFromEmpty)) {
            renderActivityNotificationsList(notifications);
          }
        }
      }, error => {
        console.warn('Activity notifications live listener failed:', error);
      });
  }
}
...
async function renderActivityNotificationsPage() {
  ...
  activityNotificationsOpenedAtMs = Date.now();
  try {
    ensureActivityNotificationsCacheOwner();
    startActivityNotificationsLiveListener();
    renderActivityNotificationsList();
    ensureActivityNotificationsInitialSync();
  } catch (error) {
    console.error('Could not load activity notifications:', error);
    feed.innerHTML = `${headerHtml}<div class="activity-notifications-empty"><strong>Could not load notifications</strong><span>Try reopening this tab in a moment.</span></div>`;
  }
}
...
async function openActivityNotificationTarget(notificationId = '') {
  ...
  try {
    const ref = db.collection('notifications').doc(currentUser.uid).collection('items').doc(notificationId);
    const snap = await ref.get();
    if (snap.exists) notification = { notificationId: snap.id, ...(snap.data() || {}) };
    await markActivityNotificationRead(notificationId);
  } catch (error) {
    console.warn('Could not open notification target:', error);
  }
  if (!notification) return;
  const targetId = String(notification.targetActivityId || '').trim();
  if (!targetId) return;
  if (notification.targetKind === 'friend_request' && notification.actorUid) {
    if (typeof openActivityUserList === 'function') {
      openActivityUserList(String(notification.actorUid || ''));
    }
    return;
  }
  if (notification.targetKind === 'shared_watch') {
    if (typeof switchActivitySubTab === 'function') await switchActivitySubTab('sharedWatch');
    return;
  }
  if (typeof switchActivitySubTab === 'function') await switchActivitySubTab('feed');
  window.setTimeout(() => {
    const kind = notification.targetKind === 'feed' || notification.targetCollection === 'feed' ? 'feed' : 'activity';
    if (kind === 'feed') openFeedPostPage(targetId);
    else openActivityReplyPage(targetId);
  }, 120);
}
  ...
  try {
    ...
    if (!isLiked) {
      await createActivityNotification({
        ...
        type: target.collection === 'feed' ? 'feed_like' : 'activity_like',
...
async function toggleFeedReplyLike(postId, replyId, btnEl) {
  ...
  try {
    ...
    if (willBeLiked) {
      await createActivityNotification({
        recipientUid: String(target.uid || '').trim(),
        type: 'comment_like',
        targetActivityId: String(currentFeedPostActivityId || postId).trim(),
        targetKind: collection === 'feed' ? 'feed' : 'activity',
        targetCollection: collection,
        targetCommentId: replyId,
        activity: { ...(currentFeedPostActivityData || {}), ...data, id: String(currentFeedPostActivityId || postId).trim(), _collection: collection },
