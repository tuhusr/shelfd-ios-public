...
function getScreenListTrashIconSvg() {
  return `<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M6 6l1 17h10l1-17"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>`;
}
...
// Collapse: each compact card slides upward into the main card, bottom-to-top stagger
function _slCollapseStack(wrapper, panel, inner) {
  const sp = (el, prop, val) => el.style.setProperty(prop, val, 'important');
  const cp = (el, ...props) => props.forEach(p => el.style.removeProperty(p));
  const cards = Array.from(panel.querySelectorAll('.sl-activity-stack-compact-card'));
  if (!cards.length) {
    wrapper.classList.remove('is-expanded');
    cp(panel, 'height', 'margin-top', 'overflow');
    return;
  }
  // Read phase: measure all positions before any style writes
  const h = panel.scrollHeight;
  const panelTop = panel.getBoundingClientRect().top;
  const cardData = cards.map(c => {
    const r = c.getBoundingClientRect();
    return { top: r.top - panelTop, h: r.height };
  });
  // Write phase: cancel any in-progress expand animation, lock panel height
  cp(inner, 'will-change', 'transition', 'transform', 'opacity');
  sp(panel, 'height', h + 'px');
  sp(panel, 'overflow', 'hidden');
  const N = cards.length;
  const dur = 260, stagger = 22;
  // Snap each card to its current CSS state (cancels any CSS transitions in-flight)
  cards.forEach(c => {
    sp(c, 'transition', 'none');
    sp(c, 'will-change', 'transform, opacity');
  });
  // Next frame: animate cards upward with bottom-to-top stagger (composited only)
  requestAnimationFrame(() => {
    cards.forEach((c, i) => {
      // i=0 = top card (starts last), i=N-1 = bottom card (starts first)
      const delay = (N - 1 - i) * stagger;
      // Target: slide card fully above the panel's clip boundary
      const yTarget = -(cardData[i].top + cardData[i].h + 6);
      sp(c, 'transition',
        'transform ' + dur + 'ms ' + delay + 'ms cubic-bezier(.4,0,.6,1), ' +
        'opacity ' + Math.round(dur * 0.85) + 'ms ' + delay + 'ms ease-in');
      sp(c, 'transform', 'translateY(' + yTarget + 'px) scale(0.94)');
      sp(c, 'opacity', '0');
    });
  });
  // Wait for the top card (longest delay, finishes last) then collapse DOM
  ...
  const finish = () => {
    ...
    cards.forEach(c => cp(c, 'will-change', 'transition', 'transform', 'opacity'));
    wrapper.classList.remove('is-expanded');
    cp(panel, 'height', 'margin-top', 'overflow');
  };
  const onEnd = e => { if (e.propertyName === 'transform') finish(); };
  topCard.addEventListener('transitionend', onEnd);
  setTimeout(finish, dur + (N - 1) * stagger + 80); // safety fallback
}
...
function buildImportActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  const rows = items.map(item => {
    ...
    const importedWhere = [sourceLabel, status].filter(Boolean).join(' · ');
    return `<div class="import-activity-row">
      ...
      <div class="import-activity-row-copy"><strong>${escHtml(item.title || item.name || 'Untitled')}</strong><span>${escHtml(importedWhere || 'Imported')}</span></div>
    </div>`;
  }).join('');
  ...
  return `<article class="shelfd-social-card import-activity-card" data-import-activity-card="1" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}">
    ...
    <div class="import-activity-list" role="region" aria-label="Imported titles list">
      <button class="import-activity-collapse-float" type="button" onclick="event.stopPropagation(); toggleImportActivityCard(this, false)" aria-expanded="true">Collapse</button>
      <div class="import-activity-list-inner">${rows}</div>
    </div>
  </article>`;
}
