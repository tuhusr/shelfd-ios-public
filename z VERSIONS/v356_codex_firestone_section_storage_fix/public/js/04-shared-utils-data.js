...
function getOwnDataFirestoreRootPayload(safeData) {
  const sectionCounts = {};
  SCREENLIST_SECTIONS.forEach(section => {
    sectionCounts[section] = Array.isArray(safeData[section]) ? safeData[section].length : 0;
  });
  return {
    storageVersion: 2,
    sectionCounts,
    shows: firebase.firestore.FieldValue.delete(),
    movies: firebase.firestore.FieldValue.delete(),
    anime: firebase.firestore.FieldValue.delete(),
    games: firebase.firestore.FieldValue.delete(),
    manga: firebase.firestore.FieldValue.delete(),
    books: firebase.firestore.FieldValue.delete(),
    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
  };
}

function getOwnDataSectionFirestorePayload(section = '', items = []) {
  return {
...
function getWatchlistSectionDocRef(docRef, section = '') {
  if (!docRef || !section) return null;
  return docRef.collection('sections').doc(section);
}
...
async function loadWatchlistDataFromDocRef(docRef, fallbackData = null) {
  ...
  try {
    ...
    const sectionSnaps = await Promise.all(
      SCREENLIST_SECTIONS.map(section => getWatchlistSectionDocRef(docRef, section).get())
    );
    const hasSectionDocs = sectionSnaps.some(snap => snap.exists);
    if (!rootSnap.exists && !hasSectionDocs) return getEmptyListData();
    if (!hasSectionDocs) return parseOwnFirestoreData(rootData);
    const merged = getEmptyListData();
    SCREENLIST_SECTIONS.forEach((section, index) => {
      const sectionSnap = sectionSnaps[index];
      if (sectionSnap.exists) {
        const sectionData = sectionSnap.data() || {};
        try {
          merged[section] = Array.isArray(sectionData.items)
            ? sectionData.items
            : JSON.parse(sectionData.items || '[]');
        } catch (e) {
          merged[section] = [];
        }
      } else {
        try {
          merged[section] = rootData[section] ? JSON.parse(rootData[section]) : [];
        } catch (e) {
          merged[section] = [];
        }
      }
...
async function persistOwnDataToFirestore(safeData, options = {}) {
  ...
  batch.set(DOC_REF, getOwnDataFirestoreRootPayload(safeData), { merge: true });
  SCREENLIST_SECTIONS.forEach(section => {
    const sectionRef = getWatchlistSectionDocRef(DOC_REF, section);
    batch.set(sectionRef, getOwnDataSectionFirestorePayload(section, safeData[section] || []), { merge: true });
  });
  await batch.commit();
...
async function verifyOwnDataDirectWrite(expectedData) {
  ...
  if (!isSameListData(storedData, expectedData)) {
