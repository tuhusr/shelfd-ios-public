...
(function() {
  ...
  function scoreTrending(item, ctx) {
    /* Trending order from TMDB matters most. We piggyback on TMDB-supplied
       order via 1 - (index/total) when an index is provided in ctx (set by
       the caller for trending feeds); otherwise fall back to popularity. */
    const orderSignal = (ctx.indexOf && typeof ctx.indexOf === 'function')
      ? clamp01(1 - (ctx.indexOf(item) / Math.max(1, ctx.total - 1)))
      : ctx.popN(item);
    return (
      orderSignal * 0.65 +
      ctx.popN(item) * 0.20 +
  ...
  function scorePopular(item, ctx) {
    return (
      ...
      clamp01(getEffectiveRating(item) / 10) * 0.20
    );
  }
