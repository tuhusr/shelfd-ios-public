...
(function() {
  ...
  /* v675: bump cache namespace after fixing mixed-type batch key collisions.
     Old v3 entries may contain a valid IMDb payload attached to the wrong
     title, so they must not be reused. */
  const LS_PREFIX = 'shelfd:imdb-rating:v4:';
  ...
  async function enrichItemsWithImdbRatings(items = [], options = {}) {
    ...
    await Promise.all(chunks.map(async chunk => {
      ...
      chunk.forEach(({ item, tmdbId, type, imdbId }) => {
        ...
        const legacyKey = String(tmdbId || '').trim();
        const imdbKey = getBatchResultKey('', type, imdbId, '');
        const r = ratings[typedKey] || (legacyKey ? ratings[legacyKey] : null) || (imdbKey ? ratings[imdbKey] : null);
        if (r && r.ok) {
