...
function buildStackedActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  html = html.replace('class="shelfd-social-card ', 'class="shelfd-social-card sl-activity-stacked-card ');
  ...
  const stackHtml = `
    <div class="sl-activity-stack-shadow sl-activity-stack-shadow-one" aria-hidden="true"></div>
    <div class="sl-activity-stack-shadow sl-activity-stack-shadow-two" aria-hidden="true"></div>
    ${extraCount > 0 ? `<button class="sl-activity-stack-count" type="button" onclick="event.stopPropagation(); openScreenListStackedActivityPage('${escAttr(activityId)}')" aria-label="Open ${extraCount} more grouped activities">+${extraCount}</button>` : ''}`;
  return html.replace('</article>', `${stackHtml}</article>`);
}
