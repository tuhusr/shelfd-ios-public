...
/* v589: save inline bio edits back to userProfile + Firestore */
function saveMyListProfileBio(el) {
  ...
  // Persist to Firestore via the existing profile save path
  if (typeof saveProfileToFirestore === 'function') {
    saveProfileToFirestore({ bio: text }).catch(e => console.warn('Bio save failed:', e));
  } else if (typeof saveProfile === 'function') {
    saveProfile().catch(e => console.warn('Bio save failed:', e));
  }
}
...
function openProfileFavoritePicker(event, card) {
  ...
  if (card.classList.contains('profile-manual-slot')) {
    // v465: fictional character slots use a dedicated editor with Tavily image
    // search and upload+poster-crop. Other manual favorites (musicArtists)
    // keep the simple legacy form.
    if (card.dataset.manualSection === 'fictionalCharacters' && typeof openProfileCharacterEditor === 'function') {
...
function openProfileCharacterEditor(event, card) {
  if (isViewingOtherProfile() || !card) return;
  if (event) event.stopPropagation();
  const slotIndex = Number(card.dataset.manualIndex || 0);
  profileCharacterEditorState = {
    ...
    pendingImageSource: '',
    cropState: null,
    searchQuery: (card.dataset.manualName || '').trim(),
    searchResults: [],
    searchStatus: ''
  };
...
function renderProfileCharacterEditor() {
  ...
  const slotLabel = `Character ${state.slotIndex + 1}`;
    ...
    <div class="profile-character-editor-modal" role="dialog" aria-modal="true" aria-label="Edit ${slotLabel}">
      <div class="profile-character-editor-head">
        <div class="profile-character-editor-title-wrap">
          <div class="profile-character-editor-kicker">Top 3 Fictional Characters</div>
      ...
      <div class="profile-character-editor-body">
        <div class="profile-character-preview-row">
          ...
          <div class="profile-character-preview-meta">
            <label class="profile-character-field-label" for="profile-character-name-input">Character name</label>
            <input id="profile-character-name-input" class="profile-character-field-input" type="text" placeholder="e.g. Miles Morales" value="${escAttr(state.name)}" autocomplete="off" oninput="handleProfileCharacterNameInput(this.value)">
          </div>
        </div>
        ...
        ${cropPanel || `
          <div class="profile-character-section">
            <div class="profile-character-section-head">
              ...
              <div class="profile-character-section-sub">Find a character image from Tavily.</div>
            </div>
            <form class="profile-character-search-form" onsubmit="event.preventDefault(); runProfileCharacterImageSearch();">
              <input id="profile-character-search-input" class="profile-character-field-input" type="text" placeholder="Search character or actor..." value="${escAttr(state.searchQuery)}" autocomplete="off">
  ...
  overlay.classList.add('open');
  if (!state.cropState) {
    setTimeout(() => document.getElementById('profile-character-name-input')?.focus(), 30);
  }
}
...
function handleProfileCharacterNameInput(value) {
  ...
  profileCharacterEditorState.name = String(value || '').trim();
}
    ...
    <div class="profile-character-section">
      ...
      <div class="profile-character-crop-controls">
        ...
        <div class="profile-character-crop-buttons">
          ...
          <button type="button" class="profile-character-editor-save" onclick="confirmProfileCharacterCrop()">Use this crop</button>
        </div>
      </div>
    </div>
  `;
...
function confirmProfileCharacterCrop() {
  ...
  const crop = state.cropState;
  const canvas = document.createElement('canvas');
  canvas.width = crop.FRAME_W;
  canvas.height = crop.FRAME_H;
  const ctx = canvas.getContext('2d');
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  // Image draw position/size in frame pixel space
  const scaledW = crop.img.width * crop.baseScale * crop.zoom;
  const scaledH = crop.img.height * crop.baseScale * crop.zoom;
  ctx.drawImage(crop.img, crop.offsetX, crop.offsetY, scaledW, scaledH);
...
function saveProfileCharacterFromEditor() {
  ...
  // No-rating slot — leave manualRating empty for fictional characters.
  state.card.dataset.manualRating = '';
  updateProfileManualCardPreview(state.card);
  if (userProfile) readProfileDraftFromPage(userProfile);
