...
(function() {
  ...
  function renderResultRow(r, index) {
    /* v646: result-row layout is now strictly 3 stacked lines:
        1. title
        2. year
        3. media type (Movie / TV Show / Anime / Game)
       Rating and overview removed per spec. */
    ...
    const yearHtml = r.year
      ? `<span class="shelfd-search-row-year">${escHtml(r.year)}</span>`
      : '';
    const typeHtml = typeLabel
      ? `<span class="shelfd-search-row-type">${escHtml(typeLabel)}</span>`
      : '';
