...
function toggleEpisodes(id) {
  ...
  if (open) {
    // v773: defer focus-mode teardown until AFTER the close height transition
    // completes. Removing `ep-list-focused-host` synchronously released the
    // `height: 460px !important` constraint on `.ep-list-inner`, which caused
    // the inner to reflow to its natural (much taller) content height in the
    // same frame the parent .ep-list height transition was starting — that
    // mid-close reflow propagated up to the title card as a visible jolt.
    // Holding the focused-host classes in place during the 300ms close means
    // the parent height transitions cleanly from a stable start value, with
    // no inner reflow fighting it.
    const section = activeSection;
    window.setTimeout(() => {
      shelfdExitSeasonFocusMode(id);
      flushScreenListDeferredEpisodeStatus(id, section);
    }, 380);
  }
}
