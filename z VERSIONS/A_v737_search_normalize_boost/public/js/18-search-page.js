...
(function() {
  ...
  async function runSearch(rawQuery) {
    ...
    /* Logarithmic popularity boost — rewards well-known titles enough to
       leapfrog an obscure exact-match (Δ ≤ 400 between tiers). */
    function popularityBoost(p) {
      const n = Math.max(0, Number(p) || 0);
      if (!n) return 0;
      return Math.min(500, Math.log10(n + 1) * 80);
    }
    rows.sort((a, b) => {
