...
function setScreenListInlineActivityStackExpanded(wrapper, expanded) {
  ...
  // Reduce-motion: CSS height snap handles it, no JS animation needed
  ...
  if (expanded) {
    ...
    // Snap panel open (one layout frame), inner starts translated above clip boundary
    ...
    // Next frame: animate only composited properties — 120Hz-capable path
    requestAnimationFrame(() => {
      ...
      inner.addEventListener('transitionend', function onExpandEnd(e) {
        ...
        sp(panel, 'height', 'auto'); // restore responsive height
      });
    });

  } else {
    // Snapshot current height before class removal collapses CSS
    const h = panel.scrollHeight;
    sp(panel, 'height', h + 'px');
    sp(panel, 'overflow', 'hidden');
    sp(inner, 'will-change', 'transform, opacity');
    ...
    // Next frame: animate inner upward out of clip boundary
    requestAnimationFrame(() => {
      sp(inner, 'transition', 'transform 210ms cubic-bezier(.4, 0, .6, 1), opacity 170ms ease');
      sp(inner, 'transform', 'translateY(' + (-(h + 8)) + 'px)');
      sp(inner, 'opacity', '0');
      inner.addEventListener('transitionend', function onCollapseEnd(e) {
        ...
        cp(inner, 'will-change', 'transition', 'transform', 'opacity');
        cp(panel, 'height', 'margin-top', 'overflow'); // CSS takes over (height:0)
      });
    });
  }
}
