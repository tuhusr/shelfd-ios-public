...
function ensureFullscreenTrailerControlStyles() {
  if (document.getElementById('shelfd-trailer-control-styles')) return;
  const style = document.createElement('style');
  style.id = 'shelfd-trailer-control-styles';
  style.textContent = `
    #trailer-fullscreen-container .shelfd-trailer-controls,
    #trailer-video-modal .shelfd-trailer-controls {
      position: absolute;
      left: 50%;
    ...
    #trailer-fullscreen-container .shelfd-trailer-controls.is-visible,
    #trailer-video-modal .shelfd-trailer-controls.is-visible {
      opacity: 1;
      pointer-events: auto;
      transform: translate3d(-50%, 0, 0);
    }
    #trailer-fullscreen-container .shelfd-trailer-mute-btn,
    #trailer-video-modal .shelfd-trailer-mute-btn {
      min-width: 118px;
      min-height: 44px;
    ...
    #trailer-fullscreen-container .shelfd-trailer-mute-btn:active,
    #trailer-video-modal .shelfd-trailer-mute-btn:active {
      transform: scale(.97);
    }
    #trailer-fullscreen-container .shelfd-trailer-mute-btn svg,
    #trailer-video-modal .shelfd-trailer-mute-btn svg {
      flex: 0 0 auto;
      color: #c4b5fd;
    }
    #trailer-fullscreen-container .shelfd-trailer-wake-layer,
    #trailer-video-modal .shelfd-trailer-wake-layer {
      position: absolute;
      inset: 72px 0 88px;
      z-index: 1000000;
      display: block;
    ...
    #trailer-fullscreen-container.shelfd-trailer-controls-visible .shelfd-trailer-wake-layer,
    #trailer-video-modal.shelfd-trailer-controls-visible .shelfd-trailer-wake-layer {
      pointer-events: none;
    }
  `;
...
function getFullscreenTrailerMuteIcon(muted = true) {
  return muted
    ? `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 5 6 9H3v6h3l5 4V5Z"></path><path d="m22 9-6 6"></path><path d="m16 9 6 6"></path></svg><span>Muted</span>`
    : `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 5 6 9H3v6h3l5 4V5Z"></path><path d="M15.5 8.5a5 5 0 0 1 0 7"></path><path d="M18.5 5.5a9 9 0 0 1 0 13"></path></svg><span>Sound On</span>`;
}

function renderFullscreenTrailerControls() {
  return `<div class="shelfd-trailer-controls is-visible" data-trailer-controls>
    <button class="shelfd-trailer-mute-btn" type="button" data-trailer-mute-btn onclick="toggleFullscreenTrailerMute(this)" aria-label="Unmute trailer" aria-pressed="true">${getFullscreenTrailerMuteIcon(true)}</button>
  </div>
  <button class="shelfd-trailer-wake-layer" type="button" data-trailer-wake-layer aria-label="Show trailer controls"></button>`;
}

function updateFullscreenTrailerMuteButton(button, muted) {
  if (!button) return;
  button.dataset.muted = muted ? '1' : '0';
  button.setAttribute('aria-pressed', muted ? 'true' : 'false');
  button.setAttribute('aria-label', muted ? 'Unmute trailer' : 'Mute trailer');
  button.innerHTML = getFullscreenTrailerMuteIcon(muted);
}

function toggleFullscreenTrailerMute(button) {
  const root = button?.closest?.('#trailer-fullscreen-container, #trailer-video-modal');
  const iframe = root?.querySelector?.('iframe');
  if (!root || !iframe) return;
  const currentlyMuted = root.dataset.trailerMuted !== '0';
  const nextMuted = !currentlyMuted;
  root.dataset.trailerMuted = nextMuted ? '1' : '0';
  updateFullscreenTrailerMuteButton(button, nextMuted);
  sendFullscreenTrailerCommand(iframe, nextMuted ? 'mute' : 'unMute');
  if (!nextMuted) sendFullscreenTrailerCommand(iframe, 'playVideo');
  revealFullscreenTrailerControls(root);
}

function revealFullscreenTrailerControls(root) {
  const controls = root?.querySelector?.('[data-trailer-controls]');
  if (!root || !controls) return;
  root.classList.add('shelfd-trailer-controls-visible');
  controls.classList.add('is-visible');
  window.clearTimeout(root._trailerControlsTimer);
  root._trailerControlsTimer = window.setTimeout(() => {
    controls.classList.remove('is-visible');
    root.classList.remove('shelfd-trailer-controls-visible');
  }, 3200);
}

function bindFullscreenTrailerControls(root, iframe) {
  if (!root || !iframe || root.dataset.trailerControlsBound === 'true') return;
  root.dataset.trailerControlsBound = 'true';
  root.dataset.trailerMuted = '1';
  const muteButton = root.querySelector('[data-trailer-mute-btn]');
  updateFullscreenTrailerMuteButton(muteButton, true);
  const reveal = () => revealFullscreenTrailerControls(root);
  root.addEventListener('pointermove', reveal, { passive: true });
  root.addEventListener('touchstart', reveal, { passive: true });
  root.querySelector('[data-trailer-wake-layer]')?.addEventListener('click', (event) => {
...
function showTrailerModal(youtubeKey, title) {
  ...
  ensureFullscreenTrailerControlStyles();
  ...
  if (isMobile) {
    ...
    container.insertAdjacentHTML('beforeend', renderFullscreenTrailerControls());
    container.appendChild(iframeWrapper);
    document.body.appendChild(container);
    document.body.style.overflow = 'hidden';
    bindFullscreenTrailerControls(container, iframe);
    
    /* Keep fullscreen inside the Shelfd overlay instead of requesting native
       iframe fullscreen. Native fullscreen puts YouTube above our custom
       mute/unmute control, which makes the overlay control unreachable on iOS. */
    ...
    const modalHtml = `
      <div id="trailer-video-modal" class="overlay-page" style="display:block;background:rgba(0,0,0,0.95);z-index:999999;">
        <div class="overlay-page-inner" style="position:relative;">
          ...
          <div style="padding:16px;max-width:900px;margin:0 auto;">
            <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;">
              <iframe 
                ...
                src="${escAttr(embedSrc)}"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                onload="requestFullscreenTrailerPlayback(this); setTimeout(() => requestFullscreenTrailerPlayback(this), 240);"
                allowfullscreen>
              </iframe>
            </div>
          </div>
          ${renderFullscreenTrailerControls()}
        </div>
      </div>
    `;
    ...
    bindFullscreenTrailerControls(modal, modal?.querySelector('iframe'));
  }
}

window.requestFullscreenTrailerPlayback = requestFullscreenTrailerPlayback;
window.toggleFullscreenTrailerMute = toggleFullscreenTrailerMute;
