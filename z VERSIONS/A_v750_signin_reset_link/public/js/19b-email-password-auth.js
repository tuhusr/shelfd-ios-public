...
(function() {
  ...
  /* Generic confirmation for password reset — same enumeration concern.
     Even when Firebase rejects (auth/user-not-found), we show this. */
  ...
  function setMode(mode) {
    ...
    /* If we were in the reset view, going back to a tab cancels it. */
    ...
    /* Show/hide fields whose data-show matches the active mode (or is absent
       which means 'always show'). v750: rigorous hide — set both the hidden
       attr AND inline display:none, because some browsers ignore [hidden] when
       another stylesheet rule sets display. */
  ...
  /* ---------- Reset password view swap ---------- */
  function enterResetView() {
    inResetView = true;
    hideEl($('login-email-form'));
    showEl($('login-email-reset-form'));
    setMessage($('login-email-reset-error'), '');
    setMessage($('login-email-reset-message'), '');
    /* Pre-fill reset email from the main email field if present */
    const main = $('login-email-input');
    const reset = $('login-email-reset-input');
    if (main && reset && !reset.value) reset.value = main.value || '';
  }
  function returnFromResetView() {
    inResetView = false;
    showEl($('login-email-form'));
  ...
  async function handleResetSubmit(event) {
    ...
    if (typeof firebase === 'undefined' || !firebase.auth) {
      setMessage($('login-email-reset-error'), 'Sign-in service is loading — try again in a moment.', 'error');
      return;
    }
    ...
    setBusyReset(true, 'Sending…');
    setMessage($('login-email-reset-error'), '');
    setMessage($('login-email-reset-message'), '');
    try {
      await firebase.auth().sendPasswordResetEmail(email);
    } catch (err) {
      /* SECURITY: even if Firebase says auth/user-not-found, we surface the
         GENERIC confirmation. The only errors we DO surface are network /
         rate-limit because the user can act on those. */
      console.warn('[email-auth] reset email send result:', err && err.code);
      ...
      /* All other error codes (including auth/user-not-found) → generic success */
    }
    setMessage($('login-email-reset-message'), GENERIC_RESET_CONFIRM, 'success');
    setBusyReset(false);
  }

  function setBusyReset(state, label) {
    const submit = $('login-email-reset-submit');
    if (!submit) return;
