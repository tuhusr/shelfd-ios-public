...
(function initScreenListDeployAutoRefresh() {
  ...
  // v476: only the explicit version-check path may trigger the deploy splash.
  // Service-worker controllerchange events can fire for browser-driven reasons
  // unrelated to a deploy (initial registration, browser restart, cache
  // eviction, SW lifecycle), and we were treating every one of them as a
  // deploy — which falsely showed the "developer just sent a live update"
  // splash on the PWA. This flag is set inside checkForScreenListDeployUpdate
  // ONLY when we've confirmed the served index.html version differs from the
  // running version. controllerchange now ignores everything else.
  let versionBumpConfirmed = false;
  ...
  async function checkForScreenListDeployUpdate(force) {
    ...
    try {
      ...
      if (nextVersion && nextVersion !== currentVersion) {
        // v476: mark the version bump so that any subsequent SW controllerchange
        // is treated as part of THIS deploy and not a spurious browser-driven
        // SW activation.
        versionBumpConfirmed = true;
        reloading = true;
    ...
    try {
      ...
      if (!serviceWorkerRefreshBound) {
        ...
        navigator.serviceWorker.addEventListener('controllerchange', () => {
          if (reloading) return;
          // v476: only auto-reload+splash if THIS controller change was driven
          // by a deploy that we already confirmed via the version check. Any
          // other controllerchange (initial SW registration, browser-driven
          // SW activation, cache eviction, etc.) is silently ignored — the
          // app keeps running on the existing page and the next /index.html
          // poll will catch a real version bump if one ever lands.
          if (!versionBumpConfirmed) return;
          reloading = true;
          reloadAfterVisibleSplash();
        });
      }
  ...
  // v477: the v364 standalone PWA cache reset has been retired. It was a
  // one-time cleanup gated on a localStorage flag, but iOS Safari aggressively
  // evicts PWA localStorage (including this flag) after periods of inactivity.
  // When the flag was missing on a return visit, this function would unregister
  // the active service worker AND delete every shelfd-* cache, then reload the
  // page inside the standalone WebView. On iOS that sometimes leaves the
  // home-screen icon disconnected from the PWA context — meaning the icon
  // starts opening in browser/mobile mode instead of standalone, with no
  // user-side fix other than removing and re-adding the home-screen icon.
  // The cleanup has long served its purpose; new installs ship clean caches
  // anyway and the regular service-worker `activate` handler already prunes
  // stale `shelfd-*` caches whenever the SW updates. Disabling for safety.
  async function resetStandalonePwaCachesOnce() {
    return false;
  }

  // v476: showStartupSplashIfNeeded was previously displaying the deploy splash
  // (with the message "the developer has just sent out a live update, please
  // wait for refresh") on EVERY PWA standalone cold launch — independent of
  // whether a deploy had actually happened. That's the root cause of the
  // reported "splash appears randomly even though I didn't deploy" bug.
  // The deploy-triggered path (finishReloadSplashIfNeeded ←
  // reloadAfterVisibleSplash) still fires correctly when the version-check
  // confirms a real deploy, so removing the cold-start splash does not affect
  // genuine deploy refreshes.
