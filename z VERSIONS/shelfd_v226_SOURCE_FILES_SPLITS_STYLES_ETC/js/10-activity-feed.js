...
function buildActivityCardHTML(a, activityId, options = {}) {
  // Check if this is a feed post (text or trailer)
  ...
  const mediaCover = itemCover && itemCover !== actorPhoto ? itemCover : '';
  const posterHtml = mediaCover
    ? `<button type="button" class="activity-poster-col" onclick="${posterClick}" aria-label="Open ${escAttr(title)} media profile"><img class="activity-poster-img" src="${escAttr(mediaCover)}" alt="" loading="lazy"></button>`
    : `<button type="button" class="activity-poster-placeholder" onclick="${posterClick}" aria-label="Open ${escAttr(title)} media profile"><span class="activity-poster-placeholder-letter">${escHtml(sectionLabel.charAt(0) || '?')}</span></button>`;
  ...
  let actionLabel = getActivityVerbPhrase(eventType, item);
  let detailHtml = '';
  if (eventType === 'rated') {
    const rating = Number(item.rating || 0);
    actionLabel = `rated ${escHtml(formatRatingValueForSection(rating, section, true))}`;
    const filledStars = Math.round(rating / 2);
    let stars = '';
    for (let i = 1; i <= 5; i++) {
      stars += `<span class="activity-star ${i <= filledStars ? 'lit' : 'dim'}">★</span>`;
    }
    detailHtml = `<div class="activity-stars-row">${stars}</div>`;
  } else if (eventType === 'status-changed') {
    const toStatus = getStatusLabel(a.nextStatus || item.status);
    actionLabel = a.previousStatus
      ? `changed status ${escHtml(getStatusLabel(a.previousStatus))} → ${escHtml(toStatus)}`
      : `changed status to ${escHtml(toStatus)}`;
  } else if (eventType === 'removed') {
    const fromStatus = a.previousStatus ? getStatusLabel(a.previousStatus) : '';
    actionLabel = fromStatus ? `removed from ${escHtml(fromStatus)}` : 'removed';
  } else if (eventType === 'added') {
    actionLabel = `added to ${escHtml(getStatusLabel(item.status))}`;
  } else if (eventType === 'completed') {
    actionLabel = section === 'games' ? 'completed' : 'watched';
  } else if (eventType === 'started') {
    actionLabel = section === 'games' ? 'started playing' : 'started watching';
  } else if (eventType === 'planned') {
    actionLabel = section === 'games' ? 'added to Backlog' : 'added to Watchlist';
    ...
    if (a.commentText) detailHtml = `<div class="activity-comment-preview">${escHtml(String(a.commentText).slice(0, 100))}</div>`;
  }

  const nameHtml = hideActorName
    ? ''
    : `<span class="activity-card-name" style="cursor:pointer;" onclick="event.stopPropagation(); viewUserFromMap('${escAttr(a.uid)}')">${renderDisplayNameHTML(actor, 'Friend', '')}</span>`;
  const whoRowClass = hideActorName ? 'activity-who-row activity-who-row-user-filter' : 'activity-who-row';

  // Add like/reply interactions for all activities
  const likes = Array.isArray(a.likes) ? a.likes : [];
  const isLiked = currentUser && likes.includes(currentUser.uid);
  const likeCount = likes.length;
  
  const likeIcon = isLiked
    ? `<svg fill="currentColor" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>`
    : `<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>`;
  ...
  const interactionsHtml = options.hideInteractions ? '' : `
    <div class="activity-interactions" data-activity-interactions>
      <button class="activity-interaction-btn" data-activity-action="reply" onclick="event.stopPropagation(); openActivityReplyPage('${escAttr(activityId)}')">
        ${getScreenListReplyIconSvg()}
        <span data-activity-reply-count>${replyCount}</span>
      </button>
      <button class="activity-interaction-btn ${isLiked ? 'liked' : ''}" data-activity-action="like" onclick="event.stopPropagation(); toggleActivityLike('${escAttr(activityId)}', this)">
        <span data-like-icon-slot>${getScreenListHeartIconSvg(isLiked)}</span>
        <span data-activity-like-count>${likeCount}</span>
      </button>
    </div>
  ...
  return `<div class="activity-card ${meta.topClass}" data-activity-card-id="${escAttr(activityId)}" data-activity-id="${escAttr(activityId)}">
    ...
    <div class="activity-content-col">
      ...
      <div class="activity-action-text">${actionLabel}</div>
      <div class="activity-card-title-inline">${escHtml(title)}</div>
      ${detailHtml}
      <div class="activity-card-bottom">
