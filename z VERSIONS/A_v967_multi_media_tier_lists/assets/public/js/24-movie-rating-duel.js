/* v967: Multi-media tier list game with separate saved lists for movies, TV, anime, and games. */
(function initShelfdMovieRatingDuel() {
  if (window.__shelfdTierListGameV967) return;
  window.__shelfdTierListGameV967 = true;
  ...
  function buildDuelSession(itemId = '', mediaKey = duelMediaKey) {
    ...
    const currentBandState = getBandState(targetItem, itemsByBand, currentBand, mediaKey);
    ...
    overlay.querySelector('.movie-duel-sheet').innerHTML = `
      <div class="movie-duel-header">
        <div>
          ...
          <p>Swap categories at the top. Each tier list is saved separately and does not change My Lists ratings.</p>
        </div>
        <button type="button" class="movie-duel-close" onclick="closeMovieRatingDuel()" aria-label="Close">&times;</button>
      </div>
  ...
  function chooseMovieRatingDuelWinner(choice = '') {
    ...
    const nextBandState = getBandState(duelState.targetItem, duelState.itemsByBand, nextBand, mediaKey);
