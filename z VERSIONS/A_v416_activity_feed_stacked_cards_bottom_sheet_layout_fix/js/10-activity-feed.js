...
function buildStackedActivityCardHTML(a = {}, activityId = '', options = {}) {
  ...
  const stackHtml = `
    ...
    ${extraCount > 0 ? `<button class="sl-activity-stack-count" type="button" onclick="event.stopPropagation(); openScreenListStackedActivityPage('${escAttr(activityId)}')">+${extraCount}</button>` : ''}`;
