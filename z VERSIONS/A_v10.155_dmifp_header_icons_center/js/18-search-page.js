...
(function() {
  ...
  function ensureSearchPresetHub() {
    ...
    if (!empty || $('shelfd-search-preset-section')) return;
    const section = document.createElement('section');
    section.className = 'shelfd-search-section shelfd-search-preset-section';
    section.id = 'shelfd-search-preset-section';
    section.innerHTML = `
    ...
    `;
    empty.insertBefore(section, empty.firstChild);
  }
    ...
    return `
      <button type="button" class="shelfd-search-preset-card${locked ? ' is-locked' : ''}" data-preset-root="${escAttr(key)}">
        <span class="shelfd-search-preset-copy">
          <span class="shelfd-search-preset-title">${escHtml(title)}</span>
        </span>
  ...
  function openPresetRoot(key = '') {
    ...
    if (key === 'featured') {
      openPresetPanel({
        id: 'featured',
        html: renderPresetPanelShell({
          title: 'Featured / Official Lists',
          eyebrow: 'Premium',
          body: `<div class="shelfd-search-preset-flat-note shelfd-search-preset-flat-note-locked"><strong>Featured Lists are locked.</strong><span>Premium Shelfd list browsing will live here when the premium system is ready.</span></div>`
        })
      });
      return;
    }
  ...
  async function fetchPresetDirect(config = {}) {
    ...
    if (preset === 'popular' && typeof window.fetchDiscoverPopularMedia === 'function') {
      return window.fetchDiscoverPopularMedia('mixed');
    }
    if (preset === 'rated' && typeof window.fetchDiscoverTopRatedMedia === 'function') {
      return window.fetchDiscoverTopRatedMedia('mixed');
    }
    ...
    if (preset === 'genre') {
      ...
      return enrichAndRankPresetItems((await Promise.all(requests)).flat(), 'popular', config.mediaType || 'mixed');
    }
