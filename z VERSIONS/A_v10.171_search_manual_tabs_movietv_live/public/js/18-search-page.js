...
(function() {
  ...
  function ensureSearchPresetHub() {
    const moviePage = document.querySelector('[data-search-media-page="movietv"]');
    if (!moviePage || $('shelfd-search-preset-section')) return;
  ...
  function setActiveSearchMediaTab(key = 'movietv') {
    ...
    document.querySelectorAll('[data-search-media-page]').forEach((page) => {
      const active = String(page.getAttribute('data-search-media-page') || '') === nextKey;
  ...
  function bindSearchMediaTabs() {
    ...
    if (!shell || shell.dataset.mediaTabsBound === 'true') return;
