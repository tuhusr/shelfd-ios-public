...
/* =============================================================================
   ...
   Reuses the exact same spring-slide SAS component as seasonal anime but with
   game-specific status options (Playing / Backlog / Played / Wishlist).
   ============================================================================= */
function openGameDiscoverAddSheet(rawgId, title, poster) {
  ...
  document.getElementById('sas-backdrop')?.remove();
  document.getElementById('sas-sheet')?.remove();
  const meta = [year].filter(Boolean).join(' · ');
  const bd = document.createElement('div');
  bd.id = 'sas-backdrop'; bd.className = 'sas-backdrop';
  ...
  sheet.innerHTML = `
    ...
    <div class="sas-body">
      <div class="sas-anime-row">
        ${resolvedPoster
          ? `<img class="sas-poster" src="${_seasEscape(resolvedPoster)}" alt="${_seasEscape(resolvedTitle)}" referrerpolicy="no-referrer">`
          : '<div class="sas-poster-placeholder"></div>'}
        <div class="sas-anime-info">
          <div class="sas-title">${_seasEscape(resolvedTitle)}</div>
          ${meta ? `<div class="sas-meta">${_seasEscape(meta)}</div>` : ''}
        </div>
      </div>
      <div class="sas-prompt">Where do you want it?</div>
      <div class="sas-statuses">
        <button class="sas-status-btn" type="button" onclick="pickGameDiscoverStatus('watching')">Playing</button>
        <button class="sas-status-btn" type="button" onclick="pickGameDiscoverStatus('planned')">Backlog</button>
