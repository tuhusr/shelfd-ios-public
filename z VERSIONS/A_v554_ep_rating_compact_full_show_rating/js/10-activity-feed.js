...
function getActivityVerbPhrase(eventType, item = {}) {
  ...
  const seasonNum = Number(item?.lastSeasonFinishedNum || 0);
  switch (eventType) {
    case 'rated':     return 'rated';
    case 'status-changed': return 'changed status';
    case 'removed':   return 'removed';
    case 'completed':
      if (isGame) return 'Finished';
      if (!isMovie && seasonNum > 1) return `Finished watching Season ${seasonNum}`;
...
function renderActivityUniversalRating(ratingValue = 0) {
  ...
  if (!rating) return '';
  /* Show / season rating: full 5-star bar with N/10 score. */
...
/* v554: compact "EP rated ★ N" chip for the merged watch+rate card.
   Episode-level rating only — show rating uses the full 5-star renderer. */
function renderActivityEpisodeRatingCompact(ratingValue = 0) {
  ...
  return `<div class="sl-activity-rating sl-activity-rating-compact" aria-label="Episode rating ${escAttr(score)} out of 10">
    <span class="sl-activity-rating-prefix">EP rated</span>
...
function getActivityDisplayAction(eventType = '', item = {}, activity = {}) {
  ...
  const isGame = section === 'games';
  const isMovie = section === 'movies';
  /* v552: combined watch + rate card. If the merged activity carries both
     episode-watched metadata AND a rating, render verb as
     "watched episode X, rated" so one card describes both signals. */
  ...
  /* v553: dedicated season-finished verb.
     - First season completed: just "Finished watching"
     - Season 2+: "Finished watching season N" */
  if (eventType === 'season-finished') {
    const num = Number(activity?.seasonNum || item?.lastSeasonFinishedNum || 0);
    return num > 1 ? `Finished watching Season ${num}` : 'Finished watching';
  }
  /* v553: completion verb is "Finished watching" (movies + shows + anime),
     "Finished" for games. Multi-season shows whose status flips to watched
     append the season number. */
  if (status === 'watched' || eventType === 'completed') {
    if (isGame) return 'Finished';
    const num = Number(item?.lastSeasonFinishedNum || 0);
    if (!isMovie && num > 1) return `Finished watching Season ${num}`;
    return 'Finished watching';
  }
...
function buildActivityCardHTML(a, activityId, options = {}) {
  ...
  /* v554: rating chip selector
     - Merged "watched + rated" card uses compact "EP rated ★ N" with the
       episode rating (item.lastEpisodeRatingValue).
     - Everything else (rated, completed, season-finished) uses the full
       5-star + N/10 chip with the show rating (item.rating). */
  const isMergedWatchRate = !!a.mergedHadEpisodeWatch && a.eventType === 'rated';
  const epRating = Number(item.lastEpisodeRatingValue || 0);
  let ratingHtml;
  if (isMergedWatchRate && epRating > 0) {
