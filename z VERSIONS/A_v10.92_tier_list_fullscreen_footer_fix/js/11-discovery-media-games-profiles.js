...
function bindDiscoverHeroTrailerPreviewBounds(overlay = document.getElementById('discover-media-profile')) {
  ...
  const preview = hero?.querySelector?.('[data-discover-hero-trailer-preview]');
  const poster = hero?.querySelector?.('.discover-media-poster');
  if (!hero || !preview || !poster) return () => {};
  let rafPrimary = 0;
  let rafSecondary = 0;
  let resizeObserver = null;
  const syncBounds = () => {
    if (!hero.isConnected || !preview.isConnected || !poster.isConnected) return;
    const heroRect = hero.getBoundingClientRect();
    const posterRect = poster.getBoundingClientRect();
    if (!heroRect.height || !posterRect.height) return;
    const posterMidpoint = (posterRect.top - heroRect.top) + (posterRect.height * 0.5);
    const boundedMidpoint = Math.max(0, Math.min(heroRect.height, posterMidpoint));
    const bottomInset = Math.max(0, heroRect.height - boundedMidpoint);
    preview.style.setProperty('--discover-trailer-preview-bottom', `${Math.round(bottomInset)}px`);
  };
  const queueSync = () => {
    if (rafPrimary) cancelAnimationFrame(rafPrimary);
    if (rafSecondary) cancelAnimationFrame(rafSecondary);
    rafPrimary = requestAnimationFrame(() => {
      rafSecondary = requestAnimationFrame(syncBounds);
    });
  };
  queueSync();
  ...
  return () => {
    ...
    if (preview.isConnected) preview.style.removeProperty('--discover-trailer-preview-bottom');
  };
}
...
function hydrateDiscoverHeroTrailerPreview(overlay = document.getElementById('discover-media-profile')) {
  ...
  iframe.src = `https://www.youtube-nocookie.com/embed/${encodeURIComponent(trailerKey)}?autoplay=1&mute=1&controls=0&playsinline=1&loop=1&playlist=${encodeURIComponent(trailerKey)}&rel=0&modestbranding=1&iv_load_policy=3&enablejsapi=1&fs=0&disablekb=1`;
...
function renderDiscoverHeroTrailerPreview(trailer, title = '') {
  ...
  return `<button class="discover-media-hero-preview" type="button" data-discover-hero-trailer-preview data-trailer-key="${escAttr(trailerKey)}" data-trailer-title="${escAttr(title)}" onclick="openDiscoverHeroTrailer(event, this)" aria-label="${escAttr(`Play trailer for ${title || 'this title'}`)}">
    ...
    <span class="discover-media-hero-preview-hint"><strong>Trailer Preview</strong><em>Tap to open full screen</em></span>
  </button>`;
}
