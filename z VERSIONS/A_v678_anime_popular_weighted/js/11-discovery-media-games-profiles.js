...
/* v678: Weighted popularity ranking for the anime "Popular" grid.
   Members is the heaviest factor — it reflects pure audience reach.
   Score rewards quality. Favorites rewards deep emotional engagement.

   All three signals are min-max normalised within the fetched pool so no
   single outlier (e.g. One Piece with 10M+ members) collapses the scale
   for everything else.

   Weights:
     members   → 65 %  (audience reach — core "popular" signal)
     score     → 20 %  (quality gate — prevent tracking-only filler)
     favorites → 15 %  (passionate audience — rewards resonance)
*/
function rankAnimeByWeightedPopularity(pool = [], limit = 24) {
  if (!pool.length) return [];
  const WEIGHT_MEMBERS   = 0.65;
  ...
  const signals = pool.map(a => ({
    ...
    favorites: Number(a?.favorites || 0)
  }));

  /* ── 2. Min-max normalise each signal across the pool ── */
  ...
  minMaxNorm(signals, 'favorites');

  /* ── 3. Compute composite, sort descending, return raw Jikan items ── */
  return signals
    .map(s => ({
      item: s.item,
      composite: s.membersN   * WEIGHT_MEMBERS
               + s.scoreN     * WEIGHT_SCORE
               + s.favoritesN * WEIGHT_FAVORITES
    }))
