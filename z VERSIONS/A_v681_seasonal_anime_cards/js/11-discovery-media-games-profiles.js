...
async function loadAnimeSeasonal() {
  ...
  if (!animeSeasonalCache.has(cacheKey)) {
    ...
    try {
      const all = [];
      /* Pull up to 4 pages (≈ 100 titles). MAL seasons rarely exceed 100
         meaningful entries once you ignore extras/promotional shorts; this
         keeps the request budget under 4 calls per season. */
      for (let page = 1; page <= 4; page++) {
        const data = await J.request(`seasons/${encodeURIComponent(year)}/${encodeURIComponent(season)}`, { page });
      ...
      /* 100 % members-based ordering per spec. */
...
function addSeasonalAnimeToLibrary(malId) {
  /* Opens the Add to Shelf modal and auto-selects the anime via Jikan. */
  if (typeof openModal !== 'function') return;
  openModal();
  /* Double rAF so the modal DOM is painted before selectJikanAnime runs. */
  requestAnimationFrame(() => requestAnimationFrame(() => {
    if (typeof selectJikanAnime === 'function') selectJikanAnime(Number(malId));
  }));
}
  ...
  grid.innerHTML = rawItems.map((item, idx) => {
    ...
    return `<div class="anime-seasonal-card${hiddenClass}" data-mal-id="${malId}">
      <div class="anime-seasonal-card-poster" onclick="openJikanAnimeProfile(event,${malId})">
        ...
        ${metaLine ? `<div class="anime-seasonal-card-meta-overlay">${_seasEscape(metaLine)}</div>` : ''}
      </div>
