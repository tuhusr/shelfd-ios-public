...
(function() {
  ...
  function normalizeTmdbItem(item) {
    ...
    if (isJikan) {
      ...
      const tmdbPop = Number(item.popularity || 0);
      const voteCount = Number(item.vote_count || 0);
      popularity = tmdbPop + voteCount * 0.01;
    }
    return {
  ...
  function normalizeGameItem(item) {
    ...
    /* v736: RAWG `added` = how many users have the game in their lists —
       the best popularity proxy. Falls back to `ratings_count`. */
    const popularity = Number(item.added || 0) || Number(item.ratings_count || 0);
    return {
      key: `game:${item.id}`,
      kind: 'game',
      id: item.id,
  ...
  async function runSearch(rawQuery) {
    ...
    /* v736: Sort = letter-for-letter relevance, then popularity.
       Tiered relevance score so the result list reads top-down as
       "most-typed-prefix wins, popular title surfaces first within each
       tier". Within the same relevance tier, the most well-known title
       (popularity signal from TMDB / MAL members / RAWG `added`) lands
       at #1. Quality (rating) is only used as a final tie-breaker. */
    const ql = query.toLowerCase();
    const qWords = ql.split(/\s+/).filter(Boolean);
    function relevanceScore(title) {
      const t = String(title || '').toLowerCase();
      if (!t || !ql) return 0;
      if (t === ql) return 1000;                                  // exact title
      if (t.startsWith(ql)) return 800;                           // full-query prefix
      const tWords = t.split(/\s+/).filter(Boolean);
      if (qWords.length && qWords.every(qw => tWords.some(tw => tw.startsWith(qw)))) return 600;
      if (t.includes(' ' + ql) || t.includes('-' + ql) || t.includes(':' + ql)) return 500;
      if (t.includes(ql)) return 300;                             // substring
      if (qWords.length && qWords.every(qw => t.includes(qw))) return 100; // all words present
      return 0;
    }
    rows.sort((a, b) => {
      const as = relevanceScore(a.title);
      const bs = relevanceScore(b.title);
