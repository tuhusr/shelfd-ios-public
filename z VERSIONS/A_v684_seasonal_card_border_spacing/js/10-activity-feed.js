...
function getActivityDisplayAction(eventType = '', item = {}, activity = {}) {
  ...
  const isGame = section === 'games';
  const isMovie = section === 'movies';
  /* v552/560: merged episode-watched + episode-rated → one card whose
     verb conveys both: "watched EP X". The compact ★ N chip below shows
     the rating. */
  ...
  if ((eventType === 'episode-rated' || (eventType === 'rated' && hadEpisodeWatch)) && (hadEpisodeWatch || epLabel)) {
    return epLabel ? `watched ${epLabel}` : 'watched an episode';
  }
