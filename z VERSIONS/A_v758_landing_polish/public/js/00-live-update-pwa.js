...
(function initScreenListDeployAutoRefresh() {
  ...
  function ensureLiveUpdateSplashStyles() {
    ...
    style.textContent = `
      ...
      /* v758: startup splash — hide the "developer sent update" copy and
         the bouncing dots; show a clean spinning ring instead. */
      .screenlist-startup-ring {
        display: none;
        width: 36px; height: 36px;
        border-radius: 50%;
        border: 2px solid rgba(167,139,250,0.18);
        border-top-color: #a78bfa;
        animation: shelfdSpinRing 780ms linear infinite;
      }
      @keyframes shelfdSpinRing { to { transform: rotate(360deg); } }
      #screenlist-live-update-splash.shelfd-splash-startup .screenlist-live-update-copy { display: none !important; }
      #screenlist-live-update-splash.shelfd-splash-startup .screenlist-live-update-loader { display: none !important; }
      #screenlist-live-update-splash.shelfd-splash-startup .screenlist-startup-ring { display: block !important; }
      @media (max-width: 600px) { .screenlist-live-update-brand { width: min(500px, 90vw); } .screenlist-live-update-copy { font-size: 13px; } }
      @media (prefers-reduced-motion: reduce) { #screenlist-live-update-splash, .screenlist-live-update-card { transition: none; } .screenlist-live-update-loader span { animation: none; opacity: .86; } .screenlist-startup-ring { animation: none; opacity: 0.6; } }
    `;
  ...
  function getLiveUpdateSplash() {
    ...
    splash.innerHTML = `
      <div class="screenlist-live-update-card">
        ...
        <div class="screenlist-startup-ring" aria-hidden="true"></div>
  ...
  function showStartupSplashIfNeeded() {
    ...
    const run = () => {
      showLiveUpdateSplash();
      /* v758: tag the splash as startup-mode so CSS hides the update
         copy text and swaps the bouncing dots for a clean spinner ring.
         The update path (reloadAfterVisibleSplash) never adds this class
         so it still shows the "developer sent update" message. */
      const splash = document.getElementById('screenlist-live-update-splash');
      if (splash) splash.classList.add('shelfd-splash-startup');
