...
function getMyListWatchListAvailabilityState(item = {}, section = activeSection) {
  // Returns { state, date, providers } describing what the Watch List card should
  // show in the availability slot. State is one of:
  //   'unreleased'   — date is in the future, show "Releases <date>"
  //   'in-theaters'  — movie released within the theatrical window
  //   'released'     — released in the past and not in theaters
  ...
  if (section === 'movies' && (Date.now() - dateMs) <= SHELFD_THEATRICAL_WINDOW_MS) {
...
function queueMyListUpcomingReleaseDateHydration(item = {}, section = activeSection) {
  ...
  // Provider hydration — only fires when state is currently 'released' (or will
  // be after the date hydration resolves). Skip for unreleased/in-theaters.
