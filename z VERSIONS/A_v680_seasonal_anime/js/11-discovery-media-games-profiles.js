...
function renderAnimeSeasonalFromCache() {
  ...
  if (!filtered.length) {
    const grid = document.getElementById('anime-discover-seasonal-grid');
    if (grid) {
      grid.innerHTML = `<div class="discover-message">No ${wanted === 'all' ? '' : wanted.toUpperCase() + ' '}titles in this season.</div>`;
    }
    return;
  }
  const items = filtered.map(J.mapItem).filter(Boolean);
  renderDiscoverCards('tv', items, 'anime-discover-seasonal-grid');
}
