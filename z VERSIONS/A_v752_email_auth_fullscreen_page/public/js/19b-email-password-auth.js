...
(function() {
  ...
  function openLoginEmailPage() {
    ...
    page.scrollTop = 0;
    /* Always start in sign-in mode, never carry over reset-view state. */
    if (inResetView) returnFromResetView();
    setMode('signin');
    /* Auto-focus the first field after the page becomes visible */
    setTimeout(() => {
      const first = $('login-email-signin-id');
