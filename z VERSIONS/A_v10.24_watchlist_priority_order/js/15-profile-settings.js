...
async function openProfileSocialModal(kind) {
  /* v706: complete redesign as a full-width bottom sheet that slides
     in from the bottom. Keeps all existing data-loading logic intact. */
  ...
  modal.className = 'profile-social-overlay';
  modal.setAttribute('role', 'dialog');
  modal.setAttribute('aria-label', label);
  modal.innerHTML = `
    <div class="profile-social-sheet-v2">
      <div class="profile-social-drag-handle" aria-hidden="true">
  ...
  document.body.appendChild(modal);
  /* Double-rAF so the initial translateY(100%) is painted before transition fires */
  requestAnimationFrame(() => requestAnimationFrame(() => modal.classList.add('profile-social-open')));
  const list = modal.querySelector('.profile-social-list');
  const users = await getProfileSocialUsers(mode);
...
function bindProfilePageSwipeBack(profilePage = document.getElementById('profile-page')) {
  ...
  const getCloseThreshold = () => Math.min(132, Math.max(96, (viewportW || window.innerWidth || 390) * 0.32));
  ...
  const applyFrame = () => {
    ...
    profilePage.style.opacity = String(Math.max(0.72, 1 - progress * 0.28));
  ...
  const reset = () => {
    ...
    profilePage.style.transition = ''; profilePage.style.transform = ''; profilePage.style.opacity = ''; profilePage.style.willChange = ''; profilePage.style.boxShadow = '';
  ...
  const arm = () => {
    ...
    profilePage.style.transition = 'none'; profilePage.style.willChange = 'transform, opacity';
  ...
  const snapBack = () => {
    ...
    profilePage.style.transition = 'transform 0.22s cubic-bezier(0.2, 1, 0.3, 1), opacity 0.22s ease, box-shadow 0.22s ease, border-radius 0.22s ease';
    profilePage.style.transform = 'translate3d(0, 0, 0)'; profilePage.style.opacity = '1'; profilePage.style.boxShadow = '';
  ...
  const completeBack = () => {
    ...
    closing = true;
    clearFrame(); profilePage.classList.add('profile-swipe-back-snapping');
    profilePage.style.transition = 'transform 0.22s cubic-bezier(0.18, 0.92, 0.18, 1), opacity 0.22s ease, box-shadow 0.22s ease';
    profilePage.style.transform = 'translate3d(105vw, 0, 0)'; profilePage.style.opacity = '0.68'; profilePage.style.boxShadow = '-10px 0 24px rgba(0,0,0,0.10)';
    window.setTimeout(() => {
  ...
  const end = event => {
    ...
    if (swiping) { const shouldClose = dx >= getCloseThreshold() || (dx > 58 && velocityX > VELOCITY_CLOSE_PX_PER_MS); shouldClose ? completeBack() : snapBack(); }
    else reset();
  };
...
/* v706: Animated close for the profile back-button path.
   ...
   The swipe-back path uses finger-tracked inline transform/opacity, then
