...
const GAME_SORT_OPTIONS = [
  ...
  { key: 'avg-play-time',     label: 'Avg Play Time' },
...
function getDefaultSortKeyFor(section = activeSection, tab = activeTab) {
  ...
  // v430: Games → Playing (single-player) and Games → Live Games default-sort by
  // last launched / recently played first. last-played sort also tiebreaks on hours.
  ...
  if (normalizedSection === 'games' && normalizedTab === 'watched') {
    return 'rating-high';
  }
...
async function openLandingCreatorProfile() {
  ...
  try {
    ...
    profileViewingProfile = getCreatorPublicProfileFromUser({ ...(mirrorRaw || {}), ...creator });
    profileViewingData = await loadCreatorPublicListData();
...
async function openSignedOutCreatorListsView(creator = {}) {
  ...
  friendViewData = await autoSortAnimeBuckets(normalizeListData(publicData), false);
  viewingUser = {
    uid: CREATOR_PUBLIC_UID,
    name: creator.name || CREATOR_DEFAULT_NAME,
    photo: creator.photo || creator.customPhoto || '',
