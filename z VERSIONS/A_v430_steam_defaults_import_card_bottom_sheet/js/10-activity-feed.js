...
function installShelfdFeedSheetGestures(page) {
  ...
  const onPointerDown = (event) => {
    ...
    dragState = { startY: point.clientY, lastY: point.clientY, startedAt: Date.now() };
    page.classList.add('is-dragging');
    if (event.cancelable) event.preventDefault();
  };
  const onPointerMove = (event) => {
    if (!dragState || !inner) return;
    const point = event.touches ? event.touches[0] : event;
    const dy = Math.max(0, point.clientY - dragState.startY);
  ...
  const onPointerUp = () => {
    ...
    const sheetH = inner.offsetHeight || 1;
    const elapsed = Date.now() - dragState.startedAt;
    const velocityClose = elapsed > 0 && (dy / elapsed) > 0.6; // px/ms
    page.classList.remove('is-dragging');
    if (dy > 120 || dy / sheetH > 0.4 || velocityClose) {
  ...
  grabber.addEventListener('touchstart', onPointerDown, { passive: false });
