...
function shouldHideMyListCardGenre(section = activeSection, item = {}) {
  return isScreenListTvAnimeWatchingCard(section, item)
    || isScreenListWatchedMediaCard(section, item);
}
...
function renderCard(item, isDraggable) {
  ...
  const dragAttrs = isDraggable
    ...
    : '';
  return `
    <div class="card ${type === "show" ? "show-card" : ""}${isGameCard ? " game-library-card" : ""}${isGamesWishlistCard ? " games-wishlist-card" : ""} ${viewingUser ? "friend-view-card" : ""}${isDraggable ? ' card-draggable' : ''}" id="card-${item.id}" ${isGameCard ? `onclick="handleScreenListGameCardSurfaceClick(event,'${itemIdAttr}')"` : ''} ${dragAttrs}>
      <div class="card-header">
        ...
        <div class="card-info${isGamesWishlistCard ? ' games-wishlist-card-info' : ''}">
          ...
          ${!isGamesWishlistCard && (!shouldHideMyListCardGenre(activeSection, item) && item.genre) ? `<div class="card-genre">${escHtml(item.genre)}</div>` : ''}
