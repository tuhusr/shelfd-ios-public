    ...
    <div class="card ${type === "show" ? "show-card" : ""}${isGameCard ? " game-library-card" : ""} ${viewingUser ? "friend-view-card" : ""}${isDraggable ? ' card-draggable' : ''}" id="card-${item.id}" ${dragAttrs}>
      <div class="card-header">
        ...
        <div class="card-info">
          ...
          `) : ''}
          ${(type === "show" && item.status !== 'planned' && totalEps > 0 && watchedCount < totalEps && (activeSection === 'shows' || activeSection === 'anime')) ? `<div class="card-next-episode">Next Episode ${watchedCount + 1}/${totalEps}</div>` : ''}
          ${item.status !== 'planned' ? `<div class="rating-area">
            <div class="rating-label">Rating</div>
            ${renderStars(item.rating || 0, item.id, 'overall', 16)}
          </div>` : (() => {
            /* v650: For Watchlist / Wishlist / Backlog tabs with a future
               release date, show the release date in the slot where the
               star rating usually goes. Movies / TV / Anime use the
               existing helper; games fall back to the raw release field. */
            let label = '';
            if (typeof getMyListUpcomingReleaseLabel === 'function') {
              label = getMyListUpcomingReleaseLabel(item, activeSection) || '';
            }
            if (!label && (activeSection === 'games' || isGameCard)) {
              ...
              if (gameDate && typeof isScreenListFutureReleaseDate === 'function' && isScreenListFutureReleaseDate(gameDate)) {
                const formatted = typeof formatScreenListReleaseDateLabel === 'function'
                  ? formatScreenListReleaseDateLabel(gameDate)
                  : gameDate;
                label = formatted ? `Releases ${formatted}` : '';
              }
            }
            return label
              ? `<div class="rating-area card-future-release-area">
                  <div class="rating-label card-future-release-label">Release Date</div>
                  <div class="card-future-release-date">${escHtml(label)}</div>
                </div>`
              : '';
          })()}
        </div>
...
function toggleEp(itemId, epId) {
  ...
  preserveEpisodeScroll(itemId, () => {
    ...
    if (becameWatched) {
      markEpisodeWatchActivity(item, activeSection, { count: 1, label: ep.title || ep.name || 'episode watched' });
