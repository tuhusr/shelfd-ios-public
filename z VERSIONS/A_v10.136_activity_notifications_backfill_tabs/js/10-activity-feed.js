...
function getActivityNotificationBackfillKey() {
  return currentUser?.uid ? `shelfd-notifications-backfill-v2-${currentUser.uid}` : '';
}
...
async function backfillExistingActivityNotificationsForCurrentUser() {
  ...
  const key = getActivityNotificationBackfillKey();
  const lastRun = key ? Number(localStorage.getItem(key) || 0) || 0 : 0;
  if (lastRun && Date.now() - lastRun < 6 * 60 * 60 * 1000) return 0;
...
async function renderActivityNotificationsPage() {
  ...
  try {
    await backfillExistingActivityNotificationsForCurrentUser();
    ...
    window.setTimeout(() => markActivityNotificationsRead(notifications), 450);
  } catch (error) {
