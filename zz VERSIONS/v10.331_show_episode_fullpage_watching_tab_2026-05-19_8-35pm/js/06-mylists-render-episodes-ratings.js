...
function render() {
  ...
  /* v10.331: if the Show Episode Page overlay is open, refresh its content
     from the freshly-rendered item data so mark-all / mark-season / rating
     mutations stay in sync after each save() → render() pass. No-op if the
     overlay is closed. */
  if (typeof window !== 'undefined' && typeof window.refreshShowEpisodePageContent === 'function') {
    try { window.refreshShowEpisodePageContent(); } catch (_) {}
  }
}
...
function renderCard(item, isDraggable) {
  ...
  if (hasFullEpisodeRows) {
    /* v10.331: TV/Anime in the Watching tab open the full-page Show Episode
       Page instead of the inline dropdown. Other tabs keep the inline
       toggle path so nothing else regresses while we roll the new flow out. */
    const _useShowEpisodePage = activeTab === 'watching' && (activeSection === 'shows' || activeSection === 'anime');
    const _epOnClick = _useShowEpisodePage ? `openShowEpisodePage('${item.id}')` : `toggleEpisodes('${item.id}')`;
    episodeToggleButton = `
      <button type="button" class="ep-toggle-bar card-footer-btn" onclick="${_epOnClick}">
    ...
    /* v10.331: when the full-page Show Episode Page is open for this item,
       suppress the inline .ep-list so #ep-list-{id} stays unique to the
       overlay. The card-side block reappears after closeShowEpisodePage
       (which re-renders with the gate cleared). */
    const _suppressInlineEpList = (typeof window !== 'undefined' && window.currentShowEpisodePageId === item.id);
    episodeSection = _suppressInlineEpList ? '' : `
...
/* ============================================================================
   v10.331 — Show Episode Page (full-page TV/Anime episode tracker)
   ----------------------------------------------------------------------------
   Replaces the inline `.ep-list` dropdown for TV/Anime cards in the Watching
   tab. Reuses the existing dropdown content builder (`buildEpisodeListInnerHtml`)
   plus every existing episode helper (markAllEps, markSeasonEps, toggleSeason,
   season ratings, anime title hydration, activity feed posting, save paths) —
   zero new episode logic, just a different presentation container.

   While the overlay is open, the card-side inline `.ep-list` is suppressed by
   a render-time gate (see `window.currentShowEpisodePageId` check in the
   card renderer around the episodeSection assignment). On render(), the
   overlay's content is refreshed from fresh item data so mark-all / mark-
   season mutations stay reflected.

   Scroll position on My Lists is preserved naturally because the overlay is
   `position: fixed; inset: 0` and body has `overflow: hidden` while open —
   nothing scrolls the underlying list.
   ============================================================================ */
(function() {
  const SHOW_EP_PAGE_ID = 'show-episode-page';
  const SHOW_EP_PAGE_OPEN_CLASS = 'show-episode-page-open';

  function getShowEpisodeItem(itemId) {
    if (!itemId) return null;
    const id = String(itemId);
    const sections = ['shows', 'anime'];
    for (const sec of sections) {
      const arr = (typeof data !== 'undefined' && Array.isArray(data?.[sec])) ? data[sec] : null;
      if (!arr) continue;
      const hit = arr.find(it => it && String(it.id || '') === id);
      if (hit) return { item: hit, section: sec };
    }
    return null;
  }

  function getShowEpisodeProgress(item) {
    const eps = Array.isArray(item?.episodes) ? item.episodes : [];
    const totalFromEps = eps.length;
    const total = Number(item?.totalEpisodes || item?.totalEps || totalFromEps || 0);
    let watched = 0;
    if (totalFromEps > 0) watched = eps.filter(e => e && e.watched).length;
    else watched = Number(item?.currentEp || item?.watchedEpisodes || 0);
    return { watched, total };
  }

  function getShowEpisodeStatusLabel(item) {
    const raw = String(item?.status || '').trim().toLowerCase();
    if (raw === 'watching') return 'Watching';
    if (raw === 'watched' || raw === 'completed') return 'Watched';
    if (raw === 'paused' || raw === 'on_hold' || raw === 'on-hold') return 'Paused';
    if (raw === 'planned' || raw === 'wishlist') return 'Planned';
    if (raw === 'dropped') return 'Dropped';
    return raw ? raw.charAt(0).toUpperCase() + raw.slice(1) : 'Watching';
  }

  function buildShowEpisodeHeroHtml(item) {
    const poster = String(item?.poster || item?.coverImage || item?.cover || '').trim();
    const title = String(item?.title || 'Untitled').trim();
    const year = String(item?.year || '').trim();
    const kicker = year ? `${year}` : '';
    const statusLabel = getShowEpisodeStatusLabel(item);
    const { watched, total } = getShowEpisodeProgress(item);
    const progressText = total > 0 ? `${watched}/${total} eps` : '';
    const ratingNum = Number(item?.rating || 0);
    const ratingHtml = ratingNum > 0 ? `<span class="show-episode-page-hero-rating">${ratingNum.toFixed(1)}</span>` : '';
    const posterStyle = poster ? `style="background-image:url('${String(poster).replace(/'/g, "\\'")}')"` : '';
    const posterClass = poster ? '' : 'no-img';
    return `
      <div class="show-episode-page-hero">
        <div class="show-episode-page-hero-poster ${posterClass}" ${posterStyle}>${poster ? '' : '<span aria-hidden="true">📺</span>'}</div>
        <div class="show-episode-page-hero-meta">
          ${kicker ? `<div class="show-episode-page-hero-kicker">${escHtml(kicker)}</div>` : ''}
          <div class="show-episode-page-hero-title">${escHtml(title)}</div>
          <div class="show-episode-page-hero-status-row">
            <span class="show-episode-page-hero-status">${escHtml(statusLabel)}</span>
            ${progressText ? `<span class="show-episode-page-hero-progress">${escHtml(progressText)}</span>` : ''}
            ${ratingHtml}
          </div>
        </div>
      </div>
  ...
  /* The episode-tracker container hosts the inline `.ep-list` block that we
     transplant in via DOM move (preserves IDs + listeners + lazy-hydration
     state). On render() refresh, the same inline ep-list is rebuilt because
     it's the card-side gate that controls who owns the live DOM. */
  function buildShowEpisodePageHtml(item) {
    const title = String(item?.title || 'Episodes').trim();
    return `
      <div class="show-episode-page-shell" data-show-episode-shell>
        <header class="show-episode-page-topbar">
          <button type="button" class="show-episode-page-back" data-show-episode-back aria-label="Back">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 18 9 12 15 6"></polyline></svg>
          </button>
          <span class="show-episode-page-title-bar">${escHtml(title)}</span>
          <span class="show-episode-page-topbar-spacer" aria-hidden="true"></span>
        </header>
        <main class="show-episode-page-content" data-show-episode-content>
          ${buildShowEpisodeHeroHtml(item)}
          <section class="show-episode-page-tracker-host" data-show-episode-tracker-host></section>
        </main>
      </div>
    `;
  }

  function attachShowEpisodeSwipeBack(overlay) {
    const shell = overlay.querySelector('[data-show-episode-shell]');
    if (!shell) return;
    ...
    function onUp() {
      ...
      if (shouldClose) {
        ...
        setTimeout(() => closeShowEpisodePage({ instant: true }), 290);
      } else {
  ...
  /* Build a fresh `.ep-list` shell (same shape as the inline renderer at
     line ~2244) and hydrate it via buildEpisodeListInnerHtml. The overlay
     owns this node; the card-side render is gated to skip its inline copy
     while currentShowEpisodePageId === item.id. */
  function buildAndHydrateOverlayEpList(item) {
    const wrapper = document.createElement('div');
    wrapper.innerHTML = `<div class="ep-list open" id="ep-list-${item.id}"><div class="ep-list-inner"></div></div>`;
    const list = wrapper.firstElementChild;
    const inner = list.querySelector('.ep-list-inner');
    if (inner && typeof buildEpisodeListInnerHtml === 'function') {
      inner.innerHTML = buildEpisodeListInnerHtml(item);
    }
    return list;
  }

  function openShowEpisodePage(itemId) {
    const found = getShowEpisodeItem(itemId);
    if (!found) {
      if (typeof showToast === 'function') showToast('Episode tracker not available');
      return;
    }
    const { item } = found;
    closeShowEpisodePage({ instant: true });

    window.currentShowEpisodePageId = String(item.id);

    const overlay = document.createElement('section');
    overlay.id = SHOW_EP_PAGE_ID;
    overlay.className = 'show-episode-page';
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.dataset.itemId = String(item.id);
    overlay.innerHTML = buildShowEpisodePageHtml(item);

    /* Build the ep-list inside the overlay BEFORE attaching to the DOM so
       the in-flight inline copy (if any) is replaced cleanly. Subsequent
       render() calls won't re-create an inline copy thanks to the gate. */
    const host = overlay.querySelector('[data-show-episode-tracker-host]');
    if (host) host.appendChild(buildAndHydrateOverlayEpList(item));

    document.body.appendChild(overlay);
    document.body.classList.add(SHOW_EP_PAGE_OPEN_CLASS);
    requestAnimationFrame(() => requestAnimationFrame(() => overlay.classList.add('is-open')));

    /* Wire close. Back button + tap-on-empty-overlay. */
    const backBtn = overlay.querySelector('[data-show-episode-back]');
    if (backBtn) backBtn.addEventListener('click', () => closeShowEpisodePage());

    attachShowEpisodeSwipeBack(overlay);

    /* Re-render the card list so the gate suppresses the now-duplicate
       inline ep-list. Without this, the existing inline .ep-list with the
       same id stays in the DOM until the next mutation. */
    if (typeof render === 'function') {
      try { render(); } catch (_) {}
    }

    /* Hydrate posters + anime episode titles (same pattern toggleEpisodes
       uses on first inline expand). */
    if (typeof hydrateMissingSeasonPosters === 'function') {
      try { hydrateMissingSeasonPosters(item.id, found.section); } catch (_) {}
    }
    if (found.section === 'anime' && (item.malId || item.mal_id) && typeof hydrateAnimeEpisodeTitlesFromJikan === 'function') {
      try { hydrateAnimeEpisodeTitlesFromJikan(item); } catch (_) {}
    }
  }

  function closeShowEpisodePage(opts = {}) {
    const overlay = document.getElementById(SHOW_EP_PAGE_ID);
    const itemId = window.currentShowEpisodePageId || (overlay?.dataset?.itemId || '');
    window.currentShowEpisodePageId = null;

    const finish = () => {
      try { overlay?.remove(); } catch (_) {}
      document.body.classList.remove(SHOW_EP_PAGE_OPEN_CLASS);
      if (itemId && typeof shelfdExitSeasonFocusMode === 'function') {
        try { shelfdExitSeasonFocusMode(itemId); } catch (_) {}
      }
      const section = (typeof activeSection !== 'undefined') ? activeSection : '';
      if (itemId && typeof flushScreenListDeferredEpisodeStatus === 'function') {
        window.setTimeout(() => {
          try { flushScreenListDeferredEpisodeStatus(itemId, section); } catch (_) {}
        }, 380);
      }
      /* Re-render so the card's inline .ep-list reappears (gate is now off). */
      if (typeof render === 'function') {
        try { render(); } catch (_) {}
      }
    };
  ...
  /* Called from render() epilogue. Rebuilds the overlay's tracker content
     from fresh item data so mark-all/mark-season/mark-episode mutations
     stay reflected without a manual sync. Scroll position inside the
     overlay's content area is preserved. */
  function refreshShowEpisodePageContent() {
    const overlay = document.getElementById(SHOW_EP_PAGE_ID);
    if (!overlay) return;
    const itemId = overlay.dataset.itemId;
    const found = itemId ? getShowEpisodeItem(itemId) : null;
    if (!found) return;
    const { item } = found;
    const content = overlay.querySelector('[data-show-episode-content]');
    const prevScroll = content?.scrollTop || 0;
    /* Refresh hero (status/progress/rating may have changed). */
    const heroWrap = overlay.querySelector('.show-episode-page-hero');
    if (heroWrap) heroWrap.outerHTML = buildShowEpisodeHeroHtml(item);
    /* Refresh the episode tracker by rebuilding the .ep-list. */
    const host = overlay.querySelector('[data-show-episode-tracker-host]');
    if (host) {
      host.innerHTML = '';
      host.appendChild(buildAndHydrateOverlayEpList(item));
    }
    if (content) content.scrollTop = prevScroll;
  }

  window.openShowEpisodePage = openShowEpisodePage;
  window.closeShowEpisodePage = closeShowEpisodePage;
  window.refreshShowEpisodePageContent = refreshShowEpisodePageContent;
})();
