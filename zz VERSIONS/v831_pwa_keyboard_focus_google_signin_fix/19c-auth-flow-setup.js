...
(function() {
  ...
  function openShelfdSignInPage() {
    ...
    /* v831: removed setTimeout(input.focus(), 80) — iOS PWA WKWebView
       refuses to raise the keyboard for a programmatic focus that lands
       outside the original user-gesture stack AND while the slide-up
       transform is still mid-animation. Leaving it off keeps the layout
       calm; the user taps the email field themselves which always works. */
  }
  ...
  function openShelfdResetPage() {
    ...
    /* v831: see openShelfdSignInPage — no programmatic focus on mobile/PWA. */
  }
  ...
  function openShelfdSetupPage(step = 1, opts = {}) {
    ...
    /* v831: removed setTimeout(input.focus(), 80) — iOS PWA WKWebView
       refuses to raise the keyboard for a programmatic focus outside the
       original user gesture. The user taps the username field themselves. */
  }
