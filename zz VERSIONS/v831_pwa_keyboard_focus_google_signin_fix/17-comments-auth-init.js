...
function signIn() {
  ...
  /* v831: mark a sign-in attempt in flight so visibility/pageshow resume
     handlers know to refresh auth state after 2FA. Always cleared once
     onAuthStateChanged fires with a user (see auth listener below). */
  try { window.googleSignInInProgress = true; } catch (_) {}
  /* Skip the popup attempt entirely on iOS PWA standalone OR mobile Safari —
     popups are blocked and the failed open can stall the click gesture on
     some iOS versions. Use redirect so 2FA flows reliably resume via
     getRedirectResult on return. */
...
/* v631 / v831: Complete a pending redirect sign-in as soon as Firebase Auth
   loads — and ALSO every time the page is shown / becomes visible / regains
   focus. iOS PWA + 2FA: the user leaves the PWA for Gmail/Authenticator,
   completes 2FA, then returns. The redirect flow's getRedirectResult must
   be polled again on return so the credential is consumed and
   onAuthStateChanged fires. */
(function initRedirectResult() {
  if (!auth || typeof auth.getRedirectResult !== 'function') return;
  let inFlight = false;
  function pumpRedirectResult(reason) {
    if (inFlight) return;
    inFlight = true;
    try {
      auth.getRedirectResult()
        .catch(err => {
          /* Suppress the common "no pending redirect" benign error so the
             console stays clean during normal page loads. */
          if (err && err.code && err.code !== 'auth/null-user') {
            console.warn('[shelfd-auth] getRedirectResult (' + reason + '):', err && err.code);
          }
        })
        .finally(() => { inFlight = false; });
    } catch (err) {
      inFlight = false;
      console.error('Redirect-result handler failed:', err);
    }
  }
  pumpRedirectResult('boot');
  /* Re-pump on every return-to-page event. Cheap and idempotent. */
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') pumpRedirectResult('visibilitychange');
  });
  window.addEventListener('pageshow', () => pumpRedirectResult('pageshow'));
  window.addEventListener('focus', () => pumpRedirectResult('focus'));
  /* Expose so other modules can force a pump if needed. */
  try { window.__shelfdPumpRedirectResult = pumpRedirectResult; } catch (_) {}
})();
...
auth.onAuthStateChanged(async (user) => {
  /* v831: any auth-state transition clears the "Google sign-in in flight"
     latch so the next attempt is never blocked by leftover state. */
  try { window.googleSignInInProgress = false; } catch (_) {}
