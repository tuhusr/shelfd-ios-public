...
const DISCOVER_HERO_TRAILER_EXPAND_TRANSITION = 'left 340ms cubic-bezier(0.16, 1, 0.3, 1), top 340ms cubic-bezier(0.16, 1, 0.3, 1), width 340ms cubic-bezier(0.16, 1, 0.3, 1), height 340ms cubic-bezier(0.16, 1, 0.3, 1), border-radius 340ms cubic-bezier(0.16, 1, 0.3, 1)';
...
function applyDiscoverHeroTrailerExpansionProgress(overlay = document.getElementById('discover-media-profile'), rawProgress = 0) {
  ...
  preview.style.setProperty('--media-profile-trailer-content-opacity', `${Math.max(0, 1 - fadeProgress * 1.18).toFixed(3)}`);
  ...
  preview.style.setProperty('--media-profile-trailer-frame-opacity', `${(0.82 + 0.18 * ease).toFixed(3)}`);
  overlay.style.setProperty('--media-profile-trailer-page-opacity', `${Math.max(0, 1 - fadeProgress * 1.18).toFixed(3)}`);
  overlay.style.setProperty('--media-profile-trailer-page-y', `${(28 * fadeProgress).toFixed(2)}px`);
  return progress;
}
...
function bindDiscoverMediaProfileSwipeBack(overlay) {
  ...
  const handleGestureMove = (event) => {
    ...
    if (!gestureMode) {
      ...
      } else if (canTrailerCollapse && dy < -18 && absDy > absDx * 1.2) {
        armGesture('trailer-collapse');
        try { page.setPointerCapture?.(event.pointerId); } catch (e) {}
      } else if (canTrailerExpand && page.scrollTop <= 2 && dy > 18 && absDy > absDx * 1.2) {
