...
function buildCardCommentSlotHtml(item) {
  if (!item) return '';
  ...
  if (text) {
    const editBtn = isOwner ? '<button class="card-comment-edit-btn" type="button" onclick="event.stopPropagation();openCardCommentComposer(\'' + itemIdAttr + '\')" aria-label="Edit comment" title="Edit comment"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></button>' : '';
    const deleteBtn = isOwner ? '<button class="card-comment-delete-btn" type="button" onclick="event.stopPropagation();confirmDeleteCardComment(\'' + itemIdAttr + '\')" aria-label="Delete comment" title="Delete comment"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M6 6l1 14a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2l1-14"/></svg></button>' : '';
    return '<div class="card-comment-display" data-card-comment-id="' + itemIdAttr + '"><div class="card-comment-text">' + escHtml(text) + '</div>' + (isOwner ? '<div class="card-comment-actions">' + editBtn + deleteBtn + '</div>' : '') + '</div>';
  }
  if (!isOwner) return '';
...
function openCardCommentComposer(itemId) {
  ...
  const headerLabel = existingText ? 'Edit your comment' : 'Add a comment';
  const saveLabel = existingText ? 'Save' : 'Post';
  overlay.innerHTML =
    '<div class="card-comment-composer-backdrop" onclick="closeCardCommentComposer()"></div>' +
    '<div class="card-comment-composer-sheet" role="dialog" aria-modal="true" aria-labelledby="card-comment-composer-title">' +
      ...
      '<div class="card-comment-composer-header">' +
        ...
        '<button class="card-comment-composer-close" type="button" onclick="closeCardCommentComposer()" aria-label="Close">×</button>' +
      '</div>' +
      '<div class="card-comment-composer-subtitle">' + escHtml(item.title || 'Untitled') + '</div>' +
      '<textarea class="card-comment-composer-input" id="card-comment-composer-input" rows="6" maxlength="' + SCREENLIST_CARD_COMMENT_MAX + '" placeholder="Share what you thought of ' + titleAttr + '">' + escHtml(existingText) + '</textarea>' +
      '<div class="card-comment-composer-foot">' +
        ...
        '<div class="card-comment-composer-actions">' +
          '<button class="btn-secondary card-comment-composer-cancel" type="button" onclick="closeCardCommentComposer()">Cancel</button>' +
          '<button class="btn-primary card-comment-composer-save" type="button" id="card-comment-composer-save" onclick="saveCardCommentFromComposer(\'' + escAttr(itemId) + '\')">' + saveLabel + '</button>' +
        '</div>' +
      '</div>' +
    '</div>';
  document.body.appendChild(overlay);
