...
function applyDiscoverHeroTrailerExpansionProgress(overlay = document.getElementById('discover-media-profile'), rawProgress = 0) {
  ...
  const ease = progress;
  const pageFadeProgress = Math.min(1, progress * 1.55);
  ...
  preview.style.setProperty('--media-profile-trailer-content-opacity', `${Math.max(0, 1 - pageFadeProgress).toFixed(3)}`);
  ...
  overlay.style.setProperty('--media-profile-trailer-page-opacity', `${Math.max(0, 1 - pageFadeProgress).toFixed(3)}`);
  overlay.style.setProperty('--media-profile-trailer-page-y', `${(viewport.height * 0.42 * progress).toFixed(2)}px`);
  return progress;
}
