...
// v784: Light/Cream modes are quarantined while their CSS is consolidated.
// Any saved 'light' or 'cream' preference is rewritten to the default dark
// mode at load time so existing users with stale preferences fall back to
// the supported theme. The underlying applyThemeMode/normalizeThemeMode JS
// is intact and ready for re-enable when light-mode CSS is ready again.
(function() {
  const SHELFD_QUARANTINED_THEMES = new Set(['light', 'cream']);
  let saved = localStorage.getItem('theme-mode');
  if (saved && SHELFD_QUARANTINED_THEMES.has(saved)) {
