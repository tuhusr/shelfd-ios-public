...
function renderProfileSettingsPage() {
  ...
  if (themeLight)    { themeLight.disabled = false;    themeLight.checked    = themeMode === 'light'; }
  if (themeTrueDark) { themeTrueDark.checked = themeMode === 'true-dark'; }
  if (themeCream)    { themeCream.disabled = false;     themeCream.checked    = themeMode === 'cream'; }
...
function readThemeModeFromPage() {
  ...
  if (document.getElementById('theme-mode-cream')?.checked)     return 'cream';
...
function applyThemeMode(mode, persist = true) {
  ...
  if (normalized === 'cream') {
    /* Cream Light piggybacks on body.light-mode so all existing CSS rules
       apply automatically, then body.cream-mode provides the warm-paper
       identity on top. */
...
// v808 — Step 1: force every user into the current Default Theme at boot.
...
// clean. The light/cream code paths are intentionally left intact (per
// v784 quarantine) and can be re-enabled in a later step by replacing
// this IIFE body with the old "restore saved value" logic.
(function() {
  const ACTIVE = getDefaultThemeMode(); // 'true-dark'
  /* 1. Strip stale localStorage if it isn't the active default. */
  try {
  ...
  /* 3. Apply the active default theme and persist it. */
