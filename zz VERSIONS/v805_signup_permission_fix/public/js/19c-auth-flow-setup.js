  ...
  async function handleSignupSubmit(e) {
    ...
    try {
      /* ── 1. Create the Auth account first ───────────────────────────────
         ...
         first, then enforce uniqueness inside a transaction below. If the
         transaction fails (taken or rule denial), we clean up this Auth
         account via user.delete() so the email is freed for retry. */
      ...
      /* ── 2. Reserve username + write minimal user/profile doc atomically.
             Doing this inside a transaction (a) lets us perform the
             uniqueness READ with the user's own auth credentials, (b)
             guarantees we never end up with a reservation without a
             matching profile doc, and (c) uses the rule-friendly minimal
             schema { uid, username, createdAt } on the usernames doc. */
      try {
        ...
        console.error('[shelfd-auth] signup transaction failed:', txErr && txErr.code, txErr && txErr.message, txErr);
        /* Roll back the orphan Auth account so the email is freed for retry. */
        let deleteFailed = false;
        try {
          await createdUser.delete();
        } catch (delErr) {
          deleteFailed = true;
          console.warn('[shelfd-auth] could not delete partial Auth user:', delErr && delErr.code, delErr);
        }
        signupFlagOwned = false;
        window.__shelfdSignupInProgress = false;

        if (txErr && txErr.code === 'shelfd/username-taken') {
          setBanner($('shelfd-signup-error'), 'That username is taken. Try another.', 'error');
        } else if (isPermissionDenied(txErr)) {
          /* Manual rule fix required — surface the precise dev hint in the
             console and a clean user message. */
          console.error(
            '[shelfd-auth] Firestore permission-denied during signup transaction.\n' +
            '  Required rule on `usernames/{usernameLower}` (paste into Firebase Console):\n' +
            '    allow read: if request.auth != null;\n' +
            '    allow create: if request.auth != null\n' +
            '                  && request.auth.uid == request.resource.data.uid\n' +
            "                  && request.resource.data.keys().hasOnly(['uid', 'username', 'createdAt']);\n" +
            '    allow update, delete: if false;\n' +
            '  And on `users/{uid}` make sure the signed-in user can write their own doc:\n' +
            '    allow create, update: if request.auth != null && request.auth.uid == uid;'
          );
          setBanner(
            $('shelfd-signup-error'),
            "Couldn't finish account setup. The site admin needs to update the database rules — please contact support.",
            'error'
          );
        } else {
          setBanner(
            $('shelfd-signup-error'),
            'Could not finish account setup. Please try again.' + (deleteFailed ? ' (cleanup pending)' : ''),
            'error'
          );
        }
        return;
      ...
      /* ── 3. Top up the user doc with the full schema (themeMode,
             ratingPreferences, pinnedFavorites, etc.) and increment the
             registered-user counter. saveUserProfile is wrapped in its own
             try/catch so any rule mismatch on meta/userCount is logged
             internally and does not break the signup. */
      try {
      ...
      /* ── 4. Open setup flow at step 1 with the username pre-filled as a
             sensible nickname default (user can change). */
