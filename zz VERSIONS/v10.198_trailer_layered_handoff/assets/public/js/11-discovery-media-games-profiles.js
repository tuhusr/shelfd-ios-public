...
function applyDiscoverHeroTrailerExpansionProgress(overlay = document.getElementById('discover-media-profile'), rawProgress = 0) {
  ...
  preview.style.setProperty('--media-profile-trailer-content-opacity', `${Math.max(0, 1 - progress).toFixed(3)}`);
  ...
  overlay.style.setProperty('--media-profile-trailer-page-opacity', `${Math.max(0, 1 - progress).toFixed(3)}`);
