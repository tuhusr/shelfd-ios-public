...
function getMyListReviewActionVerb(section = activeSection, item = {}) {
  ...
  if (section === 'games') return status === 'watched' ? 'Played' : 'Logged';
  if (section === 'manga' || section === 'books') return status === 'watched' ? 'Read' : 'Logged';
  return status === 'watched' ? 'Watched' : 'Logged';
}
...
function renderMyListReviewRating(item = {}, section = activeSection) {
  ...
  const stars = typeof buildRatingStarsMarkup === 'function'
    ? buildRatingStarsMarkup(rating, item.id || 'review', 'review', 18, section, false)
    : '';
  const label = typeof formatRatingValueForSection === 'function'
    ? formatRatingValueForSection(rating, section)
    : String(rating);
  return `<div class="mylist-review-rating">${stars}<span>${escHtml(label)}</span></div>`;
}
...
function openFullPageMediaReview(itemId = '', section = activeSection) {
  ...
  const { item } = record;
  const author = getMyListReviewAuthor();
  const title = getDisplayTitleForItem(item, record.section) || item.title || 'Untitled';
  const poster = getMyListPosterUrlForItem(item, record.section);
  const fallbackAvatar = getMyListReviewFallbackAvatar(author.name);
  const reviewText = getMyListReviewText(item);
    ...
    <div class="mylist-media-review-shell">
      ...
      <main class="mylist-media-review-content">
          ...
          <div class="mylist-media-review-main">
            <div class="mylist-media-review-author">
              ...
              <div>
                ...
                <span>${escHtml(getMyListReviewDateLine(item, record.section))}</span>
              </div>
            </div>
            <h1>${escHtml(title)}</h1>
        ...
        <section class="mylist-media-review-body">
          ${reviewText ? `<p>${escHtml(reviewText)}</p>` : '<p class="mylist-media-review-empty">No written review yet. This page is ready for the full review experience as the feature develops.</p>'}
        </section>
      </main>
    </div>
  `;
...
function handleFullPageMediaReviewKeydown(event) {
  if (event?.key === 'Escape') closeFullPageMediaReview();
}
