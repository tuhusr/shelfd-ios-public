...
/* ===========================================================================
   ...
   =========================================================================== */
const SCREENLIST_CARD_COMMENT_MAX = 600;
...
function openCardCommentComposer(itemId) {
  ...
  /* v839: REMOVED the visualViewport-driven reposition handler.
     The previous behavior:
       - Listen to vv.resize / vv.scroll
       - On every fire, set overlay.style.top = vv.offsetTop, .height = vv.height,
         .left = vv.offsetLeft, .width = vv.width, .inset = 'unset'
     Why it had to go:
       In iOS PWA standalone WebKit, when an input inside the modal focuses
       and the soft keyboard appears, iOS scrolls the page to bring the
       focused input into view. During that scroll, vv.offsetTop briefly
       becomes a large positive value (the keyboard's effective top offset
       relative to the layout viewport). The handler then writes
       `style.top = vv.offsetTop + 'px'` onto the `position: fixed` overlay
       — which, instead of "keeping it visible," pushes the overlay down
       OUT of the visible area. The result the user saw: tap the textarea
       → keyboard appears (great — focus succeeded) → modal vanishes
       visually but stays in the DOM (because it's now positioned far below
       the visible viewport).
     The default `position: fixed; inset: 0;` from the stylesheet handles
     all keyboard cases on iOS Safari + iOS PWA without intervention: the
     overlay stays anchored to the visual viewport edges, and the centered
     sheet sits above the keyboard. No manual JS reposition needed. */
}
...
function closeCardCommentComposer() {
  ...
  /* v839: visualViewport handler removed in openCardCommentComposer —
     nothing to detach here anymore. */
...
async function saveCardCommentFromComposer(itemId) {
  ...
  save();
  closeCardCommentComposer();
  /* v840: render IMMEDIATELY after the local save so the card's comment
     body shows the new text right away. Previously render() was only
     called at the very bottom of this function — AFTER the awaited
     activity-feed sync. On iOS PWA, that await can hang (Firestore
     write to the linked feed-post doc stalls when the network round-
     trips through the standalone WKWebView), so render() never fired
     and the user saw the OLD comment text even though the underlying
     data was saved. Running render() here guarantees the visual update
     happens regardless of activity-feed sync state. */
  render();
  try {
    if (isEdit && item.cardComment.linkedActivityId) {
      ...
      if (postId) {
        ...
        save();
        /* Re-render so the new linkedActivityId is reflected (e.g. for
           future edits to update the feed post instead of creating new). */
        render();
      }
    }
  } catch (error) {
