  ...
  async function handleSignupSubmit(e) {
    ...
    try {
      ...
      try {
        ...
        /* v815: resource-exhausted (Firestore quota / rate limit). Skip any
           further Firestore writes — including the user-doc delete inside
           cleanupSignupOrphan — to avoid deepening the hole. Delete only
           the Auth user (separate quota) so the email is freed. */
      ...
      /* ── 3. Reserve the username with a single CREATE-or-fail write to
              `usernames/{usernameLower}`. Minimal schema per spec:
              { uid, username, createdAt }.

              v815: replaced the previous runTransaction() (1 READ + 1
              WRITE per attempt, up to 5 internal retries on contention =
              up to 10 Firestore ops worst case) with a plain set(). The
              Firestore rule on /usernames/{usernameLower} is
                allow create: if auth && uid matches && fields = {uid,username,createdAt};
                allow update, delete: if false;
              So set() on a new doc → CREATE event → allowed; set() on an
              existing doc → UPDATE event → denied. Uniqueness is still
              enforced server-side; we just avoid the extra read + retry
              loop that was burning quota. */
      try {
        ...
        /* v815: handle Firestore quota first — don't do any more writes
           (skip cleanupSignupOrphan's user-doc delete; Auth delete only). */
        if (isResourceExhausted(uErr)) {
          console.error(
            '[shelfd-auth] resource-exhausted on usernames write',
        ...
        /* permission-denied here is ambiguous: either the rule denied an
           UPDATE on an existing doc (username taken), or the rule itself
           isn't published. Disambiguate with one targeted get(); skip
           cleanly if even that get can't run (quota). */
        if (isPermissionDenied(uErr)) {
          let alreadyExists = false;
          let probeQuotaHit = false;
          try {
            const snap = await usernameRef.get();
            alreadyExists = !!snap.exists;
          } catch (probeErr) {
            if (isResourceExhausted(probeErr)) probeQuotaHit = true;
            /* otherwise swallow — fall through to the rule-issue branch */
          }
          if (probeQuotaHit) {
            try { await createdUser.delete(); } catch (_) {}
            signupFlagOwned = false;
          ...
          /* Rule wasn't published — log the snippet for the admin. */
          console.error(
            '[shelfd-auth] usernames/{usernameLower} reservation FAILED (rule issue)',
            'usernameLower=' + usernameLower,
            'uid=' + createdUser.uid,
            'code=' + (uErr && uErr.code),
            'message=' + (uErr && uErr.message),
        ...
        /* Unknown error class — surface code without retrying. */
        console.error(
      ...
      /* v815: if Firestore is the source of the failure and we hit a
         quota wall, show the friendly message and skip any extra writes. */
      if (isResourceExhausted(err)) {
        try { if (createdUser) await createdUser.delete(); } catch (_) {}
        setBanner($('shelfd-signup-error'), quotaUserMessage(), 'error');
        return;
      }
