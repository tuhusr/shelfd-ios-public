...
/* v810: Add a Friend tab default state — surface the creator account
...
async function openAddFriendDefault() {
  ...
  grid.innerHTML =
    '<div class="friends-empty no-search-icon" style="grid-column:1/-1;">' +
      '<p style="color:#7a6f99;font-size:14px;">Loading suggestions…</p>' +
    '</div>';
  try {
    const creator = await loadCreatorSearchUser();
    if (!creator || creator.uid === currentUser?.uid) {
      grid.innerHTML =
        '<div class="friends-empty no-search-icon" style="grid-column:1/-1;">' +
    ...
    console.warn('[shelfd-friends] add-friend default load failed:', e);
    grid.innerHTML =
      '<div class="friends-empty no-search-icon" style="grid-column:1/-1;">' +
        '<p>Search for users by username</p>' +
        '<p class="friends-empty-sub">Type a username to find people.</p>' +
