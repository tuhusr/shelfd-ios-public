...
/* v808 — Step 1: force every user into the current Default Theme.
   Coerces 'default' / 'light' / 'cream' / null / undefined / invalid /
   legacy values to 'true-dark' (which is what the "Default Theme" radio
   in Profile → Settings → Theme maps to). The underlying
   normalizeThemeMode + body.light-mode / body.cream-mode systems are
   intentionally left intact for now — to re-enable other themes later,
   replace this function body with `return normalizeThemeMode(value);` */
