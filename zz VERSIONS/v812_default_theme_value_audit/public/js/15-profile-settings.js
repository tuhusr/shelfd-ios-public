...
function renderProfileSettingsPage() {
  if (isViewingOtherProfile()) return;
  /* v812: the in-memory userProfile.themeMode is now always 'default'
...
async function handleThemeModeChange(mode) {
  if (isViewingOtherProfile()) return;
  /* v812: applyThemeMode runs the value through resolveActiveThemeMode,
     so any legacy-handler call (e.g. a stale onclick still bound to
     'light' / 'cream' / 'true-dark') gets coerced to 'default'. */
...
// v808/v812 — Step 1: force every user into the current Default Theme at boot.
// Strips stale localStorage values + stale html/body theme classes, then
// applies the Default Theme and persists it so subsequent loads start
// clean. v812: ACTIVE is now 'default' (BASE CSS, no body class) — the
// real lavender/charcoal Shelfd UI. The legacy 'true-dark' value (which
// applies the OLD pitch-black True Dark CSS via body.true-dark-mode) and
// 'light' / 'cream' all get stripped and never re-applied.
(function() {
  const ACTIVE = getDefaultThemeMode(); // 'default'
  /* 1. Strip stale localStorage if it isn't the active default — covers
        users who came in with 'true-dark' (from v808's wrong forcing),
  ...
  /* 3. Apply the active default theme and persist it. With ACTIVE now
        'default', applyThemeMode adds NO body class — the BASE CSS
        (current Shelfd lavender/charcoal UI) is what renders. */
