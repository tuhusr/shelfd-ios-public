...
function buildFeedPostCardHTML(a, activityId, options = {}) {
  ...
  /* v861: replaced the old activity-card-avatar / activity-card-avatar-placeholder
     pair with the clean sl-activity-avatar-btn pattern used by all other
     activity card renderers. The old structure placed an <img> and a <div>
     as siblings inside activity-avatar-wrap; the placeholder div was leaking
     visible as a ghost element (the "k" initial visible to the right of the
     avatar circle) and the image was rendering slightly oval because the wrap
     had no overflow clip. The new button has overflow:hidden, border-radius:999px,
     and contain:paint — guaranteeing a perfect circle with nothing leaking out. */
  const avatarHtml = avatarSrc
    ? `<button class="sl-activity-avatar-btn feed-post-avatar ${meta.ringClass}" type="button" onclick="event.stopPropagation(); openActivityUserList('${escAttr(a.uid)}','${escAttr(actor.name || a.name || '')}','${escAttr(avatarSrc)}',event.currentTarget)" title="View list" aria-label="View list"><img class="sl-activity-avatar-img" src="${escAttr(avatarSrc)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span class="sl-activity-avatar-fallback" style="display:none">${escHtml(initial)}</span></button>`
    : `<button class="sl-activity-avatar-btn feed-post-avatar ${meta.ringClass}" type="button" onclick="event.stopPropagation(); openActivityUserList('${escAttr(a.uid)}','${escAttr(actor.name || a.name || '')}','',event.currentTarget)" title="View list" aria-label="View list"><span class="sl-activity-avatar-fallback">${escHtml(initial)}</span></button>`;
