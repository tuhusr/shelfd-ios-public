...
const DISCOVER_HERO_TRAILER_EXPAND_TRANSITION = 'left 360ms cubic-bezier(0.16, 1, 0.3, 1), top 360ms cubic-bezier(0.16, 1, 0.3, 1), width 360ms cubic-bezier(0.16, 1, 0.3, 1), height 360ms cubic-bezier(0.16, 1, 0.3, 1), border-radius 360ms cubic-bezier(0.16, 1, 0.3, 1), opacity 220ms linear';

function getDiscoverTrailerViewportSize() {
  const visualViewport = window.visualViewport;
  return {
    width: Math.max(1, Math.round(visualViewport?.width || window.innerWidth || document.documentElement.clientWidth || 360)),
    height: Math.max(1, Math.round(visualViewport?.height || window.innerHeight || document.documentElement.clientHeight || 740))
  };
}
...
function clearDiscoverHeroTrailerExpansionVars(preview) {
  ...
  [
    ...
    '--media-profile-trailer-layer-opacity',
...
function resetDiscoverHeroTrailerExpansionState(overlay = document.getElementById('discover-media-profile')) {
  ...
  overlay?.classList?.remove('media-profile-trailer-expanding', 'media-profile-trailer-fullscreen', 'media-profile-trailer-collapsing', 'media-profile-trailer-gesture', 'media-profile-trailer-controls-hidden');
  document.body.classList.remove('media-profile-trailer-fullscreen-active', 'media-profile-trailer-transition-active');
...
function beginDiscoverHeroTrailerExpansion(overlay = document.getElementById('discover-media-profile')) {
  ...
  preview.style.willChange = 'left, top, width, height, border-radius';
...
function applyDiscoverHeroTrailerExpansionProgress(overlay = document.getElementById('discover-media-profile'), rawProgress = 0) {
  ...
  const ease = progress;
  const fadeProgress = progress < 0.08 ? progress / 0.08 * 0.12 : 0.12 + ((progress - 0.08) / 0.92) * 0.88;
  const start = state.startRect;
  const left = start.left + (0 - start.left) * ease;
  const top = start.top + (0 - start.top) * ease;
  const width = start.width + (viewport.width - start.width) * ease;
  const height = start.height + (viewport.height - start.height) * ease;
  preview.style.setProperty('--media-profile-trailer-left', `${left.toFixed(2)}px`);
  preview.style.setProperty('--media-profile-trailer-top', `${top.toFixed(2)}px`);
  preview.style.setProperty('--media-profile-trailer-width', `${width.toFixed(2)}px`);
  preview.style.setProperty('--media-profile-trailer-height', `${height.toFixed(2)}px`);
  ...
  preview.style.setProperty('--media-profile-trailer-content-opacity', `${Math.max(0, 1 - fadeProgress).toFixed(3)}`);
  ...
  preview.style.setProperty('--media-profile-trailer-frame-opacity', `${(0.7 + 0.3 * ease).toFixed(3)}`);
  preview.style.setProperty('--media-profile-trailer-layer-opacity', `${Math.min(1, progress * 1.28).toFixed(3)}`);
  overlay.style.setProperty('--media-profile-trailer-page-opacity', `${Math.max(0, 1 - fadeProgress).toFixed(3)}`);
  overlay.style.setProperty('--media-profile-trailer-page-y', `${(viewport.height * 0.34 * progress).toFixed(2)}px`);
  return progress;
}
...
function finishDiscoverHeroTrailerExpansion(overlay = document.getElementById('discover-media-profile')) {
  ...
  overlay.classList.remove('media-profile-trailer-expanding', 'media-profile-trailer-collapsing');
  ...
  window.setTimeout(() => {
    if (overlay.classList.contains('media-profile-trailer-fullscreen')) preview.style.transition = '';
  }, 370);
  return true;
}
...
function animateDiscoverHeroTrailerExpansionTo(overlay, targetProgress, done) {
  ...
  preview.style.transition = DISCOVER_HERO_TRAILER_EXPAND_TRANSITION;
  requestAnimationFrame(() => {
    applyDiscoverHeroTrailerExpansionProgress(overlay, targetProgress);
    window.setTimeout(() => {
      preview.style.transition = '';
...
function expandDiscoverHeroTrailerPreview(overlay = document.getElementById('discover-media-profile'), options = {}) {
  ...
  beginDiscoverHeroTrailerExpansion(overlay);
...
function cancelDiscoverHeroTrailerExpansion(overlay = document.getElementById('discover-media-profile')) {
  ...
  overlay?.classList?.remove('media-profile-trailer-fullscreen');
...
function collapseDiscoverHeroTrailerPreview(overlay = document.getElementById('discover-media-profile'), options = {}) {
  if (!overlay?.classList?.contains('media-profile-trailer-fullscreen')) return false;
...
function renderDiscoverHeroTrailerPreview(trailer, title = '') {
  ...
  return `<div class="discover-media-hero-preview" role="button" tabindex="0" data-discover-hero-trailer-preview data-trailer-key="${escAttr(trailerKey)}" data-trailer-title="${escAttr(title)}" onclick="openDiscoverHeroTrailer(event, this)" onkeydown="handleDiscoverHeroTrailerPreviewKeydown(event, this)" aria-label="${escAttr(`Expand trailer for ${title || 'this title'}`)}">
    ...
    <span class="discover-media-hero-preview-controls" data-discover-trailer-controls onclick="event.stopPropagation()">
      ...
      <span class="discover-media-hero-preview-scrub-wrap">
        <input class="discover-media-hero-preview-scrub" type="range" min="0" max="1000" value="0" step="1" data-discover-trailer-control data-discover-trailer-scrub aria-label="Trailer position" onpointerdown="beginDiscoverHeroTrailerScrub(event, this)" oninput="scrubDiscoverHeroTrailer(event, this, false)" onchange="scrubDiscoverHeroTrailer(event, this, true)" onpointerup="endDiscoverHeroTrailerScrub(event, this)" onpointercancel="endDiscoverHeroTrailerScrub(event, this)">
      </span>
      <span class="discover-media-hero-preview-time" data-discover-trailer-time>0:00</span>
      <button class="discover-media-hero-preview-native-fullscreen" type="button" data-discover-trailer-control onclick="openDiscoverHeroTrailerNativeFullscreen(event, this)" aria-label="Open native fullscreen">
...
async function openDiscoverHeroTrailerNativeFullscreen(event, btn) {
  ...
  try {
    ...
    else if (typeof showToast === 'function') showToast('Fullscreen unavailable in this browser');
  } catch (error) {
    if (typeof showToast === 'function') showToast('Fullscreen unavailable in this browser');
  }
}
...
function bindDiscoverMediaProfileSwipeBack(overlay) {
  ...
  const handleGestureMove = (event) => {
    ...
    if (gestureMode === 'trailer-collapse') {
      if (event.cancelable) event.preventDefault();
      pendingProgress = 1 - Math.min(1, Math.max(0, -dy) / Math.max(1, viewportH * 0.24));
      requestGestureFrame();
    }
  };
  ...
  const handleGestureEnd = (event) => {
    ...
    if (mode === 'trailer-collapse') {
      const shouldCollapse = pendingProgress <= 0.72 || (-dy > viewportH * 0.16 && Math.abs(dy) > Math.abs(dx));
      overlay.classList.remove('media-profile-trailer-gesture');
