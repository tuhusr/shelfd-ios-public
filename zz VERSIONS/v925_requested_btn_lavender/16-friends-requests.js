...
async function autoAddCreatorFriend() {
  ...
  friends.push(CREATOR_PUBLIC_UID);
  outgoingRequests = outgoingRequests.filter(f => f !== CREATOR_PUBLIC_UID);
  incomingRequests = incomingRequests.filter(f => f !== CREATOR_PUBLIC_UID);
  allUsersCache = [];
  try {
    const arrayUnion = firebase.firestore.FieldValue.arrayUnion;
    ...
    await db.collection("users").doc(currentUser.uid).set({
      ...
      incomingRequests: arrayRemove(CREATOR_PUBLIC_UID)
    }, { merge: true });
  } catch (e) {
    console.error('[shelfd-friends] autoAddCreatorFriend failed:', e);
    /* Roll back local state */
    friends = friends.filter(f => f !== CREATOR_PUBLIC_UID);
