/* v952: Movies-only "Rate by Matchups" private ladder game. */
(function initShelfdMovieRatingDuel() {
  if (window.__shelfdMovieRatingDuelV952) return;
  window.__shelfdMovieRatingDuelV952 = true;
  ...
  function pickOpponentForBand(targetItem, itemsByBand = {}, band = 0) {
    const pool = sortMoviesForDuel(itemsByBand[band] || []).filter(item => cleanText(item?.id || '') !== cleanText(targetItem?.id || ''));
    return pool[0] || null;
  }
  ...
  function buildMovieDuelSession(itemId = '') {
    ...
    const opponent = pickOpponentForBand(targetItem, itemsByBand, currentBand);
    if (!currentBand || !opponent) {
    ...
    overlay.querySelector('.movie-duel-sheet').innerHTML = `
      <div class="movie-duel-header">
        <div>
          ...
          <p>Beat your ${html(compareBand)}/10 ladder to move up. The first loss ends the run.</p>
        </div>
        <button type="button" class="movie-duel-close" onclick="closeMovieRatingDuel()" aria-label="Close">×</button>
      </div>
      <div class="movie-duel-band-chip">Testing for ${html(compareBand)}/10</div>
  ...
  function chooseMovieRatingDuelWinner(choice = '') {
    ...
    const nextOpponent = pickOpponentForBand(duelState.targetItem, duelState.itemsByBand, nextBand);
    if (!nextOpponent) {
    ...
    duelState.currentOpponent = nextOpponent;
    renderMovieRatingDuelRound();
  }
