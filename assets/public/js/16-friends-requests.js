...
async function loadActivityTabFeed() {
  ...
  if (!currentUser || !friends.length) {
    feed.innerHTML = `${buildActivityFeedHeaderHTML('Activity Feed', { showRefresh: false })}<div class="activity-feed-empty"><strong>No friends yet</strong>Add friends to see what they are watching, playing, and rating.</div>`;
    return;
  }
