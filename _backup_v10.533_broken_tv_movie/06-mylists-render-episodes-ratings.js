...
function render() {
  ...
  /* v10.503: Movies + TV "Pro" toolbar button — sits between the Sort
     button and the Search button. Only shows on the Movies, Shows
     (TV), and Anime shelf sections; gets removed on every other
     section so the toolbar stays clean. Tap opens a category-
     specific bottom sheet (Movies Pro / TV Pro) listing the
     paid-tier insights. */
  const toolbarRightForPro = document.querySelector('.toolbar-right');
  const searchToggleBtn = document.getElementById('mylist-search-toggle-btn');
  const proSheetKind =
    activeSection === 'movies' ? 'movies'
    : (activeSection === 'shows' || activeSection === 'anime') ? 'tv'
    : null;
  let proBtn = document.getElementById('mylist-pro-toolbar-btn');
  if (!proSheetKind) {
    if (proBtn && proBtn.parentNode) proBtn.parentNode.removeChild(proBtn);
  } else {
    if (!proBtn) {
      proBtn = document.createElement('button');
      proBtn.id = 'mylist-pro-toolbar-btn';
      proBtn.type = 'button';
      proBtn.className = 'btn-secondary mylist-pro-toolbar-btn';
      proBtn.addEventListener('click', e => {
        e.stopPropagation();
        const kind = proBtn.getAttribute('data-pro-kind') || 'movies';
        if (typeof window.openShelfProSheet === 'function') {
          window.openShelfProSheet(kind);
        }
      });
    }
    /* Ensure positional order: [sortBtn, proBtn, searchBtn]. The sort
       button is already inserted before search; we want pro between
       them, so insert pro before the search toggle (which is the last
       child of .toolbar-right). */
    if (toolbarRightForPro && searchToggleBtn && proBtn.parentNode !== toolbarRightForPro) {
      toolbarRightForPro.insertBefore(proBtn, searchToggleBtn);
    } else if (toolbarRightForPro && searchToggleBtn && proBtn.nextSibling !== searchToggleBtn) {
      toolbarRightForPro.insertBefore(proBtn, searchToggleBtn);
    }
    proBtn.setAttribute('data-pro-kind', proSheetKind);
    proBtn.setAttribute('aria-label', proSheetKind === 'movies' ? 'Movies Pro' : 'TV Pro');
    proBtn.setAttribute('title', proSheetKind === 'movies' ? 'Movies Pro' : 'TV Pro');
    proBtn.innerHTML = `
      <span class="mylist-pro-toolbar-btn-label">Pro</span>
      <svg class="mylist-pro-toolbar-btn-lock" viewBox="0 0 24 24" width="12" height="12" aria-hidden="true">
...
function renderCard(item, isDraggable) {
  ...
  const useRatingBubble = !shouldHideMyListCardOverallRating && (
    ...
    || (isMusicSection && ['watching', 'watched'].includes(itemStatus))
    || (activeSection === 'movies' && itemStatus === 'watched')
    || (activeSection === 'games' && ['watching', 'live', 'competitive', 'watched'].includes(itemStatus))
  );
  ...
  return `<section class="discover-media-page discover-standard-title-page game-media-page discover-desktop-title-page" role="dialog" aria-modal="true" aria-label="${escAttr(title)} details">
    ...
    <div class="discover-media-hero game-media-hero${heroTrailer ? ' has-trailer-preview' : ''}" style="${poster ? `background-image:url('${escAttr(poster)}')` : ''}">
      ...
      <div class="discover-media-hero-content">
        ...
        ${genres.length ? `<div class="discover-media-chips">${genres.map(name => `<span>${escHtml(name)}</span>`).join('')}</div>` : ''}
        ${renderGameProfileFloatingExports(details)}
        ${typeof renderDiscoverWhereToWatchInline === 'function' ? renderDiscoverWhereToWatchInline(details) : ''}
      </div>
    </div>
    <div class="discover-media-body discover-media-body-cinema">
...
async function openGameMediaProfile(event, rawgId = '', seedOverride = null, transitionOrigin = null) {
  ...
  try {
    ...
    if (!resolvedId || !/^\d+$/.test(String(resolvedId))) {
      ...
      /* v10.533: clear any trailer-preview state left over from the shell
         before swapping innerHTML, matching openDiscoverMediaProfile so the
         games trailer expand/collapse starts from a clean rect. */
      if (typeof destroyDiscoverHeroTrailerPreview === 'function') destroyDiscoverHeroTrailerPreview(overlay);
    ...
    /* v10.533: see comment above — parity with openDiscoverMediaProfile. */
    if (typeof destroyDiscoverHeroTrailerPreview === 'function') destroyDiscoverHeroTrailerPreview(overlay);
    ...
    /* v10.533: see comments above — parity with openDiscoverMediaProfile. */
    if (typeof destroyDiscoverHeroTrailerPreview === 'function') destroyDiscoverHeroTrailerPreview(overlay);
...
/* v10.503: Movies + TV Pro feature bottom sheet. Mirrors the music
   album-shelf Pro sheet (`openAdvancedStatsProSheet` in
   27-music-album-shelf-page.js) but lives at the My List shelf
   toolbar level so it can be invoked from the Movies and TV
   (shows/anime) shelves. Two different content payloads — selected by
   the `kind` argument: 'movies' or 'tv'. Reuses the same
   `.mylist-album-shelf-pro-sheet*` CSS classes so styling stays
   consistent across all three Pro sheets. */
window.openShelfProSheet = function(kind) {
  if (document.querySelector('.mylist-album-shelf-pro-sheet')) return;
  const config = getShelfProSheetConfig(kind);
  if (!config) return;
  const sheet = document.createElement('div');
  sheet.className = 'mylist-album-shelf-pro-sheet';
  sheet.setAttribute('role', 'dialog');
  sheet.setAttribute('aria-modal', 'true');
  sheet.setAttribute('aria-label', config.ariaLabel);
  sheet.innerHTML = `
    <div class="mylist-album-shelf-pro-sheet-scrim" data-pro-sheet-dismiss></div>
    <div class="mylist-album-shelf-pro-sheet-panel" role="document">
      <button type="button" class="mylist-album-shelf-pro-sheet-handle" data-pro-sheet-dismiss aria-label="Close"></button>
      <div class="mylist-album-shelf-pro-sheet-eyebrow">
        <svg class="mylist-album-shelf-pro-sheet-lock" viewBox="0 0 24 24" width="14" height="14" aria-hidden="true">
          <path fill="currentColor" d="M12 2a5 5 0 0 0-5 5v3H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2h-1V7a5 5 0 0 0-5-5Zm-3 8V7a3 3 0 1 1 6 0v3H9Z"/>
        </svg>
        <span>Pro feature</span>
      </div>
      ${config.tagline ? `<p class="mylist-album-shelf-pro-sheet-tagline">${config.tagline}</p>` : ''}
      <h2 class="mylist-album-shelf-pro-sheet-title">${config.title}</h2>
      ${config.sub ? `<p class="mylist-album-shelf-pro-sheet-sub">${config.sub}</p>` : ''}
      ${config.sections.map(section => `
        <div class="mylist-album-shelf-pro-sheet-section-label">${section.label}</div>
        <ul class="mylist-album-shelf-pro-sheet-list">
          ${section.items.map(item => `<li><span class="mylist-album-shelf-pro-sheet-bullet" aria-hidden="true"></span>${item}</li>`).join('')}
        </ul>
      `).join('')}
      <div class="mylist-album-shelf-pro-sheet-footer">
        <span class="mylist-album-shelf-pro-sheet-footnote">Available with Shelfd Pro</span>
      </div>
    </div>
  `;
  document.body.appendChild(sheet);
  document.body.classList.add('mylist-album-shelf-pro-sheet-open');
  requestAnimationFrame(() => requestAnimationFrame(() => sheet.classList.add('is-open')));

  function dismiss() {
    sheet.classList.remove('is-open');
    document.body.classList.remove('mylist-album-shelf-pro-sheet-open');
    document.removeEventListener('keydown', onKey);
    setTimeout(() => { try { sheet.remove(); } catch (_) {} }, 260);
  }
  function onKey(e) { if (e.key === 'Escape') dismiss(); }
  sheet.querySelectorAll('[data-pro-sheet-dismiss]').forEach(el => {
    el.addEventListener('click', e => { e.stopPropagation(); dismiss(); });
  });
  document.addEventListener('keydown', onKey);
};

function getShelfProSheetConfig(kind) {
  if (kind === 'movies') {
    return {
      ariaLabel: 'Movies Pro features',
      title: 'Movies Pro',
      tagline: "We're done waiting for our yearly wrap — movie lovers can see their stats and statistics whenever they want.",
      sub: 'Deep insights into how you watch, rate, and rewatch movies.',
      sections: [
        {
          label: 'Movie Insights',
          items: [
            'Most watched movie genres',
            'Highest-rated movie genres',
            'Lowest-rated movie genres',
            'Average rating by movie genre',
            'Average movie runtime',
            'Longest movie watched',
            'Shortest movie watched',
            'Most rewatched movie',
            'Movies watched by release decade',
            'Movies watched by release year',
            'Highest-rated movies of the month/year',
            'Lowest-rated movies of the month/year',
            'Movies you rated higher than your usual average',
            'Movies you rated lower than your usual average'
          ]
        },
        {
          label: 'Actor & Director Stats',
          items: [
            'Most watched actor',
            'Most watched actress',
            'Most watched director',
            'Highest-rated actor based on your movie ratings',
            'Highest-rated actress based on your movie ratings',
            'Highest-rated director based on your movie ratings',
            'Actor you rewatch the most',
            'Director you rewatch the most',
            'Actors/directors you watched most this month/year',
            'Actors/directors connected to your highest-rated movies'
          ]
        },
        {
          label: 'Movie Genre Breakdowns',
          items: [
            'Most watched genre',
            'Highest-rated genre',
            'Lowest-rated genre',
            'Genre rating trends over time',
            'Genre watch trends by month/year',
            'Genre you rewatch most',
            'Genre you rate most consistently',
            'Genre you are most critical of'
          ]
        },
        {
          label: 'Movie Franchise Tracking',
          items: [
            'Most watched movie franchise',
            'Highest-rated movie franchise',
            'Franchise completion percentage',
            'Franchise average rating',
            'Best-rated franchise entry',
            'Lowest-rated franchise entry',
            'Franchise rewatch count',
            'Total franchise runtime',
            'Upcoming or unwatched franchise entries'
          ]
        },
        {
          label: 'Monthly & Yearly Movie Recaps',
          items: [
            'Monthly movie recap',
            'Yearly movie recap',
            'Top movies of the month/year',
            'Most rewatched movies of the month/year',
            'Most watched movie genre of the month/year',
            'Highest-rated movie genre of the month/year',
            'Most watched actor/actress/director of the month/year',
            'Biggest movie rating trends',
            'Movie franchise progress recap',
            'Movies completed this month/year'
          ]
        }
      ]
    };
  ...
  if (kind === 'tv') {
    return {
      ariaLabel: 'TV Pro features',
      title: 'TV Pro',
      tagline: "We're done waiting for our yearly wrap — TV lovers can see their stats and statistics whenever they want.",
      sub: 'Deep insights into how you watch, rate, and finish shows.',
      sections: [
        {
          label: 'TV Show Insights',
          items: [
            'Total episodes watched',
            'Total seasons completed',
            'Average episode runtime',
            'Average season length',
            'Longest show watched',
            'Most watched show',
            'Most rewatched show',
            'Highest-rated shows of the month/year',
            'Lowest-rated shows of the month/year',
            'Shows completed this month/year',
            'Shows dropped or left unfinished',
            'Shows you are behind on',
            'Shows with the most progress updates'
          ]
        },
        {
          label: 'TV People Stats',
          items: [
            'Most watched actor',
            'Most watched actress',
            'Most watched showrunner / creator',
            'Highest-rated actor based on your TV ratings',
            'Highest-rated actress based on your TV ratings',
            'Highest-rated showrunner / creator',
            'Actors you watched across the most shows',
            'Actors connected to your highest-rated shows',
            'Most watched TV cast members this month/year'
          ]
        },
        {
          label: 'TV Genre Breakdowns',
          items: [
            'Most watched TV genre',
            'Highest-rated TV genre',
            'Lowest-rated TV genre',
            'Average rating by TV genre',
            'Genre completion trends',
            'Genre you finish most often',
            'Genre you drop most often',
            'Genre you rewatch most',
            'TV genre trends by month/year'
          ]
        },
        {
          label: 'Season & Episode Stats',
          items: [
            'Most watched season',
            ...
            'Seasons completed this month/year',
            'Average episodes watched per week/month',
            'Longest episode streak',
            'Most active TV watching month'
          ]
        },
        {
          label: 'Network / Platform Stats',
          items: [
            'Most watched TV network',
            'Most watched streaming platform',
            'Highest-rated network/platform',
            'Shows watched by platform',
            'Shows completed by platform',
            'Platform with your most unfinished shows'
          ]
        },
        {
          label: 'Monthly & Yearly TV Recaps',
          items: [
            'Monthly TV recap',
            'Yearly TV recap',
            'Top shows of the month/year',
            'Top seasons of the month/year',
            'Most watched TV genre of the month/year',
            'Highest-rated TV genre of the month/year',
            'Most completed shows of the month/year',
            'Most watched actor/actress/showrunner of the month/year',
            'Shows you started, finished, dropped, or continued',
            'TV progress recap across watching, completed, paused, and dropped shows'
          ]
        }
      ]
    };
