...
async function openJikanAnimeProfile(event, malId, transitionOrigin = null) {
  ...
  try {
    ...
    bindDiscoverMediaProfileActions(overlay);
    hydrateDiscoverHeroTrailerPreview(overlay);
    /* v10.533: parity with openDiscoverMediaProfile — anime profiles missed
       the More Like This hydration call, so the recommendation row stayed
       empty even when DeepSeek returned matches. */
    if (typeof hydrateDeepSeekMoreLikeThis === 'function') hydrateDeepSeekMoreLikeThis(profileType, mergedDetails);
    hydrateDiscoverProviderLogoFallbacks();
  } catch (e) {
